...
dest){this.debug.matchArguments("jayus.applyObject",arguments,"src",5,"dest",5);for(var item in src)if(src.hasOwnProperty(item))dest[item]=src[item];return dest},getNewId:function jayus_getNewId(){return parseInt(Math.random()*1E6,10)},toJSON:function jayus_toJSON(entity){var result={objects:[],id:entity.id};entity.addToResult(result);return result},fromJSON:function jayus_toJSON(result){},isObjectInResult:function jayus_isObjectInResult(result,object){var id=object.id;for(var i=0;i<result.objects.length;i++)if(result.objects[i].id===
id)return true;return false},getObjectFromResult:function jayus_getObjectFromResult(result,id){for(var i=0;i<result.objects.length;i++){var object=result.objects[i];if(object.id===id)return new jayus[object.__type__].fromResult(result,object)}return null},entities:[],createClass:function jayus_createClass(props){var constructor=function(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments)};if(typeof jayus.debug.className==="string")constructor=eval("(function "+jayus.debug.className+
"(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments);})");jayus.debug.className=null;constructor.prototype=props;if(typeof constructor.prototype.init!=="function")constructor.prototype.init=function(){};constructor.init=function HelperInit(object,args){constructor.prototype.init.apply(object,args)};constructor.extend=jayus.extendMethod;return constructor},extendMethod:function jayus_extendMethod(props){var constructor=function(){this.id=jayus.getNewId();jayus.entities.push(this);
this.init.apply(this,arguments)};if(typeof jayus.debug.className==="string")constructor=eval("(function "+jayus.debug.className+"(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments);})");jayus.debug.className=null;constructor.prototype=Object.create(this.prototype);jayus.applyObject(props,constructor.prototype);if(typeof constructor.prototype.init!=="function")constructor.prototype.init=function(){};constructor.init=function HelperInit(object,args){constructor.prototype.init.apply(object,
args)};constructor.extend=jayus.extendMethod;return constructor},getUrlParameters:function jayus_getUrlParameters(){for(var i=0,vars={},param,paramArray=window.location.href.slice(window.location.href.indexOf("?")+1).split("&");i<paramArray.length;i++){param=paramArray[i].split("=");vars[param[0]]=param[1]}return vars},init:function jayus_init(){var elem=document.createElement("canvas");if(!(typeof elem.getContext==="function"&&typeof elem.getContext("2d")==="object"))throw new Error("Fatal error, browser does not support the canvas element, unable to initiate Jayus");
this.applyObject(this.Responder.prototype,this);this.addDefaultHandlers();this.ua=this.detectBrowser();this.ua.lineDash=typeof this.getContext().setLineDash==="function";window.requestAnimationFrame=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame;this.ua.requestAnimationFrame=typeof window.requestAnimationFrame==="function";this.keyboard.init();this.colors.cssNames=[];for(var i=0;i<this.colors.displayNames.length;i++)this.colors.cssNames.push(this.colors.displayNames[i].replace(" ",
...
jayus.chart.end();jayus.chart.update()}}else jayus.frameIntervalRunning=false},fromObject:function jayus_fromObject(obj){var type=obj.__type__;if(typeof jayus[type]==="function"){for(var key in obj){var value=obj[key];if(value instanceof Array)for(var i=0;i<value.length;i++){if(typeof value[i]==="object"&&typeof value[i].type==="string")value[i]=this.fromObject(value[i])}else if(typeof value==="object"&&typeof value.type==="string")obj[key]=this.fromObject(value)}var ret=new jayus[type];ret.apply(obj);
return ret}},groupToObject:function Group_toObject(object){if(this.propagateCursor!==true)object.propagateCursor=this.propagateCursor;if(this.children!==jayus.Group.children){object.children=[];for(var i=0;i<this.children.items.length;i++)object.children.push(this.children.items[i].toObject())}},groupTnitFromObject:function Group_initFromObject(object){if(typeof object.propagateCursor==="boolean")this.propagateCursor=object.propagateCursor;if(typeof object.children==="object"){this.children=new jayus.List(this);
for(var i=0;i<object.children.length;i++)this.children.items.push(jayus.fromObject(object.children[i]))}},addDefaultHandlers:function jayus_addDefaultHandlers(){jayus.addHandler("keyPress",function(e){if(e.key==="grave"&&e.event.ctrlKey){if(jayus.chart.visible)jayus.chart.hide();else jayus.chart.show();return true}})},chart:{routines:{render:{description:"Rendering",time:0,color:"saddlebrown"},box2d:{description:"Box2D updating",time:0,color:"blue"},animation:{description:"Animation",time:0,color:"orange"},
cursor:{description:"Cursor events",time:0,color:"Chartreuse"}},routineNameStack:[],routineTimeStack:[],routineInitStack:[],markColor:"grey",markFont:"10px sans-serif",display:null,graphSurface:null,topMS:40,index:0,width:500,height:380,visible:false,initialized:false,windowSize:120,windowIndex:0,barHeight:20,barSpacing:10,routineData:{},getRoutineColor:function jayus_chart_getRoutineColor(name){switch(name){case "render":return"saddlebrown";case "box2d":return"blue";case "cursor":return"Chartreuse";
case "animation":return"orange"}return"white"},begin:function jayus_chart_begin(routine){this.routineNameStack.push(routine);this.routineTimeStack.push(Date.now());this.routineInitStack.push(0)},end:function jayus_chart_end(){var time=Date.now()-this.routineTimeStack.pop(),name=this.routineNameStack.pop(),inits=this.routineInitStack.pop(),data,i;if(typeof this.routines[name]!=="object")this.routines[name]={description:"",time:0,color:"red"};if(this.initialized){data=this.routineData[name];if(data===
undefined){data={visible:true,latencies:[],averageLatency:0,inits:[],initTotal:0,color:this.getRoutineColor(name),widget:null};for(i=0;i<this.windowSize;i++){data.latencies.push(0);data.inits.push(0)}this.routineData[name]=data;data.row=document.createElement("tr");var title=document.createElement("td");title.style.color="#CCC";title.style.fontSize="11pt";title.innerText=name;data.latencyLabel=document.createElement("td");data.latencyLabel.style.color="#BBB";data.latencyLabel.style.fontSize="10pt";
data.latencyLabel.style.textAlign="right";data.latencyLabel.innerText="0 ms";data.progressBar=document.createElement("div");data.progressBar.style.margin="4px";$(data.progressBar).progressbar({value:0});data.progressBarValue=data.progressBar.childNodes[0];$(data.progressBarValue).css("background",data.color);data.row.appendChild(title);data.row.appendChild(data.latencyLabel);data.row.appendChild(data.progressBar);jayus.chart.latencyTable.appendChild(data.row)}data.time+=time;data.latencyTotal-=data.latencies[this.windowIndex];
data.latencies[this.windowIndex]=time;data.inits+=inits}this.routines[name].time+=time},tallyInit:function jayus_chart_tallyInit(type){this.routineInitStack[this.routineInitStack.length-1]++},show:function jayus_chart_show(){if(!this.initialized){this.vBox=new jayus.vBox;this.vBox.setSize(this.width,this.height);this.topRow=new jayus.Scene(this.width,30);var bg=new jayus.LinearGradient(0,0,0,30);bg.addColorStop(0,"#DDD");bg.addColorStop(1,"#CCC");this.topRow.setBg({fill:bg});this.topRow.heightPolicy=
new jayus.SizePolicy;this.topRow.heightPolicy.size=30;this.topRow.heightPolicy.expand=false;this.fpsLabel=jayus.parse({type:"Text",setText:"",setFont:"12px sans-serif",setBrush:{fill:"#111"},setOrigin:[0,4]});this.topRow.children.add(this.fpsLabel);setInterval(function(){var label=jayus.chart.fpsLabel;var fps=Math.round(jayus.fps*10)/10+"";if(fps.length===2)fps+=".0";label.setText("Framerate: "+fps);label.setX(label.parent.width-4-label.width)},100);this.graph=new jayus.Scene;this.graph.setOptimizedBuffering(false);
this.vBox.children.add(this.topRow,this.graph);this.graphSurface=new jayus.Surface(this.graph.width,this.graph.height);this.graph.children.add(this.graphSurface);this.graph.addHandler("dirty",function(dirty){if(dirty&4){var addLabel=function(ms){var y=jayus.chart.graph.height-ms/jayus.chart.topMS*jayus.chart.graph.height,line=new jayus.PaintedShape(new jayus.Polygon.Line(0,Math.floor(y)+0.5,jayus.chart.graph.width,Math.floor(y)+0.5),{stroke:"dimgrey"}),text=(new jayus.Text(ms+" ms","10px sans-serif",
{fill:"dimgrey"})).setOrigin(4,y-12);jayus.chart.graph.children.update(ms+"line",line.setId(ms+"line")).update(ms+"text",text.setId(ms+"text"))};addLabel(16);addLabel(33);jayus.chart.graphSurface.setHeight(this.height)}});this.graph.fire("resized");this.display=new jayus.Display(this.width,this.height);this.display.children.add(this.vBox);this.display.addHandler("dirty",function(dirty){if(dirty&4)jayus.chart.vBox.setWidth(this.width)});var div=document.createElement("div");var canvas=this.display.canvas;
canvas.style.border="#aaa 1px solid";canvas.style.backgroundColor="black";div.appendChild(canvas);table=document.createElement("table");table.style.width="100%";div.appendChild(table);$(table).append('<colgroup><col span="1" style="width: 15%;"><col span="1" style="width: 15%;"><col span="1" style="width: 70%;"></colgroup>');jayus.chart.latencyTable=table;$(div).dialog({title:"Jayus Debug Panel",open:function(){$(this).data("width.dialog",$(canvas).width()+60)},resize:function(){jayus.chart.display.setWidth($(jayus.chart.dialogContainer).width()-
4);return false},close:function(){jayus.chart.visible=false}});jayus.chart.dialog=div.parentNode;jayus.chart.dialogContainer=jayus.chart.dialog.childNodes[1];this.display.setSize($(jayus.chart.dialogContainer).width()-4,$(jayus.chart.dialogContainer).height()-4);this.initialized=true}this.visible=true;$(jayus.chart.dialog).show();jayus.fire("debugPanelShown")},hide:function jayus_chart_hide(){if(this.initialized&&this.visible){$(jayus.chart.dialog).hide();this.graphSurface.clear();this.index=0;this.visible=
...
var totalLatency=0;var routineNames=[];for(routine in this.routineData){data=this.routineData[routine];routineNames.push(routine);data.averageLatency=jayus.math.average(data.latencies);totalLatency+=data.averageLatency}for(i=0;i<routineNames.length;i++){data=this.routineData[routineNames[i]];height=Math.floor(data.time/this.topMS*this.graph.height);ctx.fillStyle=data.color;ctx.fillRect(this.index,y-height,1,height-0.5);y-=height;data.time=0;data.inits=0}ctx.clearRect(this.index+1,0,2,this.graph.height);
this.index++;if(this.index>=this.graph.width)this.index=-1;for(i=0;i<routineNames.length;i++){data=this.routineData[routineNames[i]];$(data.progressBar).progressbar({value:100*data.averageLatency/totalLatency});var num=Math.round(10*data.averageLatency)/10+"";if(num.length===1&&num!=="0")num+=".0";$(data.latencyLabel).text(num+" ms")}var parent=$(jayus.chart.latencyTable);var items=parent.children();items.sort(function(a,b){if(a.nodeName==="TR"&&b.nodeName==="TR"){var vA=parseFloat(a.childNodes[2].attributes["aria-valuenow"].value);
var vB=parseFloat(b.childNodes[2].attributes["aria-valuenow"].value);return vA<vB?1:0}});parent.append(items);this.windowIndex++;if(this.windowIndex===this.windowSize)this.windowIndex=0;this.graphSurface.dirty(jayus.DIRTY.ALL);this.display.refreshBuffer()}}},getFontDescriptor:function jayus_getFontDescriptor(font){this.debug.match("jayus.getFontDescriptor",font,"font",2);if(!this.isFontCached(font))this.cacheFont(font);return this.fontCache[font]},cacheFont:function jayus_cacheFont(font){this.debug.match("jayus.cacheFont",
font,"font",2);var i,ctx,charWidths,descriptor;if(!this.isFontCached(font)){charWidths=[];ctx=this.getContext();ctx.save();ctx.font=font;for(i=0;i<this.fontCacheMaxChar+1;i++)charWidths[i]=ctx.measureText(String.fromCharCode(i)).width;ctx.restore();descriptor=this.getVerticalFontMetrics(font);descriptor.charWidths=charWidths;this.fontCache[font]=descriptor}},cacheFonts:function jayus_cacheFonts(fonts){this.debug.matchArray("jayus.cacheFonts",fonts,"fonts",2);for(var i=0;i<fonts.length;i++)this.cacheFont(fonts[i])},
isFontCached:function jayus_isFontCached(font){this.debug.match("jayus.isFontCached",font,"font",2);return typeof this.fontCache[font]==="object"},measureText:function jayus_measureText(text,font){return this.measureTextOnto(text,font,{})},measureTextOnto:function jayus_measureTextOnto(text,font,ret){this.debug.matchArguments("jayus.measureTextOnto",arguments,"text",2,"font",2,"ret",5);this.cacheFont(font);var descriptor=jayus.fontCache[font],charWidths=descriptor.charWidths,i,width=0;for(i=0;i<text.length;i++)width+=
charWidths[text.charCodeAt(i)];ret.width=width;ret.ascent=descriptor.ascent;ret.descent=descriptor.descent;ret.height=descriptor.height;return ret},getVerticalFontMetrics:function jayus_getVerticalFontMetrics(font){var result={},text=document.createElement("span"),block=document.createElement("div"),div=document.createElement("div");text.style.font=font;text.innerText="Hg";block.style.display="inline-block";block.style.width="1px";block.style.height="0";div.appendChild(text);div.appendChild(block);
document.body.appendChild(div);try{block.style.verticalAlign="baseline";result.ascent=block.offsetTop-text.offsetTop;block.style.verticalAlign="bottom";result.height=block.offsetTop-text.offsetTop;result.descent=result.height-result.ascent}finally{document.body.removeChild(div)}return result},box2d:{enabled:true,scale:0.5,worlds:[],velocityIterations:10,positionIterations:10,physicalEntities:[],addWorld:function jayus_box2d_addWorld(world){jayus.debug.match("jayus.box2d.addWorld",world,"world",5);
this.worlds.push(world)},removeWorld:function jayus_box2d_removeWorld(world){this.worlds.splice(this.worlds.indexOf(this.world),1)},step:function jayus_box2d_step(elapsedSecs){jayus.chart.begin("box2d");var i,world;for(i=0;i<this.worlds.length;i++){world=this.worlds[i];world.Step(elapsedSecs,this.velocityIterations,this.positionIterations);world.ClearForces()}for(i=0;i<this.physicalEntities.length;i++)this.physicalEntities[i].updateFromBody();jayus.chart.end()},drawDebug:function jayus_box2d_drawDebug(){for(var i=
...
jayus.Dependency=jayus.Animatable.extend({id:0,dependents:null,dependentCount:0,frozen:0,setId:function Dependency_setId(id){jayus.debug.match("Dependency.setId",id,"id",0);this.id=id;return this},initFromObject:function Dependency_initFromObject(object){jayus.debug.match("Dependency.initFromObject",object,"object",5);this.id=id;this.dependents=object.dependents;if(this.dependents!==null){this.dependentCount=object.dependents.length;for(var i=0;i<this.dependentCount;i++)this.dependents[i]=jayus.getObject(this.dependents[i])}else this.dependentCount=
0;this.dirty(jayus.DIRTY.ALL)},addToResult:function Dependency_addToResult(result){if(jayus.isObjectInResult(result,this))return;var object={};object.__type__="Dependency";object.id=this.id;result.objects.push(object);if(this.dependentCount){object.dependents=[];for(var i=0;i<this.dependentCount;i++){var val=this.dependents[i];val.addToResult(result);object.dependents.push(val.id)}}else object.dependents=null;return object},attach:function Dependency_attach(dependent){jayus.debug.match("Dependency.attach",
dependent,"dependent",5);if(!this.dependentCount)this.dependents=[];this.dependents.push(dependent);this.dependentCount++},detach:function Dependency_detach(dependent){if(this.dependentCount){this.dependents.splice(this.dependents.indexOf(dependent),1);this.dependentCount--}},dirty:function Dependency_dirty(type){if(!this.frozen)this.informDependents(type)},informDependents:function Dependency_informDependents(type){for(var i=0;i<this.dependentCount;i++){jayus.debug.verifyMethod(this.dependents[i],
"componentDirtied");this.dependents[i].componentDirtied(this,type)}},forEachDependent:function Dependency_forEachDependent(func,args){for(var i=0;i<this.dependentCount;i++)func.apply(this.dependents[i],args)}});jayus.debug.className="List";
...
alpha){this.alpha=alpha;this.dirty(jayus.DIRTY.STYLE)}return this},setFill:function Brush_setFill(style){if(style!==null)jayus.debug.match("Brush.setFill",style,"style",0);if(this.fillType>=4)this.fill.detach(this);this.fill=style;this.fillType=this.findStyleType(style);if(this.fillType>=4)this.fill.attach(this);this.filling=!!this.fillType;this.dirty(jayus.DIRTY.STYLE);return this},setStroke:function Brush_setStroke(style){if(style!==null)jayus.debug.match("Brush.setStroke",style,"style",0);if(this.strokeType>=
4)this.stroke.detach(this);this.stroke=style;this.strokeType=this.findStyleType(style);if(this.strokeType>=4)this.stroke.attach(this);this.stroking=!!this.strokeType;this.dirty(jayus.DIRTY.STYLE);return this},setLineWidth:function Brush_setLineWidth(lineWidth){if(lineWidth!==null)jayus.debug.match("Brush.setLineWidth",lineWidth,"lineWidth",4);if(this.lineWidth!==lineWidth){this.lineWidth=lineWidth;this.dirty(jayus.DIRTY.STYLE)}return this},setLineCap:function Brush_setLineCap(lineCap){if(lineCap!==
null)jayus.debug.match("Brush.setLineCap",lineCap,"lineCap");if(this.lineCap!==lineCap){this.lineCap=lineCap;this.dirty(jayus.DIRTY.STYLE)}return this},setLineJoin:function Brush_setLineJoin(lineJoin){if(lineJoin!==null)jayus.debug.match("Brush.setLineJoin",lineJoin,"lineJoin",2);if(this.lineJoin!==lineJoin){this.lineJoin=lineJoin;this.dirty(jayus.DIRTY.STYLE)}return this},setMiterLimit:function Brush_setMiterLimit(miterLimit){if(miterLimit!==null)jayus.debug.match("Brush.setMiterLimit",miterLimit,
"miterLimit",2);if(this.miterLimit!==miterLimit){this.miterLimit=miterLimit;this.dirty(jayus.DIRTY.STYLE)}return this},setLineDash:function Brush_setLineDash(lineDash){if(lineDash!==null)jayus.debug.matchArray("Style.setLineDash",lineDash,"lineDash",4);this.lineDash=lineDash;this.dirty(jayus.DIRTY.STYLE);return this},setLineDashOffset:function Brush_setLineDashOffset(lineDashOffset){if(lineDashOffset!==null)jayus.debug.match("Brush.setLineDashOffset",lineDashOffset,"lineDashOffset",4);if(this.lineDashOffset!==
lineDashOffset){this.lineDashOffset=lineDashOffset;this.dirty(jayus.DIRTY.STYLE)}return this},setShadow:function Brush_setShadow(shadow){if(shadow!==null)jayus.debug.match("Brush.setShadow",shadow,"shadow",0);if(this.shadowType===2)this.shadow.detach(this);this.shadow=shadow;if(shadow===null)this.shadowType=0;else if(typeof shadow==="string")this.shadowType=1;else if(shadow instanceof jayus.Color){this.shadowType=2;this.shadow.attach(this)}else throw new Error("Style.setShadow() - Invalid shadow style: "+
...
jayus.Gradient=jayus.Dependency.extend({componentType:"GRADIENT",stopPositions:null,stopColors:null,nativeGradient:null,reformNative:true,componentDirtied:function Gradient_componentDirtied(component,type){this.reformNative=true;this.dirty(jayus.DIRTY.STYLE)},translate:function Gradient_translate(x,y){jayus.debug.matchCoordinate("Gradient.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}this.start.translate(x,y);this.end.translate(x,y);this.dirty(jayus.DIRTY.ALL);return this},addColorStop:function Gradient_addColorStop(position,
color){jayus.debug.matchArguments("Gradient.addColorStop",arguments,"position",4,"color",0);this.stopPositions.push(position);this.stopColors.push(color);return this},getNative:function Gradient_getNative(){if(this.reformNative){this.refresh();this.reformNative=false}return this.nativeGradient}});jayus.debug.className="LinearGradient";
...
jayus.Matrix=jayus.createClass({a:1,b:0,tx:0,c:0,d:1,ty:0,init:function Matrix_init(a,b,tx,c,d,ty){if(arguments.length){jayus.debug.matchArgumentsAs("Matrix.init",arguments,4,"a","b","tx","c","d","ty");this.a=a;this.b=b;this.tx=tx;this.c=c;this.d=d;this.ty=ty}},initFromObject:function Matrix_initFromObject(object){jayus.debug.match("Matrix.initFromObject",object,"object",5);this.a=object.a;this.b=object.b;this.tx=object.tx;this.c=object.c;this.d=object.d;this.ty=object.ty;this.dirty(jayus.DIRTY.ALL)},
addToObject:function Matrix_addToObject(result){if(jayus.isObjectInResult(result,this))return;var object={};object.__type__="Matrix";object.id=this.id;object.a=this.a;object.b=this.b;object.tx=this.tx;object.c=this.c;object.d=this.d;object.ty=this.ty;result.objects.push(object)},translate:function Matrix_translate(x,y){jayus.debug.matchCoordinate("Matrix.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}this.tx+=this.a*x+this.b*y;this.ty+=this.d*y+this.c*x;return this},scale:function Matrix_scale(x,
y){jayus.debug.matchCoordinate("Matrix.scale",x,y);if(arguments.length===1){y=x.y;x=x.x}this.a*=x;this.c*=x;this.d*=y;this.b*=y;return this},rotate:function Matrix_rotate(angle){jayus.debug.match("Matrix.rotate",angle,"angle",4);var sin=Math.sin(angle),cos=Math.cos(angle);var a=this.a*cos+this.b*sin;this.b=-this.a*sin+this.b*cos;this.a=a;var c=this.d*sin+this.c*cos;this.d=this.d*cos-this.c*sin;this.c=c;return this},multiply:function Matrix_multiply(m1){jayus.debug.match("Matrix.multiply",m1,"m1",
10);m2=this;return new jayus.Matrix(m1.a*m2.a+m1.b*m2.c,m1.a*m2.b+m1.b*m2.d,m1.a*m2.tx+m1.b*m2.ty+m1.tx,m1.c*m2.a+m1.d*m2.c,m1.c*m2.b+m1.d*m2.d,m1.c*m2.tx+m1.d*m2.ty+m1.ty)},transformPoint:function Matrix_transformPoint(x,y){jayus.debug.matchCoordinate("Matrix.transformPoint",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.transformPointOnto(x,y,new jayus.Point)},transformPointOnto:function Matrix_transformPointOnto(x,y,ret){jayus.debug.matchArguments("Matrix.transformPointOnto",arguments,"x",
4,"y",4,"ret",13);ret.x=x*this.a+y*this.b+this.tx;ret.y=x*this.c+y*this.d+this.ty;return ret},inverseTransformPoint:function Matrix_inverseTransformPoint(x,y){jayus.debug.matchCoordinate("Matrix.inverseTransformPoint",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.inverseTransformPointOnto(x,y,new jayus.Point)},inverseTransformPointOnto:function Matrix_inverseTransformPointOnto(x,y,ret){jayus.debug.matchArguments("Matrix.inverseTransformPointOnto",arguments,"x",4,"y",4,"ret",13);var det=this.getDeterminant();
...
cloneOnto:function Matrix_cloneOnto(ret){jayus.debug.match("Matrix.cloneOnto",ret,"ret",10);ret.init(this.a,this.b,this.tx,this.c,this.d,this.ty);return ret},toString:function Matrix_toString(){return"[ "+this.a+", "+this.b+", "+this.tx+" ]\n"+"[ "+this.b+", "+this.d+", "+this.ty+" ]\n"+"[ "+0+", "+0+", "+1+" ]"},equals:function Matrix_equals(mat){jayus.debug.match("Matrix.equals",mat,"mat",10);return this.a===mat.a&&this.b===mat.b&&this.c===mat.c&&this.d===mat.d&&this.tx===mat.tx&&this.ty===mat.ty},
applyToContext:function Matrix_applyToContext(ctx){jayus.debug.matchContext("Matrix.applyToContext",ctx);ctx.transform(this.a,this.c,this.b,this.d,this.tx,this.ty)}});jayus.debug.className="SizePolicy";jayus.SizePolicy=jayus.createClass({size:0,weight:1,expand:true});jayus.images={images:{},pendingImageCount:0,checkFilepath:function jayus_images_checkFilepath(filepath){jayus.debug.match("jayus.images.checkFilepath",filepath,"filepath",2);if(filepath.indexOf("../")!==-1)console.warn('jayus.images.checkFilepath() - Filepath "'+filepath+'" contains "./" or "../", avoid using these')},has:function jayus_images_has(filepath){jayus.debug.match("jayus.images.has",filepath,"filepath",2);this.checkFilepath(filepath);return typeof this.images[filepath]==="object"},isLoaded:function jayus_images_isLoaded(filepath){jayus.debug.match("jayus.images.isLoaded",
...
jayus.Point=jayus.Dependency.extend({componentType:"POINT",x:0,y:0,init:function Point_init(x,y){if(arguments.length===1)this.initFromObject(x);else if(arguments.length){jayus.debug.matchArgumentsAs("Point.init",arguments,4,"x","y");this.x=x;this.y=y}},toObject:function Point_toObject(){return{__type___:"Point",x:this.x,y:this.y}},initFromObject:function Point_initFromObject(object){jayus.debug.match("Point.initFromObject",object,"object",5);this.x=object.x;this.y=object.y;return this.dirty(jayus.DIRTY.ALL)},
setX:function Point_setX(x){jayus.debug.match("Point.setX",x,"x",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setX,this.x,x)}if(this.x!==x){this.x=x;this.dirty(2)}return this},setY:function Point_setY(y){jayus.debug.match("Point.setY",y,"y",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setY,this.y,y)}if(this.y!==y){this.y=y;this.dirty(2)}return this},set:function Point_set(x,y){jayus.debug.matchArgumentsAs("Point.set",
arguments,4,"x","y");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.set,[this.x,this.y],[x,y])}if(this.x!==x||this.y!==y){this.x=x;this.y=y;this.dirty(2)}return this},translate:function Point_translate(x,y){jayus.debug.matchCoordinate("Point.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.set(this.x+x,this.y+y)},toString:function Point_toString(){return"(Point:"+this.x+","+this.y+")"},clone:function Point_clone(){return new jayus.Point(this.x,
...
jayus.Rectangle=jayus.Shape.extend({componentType:"RECTANGLE",shapeType:4,x:0,y:0,width:0,height:0,keepAligned:true,init:function Rectangle_init(x,y,width,height){if(arguments.length===1)this.initFromObject(x);else if(arguments.length){jayus.debug.matchRectangle("Rectangle.init",x,y,width,height);if(arguments.length===2){height=y.height;width=y.width;y=x.y;x=x.x}else if(arguments.length===3){height=width;width=height;y=x.y;x=x.x}this.x=x;this.y=y;this.width=width;this.height=height}},initFromObject:function Rectangle_initFromObject(object){jayus.debug.match("Rectangle.initFromObject",
object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.x=object.x;this.y=object.y;this.width=object.width;this.height=object.height;this.keepAligned=object.keepAligned;this.dirty(jayus.DIRTY.ALL)},addToResult:function Rectangle_addToResult(result){if(jayus.isObjectInResult(result,this))return;var object=jayus.Dependency.prototype.addToResult.call(this);object.__type__="Rectangle";object.x=this.x;object.y=this.y;object.width=this.width;object.height=this.height;object.keepAligned=
this.keepAligned;return object},getOrigin:function Rectangle_getOrigin(){return new jayus.Point(this.x,this.y)},getOriginOnto:function Rectangle_getOriginOnto(ret){ret.x=this.x;ret.y=this.y;return ret},setX:jayus.Point.prototype.setX,setY:jayus.Point.prototype.setY,setOrigin:function Rectangle_setOrigin(x,y){jayus.debug.matchCoordinate("Rectangle.set",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setOrigin,[this.x,
this.y],[x,y])}if(this.x!==x||this.y!==y){this.x=x;this.y=y;this.dirty(2)}return this},translate:function Rectangle_translate(x,y){jayus.debug.matchCoordinate("Rectangle.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setOrigin(this.x+x,this.y+y)},setWidth:function Rectangle_setWidth(width){jayus.debug.match("Rectangle.setWidth",width,"width",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setWidth,this.width,width)}if(this.width!==width){this.width=
width;this.dirty(4)}return this},setHeight:function Rectangle_setHeight(height){jayus.debug.match("Rectangle.setHeight",height,"height",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setHeight,this.height,height)}if(this.height!==height){this.height=height;this.dirty(4)}return this},setSize:function Rectangle_setSize(width,height){jayus.debug.matchSize("Rectangle.setSize",width,height);if(arguments.length===1){height=width.height;width=width.width}if(this.actionsToAnimate){this.actionsToAnimate--;
...
getBottom:function Rectangle_getBottom(){return this.y+this.height},setLeft:function Rectangle_setLeft(left){jayus.debug.match("Rectangle.setLeft",left,"left",4);return this.setFrame(left,this.y,this.x+this.width-left,this.height)},setRight:function Rectangle_setRight(right){jayus.debug.match("Rectangle.setRight",right,"right",4);return this.setWidth(this.width+right-this.x)},setTop:function Rectangle_setTop(top){jayus.debug.match("Rectangle.setTop",top,"top",4);return this.setFrame(this.x,top,this.width,
this.y+this.height-top)},setBottom:function Rectangle_setBottom(bottom){jayus.debug.match("Rectangle.setBottom",bottom,"bottom",4);return this.setHeight(this.height+bottom-this.y)},setFrame:function Rectangle_setFrame(x,y,width,height){if(arguments.length===3){jayus.debug.matchArguments("Rectangle.setFrame",arguments,"origin",13,"width",4,"height",4);height=width;width=y;y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Rectangle.setFrame",arguments,4,"x","y","width","height");if(this.actionsToAnimate){this.actionsToAnimate--;
return new jayus.MethodAnimator(this,this.setFrame,[this.x,this.y,this.width,this.height],[x,y,width,height])}this.x=x;this.y=y;this.width=width;this.height=height;this.dirty(jayus.DIRTY.FRAME);return this},includePoint:function Rectangle_includePoint(x,y){jayus.debug.matchCoordinate("Rectangle.includePoint",x,y);if(arguments.length===1){y=x.y;x=x.x}var x1=Math.min(this.x,x),y1=Math.min(this.y,y),x2=Math.max(this.x+this.width,x),y2=Math.max(this.y+this.height,y);return this.setFrame(x1,y1,x2-x1,y2-
y1)},includeRectangle:function Rectangle_includeRectangle(x,y,width,height){if(arguments.length===1){jayus.debug.match("Rectangle.includeRectangle()",x,"section",20);height=x.height;width=x.width;y=x.y;x=x.x}else if(arguments.length===3){jayus.debug.matchArguments("Rectangle.includeRectangle()",arguments,"origin",13,"width",4,"height",4);height=width;width=y;y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Rectangle.includeRectangle()",arguments,4,"x","y","width","height");var x1=Math.min(this.x,x),
y1=Math.min(this.y,y),x2=Math.max(this.x+this.width,x+width),y2=Math.max(this.y+this.height,y+height);return this.setFrame(x1,y1,x2-x1,y2-y1)},alignToCanvas:function Rectangle_alignToCanvas(){return this.setFrame(Math.floor(this.x)+0.5,Math.floor(this.y)+0.5,Math.round(this.width),Math.round(this.height))},intersectsAt:function Rectangle_intersectsAt(x,y){jayus.debug.matchArgumentsAs("Rectangle.intersectsAt",arguments,4,"x","y");return this.x<=x&&x<=this.x+this.width&&this.y<=y&&y<=this.y+this.height},
toString:function Rectangle_toString(){return"( Rectangle: "+this.x+", "+this.y+", "+this.width+", "+this.height+" )"},getScope:function Rectangle_getScope(){return this.clone()},getTransformed:function Rectangle_getTransformed(matrix){jayus.debug.match("Rectangle.getTransformed",matrix,"matrix",10);return this.toPolygon().getTransformed(matrix)},clone:function Rectangle_clone(){return this.cloneOnto(new jayus.Rectangle)},cloneOnto:function Rectangle_cloneOnto(ret){jayus.debug.match("Rectangle.cloneOnto",
ret,"ret",20);ret.x=this.x;ret.y=this.y;ret.width=this.width;ret.height=this.height;return ret},toPolygon:function Rectangle_toPolygon(){return new jayus.Polygon.Rectangle(this.x,this.y,this.width,this.height)},etchOntoContext:function Rectangle_etchOntoContext(ctx){jayus.debug.matchContext("Rectangle.etchOntoContext",ctx);ctx.beginPath();if(this.keepAligned)ctx.rect(Math.round(this.x)+0.5,Math.round(this.y)+0.5,Math.round(this.width),Math.round(this.height));else ctx.rect(this.x,this.y,this.width,
this.height);return this}});jayus.debug.className="Circle";
...
2){radius=y;y=x.y;x=x.x}this.x=x;this.y=y;this.radius=radius}},pointChanged:function Circle_pointChanged(){},toObject:function Circle_toObject(){var object={};object.__type__="Circle";object.x=this.x;object.y=this.y;object.radius=this.radius;if(this.toPolygonDetail!==10)object.toPolygonDetail=this.toPolygonDetail;return object},initFromObject:function Circle_initFromObject(object){jayus.debug.match("Circle.initFromObject",object,"object",5);this.x=object.x;this.y=object.y;this.radius=object.radius;
if(typeof object.toPolygonDetail==="number")this.toPolygonDetail=object.toPolygonDetail;return this.dirty(jayus.DIRTY.ALL)},getCenter:function Circle_getCenter(){return new jayus.Point(this.x,this.y)},setX:jayus.Point.prototype.setX,setY:jayus.Point.prototype.setY,setCenter:jayus.Point.prototype.set,translate:function Circle_translate(x,y){jayus.debug.matchCoordinate("Circle.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setCenter(this.x+x,this.y+y)},alignToCanvas:function Circle_alignToCanvas(){return this},
getRadius:function Circle_getRadius(){return this.radius},setRadius:function Circle_setRadius(radius){jayus.debug.match("Circle.setRadius",radius,"radius",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setRadius,this.radius,radius)}if(this.radius!==radius){this.radius=radius;this.dirty(4)}return this},intersectsAt:function Circle_intersectsAt(x,y){jayus.debug.matchArgumentsAs("Circle.intersectsAt",arguments,4,"x","y");return(this.x-x)*(this.x-x)+(this.y-
y)*(this.y-y)<=this.radius*this.radius},getScope:function Circle_getScope(){return new jayus.Rectangle(this.x-this.radius,this.y-this.radius,this.radius*2,this.radius*2)},toString:function Circle_toString(){return"(Circle: "+this.x+","+this.y+","+this.radius+")"},clone:function Circle_clone(){return new jayus.Circle(this.x,this.y,this.radius)},cloneOnto:function Circle_cloneOnto(ret){ret.x=this.x;ret.y=this.y;ret.radius=this.radius;return ret},toPolygon:function Circle_toPolygon(detail){if(!arguments.length)detail=
this.toPolygonDetail;else jayus.debug.match("Circle.toPolygon",detail,"detail",4);return new jayus.Polygon.Regular(detail,this.radius)},etchOntoContext:function Circle_etchOntoContext(ctx){jayus.debug.matchContext("Circle.etchOntoContext",ctx);ctx.beginPath();ctx.arc(this.x,this.y,this.radius,0,Math.PI*2)}});jayus.debug.className="Polygon";
...
y}if(this.frame===null)this.frame=new jayus.Rectangle;this.frame.setFrame(x1,y1,x2-x1,y2-y1)},intersectsAt:function Polygon_intersectsAt(x,y){jayus.debug.matchArgumentsAs("Polygon.intersectsAt",arguments,4,"x","y");var ctx=jayus.getContext();ctx.save();this.etchOntoContext(ctx);var ret=ctx.isPointInPath(x,y);ctx.restore();return ret},setClosed:function Polygon_setClosed(on){jayus.debug.match("Polygon.setClosed",on,"on",1);if(this.closed!==on){this.closed=on;this.dirty(16)}return this},toString:function Polygon_toString(){var msg=
"(Polygon:";for(var i=0;i<this.xPoints.length;i++)msg+="("+this.xPoints[i]+","+this.yPoints[i]+"),";return msg.slice(0,msg.length-1)+")"},clone:function Polygon_clone(){return new jayus.Polygon(this.xPoints.slice(0),this.yPoints.slice(0))},cloneOnto:function Polygon_cloneOnto(ret){ret.xPoints=this.xPoints.slice(0);ret.yPoints=this.yPoints.slice(0);ret.frameDirty=true;return ret},transform:function Polygon_transform(matrix){jayus.debug.match("Polygon.transform",matrix,"matrix",10);var i,point=new jayus.Point;
for(i=0;i<this.xPoints.length;i++){matrix.transformPointOnto(this.xPoints[i],this.yPoints[i],point);this.xPoints[i]=point.x;this.yPoints[i]=point.y}this.frameDirty=true;return this},getTransformed:function Polygon_getTransformed(matrix){jayus.debug.match("Polygon.getTransformed",matrix,"matrix",10);return this.getTransformedOnto(matrix,new jayus.Polygon)},getTransformedOnto:function Polygon_getTransformedOnto(matrix,ret){jayus.debug.matchArguments("Polygon.getTransformedOnto",arguments,"matrix",10,
"ret",15);var i,point=new jayus.Point;ret.xPoints=[];ret.yPoints=[];for(i=0;i<this.xPoints.length;i++){matrix.transformPointOnto(this.xPoints[i],this.yPoints[i],point);ret.xPoints.push(point.x);ret.yPoints.push(point.y)}return ret},etchOntoContext:function Polygon_etchOntoContext(ctx){jayus.debug.matchContext("Polygon.etchOntoContext",ctx);if(this.xPoints.length){ctx.beginPath();ctx.moveTo(this.xPoints[0],this.yPoints[0]);for(var i=0;i<this.xPoints.length;i++)ctx.lineTo(this.xPoints[i],this.yPoints[i]);
if(this.closed)ctx.closePath()}return this}});jayus.Polygon.Line=function Polygon_Line(x1,y1,x2,y2){if(arguments.length===2){jayus.debug.matchArguments("Polygon.Line()",arguments,"start",13,"end",13);y2=y1.y;x2=y1.x;y1=x1.y;x1=x1.x}else jayus.debug.matchArgumentsAs("Polygon.Line",arguments,4,"x1","y1","x2","y2");return new jayus.Polygon([x1,x2],[y1,y2])};
...
jayus.debug.defaultDebugRenderer;this.exposingAll=true;if(this.isParent)this.forEachChild(function(){this.exposeAll()});this.dirty(jayus.DIRTY.ALL)},unexposeAll:function Entity_exposeAll(){this.debugRenderer=null;this.exposingAll=false;if(this.isParent)this.forEachChild(function(){this.unexposeAll()});this.dirty(jayus.DIRTY.ALL)},toObject:function Entity_toObject(){var object={};object.__type__="Entity";var i,key,val,valType;for(i=0;i<jayus.Entity.prototype.properties.length;i++){key=jayus.Entity.prototype.properties[i];
val=this[key];valType=typeof val;if(val!==jayus.Entity.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}return object},initFromObject:function Entity_initFromObject(object){jayus.debug.match("Entity.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.frozen++;var i,key,val,valType;for(i=0;i<this.properties.length;i++){key=this.properties[i];val=object[key];valType=typeof val;if(valType==="object")val=jayus.fromObject(val);this["set"+
key[0].toUpperCase()+key.slice(1)](val)}this.frozen--;return this.dirty(jayus.DIRTY.ALL)},apply:function Entity_apply(obj){jayus.debug.match("Entity.apply",obj,"type",5);for(var property in obj)if(obj.hasOwnProperty(property))if(typeof this[property]==="function")if(obj[property]instanceof Array)this[property].apply(this,obj[property]);else this[property](obj[property]);else this[property]=obj[property];return this},isTransformed:false,actionsToAnimate:0,animate:function Animatable_animate(){this.actionsToAnimate++;
return this},init:function Entity_init(){this.addHandler("dirty",function(type){if(type&2)this.matrixDirty=true});if(this.enableDragEvents)this.handle({leftPress:function(e){this.leftDragging=true;return this.fire("leftDragStart",e)},leftRelease:function(e){if(this.leftDragging){this.leftDragging=false;return this.fire("leftDragEnd",e)}},middlePress:function(e){this.middleDragging=true;return this.fire("middleDragStart",e)},middleRelease:function(e){if(this.middleDragging){this.middleDragging=false;
return this.fire("middleDragEnd",e)}},rightPress:function(e){this.rightDragging=true;return this.fire("rightDragStart",e)},rightRelease:function(e){if(this.rightDragging){this.rightDragging=false;return this.fire("rightDragEnd",e)}},cursorMove:function(e){var ret=false;if(this.leftDragging)ret=this.fire("leftDrag",e)||ret;if(this.middleDragging)ret=this.fire("middleDrag",e)||ret;if(this.rightDragging)ret=this.fire("rightDrag",e)||ret;return ret},cursorOut:function(){if(this.leftDragging)this.fire("leftDragEnd");
if(this.middleDragging)this.fire("middleDragEnd");if(this.rightDragging)this.fire("rightDragEnd");this.leftDragging=this.middleDragging=this.rightDragging=false}})},dirty:function Entity_dirty(type){jayus.debug.match("Entity.dirty",type,"type",4);if(!this.frozen){this.fire("dirty",type);this.dirtied=true;this.informDependents(type)}return this},getAbsoluteParent:function Entity_getAbsoluteParent(){return this.hasParent?this.parent.getAbsoluteParent():this},setParent:function Entity_setParent(parent){if(this.hasParent)throw new Error("Entity.setParent() - Entity already has a parent");
if(parent.exposingAll)this.expose();this.hasParent=true;this.parent=parent;this.attach(parent);this.fire("added")},removeParent:function Entity_removeParent(){this.hasParent=false;this.parent=null;this.underCursor=false;this.detach(parent);this.fire("removed")},setVisible:function Entity_setVisible(on){jayus.debug.match("Entity.setVisible",on,"on",1);if(this.visible!==on){this.visible=on;this.dirty(1)}return this},show:function Entity_show(){return this.setVisible(true)},hide:function Entity_hide(){return this.setVisible(false)},
setAlpha:function Entity_setAlpha(alpha){jayus.debug.match("Entity.setAlpha",alpha,"alpha",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setAlpha,this.alpha,alpha)}if(this.alpha!==alpha){this.alpha=alpha;this.dirty(16)}return this},updateCursor:function Entity_updateCursor(x,y){if(this.underCursor)if(!this.intersectsAt(x,y)){this.underCursor=false;this.fire("cursorOut");if(this.isParent)this.removeCursorFromChildren()}else{if(this.isParent)this.updateCursorOnChildren(x,
y)}else if(this.intersectsAt(x,y)){this.underCursor=true;this.fire("cursorOver");if(this.isParent)this.updateCursorOnChildren(x,y)}},removeCursor:function Entity_removeCursor(){if(this.underCursor){this.underCursor=false;this.fire("cursorOut");if(this.isParent)this.removeCursorFromChildren()}},setDragButton:function Entity_setDragButton(button){if(this.dragButton!==button){this.dragButton=button;if(this.draggable){this.setDraggable(false);this.setDraggable(true)}}return this},setDraggable:function Entity_setDraggable(on){jayus.debug.match("Entity.setDraggable",
on,"on",1);if(!this.draggable&&on){this.addHandler(this.dragButton+"Press",function(e){this.dragging=true;this.canReleaseCursor=false;this.fire("dragStart");jayus.interceptNextEvent(this.dragButton+"Release",function(e){this.dragging=false;this.canReleaseCursor=true;this.display.updateCursor(this.display.cursor.x,this.display.cursor.y)},{context:this});this.display=e.display;var that=this;var dragHandler=function(e){if(that.dragging)that.translate(e.deltaX,e.deltaY);else that.display.removeHandler("cursorMove",
"dragHandler");return true};e.display.addHandler("cursorMove",dragHandler,{id:"dragHandler"})});this.draggable=true}else if(this.draggable&&!on);return this},matrix:null,matrixDirty:true,getMatrix:function Entity_getMatrix(){if(this.matrixDirty){if(this.matrix===null)this.matrix=new jayus.Matrix;else this.matrix.identity();this.applyTransforms(this.matrix);this.matrixDirty=false}return this.matrix},applyTransforms:function Entity_applyTransforms(ctx){if(this.isTransformed)if(this.xAnchor||this.yAnchor){ctx.translate(this.xAnchor,
this.yAnchor);if(this.xScale!==1||this.yScale!==1)ctx.scale(this.xScale,this.yScale);if(this.angle!==0)ctx.rotate(this.angle);ctx.translate(-this.xAnchor,-this.yAnchor)}else{if(this.xScale!==1||this.yScale!==1)ctx.scale(this.xScale,this.yScale);if(this.angle!==0)ctx.rotate(this.angle)}return this},setAngle:function Entity_setAngle(angle){jayus.debug.match("Entity.setAngle",angle,"angle",4);if(this.actionsToAnimate){this.actionsToAnimate--;return jayus.MethodAnimator(this,this.setAngle,this.angle,
angle)}if(this.angle!==angle){this.angle=angle;this.isTransformed=true;this.matrixDirty=true;this.dirty(8)}return this},rotate:function Entity_rotate(angle){jayus.debug.match("Entity.rotate",angle,"angle",4);return this.setAngle(this.angle+angle)},setScale:function Entity_setScale(x,y){if(arguments.length===1)y=x;if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setScale,[this.xScale,this.yScale],[x,y])}if(this.xScale!==x||this.yScale!==y){this.xScale=x;this.yScale=
y;this.isTransformed=true;this.matrixDirty=true;this.dirty(8)}return this},scale:function Entity_scale(x,y){if(arguments.length===1){jayus.debug.match("Entity.scale",x,"scale",4);y=x}else jayus.debug.matchArgumentsAs("Entity.scale",arguments,4,"x","y");return this.setScale(this.xScale*x,this.yScale*y)},setAnchor:function Entity_setAnchor(x,y){jayus.debug.matchCoordinate("Entity.setAnchor",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.xAnchor!==x||this.yAnchor!==y){this.xAnchor=x;this.yAnchor=
y;this.matrixDirty=true;this.dirty(8)}return this},screenToLocal:function Entity_screenToLocal(x,y){jayus.debug.matchCoordinate("Entity.screenToLocal",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.hasParent){var pos=this.parent.screenToLocal(x,y);return this.parentToLocal(pos)}return new jayus.Point(x,y)},parentToLocal:function Entity_parentToLocal(x,y){jayus.debug.matchCoordinate("Entity.parentToLocal",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.hasParent){x-=this.x;y-=this.y;x/=this.xScale;
y/=this.yScale;if(this.angle!==0){var x2=x,y2=y,cos=Math.cos(this.angle),sin=Math.sin(this.angle);x=cos*x2-sin*y2;y=cos*y2+sin*x2}}return new jayus.Point(x,y)},localToScreen:function Entity_localToScreen(x,y){jayus.debug.matchCoordinate("Entity.localToScreen",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.hasParent)return this.parent.localToScreen(this.localToParent(x,y));return new jayus.Point(x,y)},localToScreenOnto:function Entity_localToScreenOnto(x,y,ret){jayus.debug.matchArguments("Entity.localToScreenOnto",
arguments,"x",4,"y",4,"ret",13);if(this.hasParent){this.localToParentOnto(x,y,ret);this.parent.localToScreenOnto(ret.x,ret.y,ret)}else{ret.x=x;ret.y=y}return ret},localToParent:function Entity_localToParent(x,y){jayus.debug.matchCoordinate("Entity.localToParent",x,y);if(arguments.length===1){y=x.y;x=x.x}var ret=new jayus.Point;this.localToParentOnto(x,y,ret);return ret},localToParentOnto:function Entity_localToParentOnto(x,y,ret){jayus.debug.matchArguments("Entity.localToParentOnto",arguments,"x",
4,"y",4,"ret",13);if(this.angle!==0){var x2=x,y2=y,cos=Math.cos(this.angle),sin=Math.sin(this.angle);x=cos*x2-sin*y2;y=cos*y2+sin*x2}x*=this.xScale;y*=this.yScale;x+=this.x;y+=this.y;ret.x=x;ret.y=y;return ret},localToParentOntoORIGINAL:function Entity_localToParentOntoORIGINAL(x,y,ret){jayus.debug.matchArguments("Entity.localToParentOntoORIGINAL",arguments,"x",4,"y",4,"ret",13);if(this.hasParent){if(this.angle!==0){var x2=x,y2=y,cos=Math.cos(this.angle),sin=Math.sin(this.angle);x=cos*x2-sin*y2;y=
cos*y2+sin*x2}x*=this.xScale;y*=this.yScale;x+=this.x;y+=this.y}ret.x=x;ret.y=y;return ret},running:false,xVelocity:0,yVelocity:0,setVelocity:function Entity_setVelocity(x,y){jayus.debug.matchCoordinate("Entity.setVelocity",x,y);if(arguments.length===1){y=x.y;x=x.x}this.xVelocity=x;this.yVelocity=y;if(!this.running){jayus.animators.push(this);this.running=true}},clearVelocity:function Entity_clearVelocity(){this.velocity=null;jayus.animators.splice(jayus.animators.indexOf(this),1);this.running=false},
tick:function Entity_tick(now,elapsed){this.translate(this.xVelocity*elapsed,this.yVelocity*elapsed)},intersects:function Entity_intersects(object){return jayus.intersectTest(this,object)},intersectCount:function Entity_intersectCount(entities){jayus.debug.matchArray("Entity.intersectCount",entities,"entities",12);if(!entities.length)throw new RangeError("Entity.intersectCount() - Invalid entities"+jayus.debug.toString(entities)+" sent, length of at least 1 required");var i,count=0;for(i=0;i<entities.length;i++)if(jayus.intersectTest(this,
entities[i]))count++;return count},intersectsAny:function Entity_intersectsAny(entities){if(entities instanceof jayus.List)entities=entities.items;jayus.debug.matchArray("Entity.intersectsAny",entities,"entities",12);for(var i=0;i<entities.length;i++)if(jayus.intersectTest(this,entities[i]))return true;return false},intersectsAll:function Entity_intersectsAll(entities){if(entities instanceof jayus.List)entities=entities.items;jayus.debug.matchArray("Entity.intersectsAll",entities,"entities",12);if(!entities.length)throw new RangeError("Entity.intersectsAll() - Invalid entities"+
jayus.debug.toString(entities)+" sent, length of at least 1 required");for(var i=0;i<entities.length;i++)if(!jayus.intersectTest(this,entities[i]))return false;return true},intersectsWhich:function Entity_intersectsWhich(entities){if(entities instanceof jayus.List)entities=entities.items;jayus.debug.matchArray("Entity.intersectsWhich",entities,"entities",12);var i,ret=[];for(i=0;i<entities.length;i++)if(jayus.intersectTest(this,entities[i]))ret.push(entities[i]);return ret},setStyle:function RectEntity_setStyle(style){jayus.debug.match("RectEntity.setStyle",
style,"style",5);if(this.hasStyle)this.style.detach(this);if(!(style instanceof jayus.Brush))style=new jayus.Brush(style);this.style=style;this.style.attach(this);this.hasStyle=true;return this.dirty(jayus.DIRTY.STYLE)},clearBg:function RectEntity_clearBg(){if(this.hasStyle){this.style.detach(this);this.style=null;this.hasStyle=false;this.dirty(jayus.DIRTY.STYLE)}return this},rasterize:function Entity_rasterize(){var scope=this.getScope(),surface=new jayus.Surface(scope.width,scope.height),x=this.x,
y=this.y;this.x=0;this.y=0;this.drawOnto(surface);this.x=x;this.y=y;return surface},drawOnto:function Entity_drawOnto(surface){var ctx=surface.context;ctx.save();this.drawOntoContext(ctx,0,0);ctx.restore();return this}});jayus.debug.className="RectEntity";
...
this[key];valType=typeof val;if(val!==jayus.RectEntity.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}if(this.bg!==null)object.bg=this.bg.toObject();if(this.bounds!==null)object.bounds=this.bounds.toObject();return object},initFromObject:function RectEntity_initFromObject(object){jayus.debug.match("RectEntity.initFromObject",object,"object",5);jayus.Entity.prototype.initFromObject.call(this,object);this.frozen++;var i,key,val,valType;for(i=0;i<this.properties.length;i++){key=
this.properties[i];this[key]=object[key]}if(typeof object.bounds==="object")this.bounds=jayus.fromObject(object.bounds);if(typeof object.bg==="object")this.bg=jayus.fromObject(object.bg);this.frozen--;return this.dirty(jayus.DIRTY.ALL)},setX:jayus.Rectangle.prototype.setX,setY:jayus.Rectangle.prototype.setY,getOrigin:jayus.Rectangle.prototype.getOrigin,setOrigin:jayus.Rectangle.prototype.setOrigin,translate:jayus.Rectangle.prototype.translate,getLeft:jayus.Rectangle.prototype.getLeft,getRight:jayus.Rectangle.prototype.getRight,
getTop:jayus.Rectangle.prototype.getTop,getBottom:jayus.Rectangle.prototype.getBottom,setLeft:jayus.Rectangle.prototype.setLeft,setRight:jayus.Rectangle.prototype.setRight,setTop:jayus.Rectangle.prototype.setTop,setBottom:jayus.Rectangle.prototype.setBottom,getPosAt:function RectEntity_getPosAt(anchorX,anchorY){jayus.debug.matchCoordinate("RectEntity.getPosAt",anchorX,anchorY);if(arguments.length===1){anchorY=anchorX.y;anchorX=anchorX.x}return this.localToParent(anchorX*this.width,anchorY*this.height)},
setPosAt:function RectEntity_setPosAt(anchorX,anchorY,x,y){if(arguments.length===3){y=x.y;x=x.x}jayus.debug.matchArgumentsAs("RectEntity.setPosAt",[anchorX,anchorY,x,y],4,"anchorX","anchorY","x","y");return this.translate(x-(this.x+anchorX*this.width),y-(this.y+anchorY*this.height))},setRelativeAnchor:function Entity_setRelativeAnchor(x,y){jayus.debug.matchCoordinate("RectEntity.setRelativeAnchor",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setAnchor(this.width*x,this.height*y)},setBody:function RectEntity_setBody(body){jayus.debug.match("RectEntity.setBody",
body,"body",5);if(this.body!==null)delete this.body.entity;else{jayus.box2d.physicalEntities.push(this);this.setAnchor(this.width/2,this.height/2)}this.body=body;this.body.entity=this;return this},removeBody:function RectEntity_removeBody(){if(this.body!==null){delete this.body.entity;this.body=null;jayus.box2d.physicalEntities.splice(jayus.box2d.physicalEntities.indexOf(this))}return this},updateFromBody:function RectEntity_updateFromBody(){var awake=this.body.IsAwake();if(awake){var pos=this.body.GetPosition();
this.setPosAt(0.5,0.5,pos.x,pos.y);this.angle=this.body.GetAngle();this.matrixDirty=true}},setWidth:function RectEntity_setWidth(width){jayus.debug.match("RectEntity.setWidth",width,"width",4);if(!this.hasFlexibleWidth)throw new Error("RectEntity.setWidth() - Entity width is not flexible");if(width!==this.width)this.changeSize(width,this.height);return this},widthDesc:null,setRemoteWidth:function RectEntity_setRemoteWidth(source,key){if(!this.hasFlexibleWidth)throw new Error("RectEntity.setRemoteWidth() - Entity width is not flexible");
...
jayus.Text=jayus.RectEntity.extend({text:"",font:"12pt sans-serif",brush:null,hasBrush:false,alignment:0,fontDesc:null,lines:null,lineWidths:null,hasFlexibleWidth:false,hasFlexibleHeight:false,init:function Text_init(text,font,brush){jayus.Entity.prototype.init.apply(this);if(arguments.length===1)jayus.debug.match("Text.init",text,"text",2);else if(arguments.length===2)jayus.debug.matchArguments("Text.init",arguments,"text",2,"font",2);else if(arguments.length===3)jayus.debug.matchArguments("Text.init",arguments,
"text",2,"font",2,"brush",5);if(arguments.length){this.text=text;if(arguments.length>1){this.font=font;if(arguments.length>2)this.setBrush(brush)}}this.refreshMetrics()},componentDirtied:function Text_componentDirtied(){this.dirty(16)},refreshMetrics:function Text_refreshMetrics(){if(this.fontDesc===null)this.fontDesc=jayus.getFontDescriptor(this.font);this.lines=this.text.split("\n");if(this.lineWidths===null)this.lineWidths=[];else this.lineWidths.length=this.lines.length;var tempRet={};for(var i=
0;i<this.lines.length;i++)this.lineWidths[i]=jayus.measureTextOnto(this.lines[i],this.font,tempRet).width;this.changeSize(jayus.math.max(this.lineWidths),this.lines.length*this.fontDesc.height)},getScope:function Text_getScope(){var scope=this.getFrame().getScope();if(this.hasBrush)if(this.brush.stroking||this.brush.shadowing){var scale=Math.max(this.xScale,this.yScale),left=0,right=0,top=0,bottom=0;if(this.brush.stroking){var width=this.brush.lineWidth*scale/2;left=right=top=bottom=Math.sqrt(width*
width*2)}if(this.brush.shadowing){left=Math.max(left,left+this.brush.shadowBlur-this.brush.shadowOffsetX);right=Math.max(right,right+this.brush.shadowBlur+this.brush.shadowOffsetX);top=Math.max(top,top+this.brush.shadowBlur-this.brush.shadowOffsetY);bottom=Math.max(bottom,bottom+this.brush.shadowBlur+this.brush.shadowOffsetY)}scope.setSize(scope.width+left+right,scope.height+top+bottom);scope.translate(-left,-top)}return scope},setText:function Text_setText(text){jayus.debug.match("Text.setText",
...
jayus.Image=jayus.RectEntity.extend({image:null,imageSource:null,hasSection:false,section:null,keepAligned:false,loaded:false,hasFlexibleWidth:false,hasFlexibleHeight:false,init:function Image_init(image){jayus.Entity.prototype.init.apply(this);if(typeof image==="string"){var filepath=image;this.imageSource=filepath;if(jayus.images.isLoaded(filepath)){this.loaded=true;this.image=jayus.images.images[filepath];this.width=this.image.width;this.height=this.image.height}else{var that=this;jayus.addHandler("imageLoaded",
function(data){if(data.filepath===filepath){that.loaded=true;that.image=data.image;if(!that.hasSection){that.width=data.image.width;that.height=data.image.height;that.dirty(4)}}});jayus.images.load(filepath)}}else{this.loaded=true;this.image=image;this.width=this.image.width;this.height=this.image.height}},toObject:function Image_toObject(){var object=jayus.RectEntity.prototype.toObject.apply(this);object.__type__="Image";if(this.section!==null)object.section=this.section.toObject();if(this.keepAligned!==
false)object.keepAligned=this.keepAligned;if(this.image instanceof HTMLImageElement)object.image=this.imageSource;else if(this.image instanceof jayus.Surface)object.image=this.image.toObject();delete object.width;delete object.height;return object},initFromObject:function Image_initFromObject(object){this.RectEntity.prototype.initFromObject.call(this,object);this.hasSection=typeof object.section==="object";if(this.hasSection)this.section=new jayus.Rectangle(object.section);if(typeof object.keepAligned===
"boolean")this.keepAligned=object.keepAligned;if(typeof object.image==="string")this.setSource(object.image);else if(typeof object.image==="object")this.setSource(new jayus.Surface(object.image));return this.dirty(jayus.DIRTY.ALL)},setSource:function Image_setSource(image){if(typeof image==="string"){var filepath=image;if(jayus.images.isLoaded(filepath)){this.loaded=true;this.image=jayus.images.images[filepath];this.width=this.image.width;this.height=this.image.height;this.dirty(4)}else{var that=
this;jayus.images.load(filepath,function(source,image){that.loaded=true;that.image=jayus.images.images[filepath];that.width=image.width;that.height=image.height;that.dirty(4)})}}else{this.loaded=true;this.image=image;this.width=this.image.width;this.height=this.image.height;this.dirty(4)}},setSection:function Image_setSection(x,y,width,height){if(arguments.length===1){jayus.debug.match("Image.setSection()",x,"section",20);height=x.height;width=x.width;y=x.y;x=x.x}else if(arguments.length===3){jayus.debug.matchArguments("Image.setSection()",
...
jayus.Sprite=jayus.Image.extend({animation:null,animator:null,spriteIndexX:null,spriteIndexY:null,toObject:function Sprite_toObject(){var object=jayus.Image.prototype.toObject.apply(this);object.__type__="Sprite";if(this.spriteIndexX!==null)object.spriteIndexX=this.spriteIndexX;if(this.spriteIndexY!==null)object.spriteIndexY=this.spriteIndexY;return object},initFromObject:function Sprite_initFromObject(object){jayus.Image.prototype.initFromObject.apply(this);if(typeof object.spriteIndexX==="number")this.spriteIndexX=
object.spriteIndexX;if(typeof object.spriteIndexY==="number")this.spriteIndexY=object.spriteIndexY;return this.dirty(jayus.DIRTY.ALL)},setSpriteSheet:function Sprite_setSpriteSheet(sheet){this.sheet=sheet;return this},setSprite:function Sprite_setSprite(x,y){jayus.debug.matchCoordinate("Sprite.setSprite",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.sheet!==null||typeof this.image.sheet==="object"){if(this.spriteIndexX!==x||this.spriteIndexY!==y){var sheet=this.sheet||this.image.sheet;this.spriteIndexX=
x;this.spriteIndexY=y;this.setSection(sheet.marginX+x*sheet.spriteWidth,sheet.marginY+y*sheet.spriteHeight,sheet.spriteWidth,sheet.spriteHeight);this.dirty(4)}}else console.warn("Sprite.setSprite() - Image does not contain attached spritesheet");return this},playAnimation:function Sprite_playAnimation(name,func){jayus.debug.match("Sprite.playAnimation",name,"name",2);jayus.debug.matchOptional("Sprite.playAnimation",func,"func",3);this.setAnimation(name,false);if(arguments.length>1)this.animator.addHandler("finished",
func);return this},loopAnimation:function Sprite_loopAnimation(name){jayus.debug.match("Sprite.loopAnimation",name,"name",2);return this.setAnimation(name,true)},stopAnimation:function Sprite_stopAnimation(){if(this.animator!==null)this.animator.stop()},setAnimation:function Sprite_setAnimation(name,looped){jayus.debug.matchArguments("Sprite.setAnimation",arguments,"name",2,"looped",1);if(this.sheet!==null||typeof this.image.sheet==="object"){var sheet=this.sheet||this.image.sheet;if(sheet.hasAnimation(name)){if(this.animator!==
null)this.animator.stop();this.animation=name;var data=sheet.animations[name],sequence=data.sprites,sprite=this;if(data.flipX||data.flipY){this.setAnchor(this.width/2,this.height/2);if(data.flipX)this.setScale(-1,this.yScale);else this.setScale(1,this.yScale);if(data.flipY)this.setScale(this.xScale,-1);else this.setScale(this.xScale,1)}else this.setScale(1,1);this.animator=new jayus.Animator.Discrete(sequence.length,function(index){sprite.setSprite(sequence[index])});this.animator.setDuration(sheet.animations[name].duration).setLooped(looped).start()}else throw new Error("Sprite.setAnimation() - Spritesheet does not contain sprite/animation: "+
...
this.redrawAll=true;this.dirty(16)},formContents:function Scene_formContents(width,height){if(this.buffered){this.canvas.width=width;this.canvas.height=height}},init:function Scene_init(width,height){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);this.children.typeId=12;if(arguments.length){jayus.debug.matchArgumentsAs("Scene.init",arguments,4,"width","height");this.width=width;this.height=height}},toObject:function Scene_toObject(){var object=jayus.RectEntity.prototype.toObject.apply(this);
jayus.groupToObject.call(this,object);object.__type__="Scene";if(this.optimizeBuffering!==jayus.Scene.prototype.optimizeBuffering)object.optimizeBuffering=this.optimizeBuffering;if(this.groupDamagedRegions!==jayus.Scene.prototype.groupDamagedRegions)object.groupDamagedRegions=this.groupDamagedRegions;if(this.damagedRegionPadding!==jayus.Scene.prototype.damagedRegionPadding)object.damagedRegionPadding=this.damagedRegionPadding;return object},initFromObject:function Scene_initFromObject(object){jayus.debug.match("Scene.initFromObject",
object,"object",5);this.frozen++;jayus.RectEntity.prototype.initFromObject.call(this,object);jayus.groupFromObject.call(this,object);if(typeof object.optimizeBuffering==="boolean")this.optimizeBuffering=object.optimizeBuffering;if(typeof object.groupDamagedRegions==="boolean")this.groupDamagedRegions=object.groupDamagedRegions;if(typeof object.damagedRegionPadding==="number")this.damagedRegionPadding=object.damagedRegionPadding;this.frozen--;this.dirty(jayus.DIRTY.ALL)},optimizeBuffering:false,groupDamagedRegions:true,
damagedRegionPadding:2,setOptimizedBuffering:function Scene_setOptimizedBuffering(on){jayus.debug.match("Scene.setOptimizedBuffering",on,"on",1);if(this.optimizeBuffering!==on){this.optimizeBuffering=on;this.redrawAll=true;this.dirty(16)}return this},refreshBuffer:function Scene_refreshBuffer(){if(this.buffered){var width=this.width,height=this.height,canvas=this.canvas,ctx=this.context;if(width!==canvas.width||height!==canvas.height){canvas.width=width;canvas.height=height}if(this.optimizeBuffering&&
!this.redrawAll)this.optimizedRefreshBuffer();else{this.fullyRefreshBuffer();this.redrawAll=false}this.dirtied=false}},optimizedRefreshBuffer:function Scene_optimizedRefreshBuffer(){if(this.buffered){var ctx=this.context,i,item,region,damagedRegions=[];for(i=0;i<this.children.items.length;i++){item=this.children.items[i];item.clean=true;if(item.scopeChanged){damagedRegions.push(item.getScope().includeRectangle(item.prevScope));item.prevScope=item.getScope();item.scopeChanged=false;item.clean=false}else if(item.dirtied){damagedRegions.push(item.getScope());
...
jayus.Box=jayus.RectEntity.extend({width:100,height:100,spacing:8,reversed:false,isParent:true,forming:false,init:function Box_init(){jayus.Entity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function Box_dirtyHandler(type){if(type&4)this.formContents()})},componentDirtied:function Box_componentDirtied(component,type){if(!this.forming)this.dirty(jayus.DIRTY.ALL)},listItemAdded:function Box_listItemAdded(list,item){item.setParent(this);
this.dirty(jayus.DIRTY.ALL)},listItemsAdded:function Box_listItemsAdded(list,items){for(var i=0;i<items.length;i++)items[i].setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemRemoved:function Box_listItemRemoved(list,item){item.removeParent();this.dirty(jayus.DIRTY.ALL)},listItemsRemoved:function Box_listItemsRemoved(list,items){for(var i=0;i<items.length;i++)items[i].removeParent();this.dirty(jayus.DIRTY.ALL)},setReversed:function Box_setReversed(on){jayus.debug.match("Box.setReversed",on,"on",
1);if(this.reversed!==on){this.reversed=on;this.formContents()}return this},setSpacing:function Box_setSpacing(spacing){jayus.debug.match("Box.setSpacing",spacing,"spacing",4);if(this.spacing!==spacing){this.spacing=spacing;this.formContents()}return this},findGreatestMinimalChildWidth:function Box_findGreatestMinimalChildWidth(){var width=0;for(var i=0;i<this.children.items.length;i++)width=Math.max(width,this.children.items[i].minW);return width},findGreatestMinimalChildHeight:function Box_findGreatestMinimalChildHeight(){var height=
0;for(var i=0;i<this.children.items.length;i++)height=Math.max(height,this.children.items[i].minH);return height},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.Box.prototype);jayus.debug.className="hBox";
jayus.hBox=jayus.Box.extend({formContents:function hBox_formContents(){this.forming=true;var i,item,x=0,space=this.width-(this.children.items.length-1)*this.spacing,totalWeight=0,totalFixedSize=0,itemWidth,itemHeight;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(typeof item.policy!=="object")item.policy=new jayus.SizePolicy;if(item.hasFlexibleWidth){totalFixedSize+=item.policy.size;if(item.policy.expand)totalWeight+=item.policy.weight}else totalFixedSize+=item.width}var extraSpace=
space-totalFixedSize;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(item.hasFlexibleHeight)itemHeight=this.height;else itemHeight=item.height;if(item.hasFlexibleWidth)if(item.policy.expand)itemWidth=item.policy.size+extraSpace*(item.policy.weight/totalWeight);else itemWidth=item.policy.size;else itemWidth=item.width;item.x=x;x+=itemWidth+this.spacing;item.changeSize(itemWidth,itemHeight)}this.forming=false}});jayus.debug.className="vBox";
jayus.vBox=jayus.Box.extend({formContents:function vBox_formContents(){this.forming=true;var i,item,y=0,space=this.height-(this.children.items.length-1)*this.spacing,totalWeight=0,totalFixedSize=0,itemWidth,itemHeight;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(typeof item.heightPolicy!=="object")item.heightPolicy=new jayus.SizePolicy;if(item.hasFlexibleHeight){totalFixedSize+=item.heightPolicy.size;if(item.heightPolicy.expand)totalWeight+=item.heightPolicy.weight}else totalFixedSize+=
item.height}var extraSpace=space-totalFixedSize;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(item.hasFlexibleWidth)itemWidth=this.width;else itemWidth=item.width;if(item.hasFlexibleHeight)if(item.heightPolicy.expand)itemHeight=item.heightPolicy.size+extraSpace*(item.heightPolicy.weight/totalWeight);else itemHeight=item.heightPolicy.size;else itemHeight=item.height;item.y=y;y+=itemHeight+this.spacing;item.changeSize(itemWidth,itemHeight)}this.forming=false}});jayus.debug.className="TiledMap";
