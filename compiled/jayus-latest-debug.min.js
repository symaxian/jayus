...
2)this.depdendents=this.dependents[1];else this.dependents.splice(this.dependents.indexOf(dependent),1);this.dependentCount--}},dirty:function Dependency_dirty(type){if(!this.frozen)this.informDependents(type)},informDependents:function Dependency_informDependents(type){if(this.dependentCount)if(this.dependentCount===1){jayus.debug.verifyMethod(this.dependents,"componentDirtied");this.dependents.componentDirtied(this,type)}else for(var i=0;i<this.dependentCount;i++){jayus.debug.verifyMethod(this.dependents[i],
"componentDirtied");this.dependents[i].componentDirtied(this,type)}},forEachDependent:function Dependency_forEachDependent(func,args){if(this.dependentCount===1)func.apply(this.dependents,args);else for(var i=0;i<this.dependentCount;i++)func.apply(this.dependents[i],args)}});jayus.debug.className="List";
...
apply:function Brush_apply(styling){jayus.debug.match("Brush.apply",styling,"styling",5);this.frozen++;for(var key in styling)if(styling.hasOwnProperty(key))this["set"+key[0].toUpperCase()+key.slice(1)](styling[key]);this.frozen--;this.dirty(jayus.DIRTY.STYLE);return this},findStyleType:function Brush_findStyleType(style){if(style===null)return 0;if(typeof style==="string")return 1;if(style instanceof CanvasGradient)return 2;if(style instanceof CanvasPattern)return 3;if(style instanceof jayus.Color)return 4;
if(style instanceof jayus.LinearGradient)return 5;if(style instanceof jayus.RadialGradient)return 6;throw new Error("Style.findStyleType() - Invalid fill/stroke style: "+jayus.debug.toString(style));},getNativeStyle:function Brush_getNativeStyle(type,style){if(type<=3)return style;if(type===4)return style.toCSS();if(type===5||type===6)return style.getNative();throw new Error("Style.applyTo() - Invalid fill/stroke type identifier: "+jayus.debug.toString(type));},applyTo:function Brush_applyTo(ctx){if(this.alpha!==
1)ctx.globalAlpha*=this.alpha;if(this.filling)ctx.fillStyle=this.getNativeStyle(this.fillType,this.fill);if(this.stroking){ctx.strokeStyle=this.getNativeStyle(this.strokeType,this.stroke);if(this.lineWidth!==null)ctx.lineWidth=this.lineWidth;if(this.lineCap!==null)ctx.lineCap=this.lineCap;if(this.lineJoin!==null)ctx.lineJoin=this.lineJoin;if(this.miterLimit!==null)ctx.miterLimit=this.miterLimit;if(this.lineDash!==null&&jayus.ua.lineDash){ctx.setLineDash(this.lineDash);if(this.lineDashOffset!==null)ctx.lineDashOffset=
this.lineDashOffset}}if(this.shadowType){if(this.shadowType===1)ctx.shadowColor=this.shadow;else if(this.shadowType===2)ctx.shadowColor=this.shadow.toCSS();else throw new Error("Style.applyTo() - Invalid shadow type identifier: "+jayus.debug.toString(this.shadowType));if(this.shadowOffsetX!==null)ctx.shadowOffsetX=this.shadowOffsetX;if(this.shadowOffsetY!==null)ctx.shadowOffsetY=this.shadowOffsetY;if(this.shadowBlur!==null)ctx.shadowBlur=this.shadowBlur}},paintShape:function Brush_paintShape(ctx){this.applyTo(ctx);
if(this.stroking&&this.strokeFirst)ctx.stroke();if(this.filling)ctx.fill();if(this.stroking&&!this.strokeFirst)ctx.stroke()},paintRect:function Brush_paintRect(ctx,x,y,width,height){ctx.save();this.applyTo(ctx);if(this.stroking&&this.strokeFirst)ctx.strokeRect(x,y,width,height);if(this.filling)ctx.fillRect(x,y,width,height);if(this.stroking&&!this.strokeFirst)ctx.strokeRect(x,y,width,height);ctx.restore()},setAlpha:function Brush_setAlpha(alpha){if(alpha!==null)jayus.debug.match("Brush.setAlpha",
alpha,"alpha",4);if(alpha===null)alpha=1;if(this.alpha!==alpha){this.alpha=alpha;this.dirty(jayus.DIRTY.STYLE)}return this},setFill:function Brush_setFill(style){if(style!==null)jayus.debug.match("Brush.setFill",style,"style",0);if(this.fillType>=4)this.fill.detach(this);this.fill=style;this.fillType=this.findStyleType(style);if(this.fillType>=4)this.fill.attach(this);this.filling=!!this.fillType;this.dirty(jayus.DIRTY.STYLE);return this},setStroke:function Brush_setStroke(style){if(style!==null)jayus.debug.match("Brush.setStroke",
...
this.setPosAt(0.5,0.5,pos.x,pos.y);this.angle=this.body.GetAngle();this.matrixDirty=true}},setWidth:function RectEntity_setWidth(width){jayus.debug.match("RectEntity.setWidth",width,"width",4);if(width!==this.width){this.width=width;this.dirty(4)}return this},setHeight:function RectEntity_setHeight(height){jayus.debug.match("RectEntity.setHeight",height,"height",4);if(height!==this.height){this.height=height;this.dirty(4)}return this},setSize:function RectEntity_setSize(width,height){jayus.debug.matchSize("RectEntity.setSize",
width,height);if(arguments.length===1){height=width.height;width=width.width}this.width=width;this.height=height;this.dirty(4);return this},intersectsAt:function RectEntity_intersectsAt(x,y){return this.getBounds().intersectsAt(x,y)},getUnFrame:function RectEntity_getUnFrame(){return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getFrame:function RectEntity_getFrame(){if(this.isTransformed)return(new jayus.Polygon.Rectangle(0,0,this.width,this.height)).transform(this.getMatrix());return new jayus.Rectangle(this.x,
this.y,this.width,this.height)},getScope:function RectEntity_getScope(){return this.getFrame().getScope()},getBounds:function RectEntity_getBounds(){if(this.bounds!==null){this.bounds.cloneOnto(this.boundsClone);return this.boundsClone.translate(this.x,this.y)}return this.getFrame().getScope()},setBounds:function RectEntity_setBounds(shape){jayus.debug.match("RectEntity.setBounds",shape,"shape",19);if(this.bounds!==null)this.bounds.detach(this);this.bounds=shape;this.bounds.attach(this);this.boundsClone=
this.bounds.clone();this.shapeType=0;return this},clearBounds:function RectEntity_clearBounds(){if(this.bounds!==null){this.bounds.detach(this);this.bounds=null;this.boundsClone=null;this.shapeType=4}return this},componentDirtied:function RectEntity_componentDirtied(component,type){if(component===this.bounds)this.bounds.cloneOnto(this.boundsClone)},setBg:function RectEntity_setBg(brush){jayus.debug.match("RectEntity.setBg",brush,"brush",5);if(this.hasBg)this.bg.detach(this);if(!(brush instanceof jayus.Brush))brush=
new jayus.Brush(brush);this.bg=brush;this.bg.attach(this);this.hasBg=true;return this.dirty(jayus.DIRTY.BACKGROUND)},clearBg:function RectEntity_clearBg(){if(this.hasBg){this.bg.detach(this);this.bg=null;this.hasBg=false;this.dirty(jayus.DIRTY.BACKGROUND)}return this},buffered:false,bufferBg:true,canvas:null,context:null,setBuffering:function RectEntity_setBuffering(on){jayus.debug.match("RectEntity.setBuffering",on,"on",1);if(!this.buffered&&on){this.canvas=document.createElement("canvas");this.context=
this.canvas.getContext("2d");this.refreshBuffer()}else if(this.buffered&&!on){this.canvas=null;this.context=null}this.buffered=on;return this},refreshBuffer:function RectEntity_refreshBuffer(){if(this.buffered){var canvas=this.canvas;if(this.width!==canvas.width||this.height!==canvas.height){canvas.width=this.width;canvas.height=this.height}this.fullyRefreshBuffer()}},fullyRefreshBuffer:function RectEntity_fullyRefreshBuffer(){if(this.buffered){var ctx=this.context;ctx.clearRect(0,0,this.width,this.height);
ctx.save();if(this.bufferBg&&this.hasBg)this.bg.paintRect(ctx,0,0,this.width,this.height);this.paintContents(ctx);ctx.restore();if(this.showDamage)this.paintDamage(this)}},keepAligned:false,applyTransforms:function RectEntity_applyTransforms(ctx){if(this.xScale!==1||this.yScale!==1||this.angle)if(this.xAnchor||this.yAnchor){if(this.keepAligned)ctx.translate(Math.round(this.x)+this.xAnchor,Math.round(this.y)+this.yAnchor);else ctx.translate(this.x+this.xAnchor,this.y+this.yAnchor);if(this.xScale!==
1||this.yScale!==1)ctx.scale(this.xScale,this.yScale);if(this.angle!==0)ctx.rotate(this.angle);ctx.translate(-this.xAnchor,-this.yAnchor)}else{if(this.keepAligned)ctx.translate(Math.round(this.x),Math.round(this.y));else ctx.translate(this.x,this.y);if(this.xScale!==1||this.yScale!==1)ctx.scale(this.xScale,this.yScale);if(this.angle!==0)ctx.rotate(this.angle)}else if(this.x||this.y)if(this.keepAligned)ctx.translate(Math.round(this.x),Math.round(this.y));else ctx.translate(this.x,this.y);return this},
drawOntoContext:function RectEntity_drawOntoContext(ctx){jayus.debug.matchContext("RectEntity.drawOntoContext",ctx);ctx.save();if(this.alpha!==1)ctx.globalAlpha*=this.alpha;this.applyTransforms(ctx);if(this.buffered){if(this.dirtied)this.refreshBuffer();if(!this.bufferBg&&this.hasBg)if(this.alignBg)this.bg.paintRect(ctx,0.5,0.5,Math.round(this.width),Math.round(this.height));else this.bg.paintRect(ctx,0,0,this.width,this.height);ctx.drawImage(this.canvas,0,0)}else{if(this.hasBg)if(this.alignBg)this.bg.paintRect(ctx,
0.5,0.5,Math.round(this.width),Math.round(this.height));else this.bg.paintRect(ctx,0,0,this.width,this.height);this.paintContents(ctx)}this.dirtied=false;ctx.restore();if(this.debugRenderer!==null)this.debugRenderer(ctx);return this}});jayus.debug.className="PaintedShape";
...
else this.lineWidths.length=this.lines.length;var tempRet={};for(var i=0;i<this.lines.length;i++)this.lineWidths[i]=jayus.measureTextOnto(this.lines[i],this.font,tempRet).width;this.setSize(jayus.math.max(this.lineWidths),this.lines.length*this.fontDesc.height)},getScope:function Text_getScope(){var scope=this.getFrame().getScope();if(this.hasBrush)if(this.brush.stroking||this.brush.shadowing){var scale=Math.max(this.xScale,this.yScale),left=0,right=0,top=0,bottom=0;if(this.brush.stroking){var width=
...
jayus.TextBox=jayus.Text.extend({init:function TextBox_init(){jayus.Text.prototype.init.apply(this,arguments);this.addHandler("dirty",function(type){if(type&4)this.refreshMetrics()})},hasFlexibleWidth:function TextBox_hasFlexibleWidth(){return true},hasFlexibleHeight:function TextBox_hasFlexibleHeight(){return true},refreshMetrics:function TextBox_refreshMetrics(){if(this.fontDesc===null)this.fontDesc=jayus.getFontDescriptor(this.font);var tempRet={},font=this.font,getWordLen=function(str){return jayus.measureTextOnto(str,
font,tempRet).width},divideText=function(str,delims){var i,out=[],curr="";for(i=0;i<str.length;i++)if(delims.indexOf(str[i])>=0){out.push(curr);out.push(str[i]);curr=""}else curr+=str[i];out.push(curr);return out},lines=[],lineWidths=[],words=divideText(this.text," \n"),i=0,currentLine,currentLineLength,nextWord,nextWordLength;while(i!==words.length){currentLine="\n";currentLineLength=0;nextWord=words[i];if(nextWord===" "||nextWord==="\n")nextWordLength=0;else nextWordLength=getWordLen(nextWord);
do{if(nextWord==="\n"){i++;break}currentLine+=nextWord;currentLineLength+=nextWordLength;i++;if(i===words.length)break;nextWord=words[i];nextWordLength=getWordLen(nextWord)}while(currentLineLength+nextWordLength<this.width);if(currentLine.length){lines.push(currentLine);lineWidths.push(currentLineLength)}}this.lines=lines;this.lineWidths=lineWidths}});jayus.debug.className="Grid";
...
this.setChild(child);this.addHandler("dirty",function(type){if(type&4||type&2)this.formContents()})},setChild:function Frame_setChild(child){jayus.Wrapper.setChild.call(this,child);this.setSize(child.width+this.marginLeft+this.marginRight,child.height+this.marginTop+this.marginBottom);return this},componentDirtied:function Frame_componentDirtied(component,type){if(!this.forming)if(type&4)this.setSize(this.child.width+this.marginLeft+this.marginRight,this.child.height+this.marginTop+this.marginBottom);
else this.dirty(jayus.DIRTY.ALL)},setBrush:function Frame_setBrush(brush){jayus.debug.match("Frame.setBrush",brush,"brush",5);if(this.hasBrush)this.brush.detach(this);if(!(brush instanceof jayus.Brush))brush=new jayus.Brush(brush);this.brush=brush;this.brush.attach(this);this.hasBrush=true;return this.dirty(16)},clearBrush:function Frame_clearBrush(){if(this.hasBrush){this.brush.detach(this);this.brush=null;this.hasBrush=false;this.dirty(16)}return this},hasFlexibleWidth:function Frame_hasFlexibleWidth(){return this.child.hasFlexibleWidth()},
hasFlexibleHeight:function Frame_hasFlexibleHeight(){return this.child.hasFlexibleHeight()},formContents:function Frame_formContents(){this.forming=true;this.child.setOrigin(this.marginLeft,this.marginTop);this.child.setSize(this.width-this.marginLeft-this.marginRight,this.height-this.marginTop-this.marginBottom);this.forming=false},setMargin:function Frame_setMargin(left,right,top,bottom){if(arguments.length===1){jayus.debug.match("Frame.setMargin",left,"margin",4);bottom=top=right=left}else jayus.debug.matchArgumentsAs("Frame.setMargin",
arguments,4,"left","right","top","bottom");this.marginLeft=left;this.marginRight=right;this.marginTop=top;this.marginBottom=bottom;this.child.frozen++;if(this.child.hasFlexibleWidth())this.child.setWidth(this.width-this.marginLeft-this.marginRight);else this.setSize(this.child.width+left+right,this.child.height);if(this.child.hasFlexibleHeight())this.child.setHeight(this.height-this.marginTop-this.marginBottom);else this.setSize(this.child.width,this.child.height+top+bottom);this.child.frozen--;return this},
setMarginLeft:function Frame_setMarginLeft(margin){jayus.debug.match("Frame.setMarginLeft",margin,"margin",4);return this.setMargin(margin,this.marginRight,this.marginTop,this.marginBottom)},setMarginRight:function Frame_setMarginRight(margin){jayus.debug.match("Frame.setMarginRight",margin,"margin",4);return this.setMargin(this.marginLeft,margin,this.marginTop,this.marginBottom)},setMarginTop:function Frame_setMarginTop(margin){jayus.debug.match("Frame.setMarginTop",margin,"margin",4);return this.setMargin(this.marginLeft,
this.marginRight,margin,this.marginBottom)},setMarginBottom:function Frame_setMarginBottom(margin){jayus.debug.match("Frame.setMarginBottom",margin,"margin",4);return this.setMargin(this.marginLeft,this.marginRight,this.marginTop,margin)},paintContents:function Frame_paintContents(ctx){jayus.debug.matchContext("Frame.paintContents",ctx);if(this.hasBrush)if(this.brush.fill||this.brush.stroking){ctx.save();this.brush.applyTo(ctx);ctx.beginPath();ctx.rect(0,0,this.width,this.height);if(this.brush.stroking&&
this.brush.strokeFirst)ctx.stroke();if(this.brush.filling)ctx.fill();if(this.brush.stroking&&!this.brush.strokeFirst)ctx.stroke();ctx.restore()}ctx.save();this.child.drawOntoContext(ctx);this.child.dirtied=false;ctx.restore()}},jayus.copyObject(jayus.Wrapper)));jayus.debug.className="EditableFrame";
...
jayus.FlowLayout=jayus.RectEntity.extend({alignment:0,width:100,height:100,forming:false,init:function FlowLayout_init(){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function(type){if(type&4)this.formContents()})},hasFlexibleWidth:function Scene_hasFlexibleWidth(){return true},hasFlexibleHeight:function Scene_hasFlexibleHeight(){return false},componentDirtied:function Box_componentDirtied(component,type){if(!this.forming)this.dirty(jayus.DIRTY.ALL)},
listItemAdded:function FlowLayout_listItemAdded(list,item){item.setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemsAdded:function FlowLayout_listItemsAdded(list,items){for(var i=0;i<items.length;i++)items[i].setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemRemoved:function FlowLayout_listItemRemoved(list,item){item.removeParent();this.dirty(jayus.DIRTY.ALL)},listItemsRemoved:function FlowLayout_listItemsRemoved(list,items){for(var i=0;i<items.length;i++)items[i].removeParent();this.dirty(jayus.DIRTY.ALL)},
setAlignment:function FlowLayout_setAlignment(alignment){jayus.debug.match("FlowLayout.setAlignment",alignment,"alignment",4);if(this.alignment!==alignment){this.alignment=alignment;this.formContents()}return this},getAlignment:function FlowLayout_getAlignment(){return this.alignment},formContents:function FlowLayout_formContents(){this.forming=true;var i=0,j,y=0,currentLine,currentLineWidth,h,x,item,nextItem,nextItemWidth,lineHeights=[];while(i!==this.children.items.length){currentLine=[];currentLineWidth=
0;h=0;x=0;nextItem=this.children.items[i];nextItemWidth=nextItem.width;do{nextItem.x=x;nextItem.y=y;currentLine.push(nextItem);currentLineWidth+=nextItemWidth;x+=nextItemWidth;if(nextItem.height>h)h=nextItem.height;i++;if(i===this.children.items.length)break;nextItem=this.children.items[i];nextItemWidth=nextItem.width}while(currentLineWidth+nextItemWidth<this.width);if(this.alignment!==0)for(j=0;j<currentLine.length;j++){item=currentLine[j];item.x+=(this.width-currentLineWidth)*this.alignment}for(j=
0;j<currentLine.length;j++){item=currentLine[j];if(typeof item.verticalAlign==="number")item.y=item.y+item.verticalAlign*(h-item.height)}y+=h;lineHeights.push(h)}for(i=0;i<this.children.items.length;i++){item=this.children.items[i];item.dirty(2)}this.lineHeights=lineHeights;this.forming=false},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.FlowLayout.prototype);jayus.debug.className="Stack";
jayus.Stack=jayus.RectEntity.extend({spacing:8,reversed:false,isParent:true,init:function Stack_init(){jayus.RectEntity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function(type){if(type&4);})},listItemAdded:function Stack_listItemAdded(list,item){this.formContents()},listItemRemoved:function Stack_listItemRemoved(list,item){this.formContents()},listItemsAdded:function Stack_listItemsAdded(list,items){this.formContents()},
listItemsRemoved:function Stack_listItemsRemoved(list,items){this.formContents()},hasFlexibleWidth:function Stack_hasFlexibleWidth(){return false},hasFlexibleHeight:function Stack_hasFlexibleHeight(){return false},setReversed:function Stack_setReversed(on){jayus.debug.match("Stack.setReversed",on,"on",1);if(this.reversed!==on){this.reversed=on;this.formContents()}return this},setSpacing:function Stack_setSpacing(spacing){jayus.debug.match("Stack.setSpacing",spacing,"spacing",4);if(this.spacing!==
spacing){this.spacing=spacing;this.formContents()}return this},findGreatestMinimalChildWidth:function Stack_findGreatestMinimalChildWidth(){var i,width=0;for(i=0;i<this.children.items.length;i++)width=Math.max(width,this.children.items[i].minW);return width},findGreatestMinimalChildHeight:function Stack_findGreatestMinimalChildHeight(){var i,height=0;for(i=0;i<this.children.items.length;i++)height=Math.max(height,this.children.items[i].minH);return height},reform:function Stack_reform(){this.formContents(this.width,
this.height)},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.Stack.prototype);
...
heigth=this.height;if(this.automaticHeight)height=h;this.setSize(width,height)}});
...
h+=item.height+this.spacing;if(this.automaticWidth){var newW=item.width;if(newW>w)w=newW}}var height=h-this.spacing,width=this.width;if(this.automaticWidth)width=w;this.setSize(width,height)}});jayus.debug.className="Box";
jayus.Box=jayus.RectEntity.extend({width:100,height:100,spacing:8,reversed:false,isParent:true,forming:false,init:function Box_init(){jayus.Entity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function(type){if(type&4)this.formContents()})},componentDirtied:function Box_componentDirtied(component,type){if(!this.forming)this.dirty(jayus.DIRTY.ALL)},listItemAdded:function Box_listItemAdded(list,item){item.setParent(this);this.dirty(jayus.DIRTY.ALL)},
listItemsAdded:function Box_listItemsAdded(list,items){for(var i=0;i<items.length;i++)items[i].setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemRemoved:function Box_listItemRemoved(list,item){item.removeParent();this.dirty(jayus.DIRTY.ALL)},listItemsRemoved:function Box_listItemsRemoved(list,items){for(var i=0;i<items.length;i++)items[i].removeParent();this.dirty(jayus.DIRTY.ALL)},setReversed:function Box_setReversed(on){jayus.debug.match("Box.setReversed",on,"on",1);if(this.reversed!==on){this.reversed=
on;this.formContents()}return this},setSpacing:function Box_setSpacing(spacing){jayus.debug.match("Box.setSpacing",spacing,"spacing",4);if(this.spacing!==spacing){this.spacing=spacing;this.formContents()}return this},findGreatestMinimalChildWidth:function Box_findGreatestMinimalChildWidth(){var width=0;for(var i=0;i<this.children.items.length;i++)width=Math.max(width,this.children.items[i].minW);return width},findGreatestMinimalChildHeight:function Box_findGreatestMinimalChildHeight(){var height=
...
item.policy.size+extraSpace*(item.policy.weight/totalWeight);else itemHeight=item.policy.size;else itemHeight=item.height;item.y=y;y+=itemHeight+this.spacing;item.setSize(itemWidth,itemHeight)}this.forming=false}});jayus.debug.className="TiledMap";
