...
dest){this.debug.matchArguments("jayus.applyObject",arguments,"src",5,"dest",5);for(var item in src)if(src.hasOwnProperty(item))dest[item]=src[item];return dest},createClass:function jayus_createClass(props){var constructor=function(){this.init.apply(this,arguments)};if(typeof jayus.debug.className==="string")constructor=eval("(function "+jayus.debug.className+"(){this.init.apply(this,arguments);})");jayus.debug.className=null;constructor.prototype=props;if(typeof constructor.prototype.init!=="function")constructor.prototype.init=
function(){};constructor.init=function HelperInit(object,args){constructor.prototype.init.apply(object,args)};constructor.extend=jayus.extendMethod;return constructor},extendMethod:function jayus_extendMethod(props){var constructor=function(){this.init.apply(this,arguments)};if(typeof jayus.debug.className==="string")constructor=eval("(function "+jayus.debug.className+"(){this.init.apply(this,arguments);})");jayus.debug.className=null;constructor.prototype=Object.create(this.prototype);jayus.applyObject(props,
constructor.prototype);if(typeof constructor.prototype.init!=="function")constructor.prototype.init=function(){};constructor.init=function HelperInit(object,args){constructor.prototype.init.apply(object,args)};constructor.extend=jayus.extendMethod;return constructor},getUrlParameters:function jayus_getUrlParameters(){for(var i=0,vars={},param,paramArray=window.location.href.slice(window.location.href.indexOf("?")+1).split("&");i<paramArray.length;i++){param=paramArray[i].split("=");vars[param[0]]=
...
window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame;this.ua.requestAnimationFrame=typeof window.requestAnimationFrame==="function";this.keyboard.init();this.colors.cssNames=[];for(var i=0;i<this.colors.displayNames.length;i++)this.colors.cssNames.push(this.colors.displayNames[i].replace(" ","").replace(" ","").toLowerCase());this.initialized=true},detectBrowser:function jayus_detectBrowser(){var dataBrowser=[{src:navigator.userAgent,
...
{src:navigator.userAgent,str:"Gecko",id:"Mozilla",ver:"rv"},{src:navigator.userAgent,str:"Mozilla",id:"Netscape",ver:"Mozilla"}];var dataOS=[{src:navigator.platform,str:"Win",id:"Windows"},{src:navigator.platform,str:"Mac",id:"Mac"},{src:navigator.userAgent,str:"iPhone",id:"iPhone/iPod"},{src:navigator.platform,str:"Linux",id:"Linux"}];var verString;var searchString=function(data){for(var i=0;i<data.length;i++){var dataString=data[i].src;var dataProp=data[i].prop;verString=data[i].ver||data[i].id;
...
item.hasPendingMousemoveEvent=false}}jayus.chart.end();jayus.chart.begin("animation");for(i=0;i<jayus.animators.length;i++)jayus.animators[i].tick(now,elapsedSecs);jayus.chart.end();if(jayus.box2d.enabled)jayus.box2d.step(elapsedSecs);jayus.chart.begin("render");for(i=0;i<jayus.displays.length;i++){item=jayus.displays[i];if(item.dirtied)item.refreshBuffer()}if(jayus.box2d.enabled)jayus.box2d.drawDebug();jayus.chart.end();jayus.chart.update()}}else jayus.frameIntervalRunning=false},parse:function jayus_parse(obj){var type=
obj.type;if(typeof jayus[type]==="function"){for(var key in obj){var value=obj[key];if(value instanceof Array)for(var i=0;i<value.length;i++){if(typeof value[i]==="object"&&typeof value[i].type==="string")value[i]=this.parse(value[i])}else if(typeof value==="object"&&typeof value.type==="string")obj[key]=this.parse(value)}var ret=new jayus[type];ret.apply(obj);return ret}},addDefaultHandlers:function jayus_addDefaultHandlers(){jayus.addHandler("keyPress",function(e){if(e.key==="grave"&&e.event.ctrlKey){if(jayus.chart.visible)jayus.chart.hide();
else jayus.chart.show();return true}})},chart:{routines:{render:{description:"Rendering",time:0,color:"saddlebrown"},box2d:{description:"Box2D updating",time:0,color:"blue"},animation:{description:"Animation",time:0,color:"orange"},cursor:{description:"Cursor events",time:0,color:"Chartreuse"}},routineNameStack:[],routineTimeStack:[],routineInitStack:[],markColor:"grey",markFont:"10px sans-serif",display:null,graphSurface:null,topMS:40,index:0,width:500,height:380,visible:false,initialized:false,
windowSize:120,windowIndex:0,barHeight:20,barSpacing:10,routineData:{},getRoutineColor:function jayus_chart_getRoutineColor(name){switch(name){case "render":return"saddlebrown";case "box2d":return"blue";case "cursor":return"Chartreuse";case "animation":return"orange"}return"white"},begin:function jayus_chart_begin(routine){this.routineNameStack.push(routine);this.routineTimeStack.push(Date.now());this.routineInitStack.push(0)},end:function jayus_chart_end(){var time=Date.now()-this.routineTimeStack.pop(),
name=this.routineNameStack.pop(),inits=this.routineInitStack.pop(),data,i;if(typeof this.routines[name]!=="object")this.routines[name]={description:"",time:0,color:"red"};if(this.initialized){data=this.routineData[name];if(data===undefined){data={visible:true,latencies:[],averageLatency:0,inits:[],initTotal:0,color:this.getRoutineColor(name),widget:null};for(i=0;i<this.windowSize;i++){data.latencies.push(0);data.inits.push(0)}this.routineData[name]=data;data.row=document.createElement("tr");var title=
document.createElement("td");title.style.color="#CCC";title.style.fontSize="11pt";title.innerText=name;data.latencyLabel=document.createElement("td");data.latencyLabel.style.color="#BBB";data.latencyLabel.style.fontSize="10pt";data.latencyLabel.style.textAlign="right";data.latencyLabel.innerText="0 ms";data.progressBar=document.createElement("div");data.progressBar.style.margin="4px";$(data.progressBar).progressbar({value:0});data.progressBarValue=data.progressBar.childNodes[0];$(data.progressBarValue).css("background",
data.color);data.row.appendChild(title);data.row.appendChild(data.latencyLabel);data.row.appendChild(data.progressBar);jayus.chart.latencyTable.appendChild(data.row)}data.time+=time;data.latencyTotal-=data.latencies[this.windowIndex];data.latencies[this.windowIndex]=time;data.inits+=inits}this.routines[name].time+=time},tallyInit:function jayus_chart_tallyInit(type){this.routineInitStack[this.routineInitStack.length-1]++},show:function jayus_chart_show(){if(!this.initialized){this.vBox=new jayus.vBox;
this.vBox.setSize(this.width,this.height);this.topRow=new jayus.Scene(this.width,30);var bg=new jayus.LinearGradient(0,0,0,30);bg.addColorStop(0,"#DDD");bg.addColorStop(1,"#CCC");this.topRow.setBg({fill:bg});this.topRow.heightPolicy=new jayus.SizePolicy;this.topRow.heightPolicy.size=30;this.topRow.heightPolicy.expand=false;this.fpsLabel=jayus.parse({type:"Text",setText:"",setFont:"12px sans-serif",setBrush:{fill:"#111"},setOrigin:[0,4]});this.topRow.children.add(this.fpsLabel);setInterval(function(){var label=
jayus.chart.fpsLabel;var fps=Math.round(jayus.fps*10)/10+"";if(fps.length===2)fps+=".0";label.setText("Framerate: "+fps);label.setX(label.parent.width-4-label.width)},100);this.graph=new jayus.Scene;this.graph.setOptimizedBuffering(false);this.vBox.children.add(this.topRow,this.graph);this.graphSurface=new jayus.Surface(this.graph.width,this.graph.height);this.graph.children.add(this.graphSurface);this.graph.addHandler("dirty",function(dirty){if(dirty&4){var addLabel=function(ms){var y=jayus.chart.graph.height-
...
this.height);this.display.children.add(this.vBox);this.display.addHandler("dirty",function(dirty){if(dirty&4)jayus.chart.vBox.setWidth(this.width)});var div=document.createElement("div");var canvas=this.display.canvas;canvas.style.border="#aaa 1px solid";canvas.style.backgroundColor="black";div.appendChild(canvas);table=document.createElement("table");table.style.width="100%";div.appendChild(table);$(table).append('<colgroup><col span="1" style="width: 15%;"><col span="1" style="width: 15%;"><col span="1" style="width: 70%;"></colgroup>');
jayus.chart.latencyTable=table;$(div).dialog({title:"Jayus Debug Panel",open:function(){$(this).data("width.dialog",$(canvas).width()+60)},resize:function(){jayus.chart.display.setWidth($(jayus.chart.dialogContainer).width()-4);return false},close:function(){jayus.chart.visible=false}});jayus.chart.dialog=div.parentNode;jayus.chart.dialogContainer=jayus.chart.dialog.childNodes[1];this.display.setSize($(jayus.chart.dialogContainer).width()-4,$(jayus.chart.dialogContainer).height()-4);this.initialized=
true}this.visible=true;$(jayus.chart.dialog).show();jayus.fire("debugPanelShown")},hide:function jayus_chart_hide(){if(this.initialized&&this.visible){$(jayus.chart.dialog).hide();this.graphSurface.clear();this.index=0;this.visible=false}jayus.fire("debugPanelHidden")},mark:function jayus_chart_mark(text){if(this.visible){var ctx=this.graphSurface.context,metrics=jayus.measureText(text,this.markFont);ctx.fillStyle=this.markColor;ctx.fillRect(this.index,0,1,this.graph.height-0.5);ctx.fillText(text,
this.index-metrics.width-2,metrics.ascent)}},order:["render","box2d","animation","cursor"],update:function jayus_chart_update(){if(this.visible){var ctx=this.graphSurface.context,y=this.graphSurface.height,i,routine,data,width,height;var totalLatency=0;var routineNames=[];for(routine in this.routineData){data=this.routineData[routine];routineNames.push(routine);data.averageLatency=jayus.math.average(data.latencies);totalLatency+=data.averageLatency}for(i=0;i<routineNames.length;i++){data=this.routineData[routineNames[i]];
height=Math.floor(data.time/this.topMS*this.graph.height);ctx.fillStyle=data.color;ctx.fillRect(this.index,y-height,1,height-0.5);y-=height;data.time=0;data.inits=0}ctx.clearRect(this.index+1,0,2,this.graph.height);this.index++;if(this.index>=this.graph.width)this.index=-1;for(i=0;i<routineNames.length;i++){data=this.routineData[routineNames[i]];$(data.progressBar).progressbar({value:100*data.averageLatency/totalLatency});var num=Math.round(10*data.averageLatency)/10+"";if(num.length===1&&num!=="0")num+=
".0";$(data.latencyLabel).text(num+" ms")}var parent=$(jayus.chart.latencyTable);var items=parent.children();items.sort(function(a,b){if(a.nodeName==="TR"&&b.nodeName==="TR"){var vA=parseFloat(a.childNodes[2].attributes["aria-valuenow"].value);var vB=parseFloat(b.childNodes[2].attributes["aria-valuenow"].value);return vA<vB?1:0}});parent.append(items);this.windowIndex++;if(this.windowIndex===this.windowSize)this.windowIndex=0;this.graphSurface.dirty(jayus.DIRTY.ALL);this.display.refreshBuffer()}}},
getFontDescriptor:function jayus_getFontDescriptor(font){this.debug.match("jayus.getFontDescriptor",font,"font",2);if(!this.isFontCached(font))this.cacheFont(font);return this.fontCache[font]},cacheFont:function jayus_cacheFont(font){this.debug.match("jayus.cacheFont",font,"font",2);var i,ctx,charWidths,descriptor;if(!this.isFontCached(font)){charWidths=[];ctx=this.getContext();ctx.save();ctx.font=font;for(i=0;i<this.fontCacheMaxChar+1;i++)charWidths[i]=ctx.measureText(String.fromCharCode(i)).width;
ctx.restore();descriptor=this.getVerticalFontMetrics(font);descriptor.charWidths=charWidths;this.fontCache[font]=descriptor}},cacheFonts:function jayus_cacheFonts(fonts){this.debug.matchArray("jayus.cacheFonts",fonts,"fonts",2);for(var i=0;i<fonts.length;i++)this.cacheFont(fonts[i])},isFontCached:function jayus_isFontCached(font){this.debug.match("jayus.isFontCached",font,"font",2);return typeof this.fontCache[font]==="object"},measureText:function jayus_measureText(text,font){return this.measureTextOnto(text,
font,{})},measureTextOnto:function jayus_measureTextOnto(text,font,ret){this.debug.matchArguments("jayus.measureTextOnto",arguments,"text",2,"font",2,"ret",5);this.cacheFont(font);var descriptor=jayus.fontCache[font],charWidths=descriptor.charWidths,i,width=0;for(i=0;i<text.length;i++)width+=charWidths[text.charCodeAt(i)];ret.width=width;ret.ascent=descriptor.ascent;ret.descent=descriptor.descent;ret.height=descriptor.height;return ret},getVerticalFontMetrics:function jayus_getVerticalFontMetrics(font){var result=
{},text=document.createElement("span"),block=document.createElement("div"),div=document.createElement("div");text.style.font=font;text.innerText="Hg";block.style.display="inline-block";block.style.width="1px";block.style.height="0";div.appendChild(text);div.appendChild(block);document.body.appendChild(div);try{block.style.verticalAlign="baseline";result.ascent=block.offsetTop-text.offsetTop;block.style.verticalAlign="bottom";result.height=block.offsetTop-text.offsetTop;result.descent=result.height-
result.ascent}finally{document.body.removeChild(div)}return result},box2d:{enabled:true,scale:0.5,worlds:[],velocityIterations:10,positionIterations:10,physicalEntities:[],addWorld:function jayus_box2d_addWorld(world){jayus.debug.match("jayus.box2d.addWorld",world,"world",5);this.worlds.push(world)},removeWorld:function jayus_box2d_removeWorld(world){this.worlds.splice(this.worlds.indexOf(this.world),1)},step:function jayus_box2d_step(elapsedSecs){jayus.chart.begin("box2d");var i,world;for(i=0;i<
this.worlds.length;i++){world=this.worlds[i];world.Step(elapsedSecs,this.velocityIterations,this.positionIterations);world.ClearForces()}for(i=0;i<this.physicalEntities.length;i++)this.physicalEntities[i].updateFromBody();jayus.chart.end()},drawDebug:function jayus_box2d_drawDebug(){for(var i=0;i<this.worlds.length;i++)this.worlds[i].DrawDebugData()}},getContext:function jayus_getContext(){if(this.displays.length)return this.displays[0].context;return document.createElement("canvas").getContext("2d")},
TYPES:{DEFINED:0,BOOLEAN:1,STRING:2,FUNCTION:3,NUMBER:4,OBJECT:5,ARRAY:6,CONTEXT:7,COLOR:8,BRUSH:9,MATRIX:10,LIST:11,ENTITY:12,POINT:13,CIRCLE:14,POLYGON:15,PATH:16,SURFACE:17,SCENE:18,SHAPE:19,RECTANGLE:20,BUFFER:21,ANIMATOR:22,CANVAS:23},debug:{pauseOnEscape:true,types:["undefined","boolean","string","function","number","object","array","Context","Color","Brush","Matrix","List","Entity","Point","Circle","Polygon","Path","Surface","Scene","Shape","Rectangle","Buffer","Animator","Canvas"],is:function jayus_debug_is(type,
v){switch(type){case 0:return typeof v!=="undefined";case 1:return typeof v==="boolean";case 2:return typeof v==="string";case 3:return typeof v==="function";case 4:return typeof v==="number"&&isFinite(v);case 5:return typeof v==="object"&&v!==null;case 6:return v instanceof Array;case 7:return v instanceof CanvasRenderingContext2D;case 9:return typeof v==="object";case 8:case 10:case 11:case 12:case 13:case 14:case 15:case 16:case 17:case 18:return typeof v==="object"&&v instanceof jayus[jayus.debug.types[type]];
case 19:return this.is(13,v)||this.is(20,v)||this.is(14,v)||this.is(15,v)||this.is(16,v);case 20:return v instanceof jayus.Rectangle||v.isRect;case 21:if(typeof ImageData==="function")return v instanceof ImageData;return this.is(5,v)&&this.is(4,v.width)&&this.is(4,v.height)&&this.is(6,v.data);case 22:return!!v.isAnimator;case 23:return v instanceof HTMLCanvasElement}throw new Error("jayus.debug.is() - Unknown type("+jayus.debug.types[type]+") specified");},match:function jayus_debug_match(method,
value,name,type){if(!this.is(type,value))throw new TypeError(method+"() - Invalid parameter "+name+this.toString(value)+" sent, "+jayus.debug.types[type]+" required");},matchOptional:function jayus_debug_matchOptional(method,value,name,type){if(typeof value!=="undefined"&&!this.is(type,value))throw new TypeError(method+"() - Invalid optional parameter "+name+this.toString(value)+" sent, "+jayus.debug.types[type]+" required");},matchArray:function jayus_debug_matchArray(method,value,name,type){if(!this.is(6,
value))throw new TypeError(method+"() - Invalid parameter "+name+this.toString(value)+" sent, Array of "+jayus.debug.types[type]+" required");for(var i=0;i<value.length;i++)if(!this.is(type,value[i]))throw new TypeError(method+"() - Invalid parameter "+name+this.toString(value)+" array sent, element at "+i+"("+value[i]+") is required to be "+jayus.debug.types[type]);},matchArguments:function jayus_debug_matchArguments(name,args){var i,paramName,argValue,paramType;for(i=2;i<arguments.length;i+=2){argValue=
...
jayus.debug.types[type]+" required");}},matchContext:function jayus_debug_matchContext(method,value){if(!this.is(7,value))throw new TypeError(method+"() - Invalid parameter ctx"+this.toString(value)+" sent, Context required");},matchCoordinate:function jayus_debug_matchCoordinate(method,x,y){if(!this.is(13,x)&&!this.is(4,x))throw new TypeError(method+"() - Invalid parameter x"+this.toString(x)+" sent, Point or Number required");if(this.is(4,x)&&!this.is(4,y))throw new TypeError(method+"() - Invalid parameter y"+
this.toString(y)+" sent, Number required");},matchSize:function jayus_debug_matchSize(method,x,y){if(!this.is(12,x)&&!this.is(4,x))throw new TypeError(method+"() - Invalid parameter width"+this.toString(x)+" sent, Entity or Number required");if(this.is(4,x)&&!this.is(4,y))throw new TypeError(method+"() - Invalid parameter height"+this.toString(y)+" sent, Number required");},matchRectangle:function jayus_debug_matchRectangle(method,a,b,c,d){switch(arguments.length){case 3:return jayus.debug.matchArguments(method,
[a,b],"origin",13,"entity",12);case 4:return jayus.debug.matchArguments(method,[a,b,c],"origin",13,"width",4,"height",4);case 5:return jayus.debug.matchArgumentsAs(method,[a,b,c,d],4,"x","y","width","height")}throw new TypeError(method+"() - Invalid number of arguments sent, 2, 3, or 4 required");},verifyMethod:function jayus_debug_verifyMethod(object,method){this.matchArguments("jayus.debug.verifyMethod",arguments,"object",5,"method",2);if(!this.is(3,object[method])){console.log(object);throw new TypeError("jayus.debug.verifyMethod() - Object"+
this.toString(object)+" does not have method "+method);}},toString:function jayus_debug_toString(v){switch(typeof v){case "undefined":return"(Undefined)";case "boolean":return"(Boolean:"+v+")";case "number":return"(Number:"+v+")";case "string":return'(String:"'+v+'")';case "object":if(v===null)return"(null)";if(v instanceof Array)return"(Array:["+v+"])";if(typeof v.toString==="function")return v.toString();return"(Object:"+v+")";case "function":return"(Function:"+v+")"}return"(Unknown)"},exposedOriginColor:"blue",
exposedBoundsColor:"red",exposedScopeColor:"green",exposedFrameColor:"orange",exposedTransformedDashing:[5,5],defaultDebugRenderer:function jayus_debug_defaultDebugRenderer(ctx){var item;ctx.save();ctx.lineWidth=1;if(this.shapeType===0){item=this.getBounds().alignToCanvas();item.etchOntoContext(ctx);ctx.strokeStyle=jayus.debug.exposedBoundsColor;ctx.stroke()}ctx.strokeStyle=jayus.debug.exposedScopeColor;item=this.getScope().alignToCanvas();ctx.strokeRect(item.x,item.y,item.width,item.height);if(this.isRect){item=
this.getFrame().alignToCanvas();item.etchOntoContext(ctx);ctx.strokeStyle=jayus.debug.exposedFrameColor;ctx.stroke();if(this.isTransformed){ctx.setLineDash(jayus.debug.exposedTransformedDashing);item=this.getUnFrame().alignToCanvas();ctx.strokeRect(item.x,item.y,item.width,item.height)}}ctx.restore()},showPoint:function jayus_debug_showPoint(display,x,y,duration){if(arguments.length===3)duration=1E3;var spec=new jayus.Scene(3,3);spec.setBg({fill:"red"});spec.setOrigin(x,y);spec.animate().setAlpha(0).setDuration(duration).addHandler("finished",
function(){display.children.remove(spec)}).start();display.children.add(spec)},pause:function jayus_debug_pause(){var i,display,ctx;if(jayus.running){jayus.stop();for(i=0;i<jayus.displays.length;i++){display=jayus.displays[i];ctx=display.context;ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle="rgba(0, 0, 0, 0.5)";ctx.fillRect(0,0,display.width,display.height);ctx.fillStyle="white";ctx.font="24px sans-serif";ctx.textAlign="center";ctx.fillText("Paused",display.width/2,display.height/2);ctx.restore()}}else{for(i=
0;i<jayus.displays.length;i++)jayus.displays[i].fullyRefreshBuffer();jayus.start()}}},math:{randomFrom:function jayus_math_randomFrom(array){jayus.debug.match("jayus.math.randomFrom",array,"array",6);if(!array.length)throw new RangeError("jayus.math.randomFrom() - Invalid array"+jayus.debug.toString(array)+" sent, array or at least length 1 required");return array[this.randomBetween(0,array.length-1)]},randomBetween:function jayus_util_randomBetween(min,max){jayus.debug.matchArgumentsAs("jayus.math.randomBetween",
arguments,4,"min","max");return min+Math.floor((max-min+1)*Math.random())},clamp:function jayus_math_clamp(min,num,max){jayus.debug.matchArgumentsAs("jayus.math.clamp",arguments,4,"min","num","max");if(num<min)return min;if(num>max)return max;return num},min:function jayus_math_min(numbers){jayus.debug.matchArray("jayus.math.min",numbers,"numbers",4);if(!numbers.length)throw new RangeError("jayus.math.min() - Invalid numbers sent"+jayus.debug.toString(numbers)+", length of at least 1 required");var i,
value,min=numbers[0];for(i=1;i<numbers.length;i++){value=numbers[i];if(value<min)min=value}return min},max:function jayus_math_max(numbers){jayus.debug.matchArray("jayus.math.max",numbers,"numbers",4);if(!numbers.length)throw new RangeError("jayus.math.max() - Invalid numbers sent"+jayus.debug.toString(numbers)+", length of at least 1 required");var i,value,max=numbers[0];for(i=1;i<numbers.length;i++){value=numbers[i];if(value>max)max=value}return max},average:function jayus_math_average(numbers){jayus.debug.matchArray("jayus.math.average",
numbers,"numbers",4);if(!numbers.length)throw new RangeError("jayus.math.average() - Invalid numbers sent"+jayus.debug.toString(numbers)+", length of at least 1 required");var i,total=0;for(i=0;i<numbers.length;i++)total+=numbers[i];return total/numbers.length},toDegrees:function jayus_math_toDegrees(angle){return angle*(180/Math.PI)},toRadians:function jayus_math_toRadians(angle){return angle*(Math.PI/180)},vFlipAngle:function jayus_math_vFlipAngle(angle){jayus.debug.match("jayus.math.vFlipAngle",
angle,"angle",4);return 2*Math.PI-angle},hFlipAngle:function jayus_math_hFlipAngle(angle){jayus.debug.match("jayus.math.hFlipAngle",angle,"angle",4);angle=Math.PI-angle;if(angle<0)angle+=2*Math.PI;return angle},getCwAngleBetween:function jayus_math_getCwAngleBetween(angle1,angle2){jayus.debug.matchArgumentsAs("jayus.math.getCwAngleBetween",arguments,4,"angle1","angle2");var distance=angle1-angle2;if(distance<0)distance+=2*Math.PI;return distance},getCcwAngleBetween:function jayus_math_getCcwAngleBetween(angle1,
angle2){jayus.debug.matchArgumentsAs("jayus.math.getCcwAngleBetween",arguments,4,"angle1","angle2");var distance=angle2-angle1;if(distance<0)distance+=2*Math.PI;return distance}},colors:{displayNames:["Alice Blue","Antique White","Aqua","Aquamarine","Azure","Beige","Bisque","Black","Blanched Almond","Blue","Blue Violet","Brown","Burly Wood","Cadet Blue","Chartreuse","Chocolate","Coral","Cornflower Blue","Cornsilk","Crimson","Cyan","Dark Blue","Dark Cyan","Dark Goldenrod","Dark Grey","Dark Green",
...
jayus.Dependency=jayus.Animatable.extend({dependents:null,dependentCount:0,frozen:0,attach:function Dependency_attach(dependent){jayus.debug.match("Dependency.attach",dependent,"dependent",5);if(this.dependentCount===1)this.dependents=[this.dependents,dependent];else if(this.dependentCount)this.dependents.push(dependent);else this.dependents=dependent;this.dependentCount++},detach:function Dependency_detach(dependent){if(this.dependentCount){if(this.dependentCount===1)this.dependents=null;else if(this.dependenCount===
2)this.depdendents=this.dependents[1];else this.dependents.splice(this.dependents.indexOf(dependent),1);this.dependentCount--}},dirty:function Dependency_dirty(type){if(!this.frozen)this.informDependents(type)},informDependents:function Dependency_informDependents(type){if(this.dependentCount===1){jayus.debug.verifyMethod(this.dependents,"componentDirtied");this.dependents.componentDirtied(this,type)}else for(var i=0;i<this.dependentCount;i++){jayus.debug.verifyMethod(this.dependents[i],"componentDirtied");
this.dependents[i].componentDirtied(this,type)}},forEachDependent:function Dependency_forEachDependent(func,args){if(this.dependentCount===1)func.apply(this.dependents,args);else for(var i=0;i<this.dependentCount;i++)func.apply(this.dependents[i],args)}});jayus.debug.className="List";
...
jayus.Brush=jayus.Dependency.extend({componentType:"BRUSH",filling:false,stroking:false,fillType:0,strokeType:0,shadowType:0,alpha:1,strokeFirst:false,fill:null,stroke:null,lineWidth:null,lineCap:null,lineJoin:null,miterLimit:null,lineDash:null,lineDashOffset:null,shadow:null,shadowOffsetX:null,shadowOffsetY:null,shadowBlur:null,init:function Brush_init(styling){if(arguments.length){jayus.debug.match("Brush.init",styling,"styling",5);this.apply(styling)}},componentDirtied:function Brush_componentDirtied(component,type){this.dirty(jayus.DIRTY.STYLE)},
apply:function Brush_apply(styling){jayus.debug.match("Brush.apply",styling,"styling",5);this.frozen++;for(var key in styling)if(styling.hasOwnProperty(key)){if(typeof this["set"+key[0].toUpperCase()+key.slice(1)]!=="function"){var msg='Brush.apply() - Unknown property "'+key+'" present in styling object sent';if(key==="shadowColor")msg+=', did you mean to set the "shadow" property?';console.warn(msg);continue}this["set"+key[0].toUpperCase()+key.slice(1)](styling[key])}this.frozen--;this.dirty(jayus.DIRTY.STYLE);
return this},findStyleType:function Brush_findStyleType(style){if(style===null)return 0;if(typeof style==="string")return 1;if(style instanceof CanvasGradient)return 2;if(style instanceof CanvasPattern)return 3;if(style instanceof jayus.Color)return 4;if(style instanceof jayus.LinearGradient)return 5;if(style instanceof jayus.RadialGradient)return 6;throw new Error("Style.findStyleType() - Invalid fill/stroke style: "+jayus.debug.toString(style));},getNativeStyle:function Brush_getNativeStyle(type,
style){if(type<=3)return style;if(type===4)return style.toCSS();if(type===5||type===6)return style.getNative();throw new Error("Style.applyTo() - Invalid fill/stroke type identifier: "+jayus.debug.toString(type));},applyTo:function Brush_applyTo(ctx){if(this.alpha!==1)ctx.globalAlpha*=this.alpha;if(this.filling)ctx.fillStyle=this.getNativeStyle(this.fillType,this.fill);if(this.stroking){ctx.strokeStyle=this.getNativeStyle(this.strokeType,this.stroke);if(this.lineWidth!==null)ctx.lineWidth=this.lineWidth;
if(this.lineCap!==null)ctx.lineCap=this.lineCap;if(this.lineJoin!==null)ctx.lineJoin=this.lineJoin;if(this.miterLimit!==null)ctx.miterLimit=this.miterLimit;if(this.lineDash!==null&&jayus.ua.lineDash){ctx.setLineDash(this.lineDash);if(this.lineDashOffset!==null)ctx.lineDashOffset=this.lineDashOffset}}if(this.shadowType){if(this.shadowType===1)ctx.shadowColor=this.shadow;else if(this.shadowType===2)ctx.shadowColor=this.shadow.toCSS();else throw new Error("Style.applyTo() - Invalid shadow type identifier: "+
jayus.debug.toString(this.shadowType));if(this.shadowOffsetX!==null)ctx.shadowOffsetX=this.shadowOffsetX;if(this.shadowOffsetY!==null)ctx.shadowOffsetY=this.shadowOffsetY;if(this.shadowBlur!==null)ctx.shadowBlur=this.shadowBlur}},paintShape:function Brush_paintShape(ctx){this.applyTo(ctx);if(this.stroking&&this.strokeFirst)ctx.stroke();if(this.filling)ctx.fill();if(this.stroking&&!this.strokeFirst)ctx.stroke()},paintRect:function Brush_paintRect(ctx,x,y,width,height){ctx.save();this.applyTo(ctx);
if(this.stroking&&this.strokeFirst)ctx.strokeRect(x,y,width,height);if(this.filling)ctx.fillRect(x,y,width,height);if(this.stroking&&!this.strokeFirst)ctx.strokeRect(x,y,width,height);ctx.restore()},setAlpha:function Brush_setAlpha(alpha){if(alpha!==null)jayus.debug.match("Brush.setAlpha",alpha,"alpha",4);if(alpha===null)alpha=1;if(this.alpha!==alpha){this.alpha=alpha;this.dirty(jayus.DIRTY.STYLE)}return this},setFill:function Brush_setFill(style){if(style!==null)jayus.debug.match("Brush.setFill",
style,"style",0);if(this.fillType>=4)this.fill.detach(this);this.fill=style;this.fillType=this.findStyleType(style);if(this.fillType>=4)this.fill.attach(this);this.filling=!!this.fillType;this.dirty(jayus.DIRTY.STYLE);return this},setStroke:function Brush_setStroke(style){if(style!==null)jayus.debug.match("Brush.setStroke",style,"style",0);if(this.strokeType>=4)this.stroke.detach(this);this.stroke=style;this.strokeType=this.findStyleType(style);if(this.strokeType>=4)this.stroke.attach(this);this.stroking=
!!this.strokeType;this.dirty(jayus.DIRTY.STYLE);return this},setLineWidth:function Brush_setLineWidth(lineWidth){if(lineWidth!==null)jayus.debug.match("Brush.setLineWidth",lineWidth,"lineWidth",4);if(this.lineWidth!==lineWidth){this.lineWidth=lineWidth;this.dirty(jayus.DIRTY.STYLE)}return this},setLineCap:function Brush_setLineCap(lineCap){if(lineCap!==null)jayus.debug.match("Brush.setLineCap",lineCap,"lineCap");if(this.lineCap!==lineCap){this.lineCap=lineCap;this.dirty(jayus.DIRTY.STYLE)}return this},
setLineJoin:function Brush_setLineJoin(lineJoin){if(lineJoin!==null)jayus.debug.match("Brush.setLineJoin",lineJoin,"lineJoin",2);if(this.lineJoin!==lineJoin){this.lineJoin=lineJoin;this.dirty(jayus.DIRTY.STYLE)}return this},setMiterLimit:function Brush_setMiterLimit(miterLimit){if(miterLimit!==null)jayus.debug.match("Brush.setMiterLimit",miterLimit,"miterLimit",2);if(this.miterLimit!==miterLimit){this.miterLimit=miterLimit;this.dirty(jayus.DIRTY.STYLE)}return this},setLineDash:function Brush_setLineDash(lineDash){if(lineDash!==
null)jayus.debug.matchArray("Style.setLineDash",lineDash,"lineDash",4);this.lineDash=lineDash;this.dirty(jayus.DIRTY.STYLE);return this},setLineDashOffset:function Brush_setLineDashOffset(lineDashOffset){if(lineDashOffset!==null)jayus.debug.match("Brush.setLineDashOffset",lineDashOffset,"lineDashOffset",4);if(this.lineDashOffset!==lineDashOffset){this.lineDashOffset=lineDashOffset;this.dirty(jayus.DIRTY.STYLE)}return this},setShadow:function Brush_setShadow(shadow){if(shadow!==null)jayus.debug.match("Brush.setShadow",
shadow,"shadow",0);if(this.shadowType===2)this.shadow.detach(this);this.shadow=shadow;if(shadow===null)this.shadowType=0;else if(typeof shadow==="string")this.shadowType=1;else if(shadow instanceof jayus.Color){this.shadowType=2;this.shadow.attach(this)}else throw new Error("Style.setShadow() - Invalid shadow style: "+jayus.debug.toString(shadow));this.dirty(jayus.DIRTY.STYLE);return this},setShadowOffsetX:function Brush_setShadowOffsetX(shadowOffsetX){if(shadowOffsetX!==null)jayus.debug.match("Brush.setShadowOffsetX",
shadowOffsetX,"shadowOffsetX",4);if(this.shadowOffsetX!==shadowOffsetX){this.shadowOffsetX=shadowOffsetX;this.dirty(jayus.DIRTY.STYLE)}return this},setShadowOffsetY:function Brush_setShadowOffsetY(shadowOffsetY){if(shadowOffsetY!==null)jayus.debug.match("Brush.setShadowOffsetY",shadowOffsetY,"shadowOffsetY",4);if(this.shadowOffsetY!==shadowOffsetY){this.shadowOffsetY=shadowOffsetY;this.dirty(jayus.DIRTY.STYLE)}return this},setShadowBlur:function Brush_setShadowBlur(shadowBlur){if(shadowBlur!==null)jayus.debug.match("Brush.setShadowBlur",
shadowBlur,"shadowBlur",4);if(this.shadowBlur!==shadowBlur){this.shadowBlur=shadowBlur;this.dirty(jayus.DIRTY.STYLE)}return this}});jayus.debug.className="Gradient";
...
return new jayus.MethodAnimator(this,this.setY,this.y,y)}if(this.y!==y){this.y=y;this.dirty(2)}return this},set:function Point_set(x,y){jayus.debug.matchArgumentsAs("Point.set",arguments,4,"x","y");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.set,[this.x,this.y],[x,y])}if(this.x!==x||this.y!==y){this.x=x;this.y=y;this.dirty(2)}return this},translate:function Point_translate(x,y){jayus.debug.matchCoordinate("Point.translate",x,y);if(arguments.length===
1){y=x.y;x=x.x}return this.set(this.x+x,this.y+y)},toString:function Point_toString(){return"(Point:"+this.x+","+this.y+")"},clone:function Point_clone(){return new jayus.Point(this.x,this.y)},cloneOnto:function Point_cloneOnto(ret){ret.x=this.x;ret.y=this.y;return ret},equals:function Point_equals(x,y){jayus.debug.matchCoordinate("Point.equals",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.x===x&&this.y===y}});jayus.debug.className="Rectangle";
...
segment[6]);newSegment[5]=temp.x;newSegment[6]=temp.y;break;case "arcTo":case "arc":case "rect":case "ellipse":throw new Error("Path.getTransformed() - Unimplemented for command "+segment[0]);}}},toPolygon:function Path_toPolygon(detail){if(!arguments.length)detail=this.toPolygonDetail;else jayus.debug.match("Circle.toPolygon",detail,"detail",4);throw new Error("TODO: Path.toPolygon() - Unimplemented");},etchOntoContext:function Path_etchOntoContext(ctx){ctx.beginPath();var i,segment,point=this.getPoint(0),
x=point.x,y=point.y;for(i=0;i<this.segments.length;i++){segment=this.segments[i];switch(segment[0]){case "moveTo":x=segment[1];y=segment[2];ctx.moveTo(x,y);break;case "closePath":ctx.closePath();break;case "lineTo":x=segment[1];y=segment[2];ctx.lineTo(x,y);break;case "bezierCurveTo":x=segment[5];y=segment[6];ctx.bezierCurveTo(segment[1],segment[2],segment[3],segment[4],x,y);break;case "quadraticCurveTo":x=segment[3];y=segment[4];ctx.quadraticCurveTo(segment[1],segment[2],x,y);break;case "arcTo":ctx.arcTo(segment[1],
segment[2],segment[3],segment[4],segment[5]);point=this.getPoint(i);x=point.x;y=point.y;break;case "arc":ctx.arc(segment[1],segment[2],segment[3],segment[4],segment[5],segment[6]);point=this.getPoint(i);x=point.x;y=point.y;break;case "rect":x=segment[1];y=segment[2];ctx.rect(x,y,segment[3],segment[4]);break;case "ellipse":throw new Error("Path.etchOntoContext() - Unimplemented for command ellipse");}}return this}});
...
jayus.debug.defaultDebugRenderer;this.dirty(jayus.DIRTY.ALL)},unexpose:function Entity_unexpose(){this.debugRenderer=null;this.dirty(jayus.DIRTY.ALL)},toggleExposed:function Entity_toggleExposed(){if(this.debugRenderer!==null)this.unexpose();else this.expose()},exposeAll:function Entity_exposeAll(){this.debugRenderer=jayus.debug.defaultDebugRenderer;this.exposingAll=true;if(this.isParent)this.forEachChild(function(){this.exposeAll()});this.dirty(jayus.DIRTY.ALL)},unexposeAll:function Entity_exposeAll(){this.debugRenderer=
null;this.exposingAll=false;if(this.isParent)this.forEachChild(function(){this.unexposeAll()});this.dirty(jayus.DIRTY.ALL)},apply:function Entity_apply(obj){jayus.debug.match("Entity.apply",obj,"type",5);for(var property in obj)if(obj.hasOwnProperty(property))if(typeof this[property]==="function")if(obj[property]instanceof Array)this[property].apply(this,obj[property]);else this[property](obj[property]);else this[property]=obj[property];return this},isTransformed:false,transforms:null,actionsToAnimate:0,
animate:function Animatable_animate(){this.actionsToAnimate++;return this},init:function Entity_init(){this.addHandler("dirty",function(type){if(type&2)this.matrixDirty=true});if(this.enableDragEvents)this.handle({leftPress:function(e){this.leftDragging=true;return this.fire("leftDragStart",e)},leftRelease:function(e){if(this.leftDragging){this.leftDragging=false;return this.fire("leftDragEnd",e)}},middlePress:function(e){this.middleDragging=true;return this.fire("middleDragStart",e)},middleRelease:function(e){if(this.middleDragging){this.middleDragging=
false;return this.fire("middleDragEnd",e)}},rightPress:function(e){this.rightDragging=true;return this.fire("rightDragStart",e)},rightRelease:function(e){if(this.rightDragging){this.rightDragging=false;return this.fire("rightDragEnd",e)}},cursorMove:function(e){var ret=false;if(this.leftDragging)ret=this.fire("leftDrag",e)||ret;if(this.middleDragging)ret=this.fire("middleDrag",e)||ret;if(this.rightDragging)ret=this.fire("rightDrag",e)||ret;return ret},cursorOut:function(){if(this.leftDragging)this.fire("leftDragEnd");
...
jayus.RectEntity=jayus.Entity.extend({shapeType:4,isRect:true,x:0,y:0,width:0,height:0,bg:null,hasBg:false,alignBg:false,bounds:null,boundsClone:null,body:null,hasFlexibleWidth:true,hasFlexibleHeight:true,setX:jayus.Rectangle.prototype.setX,setY:jayus.Rectangle.prototype.setY,getOrigin:jayus.Rectangle.prototype.getOrigin,setOrigin:jayus.Rectangle.prototype.setOrigin,translate:jayus.Rectangle.prototype.translate,getLeft:jayus.Rectangle.prototype.getLeft,getRight:jayus.Rectangle.prototype.getRight,getTop:jayus.Rectangle.prototype.getTop,
getBottom:jayus.Rectangle.prototype.getBottom,setLeft:jayus.Rectangle.prototype.setLeft,setRight:jayus.Rectangle.prototype.setRight,setTop:jayus.Rectangle.prototype.setTop,setBottom:jayus.Rectangle.prototype.setBottom,getPosAt:function RectEntity_getPosAt(anchorX,anchorY){jayus.debug.matchCoordinate("RectEntity.getPosAt",anchorX,anchorY);if(arguments.length===1){anchorY=anchorX.y;anchorX=anchorX.x}return this.localToParent(anchorX*this.width,anchorY*this.height)},setPosAt:function RectEntity_setPosAt(anchorX,
anchorY,x,y){if(arguments.length===3){y=x.y;x=x.x}jayus.debug.matchArgumentsAs("RectEntity.setPosAt",[anchorX,anchorY,x,y],4,"anchorX","anchorY","x","y");return this.translate(x-(this.x+anchorX*this.width),y-(this.y+anchorY*this.height))},setRelativeAnchor:function Entity_setRelativeAnchor(x,y){jayus.debug.matchCoordinate("RectEntity.setRelativeAnchor",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setAnchor(this.width*x,this.height*y)},setBody:function RectEntity_setBody(body){jayus.debug.match("RectEntity.setBody",
...
jayus.Image=jayus.RectEntity.extend({image:null,hasSection:false,section:null,keepAligned:false,loaded:false,hasFlexibleWidth:false,hasFlexibleHeight:false,init:function Image_init(image){jayus.Entity.prototype.init.apply(this);if(typeof image==="string"){var filepath=image;if(jayus.images.isLoaded(filepath)){this.loaded=true;this.image=jayus.images.images[filepath];this.width=this.image.width;this.height=this.image.height}else{var that=this;jayus.addHandler("imageLoaded",function(data){if(data.filepath===
filepath){that.loaded=true;that.image=data.image;if(!that.hasSection){that.width=data.image.width;that.height=data.image.height;that.dirty(4)}}});jayus.images.load(filepath)}}else{this.loaded=true;this.image=image;this.width=this.image.width;this.height=this.image.height}},setSource:function Image_setSource(image){if(typeof image==="string"){var filepath=image;if(jayus.images.isLoaded(filepath)){this.loaded=true;this.image=jayus.images.images[filepath];this.width=this.image.width;this.height=this.image.height;
this.dirty(4)}else{var that=this;jayus.images.load(filepath,function(source,image){that.loaded=true;that.image=jayus.images.images[filepath];that.width=image.width;that.height=image.height;that.dirty(4)})}}else{this.loaded=true;this.image=image;this.width=this.image.width;this.height=this.image.height;this.dirty(4)}},setSection:function Image_setSection(x,y,width,height){if(arguments.length===1){jayus.debug.match("Image.setSection()",x,"section",20);height=x.height;width=x.width;y=x.y;x=x.x}else if(arguments.length===
...
this.redrawAll=true;this.dirty(16)},formContents:function Scene_formContents(width,height){if(this.buffered){this.canvas.width=width;this.canvas.height=height}},init:function Scene_init(width,height){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);this.children.typeId=12;if(arguments.length){jayus.debug.matchArgumentsAs("Scene.init",arguments,4,"width","height");this.width=width;this.height=height}},optimizeBuffering:false,groupDamagedRegions:true,damagedRegionPadding:2,
...
jayus.Stack=jayus.RectEntity.extend({spacing:8,reversed:false,isParent:true,hasFlexibleWidth:false,hasFlexibleHeight:false,init:function Stack_init(){jayus.RectEntity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function(type){if(type&4);})},componentDirtied:function Stack_componentDirtied(component,type){this.dirty(jayus.DIRTY.ALL)},listItemAdded:function Stack_listItemAdded(list,item){this.formContents();item.setParent(this);
this.dirty(16)},listItemRemoved:function Stack_listItemRemoved(list,item){this.formContents();item.removeParent();this.dirty(16)},listItemsAdded:function Stack_listItemsAdded(list,items){this.formContents();for(var i=0;i<items.length;i++){item=items[i];item.setParent(this)}this.dirty(16)},listItemsRemoved:function Stack_listItemsRemoved(list,items){this.formContents();for(var i=0;i<items.length;i++){item=items[i];item.removeParent()}this.dirty(16)},listItemsMoved:function Stack_listItemsMoved(){this.formContents();
this.dirty(16)},setReversed:function Stack_setReversed(on){jayus.debug.match("Stack.setReversed",on,"on",1);if(this.reversed!==on){this.reversed=on;this.formContents()}return this},setSpacing:function Stack_setSpacing(spacing){jayus.debug.match("Stack.setSpacing",spacing,"spacing",4);if(this.spacing!==spacing){this.spacing=spacing;this.formContents()}return this},findGreatestMinimalChildWidth:function Stack_findGreatestMinimalChildWidth(){var i,width=0;for(i=0;i<this.children.items.length;i++)width=
Math.max(width,this.children.items[i].minW);return width},findGreatestMinimalChildHeight:function Stack_findGreatestMinimalChildHeight(){var i,height=0;for(i=0;i<this.children.items.length;i++)height=Math.max(height,this.children.items[i].minH);return height},reform:function Stack_reform(){this.formContents(this.width,this.height)},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.Stack.prototype);jayus.debug.className="hStack";
jayus.hStack=jayus.Stack.extend({automaticHeight:true,formContents:function hStack_formContents(){var w=0,h=0,newH,i,item;if(this.reversed)for(i=this.children.items.length-1;i>=0;i--){item=this.children.items[i];item.setX(w);w+=item.width+this.spacing;if(this.automaticHeight){newH=item.height;if(newH>h)h=newH}}else for(i=0;i<this.children.items.length;i++){item=this.children.items[i];item.setX(w);w+=item.width+this.spacing;if(this.automaticHeight){newH=item.height;if(newH>h)h=newH}}var width=w-this.spacing,
heigth=this.height;if(this.automaticHeight)height=h;this.changeSize(width,height)}});jayus.debug.className="vStack";
jayus.vStack=jayus.Stack.extend({automaticWidth:true,getAutomaticWidth:function vStack_getAutomaticWidth(){return this.automaticWidth},setAutomaticWidth:function vStack_setAutomaticWidth(on){jayus.debug.match("vStack.setAutomaticWidth",on,"on",1);if(this.automaticWidth!==on){this.automaticWidth=on;this.reform()}return this},formContents:function vStack_formContents(){var h=0,w=0;for(var i=0;i<this.children.items.length;i++){var item=this.children.items[i];item.frozen++;item.setY(h);item.frozen--;
h+=item.height+this.spacing;if(this.automaticWidth){var newW=item.width;if(newW>w)w=newW}}var height=h-this.spacing,width=this.width;if(this.automaticWidth)width=w;this.changeSize(width,height)}});jayus.debug.className="Box";
