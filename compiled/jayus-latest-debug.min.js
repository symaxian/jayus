...
return false}if(at>bt){temp=a;a=b;b=temp}switch(a.shapeType+b.shapeType){case 1+1:return a.x===b.x&&a.y===b.y;case 1+2:return b.intersectsAt(a.x,a.y);case 2+2:return(a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y)<=(a.radius+b.radius)*(a.radius+b.radius);case 1+4:return b.intersectsAt(a.x,a.y);case 2+4:var x=2*Math.abs(a.x-(b.x+b.width/2)),y=2*Math.abs(a.y-(b.y+b.height/2));if(x>b.width+2*a.radius||y>b.height+2*a.radius)return false;if(x<=b.width||y<=b.height)return true;return(x-b.width)*(x-b.width)+(y-b.height)*
(y-b.height)<=4*a.radius*a.radius;case 4+4:return!(a.x+a.width<b.x||a.y+a.height<b.y||a.x>b.x+b.width||a.y>b.y+b.height);case 1+8:return b.intersectsAt(a.x,a.y);case 2+8:return this.intersectsPolygonPolygon(a.toPolygon(),b);case 4+8:return this.intersectPolygonPolygon(a.toPolygon(),b);case 8+8:return this.intersectsPolygonPolygon(a,b);case 1+16:return b.intersectsAt(a.x,a.y);case 2+16:return this.intersectsPolygonPolygon(a.toPolygon(),b.toPolygon());case 4+16:return this.intersectsPolygonPolygon(a.toPolygon(),
b.toPolygon());case 8+16:return this.intersectsPolygonPolygon(a,b.toPolygon());case 16+16:return this.intersectsPolygonPolygon(a.toPolygon(),b.toPolygon())}},intersectLineLine:function jayus_intersectLineLine(a1,a2,b1,b2){jayus.debug.matchArgumentsAs("jayus.intersectLineLine",arguments,4,"a1","a2","b1","b2");var ua_t=(b2.x-b1.x)*(a1.y-b1.y)-(b2.y-b1.y)*(a1.x-b1.x),ub_t=(a2.x-a1.x)*(a1.y-b1.y)-(a2.y-a1.y)*(a1.x-b1.x),u_b=(b2.y-b1.y)*(a2.x-a1.x)-(b2.x-b1.x)*(a2.y-a1.y);if(u_b!==0){var ua=ua_t/u_b,ub=
ub_t/u_b;return 0<=ua&&ua<=1&&0<=ub&&ub<=1}return ua_t===0||ub_t===0},intersectLinePolygon:function jayus_intersectLinePolygon(a1,a2,poly){jayus.debug.matchArguments("jayus.intersectLinePolygon",arguments,"a1",13,"a2",13,"poly",15);var length=poly.xPoints.length;for(var i=0;i<length;i++)if(this.intersectLineLine(a1,a2,poly.xPoints[i],poly.yPoints[i]))return true;return false},intersectPolygonPolygon:function jayus_intersectPolygonPolygon(poly1,poly2){jayus.debug.matchArgumentsAs("jayus.intersectPolygonPolygon",
arguments,15,"poly1","poly2");var length=poly1.xPoints.length;for(var i=0;i<length;i++)if(this.intersectLinePolygon(poly1.xPoints[i],poly1.yPoints[i],poly2))return true;return false},distance:function Point_distance(a,b){this.debug.matchArgumentsAs("jayus.distance",arguments,13,"a","b");var x=a.x-b.x,y=a.y-b.y;return Math.sqrt(x*x+y*y)},copyObject:function jayus_copyObject(src){this.debug.match("jayus.copyObject",src,"src",5);return this.applyObject(src,{})},applyObject:function jayus_applyObject(src,
dest){this.debug.matchArguments("jayus.applyObject",arguments,"src",5,"dest",5);for(var item in src)if(src.hasOwnProperty(item))dest[item]=src[item];return dest},getNewId:function jayus_getNewId(){return parseInt(Math.random()*1E6,10)},entities:[],createClass:function jayus_createClass(props){var constructor=function(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments)};if(typeof jayus.debug.className==="string")constructor=eval("(function "+jayus.debug.className+"(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments);})");
jayus.debug.className=null;constructor.prototype=props;if(typeof constructor.prototype.init!=="function")constructor.prototype.init=function(){};constructor.init=function HelperInit(object,args){constructor.prototype.init.apply(object,args)};constructor.extend=jayus.extendMethod;return constructor},extendMethod:function jayus_extendMethod(props){var constructor=function(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments)};if(typeof jayus.debug.className==="string")constructor=
eval("(function "+jayus.debug.className+"(){this.id=jayus.getNewId();jayus.entities.push(this);this.init.apply(this,arguments);})");jayus.debug.className=null;constructor.prototype=Object.create(this.prototype);jayus.applyObject(props,constructor.prototype);if(typeof constructor.prototype.init!=="function")constructor.prototype.init=function(){};constructor.init=function HelperInit(object,args){constructor.prototype.init.apply(object,args)};constructor.extend=jayus.extendMethod;return constructor},
fromObject:function jayus_fromObject(obj){var type=obj.__type__;var ret=new jayus[type];ret.initFromObject(obj);return ret},groupToObject:function Group_toObject(object){if(this.propagateCursor!==true)object.propagateCursor=this.propagateCursor;if(this.children!==jayus.Group.children){object.children=[];for(var i=0;i<this.children.items.length;i++)object.children.push(this.children.items[i].toObject())}},groupInitFromObject:function Group_initFromObject(object){if(typeof object.propagateCursor===
"boolean")this.propagateCursor=object.propagateCursor;if(typeof object.children==="object"){this.children=new jayus.List(this);for(var i=0;i<object.children.length;i++)this.children.items.push(jayus.fromObject(object.children[i]))}},identifiedObjects:[],addIdentifiedObject:function jayus_addIdentifiedObject(object){this.identifiedObjects.push(object)},getUrlParameters:function jayus_getUrlParameters(){for(var i=0,vars={},param,paramArray=window.location.href.slice(window.location.href.indexOf("?")+
1).split("&");i<paramArray.length;i++){param=paramArray[i].split("=");vars[param[0]]=param[1]}return vars},init:function jayus_init(){var elem=document.createElement("canvas");if(!(typeof elem.getContext==="function"&&typeof elem.getContext("2d")==="object"))throw new Error("Fatal error, browser does not support the canvas element, unable to initiate Jayus");this.applyObject(this.Responder.prototype,this);this.addDefaultHandlers();this.ua=this.detectBrowser();this.ua.lineDash=typeof this.getContext().setLineDash===
"function";window.requestAnimationFrame=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame;this.ua.requestAnimationFrame=typeof window.requestAnimationFrame==="function";this.keyboard.init();this.colors.cssNames=[];for(var i=0;i<this.colors.displayNames.length;i++)this.colors.cssNames.push(this.colors.displayNames[i].replace(" ","").replace(" ","").toLowerCase());this.initialized=true},detectBrowser:function jayus_detectBrowser(){var dataBrowser=
...
id:"Explorer",ver:"MSIE"},{src:navigator.userAgent,str:"Gecko",id:"Mozilla",ver:"rv"},{src:navigator.userAgent,str:"Mozilla",id:"Netscape",ver:"Mozilla"}];var dataOS=[{src:navigator.platform,str:"Win",id:"Windows"},{src:navigator.platform,str:"Mac",id:"Mac"},{src:navigator.userAgent,str:"iPhone",id:"iPhone/iPod"},{src:navigator.platform,str:"Linux",id:"Linux"}];var verString;var searchString=function(data){for(var i=0;i<data.length;i++){var dataString=data[i].src;var dataProp=data[i].prop;verString=
data[i].ver||data[i].id;if(dataString){if(dataString.indexOf(data[i].str)!==-1)return data[i].id}else if(dataProp)return data[i].id}};var searchVersion=function(dataString){var index=dataString.indexOf(verString);if(index!==-1)return parseFloat(dataString.substring(index+verString.length+1))};return{browser:searchString(dataBrowser)||null,version:searchVersion(navigator.userAgent)||searchVersion(navigator.appVersion)||null,OS:searchString(dataOS)||null}},start:function jayus_start(){if(this.initialized)if(!this.running){this.running=
true;if(!this.useReqAnimFrame||!this.ua.requestAnimationFrame)this.interval=setInterval(jayus.step,1E3/this.framerate);if(!this.frameIntervalRunning){this.lastFrameTime=Date.now();this.frameIntervalRunning=true;this.step()}}else console.warn("jayus.start() - Jayus is already running");else console.warn("jayus.start() - Jayus is not initialized")},stop:function jayus_stop(){if(this.running){this.running=false;clearInterval(this.interval)}else console.warn("jayus.stop() - Called when not running")},
step:function jayus_step(){if(jayus.running){if(jayus.useReqAnimFrame&&jayus.ua.requestAnimationFrame)window.requestAnimationFrame(jayus.step);var i,item,now=Date.now(),elapsedMilliSecs=now-jayus.lastFrameTime,elapsedSecs=elapsedMilliSecs/1E3;jayus.fps=1E3/elapsedMilliSecs;jayus.lastFrameTime=now;jayus.frameEventData.elapsed=elapsedSecs;if(!jayus.isHandler.frame||!jayus.fire("frame",jayus.frameEventData)){jayus.chart.begin("cursor");for(i=0;i<jayus.displays.length;i++){item=jayus.displays[i];if(item.hasPendingMousemoveEvent){item.processPendingMousemove();
item.hasPendingMousemoveEvent=false}}jayus.chart.end();jayus.chart.begin("animation");for(i=0;i<jayus.animators.length;i++)jayus.animators[i].tick(now,elapsedSecs);jayus.chart.end();if(jayus.box2d.enabled)jayus.box2d.step(elapsedSecs);jayus.chart.begin("render");for(i=0;i<jayus.displays.length;i++){item=jayus.displays[i];if(item.dirtied)item.refreshBuffer()}if(jayus.box2d.enabled)jayus.box2d.drawDebug();jayus.chart.end();jayus.chart.update()}}else jayus.frameIntervalRunning=false},addDefaultHandlers:function jayus_addDefaultHandlers(){jayus.addHandler("keyPress",
function(e){if(e.key==="grave"&&e.event.ctrlKey){if(jayus.chart.visible)jayus.chart.hide();else jayus.chart.show();return true}})},chart:{routines:{render:{description:"Rendering",time:0,color:"saddlebrown"},box2d:{description:"Box2D updating",time:0,color:"blue"},animation:{description:"Animation",time:0,color:"orange"},cursor:{description:"Cursor events",time:0,color:"Chartreuse"}},routineNameStack:[],routineTimeStack:[],routineInitStack:[],markColor:"grey",markFont:"10px sans-serif",display:null,
graphSurface:null,topMS:40,index:0,width:500,height:380,visible:false,initialized:false,windowSize:120,windowIndex:0,barHeight:20,barSpacing:10,routineData:{},getRoutineColor:function jayus_chart_getRoutineColor(name){switch(name){case "render":return"saddlebrown";case "box2d":return"blue";case "cursor":return"Chartreuse";case "animation":return"orange"}return"white"},begin:function jayus_chart_begin(routine){this.routineNameStack.push(routine);this.routineTimeStack.push(Date.now());this.routineInitStack.push(0)},
end:function jayus_chart_end(){var time=Date.now()-this.routineTimeStack.pop(),name=this.routineNameStack.pop(),inits=this.routineInitStack.pop(),data,i;if(typeof this.routines[name]!=="object")this.routines[name]={description:"",time:0,color:"red"};if(this.initialized){data=this.routineData[name];if(data===undefined){data={visible:true,latencies:[],averageLatency:0,inits:[],initTotal:0,color:this.getRoutineColor(name),widget:null};for(i=0;i<this.windowSize;i++){data.latencies.push(0);data.inits.push(0)}this.routineData[name]=
data;data.row=document.createElement("tr");var title=document.createElement("td");title.style.color="#CCC";title.style.fontSize="11pt";title.innerText=name;data.latencyLabel=document.createElement("td");data.latencyLabel.style.color="#BBB";data.latencyLabel.style.fontSize="10pt";data.latencyLabel.style.textAlign="right";data.latencyLabel.innerText="0 ms";data.progressBar=document.createElement("div");data.progressBar.style.margin="4px";$(data.progressBar).progressbar({value:0});data.progressBarValue=
data.progressBar.childNodes[0];$(data.progressBarValue).css("background",data.color);data.row.appendChild(title);data.row.appendChild(data.latencyLabel);data.row.appendChild(data.progressBar);jayus.chart.latencyTable.appendChild(data.row)}data.time+=time;data.latencyTotal-=data.latencies[this.windowIndex];data.latencies[this.windowIndex]=time;data.inits+=inits}this.routines[name].time+=time},tallyInit:function jayus_chart_tallyInit(type){this.routineInitStack[this.routineInitStack.length-1]++},show:function jayus_chart_show(){if(!this.initialized){this.vBox=
new jayus.vBox({width:this.width,height:this.height});this.topRow=new jayus.Scene({width:this.width,height:30,heightPolicy:{size:30,expand:false},bg:{fill:{__type__:"LinearGradient",start:{x:0,y:0},end:{x:0,y:30},stopPositions:[0,1],stopColors:["#DDD","#CCC"]}}});this.fpsLabel=jayus.fromObject({__type__:"Text",font:"12px sans-serif",x:0,y:4,brush:{fill:"#111"}});this.topRow.children.add(this.fpsLabel);setInterval(function(){var label=jayus.chart.fpsLabel;var fps=Math.round(jayus.fps*10)/10+"";if(fps.length===
2)fps+=".0";label.setText("Framerate: "+fps);label.setX(label.parent.width-4-label.width)},100);this.graph=new jayus.Scene;this.graph.setOptimizedBuffering(false);this.vBox.children.add(this.topRow,this.graph);this.graphSurface=new jayus.Surface(this.graph.width,this.graph.height);this.graph.children.add(this.graphSurface);this.graph.addHandler("dirty",function(dirty){if(dirty&4){var addLabel=function(ms){var y=jayus.chart.graph.height-ms/jayus.chart.topMS*jayus.chart.graph.height,line=new jayus.PaintedShape(new jayus.Polygon.Line(0,
Math.floor(y)+0.5,jayus.chart.graph.width,Math.floor(y)+0.5),{stroke:"dimgrey"}),text=(new jayus.Text(ms+" ms","10px sans-serif",{fill:"dimgrey"})).setOrigin(4,y-12);jayus.chart.graph.children.update(ms+"line",line.setId(ms+"line")).update(ms+"text",text.setId(ms+"text"))};addLabel(16);addLabel(33);jayus.chart.graphSurface.setHeight(this.height)}});this.graph.fire("resized");this.display=new jayus.Display(this.width,this.height);this.display.children.add(this.vBox);this.display.addHandler("dirty",
function(dirty){if(dirty&4)jayus.chart.vBox.setWidth(this.width)});var div=document.createElement("div");var canvas=this.display.canvas;canvas.style.border="#aaa 1px solid";canvas.style.backgroundColor="black";div.appendChild(canvas);table=document.createElement("table");table.style.width="100%";div.appendChild(table);$(table).append('<colgroup><col span="1" style="width: 15%;"><col span="1" style="width: 15%;"><col span="1" style="width: 70%;"></colgroup>');jayus.chart.latencyTable=table;$(div).dialog({title:"Jayus Debug Panel",
open:function(){$(this).data("width.dialog",$(canvas).width()+60)},resize:function(){jayus.chart.display.setWidth($(jayus.chart.dialogContainer).width()-4);return false},close:function(){jayus.chart.visible=false}});jayus.chart.dialog=div.parentNode;jayus.chart.dialogContainer=jayus.chart.dialog.childNodes[1];this.display.setSize($(jayus.chart.dialogContainer).width()-4,$(jayus.chart.dialogContainer).height()-4);this.initialized=true}this.visible=true;$(jayus.chart.dialog).show();jayus.fire("debugPanelShown")},
hide:function jayus_chart_hide(){if(this.initialized&&this.visible){$(jayus.chart.dialog).hide();this.graphSurface.clear();this.index=0;this.visible=false}jayus.fire("debugPanelHidden")},mark:function jayus_chart_mark(text){if(this.visible){var ctx=this.graphSurface.context,metrics=jayus.measureText(text,this.markFont);ctx.fillStyle=this.markColor;ctx.fillRect(this.index,0,1,this.graph.height-0.5);ctx.fillText(text,this.index-metrics.width-2,metrics.ascent)}},order:["render","box2d","animation","cursor"],
update:function jayus_chart_update(){if(this.visible){var ctx=this.graphSurface.context,y=this.graphSurface.height,i,routine,data,width,height;var totalLatency=0;var routineNames=[];for(routine in this.routineData){data=this.routineData[routine];routineNames.push(routine);data.averageLatency=jayus.math.average(data.latencies);totalLatency+=data.averageLatency}for(i=0;i<routineNames.length;i++){data=this.routineData[routineNames[i]];height=Math.floor(data.time/this.topMS*this.graph.height);ctx.fillStyle=
data.color;ctx.fillRect(this.index,y-height,1,height-0.5);y-=height;data.time=0;data.inits=0}ctx.clearRect(this.index+1,0,2,this.graph.height);this.index++;if(this.index>=this.graph.width)this.index=-1;for(i=0;i<routineNames.length;i++){data=this.routineData[routineNames[i]];$(data.progressBar).progressbar({value:100*data.averageLatency/totalLatency});var num=Math.round(10*data.averageLatency)/10+"";if(num.length===1&&num!=="0")num+=".0";$(data.latencyLabel).text(num+" ms")}var parent=$(jayus.chart.latencyTable);
var items=parent.children();items.sort(function(a,b){if(a.nodeName==="TR"&&b.nodeName==="TR"){var vA=parseFloat(a.childNodes[2].attributes["aria-valuenow"].value);var vB=parseFloat(b.childNodes[2].attributes["aria-valuenow"].value);return vA<vB?1:0}});parent.append(items);this.windowIndex++;if(this.windowIndex===this.windowSize)this.windowIndex=0;this.graphSurface.dirty(jayus.DIRTY.ALL);this.display.refreshBuffer()}}},getFontDescriptor:function jayus_getFontDescriptor(font){this.debug.match("jayus.getFontDescriptor",
font,"font",2);if(!this.isFontCached(font))this.cacheFont(font);return this.fontCache[font]},cacheFont:function jayus_cacheFont(font){this.debug.match("jayus.cacheFont",font,"font",2);var i,ctx,charWidths,descriptor;if(!this.isFontCached(font)){charWidths=[];ctx=this.getContext();ctx.save();ctx.font=font;for(i=0;i<this.fontCacheMaxChar+1;i++)charWidths[i]=ctx.measureText(String.fromCharCode(i)).width;ctx.restore();descriptor=this.getVerticalFontMetrics(font);descriptor.charWidths=charWidths;this.fontCache[font]=
descriptor}},cacheFonts:function jayus_cacheFonts(fonts){this.debug.matchArray("jayus.cacheFonts",fonts,"fonts",2);for(var i=0;i<fonts.length;i++)this.cacheFont(fonts[i])},isFontCached:function jayus_isFontCached(font){this.debug.match("jayus.isFontCached",font,"font",2);return typeof this.fontCache[font]==="object"},measureText:function jayus_measureText(text,font){return this.measureTextOnto(text,font,{})},measureTextOnto:function jayus_measureTextOnto(text,font,ret){this.debug.matchArguments("jayus.measureTextOnto",
arguments,"text",2,"font",2,"ret",5);this.cacheFont(font);var descriptor=jayus.fontCache[font],charWidths=descriptor.charWidths,i,width=0;for(i=0;i<text.length;i++)width+=charWidths[text.charCodeAt(i)];ret.width=width;ret.ascent=descriptor.ascent;ret.descent=descriptor.descent;ret.height=descriptor.height;return ret},getVerticalFontMetrics:function jayus_getVerticalFontMetrics(font){var result={},text=document.createElement("span"),block=document.createElement("div"),div=document.createElement("div");
text.style.font=font;text.innerText="Hg";block.style.display="inline-block";block.style.width="1px";block.style.height="0";div.appendChild(text);div.appendChild(block);document.body.appendChild(div);try{block.style.verticalAlign="baseline";result.ascent=block.offsetTop-text.offsetTop;block.style.verticalAlign="bottom";result.height=block.offsetTop-text.offsetTop;result.descent=result.height-result.ascent}finally{document.body.removeChild(div)}return result},box2d:{enabled:true,scale:0.5,worlds:[],
velocityIterations:10,positionIterations:10,physicalEntities:[],addWorld:function jayus_box2d_addWorld(world){jayus.debug.match("jayus.box2d.addWorld",world,"world",5);this.worlds.push(world)},removeWorld:function jayus_box2d_removeWorld(world){this.worlds.splice(this.worlds.indexOf(this.world),1)},step:function jayus_box2d_step(elapsedSecs){jayus.chart.begin("box2d");var i,world;for(i=0;i<this.worlds.length;i++){world=this.worlds[i];world.Step(elapsedSecs,this.velocityIterations,this.positionIterations);
world.ClearForces()}for(i=0;i<this.physicalEntities.length;i++)this.physicalEntities[i].updateFromBody();jayus.chart.end()},drawDebug:function jayus_box2d_drawDebug(){for(var i=0;i<this.worlds.length;i++)this.worlds[i].DrawDebugData()}},getContext:function jayus_getContext(){if(this.displays.length)return this.displays[0].context;return document.createElement("canvas").getContext("2d")},TYPES:{DEFINED:0,BOOLEAN:1,STRING:2,FUNCTION:3,NUMBER:4,OBJECT:5,ARRAY:6,CONTEXT:7,COLOR:8,BRUSH:9,MATRIX:10,LIST:11,
ENTITY:12,POINT:13,CIRCLE:14,POLYGON:15,PATH:16,SURFACE:17,SCENE:18,SHAPE:19,RECTANGLE:20,BUFFER:21,ANIMATOR:22,CANVAS:23},debug:{pauseOnEscape:true,types:["undefined","boolean","string","function","number","object","array","Context","Color","Brush","Matrix","List","Entity","Point","Circle","Polygon","Path","Surface","Scene","Shape","Rectangle","Buffer","Animator","Canvas"],is:function jayus_debug_is(type,v){switch(type){case 0:return typeof v!=="undefined";case 1:return typeof v==="boolean";case 2:return typeof v===
"string";case 3:return typeof v==="function";case 4:return typeof v==="number"&&isFinite(v);case 5:return typeof v==="object"&&v!==null;case 6:return v instanceof Array;case 7:return v instanceof CanvasRenderingContext2D;case 9:return typeof v==="object";case 8:case 10:case 11:case 12:case 13:case 14:case 15:case 16:case 17:case 18:return typeof v==="object"&&v instanceof jayus[jayus.debug.types[type]];case 19:return this.is(13,v)||this.is(20,v)||this.is(14,v)||this.is(15,v)||this.is(16,v);case 20:return v instanceof
jayus.Rectangle||v.isRect;case 21:if(typeof ImageData==="function")return v instanceof ImageData;return this.is(5,v)&&this.is(4,v.width)&&this.is(4,v.height)&&this.is(6,v.data);case 22:return!!v.isAnimator;case 23:return v instanceof HTMLCanvasElement}throw new Error("jayus.debug.is() - Unknown type("+jayus.debug.types[type]+") specified");},match:function jayus_debug_match(method,value,name,type){if(!this.is(type,value))throw new TypeError(method+"() - Invalid parameter "+name+this.toString(value)+
" sent, "+jayus.debug.types[type]+" required");},matchOptional:function jayus_debug_matchOptional(method,value,name,type){if(typeof value!=="undefined"&&!this.is(type,value))throw new TypeError(method+"() - Invalid optional parameter "+name+this.toString(value)+" sent, "+jayus.debug.types[type]+" required");},matchArray:function jayus_debug_matchArray(method,value,name,type){if(!this.is(6,value))throw new TypeError(method+"() - Invalid parameter "+name+this.toString(value)+" sent, Array of "+jayus.debug.types[type]+
...
exposedTransformedDashing:[5,5],defaultDebugRenderer:function jayus_debug_defaultDebugRenderer(ctx){var item;ctx.save();ctx.lineWidth=1;if(this.shapeType===0){item=this.getBounds().alignToCanvas();item.etchOntoContext(ctx);ctx.strokeStyle=jayus.debug.exposedBoundsColor;ctx.stroke()}ctx.strokeStyle=jayus.debug.exposedScopeColor;item=this.getScope().alignToCanvas();ctx.strokeRect(item.x,item.y,item.width,item.height);if(this.isRect){item=this.getFrame().alignToCanvas();item.etchOntoContext(ctx);ctx.strokeStyle=
jayus.debug.exposedFrameColor;ctx.stroke();if(this.isTransformed){ctx.setLineDash(jayus.debug.exposedTransformedDashing);item=this.getUnFrame().alignToCanvas();ctx.strokeRect(item.x,item.y,item.width,item.height)}}ctx.restore()},showPoint:function jayus_debug_showPoint(display,x,y,duration){if(arguments.length===3)duration=1E3;var spec=new jayus.Scene(3,3);spec.setBg({fill:"red"});spec.setOrigin(x,y);spec.animate().setAlpha(0).setDuration(duration).addHandler("finished",function(){display.children.remove(spec)}).start();
...
jayus.start()}}},math:{randomFrom:function jayus_math_randomFrom(array){jayus.debug.match("jayus.math.randomFrom",array,"array",6);if(!array.length)throw new RangeError("jayus.math.randomFrom() - Invalid array"+jayus.debug.toString(array)+" sent, array or at least length 1 required");return array[this.randomBetween(0,array.length-1)]},randomBetween:function jayus_util_randomBetween(min,max){jayus.debug.matchArgumentsAs("jayus.math.randomBetween",arguments,4,"min","max");return min+Math.floor((max-
min+1)*Math.random())},clamp:function jayus_math_clamp(min,num,max){jayus.debug.matchArgumentsAs("jayus.math.clamp",arguments,4,"min","num","max");if(num<min)return min;if(num>max)return max;return num},min:function jayus_math_min(numbers){jayus.debug.matchArray("jayus.math.min",numbers,"numbers",4);if(!numbers.length)throw new RangeError("jayus.math.min() - Invalid numbers sent"+jayus.debug.toString(numbers)+", length of at least 1 required");var i,value,min=numbers[0];for(i=1;i<numbers.length;i++){value=
numbers[i];if(value<min)min=value}return min},max:function jayus_math_max(numbers){jayus.debug.matchArray("jayus.math.max",numbers,"numbers",4);if(!numbers.length)throw new RangeError("jayus.math.max() - Invalid numbers sent"+jayus.debug.toString(numbers)+", length of at least 1 required");var i,value,max=numbers[0];for(i=1;i<numbers.length;i++){value=numbers[i];if(value>max)max=value}return max},average:function jayus_math_average(numbers){jayus.debug.matchArray("jayus.math.average",numbers,"numbers",
4);if(!numbers.length)throw new RangeError("jayus.math.average() - Invalid numbers sent"+jayus.debug.toString(numbers)+", length of at least 1 required");var i,total=0;for(i=0;i<numbers.length;i++)total+=numbers[i];return total/numbers.length},toDegrees:function jayus_math_toDegrees(angle){return angle*(180/Math.PI)},toRadians:function jayus_math_toRadians(angle){return angle*(Math.PI/180)},vFlipAngle:function jayus_math_vFlipAngle(angle){jayus.debug.match("jayus.math.vFlipAngle",angle,"angle",4);
return 2*Math.PI-angle},hFlipAngle:function jayus_math_hFlipAngle(angle){jayus.debug.match("jayus.math.hFlipAngle",angle,"angle",4);angle=Math.PI-angle;if(angle<0)angle+=2*Math.PI;return angle},getCwAngleBetween:function jayus_math_getCwAngleBetween(angle1,angle2){jayus.debug.matchArgumentsAs("jayus.math.getCwAngleBetween",arguments,4,"angle1","angle2");var distance=angle1-angle2;if(distance<0)distance+=2*Math.PI;return distance},getCcwAngleBetween:function jayus_math_getCcwAngleBetween(angle1,angle2){jayus.debug.matchArgumentsAs("jayus.math.getCcwAngleBetween",
arguments,4,"angle1","angle2");var distance=angle2-angle1;if(distance<0)distance+=2*Math.PI;return distance}},colors:{displayNames:["Alice Blue","Antique White","Aqua","Aquamarine","Azure","Beige","Bisque","Black","Blanched Almond","Blue","Blue Violet","Brown","Burly Wood","Cadet Blue","Chartreuse","Chocolate","Coral","Cornflower Blue","Cornsilk","Crimson","Cyan","Dark Blue","Dark Cyan","Dark Goldenrod","Dark Grey","Dark Green","Dark Khaki","Dark Magenta","Dark Olive Green","Dark Orange","Dark Orchid",
"Dark Red","Dark Salmon","Dark Sea Green","Dark Slate Blue","Dark Slate Grey","Dark Turquoise","Dark Violet","Deep Pink","Deep Sky Blue","Dim Grey","Dodger Blue","Firebrick","Floral White","Forest Green","Fuchsia","Gainsboro","Ghost White","Gold","Goldenrod","Grey","Green","Green Yellow","Honeydew","Hot Pink","Indian Red","Indigo","Ivory","Khaki","Lavender","Lavender Blush","Lawn Green","Lemon Chiffon","Light Blue","Light Coral","Light Cyan","Light Goldenrod","Light Green","Light Grey","Light Pink",
"Light Salmon","Light Sea Green","Light Sky Blue","Light Slate Grey","Light Steel Blue","Light Yellow","Lime","Lime Green","Linen","Magenta","Maroon","Medium Aquamarine","Medium Blue","Medium Orchid","Medium Purple","Medium Sea Green","Medium Slate Blue","Medium Spring Green","Medium Turquoise","Medium Violet Red","Midnight Blue","Mint Cream","Misty Rose","Moccasin","Navajo White","Navy","Old Lace","Olive","Olive Drab","Orange","Orange Red","Orchid","Pale Goldenrod","Pale Green","Pale Turquoise",
"Pale Violet Red","Papaya Whip","Peach Puff","Peru","Pink","Plum","Powder Blue","Purple","Red","Rosy Brown","Royal Blue","Saddle Brown","Salmon","Sandy Brown","Sea Green","Seashell","Sienna","Silver","Sky Blue","Slate Blue","Slate Grey","Snow","Spring Green","Steel Blue","Tan","Teal","Thistle","Tomato","Turquoise","Violet","Wheat","White","White Smoke","Yellow","Yellow Green"],cssNames:null,redComponents:[240,250,0,127,240,245,255,0,255,0,138,165,222,95,127,210,255,100,255,220,0,0,0,184,169,0,189,
139,85,255,153,139,233,143,72,47,0,148,255,0,105,30,178,255,34,255,220,248,255,218,127,0,173,240,255,205,75,255,240,230,255,124,255,173,240,224,250,144,211,255,255,32,135,119,176,255,0,50,250,255,127,102,0,186,147,60,123,0,72,199,25,245,255,255,255,0,253,128,107,255,255,218,238,152,175,219,255,255,205,255,221,176,127,255,188,65,139,250,244,46,255,160,192,135,106,112,255,0,70,210,0,216,255,64,238,245,255,245,255,154],greenComponents:[248,235,255,255,255,245,228,0,235,0,43,42,184,158,255,105,127,149,
248,20,255,0,139,134,169,100,183,0,107,140,50,0,150,188,61,79,206,0,20,191,105,144,34,250,139,0,220,248,215,165,127,127,255,255,105,92,0,255,230,230,240,252,250,216,128,255,250,238,211,182,160,178,206,136,196,255,255,205,240,0,0,205,0,85,112,179,104,250,209,21,25,255,228,228,222,0,245,128,142,165,69,112,232,251,238,112,239,218,133,192,160,224,127,0,143,105,69,128,164,139,245,82,192,206,90,128,250,255,130,180,128,191,99,225,130,222,255,255,255,205],blueComponents:[255,215,255,212,255,220,196,0,205,
255,226,42,135,160,0,30,80,237,220,60,255,139,139,11,169,0,107,139,47,0,204,0,122,143,139,79,209,211,147,255,105,255,34,240,34,255,220,255,0,32,127,127,47,240,180,92,130,240,140,250,245,0,205,230,128,255,210,144,211,193,122,170,250,153,222,224,0,50,230,255,127,170,205,211,219,113,238,154,204,133,112,250,225,181,173,128,230,0,35,0,0,214,170,152,238,147,213,185,63,203,221,230,0,0,143,225,19,114,96,87,238,45,192,235,205,144,250,127,180,140,128,216,71,208,238,179,255,245,0,50]}};jayus.easing={linear:function jayus_easing_linear(t){return t},easeInQuad:function jayus_easing_easeInQuad(t){return t*t},easeOutQuad:function jayus_easing_easeOutQuad(t){return t*(2-t)},easeInOutQuad:function jayus_easing_easeInOutQuad(t){return((t/=0.5)<1?t*t:(1-t)*(t-3)+1)/2},easeInCubic:function jayus_easing_easeInCubic(t){return t*t*t},easeOutCubic:function jayus_easing_easeOutCubic(t){return--t*t*t+1},easeInOutCubic:function jayus_easing_easeInOutCubic(t){return(t/=0.5)<1?t*t*t/2:(t-=2)*t*t/
...
[];this.isHandler[event]=false}return this},removeAllHandlers:function Entity_removeAllHandlers(){this.hasHandlers=false;if(typeof this.addDefaultHandlers==="function")this.addDefaultHandlers();return this},fire:function Responder_fire(event,data){jayus.debug.match("Responder.fire",event,"event",2);jayus.debug.matchOptional("Responder.fire",data,"data",0);var i,opts,result;if(this.hasHandlers&&this.isHandler[event])for(i=0;i<this.handlers[event].length;i++){opts=this.handlers[event][i];result=!!opts.handler.call(opts.hasContext?
...
jayus.Dependency=jayus.Animatable.extend({id:0,dependents:null,dependentCount:0,frozen:0,setId:function Dependency_setId(id){jayus.debug.match("Dependency.setId",id,"id",0);this.id=id;jayus.addIdentifiedObject(this);return this},initFromObject:function Dependency_initFromObject(object){if(typeof object.id!=="undefined"){this.id=object.id;jayus.addIdentifiedObject(this)}return this},attach:function Dependency_attach(dependent){jayus.debug.match("Dependency.attach",dependent,"dependent",5);if(!this.dependentCount)this.dependents=
[];this.dependents.push(dependent);this.dependentCount++},detach:function Dependency_detach(dependent){if(this.dependentCount){this.dependents.splice(this.dependents.indexOf(dependent),1);this.dependentCount--}},dirty:function Dependency_dirty(type){if(!this.frozen)this.informDependents(type)},informDependents:function Dependency_informDependents(type){for(var i=0;i<this.dependentCount;i++){jayus.debug.verifyMethod(this.dependents[i],"componentDirtied");this.dependents[i].componentDirtied(this,type)}},
forEachDependent:function Dependency_forEachDependent(func,args){for(var i=0;i<this.dependentCount;i++)func.apply(this.dependents[i],args)}});jayus.debug.className="List";
...
args){jayus.debug.match("List.onEach",method,"method",2);jayus.debug.matchOptional("List.onEach",args,"args",6);for(var i=0;i<this.items.length;i++)this.items[i][method].apply(this.items[i],args);return this},forEach:function List_forEach(func,args){jayus.debug.match("List.forEach",func,"func",3);jayus.debug.matchOptional("List.forEach",args,"args",6);var i,ret;for(i=0;i<this.items.length;i++){ret=func.apply(this.items[i],args);if(ret!==undefined)return ret}return null},sort:function List_sort(comparator){jayus.debug.match("List.sort",
comparator,"comparator",3);this.items.sort(comparator);this.parent.listItemsMoved();return this}});jayus.debug.className="Color";
jayus.Color=jayus.Dependency.extend({componentType:"COLOR",r:0,g:0,b:0,a:1,init:function Color_init(){if(arguments.length)if(arguments.length===1&&typeof arguments[0]==="object")this.initFromObject(arguments[0]);else this.set.apply(this,arguments)},toObject:function Color_toObject(){var object={__type__:"Color",r:this.r,g:this.g,b:this.b,a:this.a};if(this.id!==0)object.id=this.id;return object},initFromObject:function Color_initFromObject(object){jayus.debug.match("Color.initFromObject",object,"object",
5);this.r=object.r;this.g=object.g;this.b=object.b;this.a=object.a;return this.dirty(jayus.DIRTY.ALL)},set:function Color_set(r,g,b,a){switch(arguments.length){case 1:g=1;case 2:jayus.debug.matchArguments("Color.set",arguments,"name",2,"alpha",4);var index=jayus.colors.displayNames.indexOf(r);if(index<0)index=jayus.colors.cssNames.indexOf(r);if(index+1)this.set(jayus.colors.redComponents[index],jayus.colors.greenComponents[index],jayus.colors.blueComponents[index],g);return this;case 3:a=1;case 4:jayus.debug.matchArgumentsAs("Color.set",
arguments,4,"r","g","b","a");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.set,[this.r,this.g,this.b,this.a],[r,g,b,a])}this.r=r;this.g=g;this.b=b;this.a=a;return this}throw new Error("Color.set() - Invalid number of arguments sent, 1, 2, 3, or 4 required");},setRed:function Color_setRed(value){jayus.debug.match("Color.setRed",value,"value",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setRed,this.r,value)}if(this.r!==
value){this.r=value;this.dirty(jayus.DIRTY.STYLE)}return this},setGreen:function Color_setGreen(value){jayus.debug.match("Color.setGreen",value,"value",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setGreen,this.g,value)}if(this.g!==value){this.g=value;this.dirty(jayus.DIRTY.STYLE)}return this},setBlue:function Color_setBlue(value){jayus.debug.match("Color.setBlue",value,"value",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,
this.setBlue,this.b,value)}if(this.b!==value){this.b=value;this.dirty(jayus.DIRTY.STYLE)}return this},setAlpha:function Color_setAlpha(value){jayus.debug.match("Color.setAlpha",value,"value",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setAlpha,this.a,value)}if(this.a!==value){this.a=value;this.dirty(jayus.DIRTY.STYLE)}return this},toHex:function Color_toHex(){var r=this.r.toString(16),g=this.g.toString(16),b=this.b.toString(16);return"#"+(r.length===
1?"0"+r:r)+(g.length===1?"0"+g:g)+(b.length===1?"0"+b:b)},toCSS:function Color_toCSS(){this.a=jayus.math.clamp(0,this.a,1);if(this.a-1)return"rgba("+this.r+","+this.g+","+this.b+","+this.a+")";return"rgb("+this.r+","+this.g+","+this.b+")"},getName:function Color_getName(){var i,table=jayus.colorTable;for(i=0;i<table.count;i++)if(this.r===table.redComponents[i]&&this.g===table.greenComponents[i]&&this.b===table.blueComponents[i])return table.displayNames[i];return""}});jayus.debug.className="Brush";
...
"shadowBlur"],objects:["fill","stroke","shadow"]},init:function Brush_init(styling){if(arguments.length){jayus.debug.match("Brush.init",styling,"styling",5);this.apply(styling)}},componentDirtied:function Brush_componentDirtied(component,type){this.dirty(jayus.DIRTY.STYLE)},toObject:function Brush_toObject(){var object={};object.__type__="Brush";var i,key,val,valType;for(i=0;i<jayus.Brush.prototype.properties.names.length;i++){key=jayus.Brush.prototype.properties.names[i];val=object[key];valType=
typeof val;if(val!==jayus.Brush.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}return object},initFromObject:function Brush_initFromObject(object){jayus.debug.match("Brush.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.frozen++;var i,key,val,valType;for(i=0;i<jayus.Brush.prototype.properties.names.length;i++){key=jayus.Brush.prototype.properties.names[i];val=object[key];valType=typeof val;if(valType!=="undefined"){if(valType===
"object")val=jayus.parse(val);if(key==="fill")this.setFill(val);else if(key==="stroke")this.setStroke(val);else if(key==="shadow")this.setShadow(val);else this[key]=val}}this.frozen--;this.dirty(jayus.DIRTY.ALL)},apply:function Brush_apply(styling){jayus.debug.match("Brush.apply",styling,"styling",5);this.frozen++;for(var key in styling)if(styling.hasOwnProperty(key)){if(typeof this["set"+key[0].toUpperCase()+key.slice(1)]!=="function"){var msg='Brush.apply() - Unknown property "'+key+'" present in styling object sent';
if(key==="shadowColor")msg+=', did you mean to set the "shadow" property?';console.warn(msg);continue}this["set"+key[0].toUpperCase()+key.slice(1)](styling[key])}this.frozen--;this.dirty(jayus.DIRTY.STYLE);return this},findStyleType:function Brush_findStyleType(style){if(style===null)return 0;if(typeof style==="string")return 1;if(style instanceof CanvasGradient)return 2;if(style instanceof CanvasPattern)return 3;if(style instanceof jayus.Color)return 4;if(style instanceof jayus.LinearGradient)return 5;
if(style instanceof jayus.RadialGradient)return 6;throw new Error("Style.findStyleType() - Invalid fill/stroke style: "+jayus.debug.toString(style));},getNativeStyle:function Brush_getNativeStyle(type,style){if(type<=3)return style;if(type===4)return style.toCSS();if(type===5||type===6)return style.getNative();throw new Error("Style.applyTo() - Invalid fill/stroke type identifier: "+jayus.debug.toString(type));},applyTo:function Brush_applyTo(ctx){if(this.alpha!==1)ctx.globalAlpha*=this.alpha;if(this.filling)ctx.fillStyle=
this.getNativeStyle(this.fillType,this.fill);if(this.stroking){ctx.strokeStyle=this.getNativeStyle(this.strokeType,this.stroke);if(this.lineWidth!==null)ctx.lineWidth=this.lineWidth;if(this.lineCap!==null)ctx.lineCap=this.lineCap;if(this.lineJoin!==null)ctx.lineJoin=this.lineJoin;if(this.miterLimit!==null)ctx.miterLimit=this.miterLimit;if(this.lineDash!==null&&jayus.ua.lineDash){ctx.setLineDash(this.lineDash);if(this.lineDashOffset!==null)ctx.lineDashOffset=this.lineDashOffset}}if(this.shadowType){if(this.shadowType===
1)ctx.shadowColor=this.shadow;else if(this.shadowType===2)ctx.shadowColor=this.shadow.toCSS();else throw new Error("Style.applyTo() - Invalid shadow type identifier: "+jayus.debug.toString(this.shadowType));if(this.shadowOffsetX!==null)ctx.shadowOffsetX=this.shadowOffsetX;if(this.shadowOffsetY!==null)ctx.shadowOffsetY=this.shadowOffsetY;if(this.shadowBlur!==null)ctx.shadowBlur=this.shadowBlur}},paintShape:function Brush_paintShape(ctx){this.applyTo(ctx);if(this.stroking&&this.strokeFirst)ctx.stroke();
if(this.filling)ctx.fill();if(this.stroking&&!this.strokeFirst)ctx.stroke()},paintRect:function Brush_paintRect(ctx,x,y,width,height){ctx.save();this.applyTo(ctx);if(this.stroking&&this.strokeFirst)ctx.strokeRect(x,y,width,height);if(this.filling)ctx.fillRect(x,y,width,height);if(this.stroking&&!this.strokeFirst)ctx.strokeRect(x,y,width,height);ctx.restore()},setAlpha:function Brush_setAlpha(alpha){if(alpha!==null)jayus.debug.match("Brush.setAlpha",alpha,"alpha",4);if(alpha===null)alpha=1;if(this.alpha!==
alpha){this.alpha=alpha;this.dirty(jayus.DIRTY.STYLE)}return this},setFill:function Brush_setFill(style){if(style!==null)jayus.debug.match("Brush.setFill",style,"style",0);if(this.fillType>=4)this.fill.detach(this);if(typeof style==="object"&&typeof style.__type__==="string")style=jayus.fromObject(style);this.fill=style;this.fillType=this.findStyleType(style);if(this.fillType>=4)this.fill.attach(this);this.filling=!!this.fillType;this.dirty(jayus.DIRTY.STYLE);return this},setStroke:function Brush_setStroke(style){if(style!==
null)jayus.debug.match("Brush.setStroke",style,"style",0);if(this.strokeType>=4)this.stroke.detach(this);if(typeof style==="object"&&typeof style.__type__==="string")style=jayus.fromObject(style);this.stroke=style;this.strokeType=this.findStyleType(style);if(this.strokeType>=4)this.stroke.attach(this);this.stroking=!!this.strokeType;this.dirty(jayus.DIRTY.STYLE);return this},setLineWidth:function Brush_setLineWidth(lineWidth){if(lineWidth!==null)jayus.debug.match("Brush.setLineWidth",lineWidth,"lineWidth",
4);if(this.lineWidth!==lineWidth){this.lineWidth=lineWidth;this.dirty(jayus.DIRTY.STYLE)}return this},setLineCap:function Brush_setLineCap(lineCap){if(lineCap!==null)jayus.debug.match("Brush.setLineCap",lineCap,"lineCap");if(this.lineCap!==lineCap){this.lineCap=lineCap;this.dirty(jayus.DIRTY.STYLE)}return this},setLineJoin:function Brush_setLineJoin(lineJoin){if(lineJoin!==null)jayus.debug.match("Brush.setLineJoin",lineJoin,"lineJoin",2);if(this.lineJoin!==lineJoin){this.lineJoin=lineJoin;this.dirty(jayus.DIRTY.STYLE)}return this},
setMiterLimit:function Brush_setMiterLimit(miterLimit){if(miterLimit!==null)jayus.debug.match("Brush.setMiterLimit",miterLimit,"miterLimit",2);if(this.miterLimit!==miterLimit){this.miterLimit=miterLimit;this.dirty(jayus.DIRTY.STYLE)}return this},setLineDash:function Brush_setLineDash(lineDash){if(lineDash!==null)jayus.debug.matchArray("Style.setLineDash",lineDash,"lineDash",4);this.lineDash=lineDash;this.dirty(jayus.DIRTY.STYLE);return this},setLineDashOffset:function Brush_setLineDashOffset(lineDashOffset){if(lineDashOffset!==
null)jayus.debug.match("Brush.setLineDashOffset",lineDashOffset,"lineDashOffset",4);if(this.lineDashOffset!==lineDashOffset){this.lineDashOffset=lineDashOffset;this.dirty(jayus.DIRTY.STYLE)}return this},setShadow:function Brush_setShadow(shadow){if(shadow!==null)jayus.debug.match("Brush.setShadow",shadow,"shadow",0);if(this.shadowType===2)this.shadow.detach(this);this.shadow=shadow;if(shadow===null)this.shadowType=0;else if(typeof shadow==="string")this.shadowType=1;else if(shadow instanceof jayus.Color){this.shadowType=
2;this.shadow.attach(this)}else throw new Error("Style.setShadow() - Invalid shadow style: "+jayus.debug.toString(shadow));this.dirty(jayus.DIRTY.STYLE);return this},setShadowOffsetX:function Brush_setShadowOffsetX(shadowOffsetX){if(shadowOffsetX!==null)jayus.debug.match("Brush.setShadowOffsetX",shadowOffsetX,"shadowOffsetX",4);if(this.shadowOffsetX!==shadowOffsetX){this.shadowOffsetX=shadowOffsetX;this.dirty(jayus.DIRTY.STYLE)}return this},setShadowOffsetY:function Brush_setShadowOffsetY(shadowOffsetY){if(shadowOffsetY!==
null)jayus.debug.match("Brush.setShadowOffsetY",shadowOffsetY,"shadowOffsetY",4);if(this.shadowOffsetY!==shadowOffsetY){this.shadowOffsetY=shadowOffsetY;this.dirty(jayus.DIRTY.STYLE)}return this},setShadowBlur:function Brush_setShadowBlur(shadowBlur){if(shadowBlur!==null)jayus.debug.match("Brush.setShadowBlur",shadowBlur,"shadowBlur",4);if(this.shadowBlur!==shadowBlur){this.shadowBlur=shadowBlur;this.dirty(jayus.DIRTY.STYLE)}return this}});jayus.debug.className="Gradient";
jayus.Gradient=jayus.Dependency.extend({componentType:"GRADIENT",stopPositions:null,stopColors:null,nativeGradient:null,reformNative:true,toObject:function Gradient_toObject(){var object={__type__:"Gradient"};if(this.stopPositions!==null)object.stopPositions=this.stopPositions;if(this.stopColors!==null)object.stopColors=this.stopColors.toObject();if(this.id!==0)object.id=this.id;return object},initFromObject:function Gradient_initFromObject(object){jayus.debug.match("Gradient.initFromObject",object,
"object",5);if(typeof object.stopPositions==="object")this.stopPositions=object.stopPositions;if(typeof object.stopColors==="object")this.stopColors=object.stopColors;this.reformNative=true;return this.dirty(jayus.DIRTY.ALL)},componentDirtied:function Gradient_componentDirtied(component,type){this.reformNative=true;this.dirty(jayus.DIRTY.STYLE)},translate:function Gradient_translate(x,y){jayus.debug.matchCoordinate("Gradient.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}this.start.translate(x,
y);this.end.translate(x,y);this.dirty(jayus.DIRTY.ALL);return this},addColorStop:function Gradient_addColorStop(position,color){jayus.debug.matchArguments("Gradient.addColorStop",arguments,"position",4,"color",0);this.stopPositions.push(position);this.stopColors.push(color);return this},getNative:function Gradient_getNative(){if(this.reformNative){this.refresh();this.reformNative=false}return this.nativeGradient}});jayus.debug.className="LinearGradient";
jayus.LinearGradient=jayus.Gradient.extend({start:null,end:null,init:overloadArgumentCount({0:function LinearGradient_init(){},2:function LinearGradient_init(start,end){jayus.debug.matchArguments("LinearGradient.init",arguments,"start",13,"end",13);this.stopPositions=[];this.stopColors=[];this.start=start;this.start.attach(this);this.end=end;this.end.attach(this)},4:function LinearGradient_init(x1,y1,x2,y2){jayus.debug.matchArgumentsAs("LinearGradient.init",arguments,4,"x1","y1","x2","y2");this.init(new jayus.Point(x1,
y1),new jayus.Point(x2,y2))}}),toObject:function LinearGradient_toObject(){var object=jayus.Gradient.prototype.toObject.apply(this);object.__type__="LinearGradient";if(this.start!==null)object.start=this.start.toObject();if(this.end!==null)object.end=this.end.toObject();return object},initFromObject:function LinearGradient_initFromObject(object){jayus.debug.match("LinearGradient.initFromObject",object,"object",5);jayus.Gradient.prototype.initFromObject.call(this,object);if(typeof object.start==="object")this.start=
new jayus.Point(object.start);if(typeof object.end==="object")this.end=new jayus.Point(object.end);return this.dirty(jayus.DIRTY.ALL)},refresh:function LinearGradient_refresh(){var i,color;this.nativeGradient=jayus.getContext().createLinearGradient(this.start.x,this.start.y,this.end.x,this.end.y);for(i=0;i<this.stopPositions.length;i++){color=this.stopColors[i];if(color instanceof jayus.Color)color=color.toCSS();this.nativeGradient.addColorStop(this.stopPositions[i],color)}return this}});
...
r1),new jayus.Circle(end,r2))},6:function RadialGradient_init(cx1,cy1,r1,cx2,cy2,r2){jayus.debug.matchArgumentsAs("RadialGradient.init",arguments,4,"cx1","cy1","r1","cx2","cy2","r2");this.init(new jayus.Circle(cx1,cy1,r1),new jayus.Circle(cx2,cy2,r2))}}),toObject:function RadialGradient_toObject(){var object=jayus.Gradient.prototype.toObject.apply(this);object.__type__="RadialGradient";if(this.start!==jayus.RadialGradient.prototype.start)object.start=this.start.toObject();if(this.end!==jayus.RadialGradient.prototype.end)object.end=
this.end.toObject();return object},initFromObject:function RadialGradient_initFromObject(object){jayus.debug.match("RadialGradient.initFromObject",object,"object",5);jayus.Gradient.prototype.initFromObject.call(this,object);if(typeof object.start==="object")this.start=new jayus.Circle(object.start);if(typeof object.end==="object")this.end=new jayus.Circle(object.end);return this.dirty(jayus.DIRTY.ALL)},refresh:function RadialGradient_refresh(){var i,color;this.nativeGradient=jayus.getContext().createRadialGradient(this.start.x,
this.start.y,this.start.radius,this.end.x,this.end.y,this.end.radius);for(i=0;i<this.stopPositions.length;i++){color=this.stopColors[i];if(color instanceof jayus.Color)color=color.toCSS();this.nativeGradient.addColorStop(this.stopPositions[i],color)}return this}});jayus.debug.className="Matrix";
jayus.Matrix=jayus.createClass({a:1,b:0,tx:0,c:0,d:1,ty:0,init:function Matrix_init(a,b,tx,c,d,ty){if(arguments.length===1)this.initFromObject(a);else if(arguments.length){jayus.debug.matchArgumentsAs("Matrix.init",arguments,4,"a","b","tx","c","d","ty");this.a=a;this.b=b;this.tx=tx;this.c=c;this.d=d;this.ty=ty}},toObject:function Matrix_toObject(){var object={__type__:"Matrix",a:this.a,b:this.b,tx:this.tx,c:this.c,d:this.d,ty:this.ty};if(this.id!==0)object.id=this.id;return object},initFromObject:function Matrix_initFromObject(object){jayus.debug.match("Matrix.initFromObject",
object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.a=object.a;this.b=object.b;this.tx=object.tx;this.c=object.c;this.d=object.d;this.ty=object.ty;this.dirty(jayus.DIRTY.ALL)},translate:function Matrix_translate(x,y){jayus.debug.matchCoordinate("Matrix.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}this.tx+=this.a*x+this.b*y;this.ty+=this.d*y+this.c*x;return this},scale:function Matrix_scale(x,y){jayus.debug.matchCoordinate("Matrix.scale",x,y);if(arguments.length===
1){y=x.y;x=x.x}this.a*=x;this.c*=x;this.d*=y;this.b*=y;return this},rotate:function Matrix_rotate(angle){jayus.debug.match("Matrix.rotate",angle,"angle",4);var sin=Math.sin(angle),cos=Math.cos(angle);var a=this.a*cos+this.b*sin;this.b=-this.a*sin+this.b*cos;this.a=a;var c=this.d*sin+this.c*cos;this.d=this.d*cos-this.c*sin;this.c=c;return this},multiply:function Matrix_multiply(m1){jayus.debug.match("Matrix.multiply",m1,"m1",10);m2=this;return new jayus.Matrix(m1.a*m2.a+m1.b*m2.c,m1.a*m2.b+m1.b*m2.d,
m1.a*m2.tx+m1.b*m2.ty+m1.tx,m1.c*m2.a+m1.d*m2.c,m1.c*m2.b+m1.d*m2.d,m1.c*m2.tx+m1.d*m2.ty+m1.ty)},transformPoint:function Matrix_transformPoint(x,y){jayus.debug.matchCoordinate("Matrix.transformPoint",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.transformPointOnto(x,y,new jayus.Point)},transformPointOnto:function Matrix_transformPointOnto(x,y,ret){jayus.debug.matchArguments("Matrix.transformPointOnto",arguments,"x",4,"y",4,"ret",13);ret.x=x*this.a+y*this.b+this.tx;ret.y=x*this.c+y*this.d+
this.ty;return ret},inverseTransformPoint:function Matrix_inverseTransformPoint(x,y){jayus.debug.matchCoordinate("Matrix.inverseTransformPoint",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.inverseTransformPointOnto(x,y,new jayus.Point)},inverseTransformPointOnto:function Matrix_inverseTransformPointOnto(x,y,ret){jayus.debug.matchArguments("Matrix.inverseTransformPointOnto",arguments,"x",4,"y",4,"ret",13);var det=this.getDeterminant();if(det){x-=this.tx;y-=this.ty;ret.x=(x*this.d-y*this.b)/
det;ret.y=(y*this.a-x*this.c)/det;return ret}return null},identity:function Matrix_identity(){this.a=this.d=1;this.b=this.tx=this.c=this.ty=0;return this},getDeterminant:function Matrix_getDeterminant(){var det=this.a*this.d-this.b*this.c;if(isFinite(det)&&Math.abs(det)>1.0E-11&&isFinite(this.tx)&&isFinite(this.ty))return det;return null},clone:function Matrix_clone(){jayus.chart.tallyInit("Matrix","Matrix.clone()");return this.cloneOnto(new jayus.Matrix)},cloneOnto:function Matrix_cloneOnto(ret){jayus.debug.match("Matrix.cloneOnto",
ret,"ret",10);ret.init(this.a,this.b,this.tx,this.c,this.d,this.ty);return ret},equals:function Matrix_equals(mat){jayus.debug.match("Matrix.equals",mat,"mat",10);return this.a===mat.a&&this.b===mat.b&&this.c===mat.c&&this.d===mat.d&&this.tx===mat.tx&&this.ty===mat.ty},applyToContext:function Matrix_applyToContext(ctx){jayus.debug.matchContext("Matrix.applyToContext",ctx);ctx.transform(this.a,this.c,this.b,this.d,this.tx,this.ty)}});jayus.debug.className="SizePolicy";
jayus.SizePolicy=jayus.createClass({size:0,weight:1,expand:true,init:function SizePolicy_init(object){if(arguments.length)this.initFromObject(object)},toObject:function SizePolicy_toObject(){object.__type__="SizePolicy";if(this.size!==0)object.size=this.size;if(this.weight!==1)object.weight=this.weight;if(this.expand!==true)object.expand=this.expand;return object},initFromObject:function SizePolicy_initFromObject(object){jayus.debug.match("SizePolicy.initFromObject",object,"object",5);if(typeof object.size===
"number")this.size=object.size;if(typeof object.weight==="number")this.weight=object.weight;if(typeof object.expand==="boolean")this.expand=object.expand;return this}});jayus.images={images:{},pendingImageCount:0,checkFilepath:function jayus_images_checkFilepath(filepath){jayus.debug.match("jayus.images.checkFilepath",filepath,"filepath",2);if(filepath.indexOf("../")!==-1)console.warn('jayus.images.checkFilepath() - Filepath "'+filepath+'" contains "./" or "../", avoid using these')},has:function jayus_images_has(filepath){jayus.debug.match("jayus.images.has",filepath,"filepath",2);this.checkFilepath(filepath);return typeof this.images[filepath]==="object"},isLoaded:function jayus_images_isLoaded(filepath){jayus.debug.match("jayus.images.isLoaded",
filepath,"filepath",2);this.checkFilepath(filepath);return typeof this.images[filepath]==="object"&&(this.images[filepath].loaded||this.images[filepath].tagName==="CANVAS")},get:function jayus_images_get(filepath){jayus.debug.match("jayus.images.get",filepath,"filepath",2);this.checkFilepath(filepath);if(!this.has(filepath)){console.warn('jayus.images.get() - Image "'+filepath+'" not yet loaded');this.load(filepath)}return this.images[filepath]},setSubimage:function jayus_images_setSubimage(sourceImage,
image,sourceX,sourceY,sourceWidth,sourceHeight){var source=this.get(sourceImage);var canvas=document.createElement("canvas");canvas.width=sourceWidth;canvas.height=sourceHeight;var ctx=canvas.getContext("2d");ctx.drawImage(source,sourceX,sourceY,sourceWidth,sourceHeight,0,0,sourceWidth,sourceHeight);this.images[image]=canvas;return this},load:function jayus_images_load(filepath,handler){jayus.debug.match("jayus.images.load",filepath,"filepath",2);this.checkFilepath(filepath);if(!this.has(filepath)){if(!jayus.debug.is(2,
filepath))throw new TypeError("jayus.images.load() - Invalid filepath"+jayus.debug.toString(filepath)+" sent as argument "+i+", String required");var image=new Image;image.isSheet=false;image.loaded=false;this.images[filepath]=image;image.onload=function image_onload(){this.loaded=true;jayus.fire("imageLoaded",{filepath:filepath,image:this});jayus.images.pendingImageCount--;if(!jayus.images.pendingImageCount)jayus.fire("imagesLoaded");if(typeof handler==="function")handler(filepath,this)};this.pendingImageCount++;
image.src=filepath}},loadAll:function jayus_images_loadAll(filepaths,handler){var i,count,checkCount;if(arguments.length===1)for(i=0;i<filepaths.length;i++)this.load(filepaths[i]);else if(arguments.length===2){count=filepaths.length;checkCount=function(){count--;if(!count)handler()};for(i=0;i<filepaths.length;i++)this.load(filepaths[i],checkCount)}}};jayus.objects={objects:{},pendingObjectCount:0,has:function jayus_objects_has(filepath){return typeof this.objects[filepath]==="object"&&this.objects[filepath]!==null},get:function jayus_objects_get(filepath){if(!this.has(filepath)){console.warn('jayus.objects.get() - Object "'+filepath+'" not yet loaded');this.load(filepath)}return this.objects[filepath]},load:function jayus_objects_load(filepath,handler){jayus.debug.match("jayus.objects.load",filepath,"filepath",2);jayus.debug.matchOptional("jayus.objects.load",
...
jayus.Point=jayus.Dependency.extend({componentType:"POINT",x:0,y:0,init:function Point_init(x,y){if(arguments.length===1)this.initFromObject(x);else if(arguments.length){jayus.debug.matchArgumentsAs("Point.init",arguments,4,"x","y");this.x=x;this.y=y}},toObject:function Point_toObject(){var object={__type___:"Point",x:this.x,y:this.y};if(this.id!==0)object.id=this.id;return object},initFromObject:function Point_initFromObject(object){jayus.debug.match("Point.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,
object);this.x=object.x;this.y=object.y;return this.dirty(jayus.DIRTY.ALL)},setX:function Point_setX(x){jayus.debug.match("Point.setX",x,"x",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setX,this.x,x)}if(this.x!==x){this.x=x;this.dirty(2)}return this},setY:function Point_setY(y){jayus.debug.match("Point.setY",y,"y",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setY,this.y,y)}if(this.y!==y){this.y=y;
this.dirty(2)}return this},set:function Point_set(x,y){jayus.debug.matchArgumentsAs("Point.set",arguments,4,"x","y");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.set,[this.x,this.y],[x,y])}if(this.x!==x||this.y!==y){this.x=x;this.y=y;this.dirty(2)}return this},translate:function Point_translate(x,y){jayus.debug.matchCoordinate("Point.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.set(this.x+x,this.y+y)},clone:function Point_clone(){return new jayus.Point(this.x,
this.y)},cloneOnto:function Point_cloneOnto(ret){ret.x=this.x;ret.y=this.y;return ret},equals:function Point_equals(x,y){jayus.debug.matchCoordinate("Point.equals",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.x===x&&this.y===y}});jayus.debug.className="Rectangle";
jayus.Rectangle=jayus.Shape.extend({componentType:"RECTANGLE",shapeType:4,x:0,y:0,width:0,height:0,keepAligned:true,init:function Rectangle_init(x,y,width,height){if(arguments.length===1)this.initFromObject(x);else if(arguments.length){jayus.debug.matchRectangle("Rectangle.init",x,y,width,height);if(arguments.length===2){height=y.height;width=y.width;y=x.y;x=x.x}else if(arguments.length===3){height=width;width=height;y=x.y;x=x.x}this.x=x;this.y=y;this.width=width;this.height=height}},toObject:function Rectangle_toObject(){var object=
{__type__:"Rectangle",x:this.x,y:this.y,width:this.width,height:this.height};if(this.keepAligned!==true)object.keepAligned=this.keepAligned;if(this.id!==0)object.id=this.id;return object},initFromObject:function Rectangle_initFromObject(object){jayus.debug.match("Rectangle.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.x=object.x;this.y=object.y;this.width=object.width;this.height=object.height;if(typeof object.keepAligned==="number")this.keepAligned=
object.keepAligned;return this.dirty(jayus.DIRTY.ALL)},getOrigin:function Rectangle_getOrigin(){return new jayus.Point(this.x,this.y)},getOriginOnto:function Rectangle_getOriginOnto(ret){ret.x=this.x;ret.y=this.y;return ret},setX:jayus.Point.prototype.setX,setY:jayus.Point.prototype.setY,setOrigin:function Rectangle_setOrigin(x,y){jayus.debug.matchCoordinate("Rectangle.set",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,
this.setOrigin,[this.x,this.y],[x,y])}if(this.x!==x||this.y!==y){this.x=x;this.y=y;this.dirty(2)}return this},translate:function Rectangle_translate(x,y){jayus.debug.matchCoordinate("Rectangle.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setOrigin(this.x+x,this.y+y)},setWidth:function Rectangle_setWidth(width){jayus.debug.match("Rectangle.setWidth",width,"width",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setWidth,this.width,width)}if(this.width!==
width){this.width=width;this.dirty(4)}return this},setHeight:function Rectangle_setHeight(height){jayus.debug.match("Rectangle.setHeight",height,"height",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setHeight,this.height,height)}if(this.height!==height){this.height=height;this.dirty(4)}return this},setSize:function Rectangle_setSize(width,height){jayus.debug.matchSize("Rectangle.setSize",width,height);if(arguments.length===1){height=width.height;width=
width.width}if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setSize,[this.width,this.height],[width,height])}if(this.width!==width||this.height!==height){this.width=width;this.height=height;this.dirty(4)}return this},expand:function Rectangle_expand(width,height){jayus.debug.matchArgumentsAs("Rectangle.expand",arguments,4,"width","height");return this.setSize(this.width+width,this.height+height)},getLeft:function Rectangle_getLeft(){return this.x},getRight:function Rectangle_getRight(){return this.x+
this.width},getTop:function Rectangle_getTop(){return this.y},getBottom:function Rectangle_getBottom(){return this.y+this.height},setLeft:function Rectangle_setLeft(left){jayus.debug.match("Rectangle.setLeft",left,"left",4);return this.setFrame(left,this.y,this.x+this.width-left,this.height)},setRight:function Rectangle_setRight(right){jayus.debug.match("Rectangle.setRight",right,"right",4);return this.setWidth(this.width+right-this.x)},setTop:function Rectangle_setTop(top){jayus.debug.match("Rectangle.setTop",
top,"top",4);return this.setFrame(this.x,top,this.width,this.y+this.height-top)},setBottom:function Rectangle_setBottom(bottom){jayus.debug.match("Rectangle.setBottom",bottom,"bottom",4);return this.setHeight(this.height+bottom-this.y)},setFrame:function Rectangle_setFrame(x,y,width,height){if(arguments.length===3){jayus.debug.matchArguments("Rectangle.setFrame",arguments,"origin",13,"width",4,"height",4);height=width;width=y;y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Rectangle.setFrame",arguments,
4,"x","y","width","height");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setFrame,[this.x,this.y,this.width,this.height],[x,y,width,height])}this.x=x;this.y=y;this.width=width;this.height=height;this.dirty(jayus.DIRTY.FRAME);return this},includePoint:function Rectangle_includePoint(x,y){jayus.debug.matchCoordinate("Rectangle.includePoint",x,y);if(arguments.length===1){y=x.y;x=x.x}var x1=Math.min(this.x,x),y1=Math.min(this.y,y),x2=Math.max(this.x+this.width,
x),y2=Math.max(this.y+this.height,y);return this.setFrame(x1,y1,x2-x1,y2-y1)},includeRectangle:function Rectangle_includeRectangle(x,y,width,height){if(arguments.length===1){jayus.debug.match("Rectangle.includeRectangle()",x,"section",20);height=x.height;width=x.width;y=x.y;x=x.x}else if(arguments.length===3){jayus.debug.matchArguments("Rectangle.includeRectangle()",arguments,"origin",13,"width",4,"height",4);height=width;width=y;y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Rectangle.includeRectangle()",
arguments,4,"x","y","width","height");var x1=Math.min(this.x,x),y1=Math.min(this.y,y),x2=Math.max(this.x+this.width,x+width),y2=Math.max(this.y+this.height,y+height);return this.setFrame(x1,y1,x2-x1,y2-y1)},alignToCanvas:function Rectangle_alignToCanvas(){return this.setFrame(Math.floor(this.x)+0.5,Math.floor(this.y)+0.5,Math.round(this.width),Math.round(this.height))},intersectsAt:function Rectangle_intersectsAt(x,y){jayus.debug.matchArgumentsAs("Rectangle.intersectsAt",arguments,4,"x","y");return this.x<=
x&&x<=this.x+this.width&&this.y<=y&&y<=this.y+this.height},getScope:function Rectangle_getScope(){return this.clone()},getTransformed:function Rectangle_getTransformed(matrix){jayus.debug.match("Rectangle.getTransformed",matrix,"matrix",10);return this.toPolygon().getTransformed(matrix)},clone:function Rectangle_clone(){return this.cloneOnto(new jayus.Rectangle)},cloneOnto:function Rectangle_cloneOnto(ret){jayus.debug.match("Rectangle.cloneOnto",ret,"ret",20);ret.x=this.x;ret.y=this.y;ret.width=this.width;
ret.height=this.height;return ret},toPolygon:function Rectangle_toPolygon(){return new jayus.Polygon.Rectangle(this.x,this.y,this.width,this.height)},etchOntoContext:function Rectangle_etchOntoContext(ctx){jayus.debug.matchContext("Rectangle.etchOntoContext",ctx);ctx.beginPath();if(this.keepAligned)ctx.rect(Math.round(this.x)+0.5,Math.round(this.y)+0.5,Math.round(this.width),Math.round(this.height));else ctx.rect(this.x,this.y,this.width,this.height);return this}});jayus.debug.className="Circle";
jayus.Circle=jayus.Shape.extend({shapeType:2,componentType:"CIRCLE",x:0,y:0,radius:1,toPolygonDetail:10,init:function Circle_init(x,y,radius){if(arguments.length===1)this.initFromObject(x);else if(arguments.length){if(arguments.length===2)jayus.debug.matchArguments("Circle.init",arguments,"center",13,"radius",13);else if(arguments.length===3)jayus.debug.matchArgumentsAs("Circle.init",arguments,4,"x","y","radius");else throw new TypeError("Circle.init() - Invalid number of parameters sent, 0, 2, or 3 required");if(arguments.length===
2){radius=y;y=x.y;x=x.x}this.x=x;this.y=y;this.radius=radius}},pointChanged:function Circle_pointChanged(){},toObject:function Circle_toObject(){var object={__type__:"Circle",x:this.x,y:this.y,radius:this.radius};if(this.toPolygonDetail!==10)object.toPolygonDetail=this.toPolygonDetail;if(this.id!==0)object.id=this.id;return object},initFromObject:function Circle_initFromObject(object){jayus.debug.match("Circle.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,
object);this.x=object.x;this.y=object.y;this.radius=object.radius;if(typeof object.toPolygonDetail==="number")this.toPolygonDetail=object.toPolygonDetail;return this.dirty(jayus.DIRTY.ALL)},getCenter:function Circle_getCenter(){return new jayus.Point(this.x,this.y)},setX:jayus.Point.prototype.setX,setY:jayus.Point.prototype.setY,setCenter:jayus.Point.prototype.set,translate:function Circle_translate(x,y){jayus.debug.matchCoordinate("Circle.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setCenter(this.x+
x,this.y+y)},alignToCanvas:function Circle_alignToCanvas(){return this},getRadius:function Circle_getRadius(){return this.radius},setRadius:function Circle_setRadius(radius){jayus.debug.match("Circle.setRadius",radius,"radius",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setRadius,this.radius,radius)}if(this.radius!==radius){this.radius=radius;this.dirty(4)}return this},intersectsAt:function Circle_intersectsAt(x,y){jayus.debug.matchArgumentsAs("Circle.intersectsAt",
arguments,4,"x","y");return(this.x-x)*(this.x-x)+(this.y-y)*(this.y-y)<=this.radius*this.radius},getScope:function Circle_getScope(){return new jayus.Rectangle(this.x-this.radius,this.y-this.radius,this.radius*2,this.radius*2)},clone:function Circle_clone(){return new jayus.Circle(this.x,this.y,this.radius)},cloneOnto:function Circle_cloneOnto(ret){ret.x=this.x;ret.y=this.y;ret.radius=this.radius;return ret},toPolygon:function Circle_toPolygon(detail){if(!arguments.length)detail=this.toPolygonDetail;
else jayus.debug.match("Circle.toPolygon",detail,"detail",4);return new jayus.Polygon.Regular(detail,this.radius)},etchOntoContext:function Circle_etchOntoContext(ctx){jayus.debug.matchContext("Circle.etchOntoContext",ctx);ctx.beginPath();ctx.arc(this.x,this.y,this.radius,0,Math.PI*2)}});jayus.debug.className="Polygon";
jayus.Polygon=jayus.Shape.extend({componentType:"POLYGON",shapeType:8,xPoints:null,yPoints:null,x1:0,y1:0,x2:0,y2:0,closed:true,frame:null,frameDirty:true,init:function Polygon_init(xPoints,yPoints){if(arguments.length===2){jayus.debug.matchArray("Polygon.init",xPoints,"xPoints",4);jayus.debug.matchArray("Polygon.init",yPoints,"yPoints",4);this.xPoints=xPoints;this.yPoints=yPoints}else{this.xPoints=[];this.yPoints=[];if(arguments.length===1){jayus.debug.matchArray("Polygon.init",xPoints,"xPoints",
13);for(var i=0;i<xPoints.length;i++){this.xPoints[i]=xPoints[i].x;this.yPoints[i]=xPoints[i].y}}}},toObject:function Polygon_toObject(){var object={__type__:"Polygon",x:this.xPoints,y:this.yPoints};if(this.closed!==true)object.closed=this.closed;if(this.id!==0)object.id=this.id;return object},initFromObject:function Polygon_initFromObject(object){jayus.debug.match("Polygon.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.x=object.xPoints;this.y=
object.yPoints;if(typeof object.closed==="boolean")this.closed=object.closed;this.reformFrame();return this.dirty(jayus.DIRTY.ALL)},translate:function Polygon_translate(x,y){jayus.debug.matchCoordinate("Polygon.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}if(x||y){for(var i=0;i<this.xPoints.length;i++){this.xPoints[i]+=x;this.yPoints[i]+=y}this.frameDirty=true}return this},alignToCanvas:function Polygon_alignToCanvas(){return this},addPoint:function Polygon_addPoint(x,y){jayus.debug.matchCoordinate("Polygon.addPoint",
x,y);if(arguments.length===1){y=x.y;x=x.x}this.xPoints.push(x);this.yPoints.push(y);this.frameDirty=true;this.dirty(16);return this},addPoints:function Polygon_addPoints(xPoints,yPoints){if(arguments.length===1){jayus.debug.matchArray("Polygon.addPoints",xPoints,"xPoints",13);for(var i=0;i<xPoints.length;i++){this.xPoints.push(xPoints[i].x);this.yPoints.push(xPoints[i].y)}}else if(arguments.length===2){jayus.debug.matchArray("Polygon.addPoints",xPoints,"xPoints",4);jayus.debug.matchArray("Polygon.addPoints",
yPoints,"yPoints",4);this.xPoints=this.xPoints.concat(xPoints);this.yPoints=this.yPoints.concat(yPoints)}this.frameDirty=true;this.dirty(16);return this},insertPoint:function Polygon_insertPoint(index,x,y){if(arguments.length===2){jayus.debug.matchArguments("Polygon.insertPoint",arguments,"index",4,"point",13);y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Polygon.insertPoint",arguments,4,"index","x","y");this.xPoints.splice(index,0,x);this.yPoints.splice(index,0,y);this.frameDirty=true;this.dirty(16);
return this},setPoint:function Polygon_setPoint(index,x,y){if(arguments.length===2){jayus.debug.matchArguments("Polygon.setPoint",arguments,"index",4,"point",13);y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Polygon.setPoint",arguments,4,"index","x","y");this.xPoints[index]=x;this.yPoints[index]=y;this.frameDirty=true;this.dirty(16);return this},getLeft:function Shape_getLeft(){return this.getFrame().getLeft()},getTop:function Shape_getTop(){return this.getFrame().getTop()},getWidth:function Shape_getWidth(){return this.getFrame().getWidth()},
getHeight:function Shape_getHeight(){return this.getFrame().getHeight()},getScope:function Polygon_getScope(){return this.getFrame()},getFrame:function Polygon_getFrame(){if(this.frameDirty){this.reformFrame();this.frameDirty=false}return this.frame},reformFrame:function Polygon_reformFrame(){var i,x,y,x1=null,x2=null,y1=null,y2=null;for(i=0;i<this.xPoints.length;i++){x=this.xPoints[i];y=this.yPoints[i];if(x1===null||x<x1)x1=x;if(x2===null||x>x2)x2=x;if(y1===null||y<y1)y1=y;if(y2===null||y>y2)y2=
y}if(this.frame===null)this.frame=new jayus.Rectangle;this.frame.setFrame(x1,y1,x2-x1,y2-y1)},intersectsAt:function Polygon_intersectsAt(x,y){jayus.debug.matchArgumentsAs("Polygon.intersectsAt",arguments,4,"x","y");var ctx=jayus.getContext();ctx.save();this.etchOntoContext(ctx);var ret=ctx.isPointInPath(x,y);ctx.restore();return ret},setClosed:function Polygon_setClosed(on){jayus.debug.match("Polygon.setClosed",on,"on",1);if(this.closed!==on){this.closed=on;this.dirty(16)}return this},clone:function Polygon_clone(){return new jayus.Polygon(this.xPoints.slice(0),
this.yPoints.slice(0))},cloneOnto:function Polygon_cloneOnto(ret){ret.xPoints=this.xPoints.slice(0);ret.yPoints=this.yPoints.slice(0);ret.frameDirty=true;return ret},transform:function Polygon_transform(matrix){jayus.debug.match("Polygon.transform",matrix,"matrix",10);var i,point=new jayus.Point;for(i=0;i<this.xPoints.length;i++){matrix.transformPointOnto(this.xPoints[i],this.yPoints[i],point);this.xPoints[i]=point.x;this.yPoints[i]=point.y}this.frameDirty=true;return this},getTransformed:function Polygon_getTransformed(matrix){jayus.debug.match("Polygon.getTransformed",
matrix,"matrix",10);return this.getTransformedOnto(matrix,new jayus.Polygon)},getTransformedOnto:function Polygon_getTransformedOnto(matrix,ret){jayus.debug.matchArguments("Polygon.getTransformedOnto",arguments,"matrix",10,"ret",15);var i,point=new jayus.Point;ret.xPoints=[];ret.yPoints=[];for(i=0;i<this.xPoints.length;i++){matrix.transformPointOnto(this.xPoints[i],this.yPoints[i],point);ret.xPoints.push(point.x);ret.yPoints.push(point.y)}return ret},etchOntoContext:function Polygon_etchOntoContext(ctx){jayus.debug.matchContext("Polygon.etchOntoContext",
ctx);if(this.xPoints.length){ctx.beginPath();ctx.moveTo(this.xPoints[0],this.yPoints[0]);for(var i=0;i<this.xPoints.length;i++)ctx.lineTo(this.xPoints[i],this.yPoints[i]);if(this.closed)ctx.closePath()}return this}});
...
jayus.Path=jayus.Shape.extend({shapeType:16,segments:null,x1:0,y1:0,x2:0,y2:0,frame:null,frameDirty:true,toPolygonDetail:10,init:function Path_init(pathString){this.segments=[];if(arguments.length){jayus.debug.match("Path.init",pathString,"pathString",2);this.addSVS(pathString)}},translate:function Path_translate(x,y){jayus.debug.matchCoordinate("Path.translate",x,y);if(arguments.length===1){y=x.y;x=x.x}for(var i=0;i<this.segments.length;i++)this.moveSegment(i,x,y);return this},alignToCanvas:function Path_alignToCanvas(){return this},
getScope:function Path_getScope(){if(this.segments.length===0)return null;this.reformFrame();return new jayus.Rectangle(this.x1,this.y1,this.x2-this.x1,this.y2-this.y1)},reformFrame:function Path_reformFrame(){var i,x,y,point,x1=null,x2=null,y1=null,y2=null;for(i=0;i<this.segments.length;i++){point=this.getPoint(i);x=point.x;y=point.y;if(x1===null||x<x1)x1=x;if(x2===null||x>x2)x2=x;if(y1===null||y<y1)y1=y;if(y2===null||y>y2)y2=y}this.x1=x1;this.x2=x2;this.y1=y1;this.y2=y2},intersectsAt:function Path_intersectsAt(x,
y){jayus.debug.matchCoordinate("Path.intersectsAt",x,y);var ctx=jayus.getContext();ctx.save();this.etchOntoContext(ctx);var ret=ctx.isPointInPath(x,y);ctx.restore();return ret},moveTo:function Path_moveTo(x,y){jayus.debug.matchArgumentsAs("Path.moveTo",arguments,4,"x","y");return this.addSegment(["moveTo",x,y])},moveBy:function Path_moveBy(x,y){jayus.debug.matchArgumentsAs("Path.moveBy",arguments,4,"x","y");var pos=this.getCurrentPoint();return this.moveTo(pos.x+x,pos.y+y)},closePath:function Path_closePath(){return this.addSegment(["closePath"])},
...
index,"index",4);return 0<=index&&index<this.segments.length},isClosed:function Path_isClosed(){return this.getInitialPoint().equals(this.getCurrentPoint())},clone:function Path_clone(){var i,j,segment,newSegment,newSegments=[];for(i=0;i<this.segments.length;i++){segment=this.segments[i];newSegment=[];for(j=0;j<segment.length;j++)newSegment.push(segment[j]);newSegments.push(newSegment)}var path=new jayus.Path;path.segments=newSegments;return path},getTransformed:function Path_getTransformed(matrix){var i,
segment,newSegment,temp,newSegments=[];for(i=0;i<this.segments.length;i++){segment=this.segments[i];newSegment=[segment[0]];newSegments.push(newSegment);switch(segment[0]){case "moveTo":temp=matrix.transformPoint(segment[1],segment[2]);newSegment[1]=temp.x;newSegment[2]=temp.y;break;case "closePath":break;case "lineTo":temp=matrix.transformPoint(segment[1],segment[2]);newSegment[1]=temp.x;newSegment[2]=temp.y;break;case "quadraticCurveTo":temp=matrix.transformPoint(segment[1],segment[2]);newSegment[1]=
temp.x;newSegment[2]=temp.y;temp=matrix.transformPoint(segment[3],segment[4]);newSegment[3]=temp.x;newSegment[4]=temp.y;break;case "bezierCurveTo":temp=matrix.transformPoint(segment[1],segment[2]);newSegment[1]=temp.x;newSegment[2]=temp.y;temp=matrix.transformPoint(segment[3],segment[4]);newSegment[3]=temp.x;newSegment[4]=temp.y;temp=matrix.transformPoint(segment[5],segment[6]);newSegment[5]=temp.x;newSegment[6]=temp.y;break;case "arcTo":case "arc":case "rect":case "ellipse":throw new Error("TODO: Path.getTransformed() - Unimplemented for command "+
segment[0]);}}},toPolygon:function Path_toPolygon(detail){if(!arguments.length)detail=this.toPolygonDetail;else jayus.debug.match("Circle.toPolygon",detail,"detail",4);throw new Error("TODO: Path.toPolygon() - Unimplemented");},etchOntoContext:function Path_etchOntoContext(ctx){ctx.beginPath();var i,segment,point=this.getPoint(0),x=point.x,y=point.y;for(i=0;i<this.segments.length;i++){segment=this.segments[i];switch(segment[0]){case "moveTo":x=segment[1];y=segment[2];ctx.moveTo(x,y);break;case "closePath":ctx.closePath();
break;case "lineTo":x=segment[1];y=segment[2];ctx.lineTo(x,y);break;case "bezierCurveTo":x=segment[5];y=segment[6];ctx.bezierCurveTo(segment[1],segment[2],segment[3],segment[4],x,y);break;case "quadraticCurveTo":x=segment[3];y=segment[4];ctx.quadraticCurveTo(segment[1],segment[2],x,y);break;case "arcTo":ctx.arcTo(segment[1],segment[2],segment[3],segment[4],segment[5]);point=this.getPoint(i);x=point.x;y=point.y;break;case "arc":ctx.arc(segment[1],segment[2],segment[3],segment[4],segment[5],segment[6]);
point=this.getPoint(i);x=point.x;y=point.y;break;case "rect":x=segment[1];y=segment[2];ctx.rect(x,y,segment[3],segment[4]);break;case "ellipse":throw new Error("Path.etchOntoContext() - Unimplemented for command ellipse");}}return this}});
jayus.Path.Circle=function Path_Circle(x,y,radius){if(arguments.length===2){jayus.debug.matchArguments("Path.Circle()",arguments,"center",13,"radius",4);radius=y;y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Path.Circle()",arguments,4,"x","y","radius");return(new jayus.Path).arc(x,y,radius,0,Math.PI*2,false)};
...
jayus.Entity=jayus.Responder.extend({componentType:"ENTITY",shapeType:0,isEntity:true,isRect:false,isParent:false,visible:true,alpha:1,dirtied:true,frozen:0,hasParent:false,parent:null,angle:0,xScale:1,yScale:1,xAnchor:0,yAnchor:0,trackCursor:true,canAcceptCursor:false,canReleaseCursor:true,underCursor:false,hasCursor:false,draggable:false,dragButton:"left",dragging:false,enableDragEvents:true,leftDragging:false,middleDragging:false,rightDragging:false,debugRenderer:null,exposingAll:false,properties:["visible",
"alpha","angle","xScale","yScale","xAnchor","yAnchor","trackCursor","canAcceptCursor","canReleaseCursor","draggable","dragButton","enableDragEvents","xVelocity","yVelocity"],expose:function Entity_expose(){this.debugRenderer=jayus.debug.defaultDebugRenderer;this.dirty(jayus.DIRTY.ALL)},unexpose:function Entity_unexpose(){this.debugRenderer=null;this.dirty(jayus.DIRTY.ALL)},toggleExposed:function Entity_toggleExposed(){if(this.debugRenderer!==null)this.unexpose();else this.expose()},exposeAll:function Entity_exposeAll(){this.debugRenderer=
jayus.debug.defaultDebugRenderer;this.exposingAll=true;if(this.isParent)this.forEachChild(function(){this.exposeAll()});this.dirty(jayus.DIRTY.ALL)},unexposeAll:function Entity_exposeAll(){this.debugRenderer=null;this.exposingAll=false;if(this.isParent)this.forEachChild(function(){this.unexposeAll()});this.dirty(jayus.DIRTY.ALL)},toObject:function Entity_toObject(){var object={__type__:"Entity"};if(this.id!==0)object.id=this.id;var i,key,val,valType;for(i=0;i<jayus.Entity.prototype.properties.length;i++){key=
jayus.Entity.prototype.properties[i];val=this[key];valType=typeof val;if(val!==jayus.Entity.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}return object},initFromObject:function Entity_initFromObject(object){jayus.debug.match("Entity.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.frozen++;var i,key,val,valType;for(i=0;i<jayus.Entity.prototype.properties.length;i++){key=jayus.Entity.prototype.properties[i];val=object[key];
valType=typeof val;if(val!==undefined){if(valType==="object")val=jayus.fromObject(val);this[key]=val}}if(typeof object.id!=="undefined")this.id=object.id;this.frozen--;return this.dirty(jayus.DIRTY.ALL)},apply:function Entity_apply(obj){jayus.debug.match("Entity.apply",obj,"type",5);for(var property in obj)if(obj.hasOwnProperty(property))if(typeof this[property]==="function")if(obj[property]instanceof Array)this[property].apply(this,obj[property]);else this[property](obj[property]);else this[property]=
obj[property];return this},isTransformed:false,actionsToAnimate:0,animate:function Animatable_animate(){this.actionsToAnimate++;return this},init:function Entity_init(){this.addHandler("dirty",function(type){if(type&2)this.matrixDirty=true});if(this.enableDragEvents)this.handle({leftPress:function(e){this.leftDragging=true;return this.fire("leftDragStart",e)},leftRelease:function(e){if(this.leftDragging){this.leftDragging=false;return this.fire("leftDragEnd",e)}},middlePress:function(e){this.middleDragging=
true;return this.fire("middleDragStart",e)},middleRelease:function(e){if(this.middleDragging){this.middleDragging=false;return this.fire("middleDragEnd",e)}},rightPress:function(e){this.rightDragging=true;return this.fire("rightDragStart",e)},rightRelease:function(e){if(this.rightDragging){this.rightDragging=false;return this.fire("rightDragEnd",e)}},cursorMove:function(e){var ret=false;if(this.leftDragging)ret=this.fire("leftDrag",e)||ret;if(this.middleDragging)ret=this.fire("middleDrag",e)||ret;
if(this.rightDragging)ret=this.fire("rightDrag",e)||ret;return ret},cursorOut:function(){if(this.leftDragging)this.fire("leftDragEnd");if(this.middleDragging)this.fire("middleDragEnd");if(this.rightDragging)this.fire("rightDragEnd");this.leftDragging=this.middleDragging=this.rightDragging=false}})},dirty:function Entity_dirty(type){jayus.debug.match("Entity.dirty",type,"type",4);if(!this.frozen){this.fire("dirty",type);this.dirtied=true;this.informDependents(type)}return this},getAbsoluteParent:function Entity_getAbsoluteParent(){return this.hasParent?
...
entities,"entities",12);var i,ret=[];for(i=0;i<entities.length;i++)if(jayus.intersectTest(this,entities[i]))ret.push(entities[i]);return ret},setStyle:function RectEntity_setStyle(style){jayus.debug.match("RectEntity.setStyle",style,"style",5);if(this.hasStyle)this.style.detach(this);if(!(style instanceof jayus.Brush))style=new jayus.Brush(style);this.style=style;this.style.attach(this);this.hasStyle=true;return this.dirty(jayus.DIRTY.STYLE)},clearBg:function RectEntity_clearBg(){if(this.hasStyle){this.style.detach(this);
this.style=null;this.hasStyle=false;this.dirty(jayus.DIRTY.STYLE)}return this},rasterize:function Entity_rasterize(){var scope=this.getScope(),surface=new jayus.Surface(scope.width,scope.height),x=this.x,y=this.y;this.x=0;this.y=0;this.drawOnto(surface);this.x=x;this.y=y;return surface},drawOnto:function Entity_drawOnto(surface){var ctx=surface.context;ctx.save();this.drawOntoContext(ctx,0,0);ctx.restore();return this}});jayus.debug.className="RectEntity";
jayus.RectEntity=jayus.Entity.extend({shapeType:4,isRect:true,x:0,y:0,width:0,height:0,bg:null,hasBg:false,alignBg:false,bounds:null,boundsClone:null,body:null,hasFlexibleWidth:true,hasFlexibleHeight:true,properties:["x","y","width","height","alignBg"],toObject:function RectEntity_toObject(){var object=jayus.Entity.prototype.toObject.apply(this);object.__type__="RectEntity";var i,key,val,valType;for(i=0;i<jayus.RectEntity.prototype.properties.length;i++){key=jayus.RectEntity.prototype.properties[i];val=
this[key];valType=typeof val;if(val!==jayus.RectEntity.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}if(this.bg!==null)object.bg=this.bg.toObject();if(this.bounds!==null)object.bounds=this.bounds.toObject();return object},initFromObject:function RectEntity_initFromObject(object){jayus.debug.match("RectEntity.initFromObject",object,"object",5);jayus.Entity.prototype.initFromObject.call(this,object);this.frozen++;var i,key,val,valType;for(i=0;i<jayus.RectEntity.prototype.properties.length;i++){key=
jayus.RectEntity.prototype.properties[i];val=object[key];if(typeof val!=="undefined")this[key]=val}if(typeof object.bounds==="object")this.bounds=jayus.fromObject(object.bounds);if(typeof object.bg==="object")this.setBg(object.bg);this.frozen--;return this.dirty(jayus.DIRTY.ALL)},setX:jayus.Rectangle.prototype.setX,setY:jayus.Rectangle.prototype.setY,getOrigin:jayus.Rectangle.prototype.getOrigin,setOrigin:jayus.Rectangle.prototype.setOrigin,translate:jayus.Rectangle.prototype.translate,getLeft:jayus.Rectangle.prototype.getLeft,
getRight:jayus.Rectangle.prototype.getRight,getTop:jayus.Rectangle.prototype.getTop,getBottom:jayus.Rectangle.prototype.getBottom,setLeft:jayus.Rectangle.prototype.setLeft,setRight:jayus.Rectangle.prototype.setRight,setTop:jayus.Rectangle.prototype.setTop,setBottom:jayus.Rectangle.prototype.setBottom,getPosAt:function RectEntity_getPosAt(anchorX,anchorY){jayus.debug.matchCoordinate("RectEntity.getPosAt",anchorX,anchorY);if(arguments.length===1){anchorY=anchorX.y;anchorX=anchorX.x}return this.localToParent(anchorX*
this.width,anchorY*this.height)},setPosAt:function RectEntity_setPosAt(anchorX,anchorY,x,y){if(arguments.length===3){y=x.y;x=x.x}jayus.debug.matchArgumentsAs("RectEntity.setPosAt",[anchorX,anchorY,x,y],4,"anchorX","anchorY","x","y");return this.translate(x-(this.x+anchorX*this.width),y-(this.y+anchorY*this.height))},setRelativeAnchor:function Entity_setRelativeAnchor(x,y){jayus.debug.matchCoordinate("RectEntity.setRelativeAnchor",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.setAnchor(this.width*
x,this.height*y)},setBody:function RectEntity_setBody(body){jayus.debug.match("RectEntity.setBody",body,"body",5);if(this.body!==null)delete this.body.entity;else{jayus.box2d.physicalEntities.push(this);this.setAnchor(this.width/2,this.height/2)}this.body=body;this.body.entity=this;return this},removeBody:function RectEntity_removeBody(){if(this.body!==null){delete this.body.entity;this.body=null;jayus.box2d.physicalEntities.splice(jayus.box2d.physicalEntities.indexOf(this))}return this},updateFromBody:function RectEntity_updateFromBody(){var awake=
this.body.IsAwake();if(awake){var pos=this.body.GetPosition();this.setPosAt(0.5,0.5,pos.x,pos.y);this.angle=this.body.GetAngle();this.matrixDirty=true}},setWidth:function RectEntity_setWidth(width){jayus.debug.match("RectEntity.setWidth",width,"width",4);if(!this.hasFlexibleWidth)throw new Error("RectEntity.setWidth() - Entity width is not flexible");if(width!==this.width)this.changeSize(width,this.height);return this},widthDesc:null,setRemoteWidth:function RectEntity_setRemoteWidth(source,key){if(!this.hasFlexibleWidth)throw new Error("RectEntity.setRemoteWidth() - Entity width is not flexible");
this.widthDesc={source:source,key:key};var that=this;source.addHandler("dirty",function(){var width=this[key];if(width!==that.width)that.changeSize(width,that.height)});return this},setHeight:function RectEntity_setHeight(height){jayus.debug.match("RectEntity.setHeight",height,"height",4);if(!this.hasFlexibleHeight)throw new Error("RectEntity.setHeight() - Entity height is not flexible");if(height!==this.height)this.changeSize(this.width,height);return this},setSize:function RectEntity_setSize(width,
height){jayus.debug.matchSize("RectEntity.setSize",width,height);if(!this.hasFlexibleWidth||!this.hasFlexibleHeight)throw new Error("RectEntity.setSize() - Entity size is not flexible");if(arguments.length===1){height=width.height;width=width.width}if(this.width!==width||this.height!==height)this.changeSize(width,height);return this},changeSize:function RectEntity_changeSize(width,height){this.width=width;this.height=height;this.dirty(4)},intersectsAt:function RectEntity_intersectsAt(x,y){return this.getBounds().intersectsAt(x,
y)},getUnFrame:function RectEntity_getUnFrame(){return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getFrame:function RectEntity_getFrame(){if(this.isTransformed)return(new jayus.Polygon.Rectangle(0,0,this.width,this.height)).transform(this.getMatrix());return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getScope:function RectEntity_getScope(){return this.getFrame().getScope()},getBounds:function RectEntity_getBounds(){if(this.bounds!==null){this.bounds.cloneOnto(this.boundsClone);
return this.boundsClone.translate(this.x,this.y)}return this.getFrame().getScope()},setBounds:function RectEntity_setBounds(shape){jayus.debug.match("RectEntity.setBounds",shape,"shape",19);if(this.bounds!==null)this.bounds.detach(this);this.bounds=shape;this.bounds.attach(this);this.boundsClone=this.bounds.clone();this.shapeType=0;return this},clearBounds:function RectEntity_clearBounds(){if(this.bounds!==null){this.bounds.detach(this);this.bounds=null;this.boundsClone=null;this.shapeType=4}return this},
componentDirtied:function RectEntity_componentDirtied(component,type){if(component===this.bounds)this.bounds.cloneOnto(this.boundsClone)},setBg:function RectEntity_setBg(brush){jayus.debug.match("RectEntity.setBg",brush,"brush",5);if(this.hasBg)this.bg.detach(this);if(!(brush instanceof jayus.Brush))brush=new jayus.Brush(brush);this.bg=brush;this.bg.attach(this);this.hasBg=true;return this.dirty(jayus.DIRTY.BACKGROUND)},clearBg:function RectEntity_clearBg(){if(this.hasBg){this.bg.detach(this);this.bg=
null;this.hasBg=false;this.dirty(jayus.DIRTY.BACKGROUND)}return this},buffered:false,bufferBg:true,canvas:null,context:null,setBuffering:function RectEntity_setBuffering(on){jayus.debug.match("RectEntity.setBuffering",on,"on",1);if(!this.buffered&&on){this.canvas=document.createElement("canvas");this.context=this.canvas.getContext("2d");this.refreshBuffer()}else if(this.buffered&&!on){this.canvas=null;this.context=null}this.buffered=on;return this},refreshBuffer:function RectEntity_refreshBuffer(){if(this.buffered){var canvas=
...
jayus.Text=jayus.RectEntity.extend({text:"",font:"12pt sans-serif",init:function Text_init(text,font,brush){jayus.Entity.prototype.init.apply(this);if(arguments.length===1)jayus.debug.match("Text.init",text,"text",2);else if(arguments.length===2)jayus.debug.matchArguments("Text.init",arguments,"text",2,"font",2);else if(arguments.length===3)jayus.debug.matchArguments("Text.init",arguments,"text",2,"font",2,"brush",5);if(arguments.length){this.text=text;if(arguments.length>1){this.font=font;if(arguments.length>
2)this.setBrush(brush)}}this.refreshMetrics()},toObject:function Text_toObject(){var object=jayus.RectEntity.prototype.toObject.apply(this);object.__type__="Text";if(this.text!=="")object.text=this.text;if(this.font!==jayus.Text.prototype.font)object.font=this.font;if(this.brush!==null)object.brush=this.brush;if(this.alignment!==0)object.alignment=this.alignment;return object},initFromObject:function Text_initFromObject(object){jayus.debug.match("Text.initFromObject",object,"object",5);this.frozen++;
jayus.RectEntity.prototype.initFromObject.call(this,object);if(typeof object.text==="string")this.setText(object.text);if(typeof object.font==="string")this.setFont(object.font);if(typeof object.brush!=="undefined")this.setBrush(object.brush);if(typeof object.alignment==="number")this.setAlignment(object.alignment);this.frozen--;this.refreshMetrics();return this.dirty(jayus.DIRTY.ALL)},componentDirtied:function Text_componentDirtied(){this.dirty(16)},refreshMetrics:function Text_refreshMetrics(){if(this.fontDesc===
null)this.fontDesc=jayus.getFontDescriptor(this.font);this.lines=this.text.split("\n");if(this.lineWidths===null)this.lineWidths=[];else this.lineWidths.length=this.lines.length;var tempRet={};for(var i=0;i<this.lines.length;i++)this.lineWidths[i]=jayus.measureTextOnto(this.lines[i],this.font,tempRet).width;this.changeSize(jayus.math.max(this.lineWidths),this.lines.length*this.fontDesc.height)},getScope:function Text_getScope(){var scope=this.getFrame().getScope();if(this.hasBrush)if(this.brush.stroking||
this.brush.shadowing){var scale=Math.max(this.xScale,this.yScale),left=0,right=0,top=0,bottom=0;if(this.brush.stroking){var width=this.brush.lineWidth*scale/2;left=right=top=bottom=Math.sqrt(width*width*2)}if(this.brush.shadowing){left=Math.max(left,left+this.brush.shadowBlur-this.brush.shadowOffsetX);right=Math.max(right,right+this.brush.shadowBlur+this.brush.shadowOffsetX);top=Math.max(top,top+this.brush.shadowBlur-this.brush.shadowOffsetY);bottom=Math.max(bottom,bottom+this.brush.shadowBlur+this.brush.shadowOffsetY)}scope.setSize(scope.width+
left+right,scope.height+top+bottom);scope.translate(-left,-top)}return scope},setText:function Text_setText(text){jayus.debug.match("Text.setText",text,"text",2);if(this.text!==text){this.text=text;this.refreshMetrics();this.dirty(16)}return this},setFont:function Text_setFont(font){jayus.debug.match("Text.setFont",font,"font",2);if(this.font!==font){this.font=font;this.fontDesc=null;this.refreshMetrics()}return this},setAlignment:function TextBox_setAlignment(alignment){jayus.debug.match("TextBox.setAlignment",
alignment,"alignment",4);if(this.alignment!==alignment){this.alignment=alignment;if(this.lines.length>1)this.dirty(16)}return this},setBrush:function Text_setBrush(brush){jayus.debug.match("Text.setBrush",brush,"brush",5);if(this.hasBrush)this.brush.detach(this);if(!(brush instanceof jayus.Brush))brush=new jayus.Brush(brush);this.brush=brush;this.brush.attach(this);this.hasBrush=true;return this.dirty(jayus.DIRTY.ALL)},clearBrush:function Text_clearBrush(){if(this.hasBrush){this.brush.detach(this);
this.brush=null;this.hasBrush=false;this.dirty(jayus.DIRTY.ALL)}return this},paintContents:function Text_paintContents(ctx){var ascent=this.fontDesc.ascent,height=this.fontDesc.height;this.brush.applyTo(ctx);ctx.font=this.font;var i,line,x;for(i=0;i<this.lines.length;i++){line=this.lines[i];x=(this.width-this.lineWidths[i])*this.alignment;if(this.brush.stroking&&this.brush.strokeFirst)ctx.strokeText(line,x,ascent+i*height);if(this.brush.filling)ctx.fillText(line,x,ascent+i*height);if(this.brush.stroking&&
!this.brush.strokeFirst)ctx.strokeText(line,x,ascent+i*height)}}});jayus.debug.className="TextBox";
jayus.TextBox=jayus.Text.extend({hasFlexibleWidth:true,hasFlexibleHeight:true,init:function TextBox_init(){jayus.Text.prototype.init.apply(this,arguments);this.addHandler("dirty",function(type){if(type&4)this.refreshMetrics()})},refreshMetrics:function TextBox_refreshMetrics(){if(this.fontDesc===null)this.fontDesc=jayus.getFontDescriptor(this.font);var tempRet={},font=this.font,getWordLen=function(str){return jayus.measureTextOnto(str,font,tempRet).width},divideText=function(str,delims){var i,out=
[],curr="";for(i=0;i<str.length;i++)if(delims.indexOf(str[i])>=0){out.push(curr);out.push(str[i]);curr=""}else curr+=str[i];out.push(curr);return out},lines=[],lineWidths=[],words=divideText(this.text," \n"),i=0,currentLine,currentLineLength,nextWord,nextWordLength;while(i!==words.length){currentLine="\n";currentLineLength=0;nextWord=words[i];if(nextWord===" "||nextWord==="\n")nextWordLength=0;else nextWordLength=getWordLen(nextWord);do{if(nextWord==="\n"){i++;break}currentLine+=nextWord;currentLineLength+=
nextWordLength;i++;if(i===words.length)break;nextWord=words[i];nextWordLength=getWordLen(nextWord)}while(currentLineLength+nextWordLength<this.width);if(currentLine.length){lines.push(currentLine);lineWidths.push(currentLineLength)}}this.lines=lines;this.lineWidths=lineWidths}});jayus.debug.className="Grid";
jayus.Grid=jayus.RectEntity.extend({isParent:true,slotWidth:10,slotHeight:10,xPadding:0,yPadding:0,items:null,init:function Grid_init(){jayus.RectEntity.prototype.init.apply(this);this.items=[[null]]},componentDirtied:function Grid_componentDirtied(component,type){},updateSize:function Grid_updateSize(){return this.updateWidth().updateHeight()},updateWidth:function Grid_updateWidth(){return this.setWidth(this.items[0].length*this.slotWidth+(this.items[0].length-1)*this.xPadding)},updateHeight:function Grid_updateHeight(){return this.setHeight(this.items.length*
...
width,height);if(arguments.length===1){height=width.height;width=width.width}if(this.slotWidth!==width||this.slotHeight!==height){this.slotWidth=width;this.slotHeight=height;this.updateSize()}return this},setPadding:function Grid_setPadding(x,y){jayus.debug.matchCoordinate("Grid.setPadding",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.xPadding!==xPadding||this.yPadding!==yPadding){this.xPadding=x;this.yPadding=y;this.dirty(jayus.DIRTY.ALL)}return this},count:function Grid_count(){var i,j,count=
0;for(i=0;i<this.getRowCount();i++)for(j=0;j<this.getColumnCount();j++)if(this.items[i][j]!==null)count++;return count},slotCount:function Grid_slotCount(){return this.items.length*this.items[0].length},hasSlot:function Grid_hasSlot(x,y){jayus.debug.matchCoordinate("Grid.hasSlot",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.hasColumn(x)&&this.hasRow(y)},hasColumn:function Grid_hasColumn(x){jayus.debug.match("Grid.hasColumn",x,"x",4);return 0<=x&&x<this.items[0].length},hasRow:function Grid_hasRow(y){jayus.debug.match("Grid.hasRow",
y,"y",4);return 0<=y&&y<this.items.length},hasChild:function Grid_hasChild(item){var i,j;if(item instanceof jayus.Entity)for(i=0;i<this.getRowCount();i++){if(this.items[0].indexOf(item)>=0)return true}else for(i=0;i<this.getRowCount();i++)for(j=0;j<this.getColumnCount();j++)if(this.items[i][j].id===item)return true;return false},hasChildAt:function Grid_hasChildAt(x,y){jayus.debug.matchCoordinate("Grid.hasChildAt",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.getChildAt(x,y)!==null},indexOf:function Grid_indexOf(item){var row,
col;if(item instanceof jayus.Entity)for(row=0;row<this.getRowCount();row++){j=this.items[row].indexOf(item);if(j>=0)return new jayus.Point(col,row)}else for(row=0;row<this.getRowCount();row++)for(col=0;col<this.getColumnCount();col++)if(this.get(col,row).id===id)return new jayus.Point(col,row);return null},getChildAt:function Grid_getChildAt(x,y){jayus.debug.matchCoordinate("Grid.getChildAt",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.hasSlot(x,y))return this.items[y][x];return null},getChild:function Grid_getChild(id){jayus.debug.match("Grid.getChild",
id,"id",0);var i,j;for(i=0;i<this.getRowCount();i++)for(j=0;j<this.getColumnCount();j++)if(this.getChildAt(j,i).id===id)return this.items[i][j];return null},setChild:overloadArgumentCount({2:function Grid_setChild(index,child){jayus.debug.matchArguments("Grid.setChild",arguments,"index",13,"child",12);return this.setChild(index.x,index.y,child)},3:function Grid_setChild(x,y,child){jayus.debug.matchArguments("Grid.setChild",arguments,"x",4,"y",4,"child",12);if(this.hasSlot(x,y)){var old=this.items[y][x];
if(old!==null)old.removeParent();this.items[y][x]=child;if(child!==null){child.setParent(this);child.translate(x*(this.slotWidth+this.xPadding),y*(this.slotHeight+this.yPadding))}this.dirty()}return this}}),getChildUnder:function Grid_getChildUnder(x,y){jayus.debug.matchCoordinate("Grid.getChildUnder",x,y);if(arguments.length===1){y=x.y;x=x.x}return this.getChildAt(this.getSlotUnder(x,y))},getSlotUnder:function Grid_getSlotUnder(x,y){jayus.debug.matchCoordinate("Grid.getSlotUnder",x,y);if(arguments.length===
1){y=x.y;x=x.x}return new jayus.Point(Math.floor((x-this.x)/(this.slotWidth+this.xPadding)),Math.floor((y-this.y)/(this.slotHeight+this.yPadding)))},getSlotFrame:function Grid_getSlotFrame(x,y){jayus.debug.matchCoordinate("Grid.getSlotFrame",x,y);if(arguments.length===1){y=x.y;x=x.x}if(this.hasSlot(x,y))return new jayus.Rectangle(this.x+x*(this.slotWidth+this.xPadding),this.y+y*(this.slotHeight+this.yPadding),this.slotWidth,this.slotHeight);return null},removeChild:overloadArgumentType({object:function Grid_removeChild(child){var index=
this.indexOf(child);if(index!==null){child.removeParent();this.items[index.y][index.x]=null}return this},string:function Grid_removeChild(id){var index=this.indexOf(id);if(index!==null){this.items[index.y][index.x].removeParent();this.items[index.y][index.x]=null}return this}}),removeChildAt:function Grid_removeChildAt(x,y){jayus.debug.matchCoordinate("Grid.removeChildAt",x,y);if(arguments.length===1){y=x.y;x=x.x}var child=this.getChildAt(x,y);if(child!==null){child.removeParent();this.items[y][x]=
null}return this},setGridSize:function Grid_setGridSize(columns,rows){return this.setColumnCount(columns).setRowCount(rows)},getRowCount:function Grid_getRowCount(){return this.items.length},setRowCount:function Grid_setRowCount(rows){jayus.debug.match("Grid.setRowCount",rows,"rows",4);while(rows<this.getRowCount())this.removeRow();while(rows>this.getRowCount())this.addRow();return this},addRow:function Grid_addRow(row){if(!arguments.length)row=[];while(row.length<this.getColumnCount())row.push(null);
...
1&&this.hasColumn(index))for(var i=0;i<this.getRowCount();i++)this.items[i].splice(index,1);this.updateSize();return this.dirty()},onEach:function Grid_onEach(method,args){jayus.debug.match("Grid.onEach",method,"method",2);return this.forEachSlot(function(x,y,item){if(item!==null)item[method].apply(item,x,y,args)})},forEach:function Grid_forEach(func,args){jayus.debug.match("Grid.forEach",func,"func",3);return this.forEachSlot(function(x,y,item){if(item!==null)func.call(this,x,y,item)})},forEachInRow:function Grid_forEachInRow(row,
...
forEachSlot:function Grid_forEachSlot(func){jayus.debug.match("Grid.forEachSlot",func,"func",3);var x,y;for(x=0;x<this.getColumnCount();x++)for(y=0;y<this.getRowCount();y++)if(func.call(this,x,y,this.items[y][x])===null)return this;return this},findCursorAcceptor:function Grid_findCursorAcceptor(){var ret=null;this.forEach(function(x,y,item){if(item.underCursor)if(item.isParent){acceptor=item.findCursorAcceptor();if(acceptor!==null){ret=acceptor;return null}if(item.canAcceptCursor){ret=item;return null}}else if(item.canAcceptCursor){ret=
item;return null}});return ret},fireOnEach:function Grid_fireOnEach(event,data){jayus.debug.match("Grid.fireOnEach",event,"event",2);var x,y,item;for(x=0;x<this.getColumnCount();x++)for(y=0;y<this.getRowCount();y++){item=this.items[y][x];if(item!==null)if(item.fire(event,data))return true}return false},fireOnCursor:function Grid_fireOnCursor(event,data){jayus.debug.match("Grid.fireOnCursor",event,"event",2);var x,y,item;for(x=0;x<this.getColumnCount();x++)for(y=0;y<this.getRowCount();y++){item=this.items[y][x];
if(item!==null&&item.underCursor){if(item.isParent&&item.fireOnCursor(event,data))return true;if(item.fire(event,data))return true}}return false},updateCursorOnChildren:function Grid_updateCursorOnChildren(x,y,data){var pos=this.parentToLocal(x,y);if(this.isTransformed)pos=this.getMatrix().inverseTransformPoint(pos.x,pos.y);x=pos.x;y=pos.y;this.forEach(function(i,j,item){if(item.trackCursor)jayus.util.updateCursorOnChild(item,x,y)})},removeCursorFromChildren:function Grid_removeCursorFromChildren(data){this.forEach(function(x,
y,item){item.removeCursor()})},runOnCursor:function Grid_runOnCursor(func,args){jayus.debug.match("Grid.runOnCursor",func,"func",3);this.forEach(function(x,y,item){if(item.underCursor){if(item.isParent&&item.runOnCursor(func,args))return true;if(func.apply(item,args))return true}})},paintContents:function Grid_paintContents(ctx){jayus.debug.matchContext("Grid.paintContents",ctx);var i,j,item;for(i=0;i<this.items.length;i++)for(j=0;j<this.items[i].length;j++){item=this.items[i][j];if(item!==null&&
item.visible)item.drawOntoContext(ctx)}}});jayus.debug.className="Image";
jayus.Image=jayus.RectEntity.extend({image:null,imageSource:null,hasSection:false,section:null,keepAligned:false,loaded:false,hasFlexibleWidth:false,hasFlexibleHeight:false,init:function Image_init(image){jayus.Entity.prototype.init.apply(this);if(arguments.length===0)return;this.setSource(image)},toObject:function Image_toObject(){var object=jayus.RectEntity.prototype.toObject.apply(this);object.__type__="Image";if(this.section!==null)object.section=this.section.toObject();if(this.keepAligned!==
false)object.keepAligned=this.keepAligned;if(this.image instanceof HTMLImageElement)object.image=this.imageSource;else if(this.image instanceof jayus.Surface)object.image=this.image.toObject();delete object.width;delete object.height;return object},initFromObject:function Image_initFromObject(object){jayus.RectEntity.prototype.initFromObject.call(this,object);this.hasSection=typeof object.section==="object";if(this.hasSection)this.section=new jayus.Rectangle(object.section);if(typeof object.keepAligned===
"boolean")this.keepAligned=object.keepAligned;if(typeof object.image==="string")this.setSource(object.image);else if(typeof object.image==="object")this.setSource(new jayus.Surface(object.image));return this.dirty(jayus.DIRTY.ALL)},setSource:function Image_setSource(image){if(typeof image==="string"){var filepath=image;this.imageSource=filepath;if(jayus.images.isLoaded(filepath)){this.loaded=true;this.image=jayus.images.images[filepath];this.width=this.image.width;this.height=this.image.height}else{var that=
this;jayus.addHandler("imageLoaded",function(data){console.log("AHA!");if(data.filepath===filepath){that.loaded=true;that.image=data.image;if(!that.hasSection){that.width=data.image.width;that.height=data.image.height;that.dirty(4)}}});jayus.images.load(filepath)}}else{this.loaded=true;this.image=image;this.width=this.image.width;this.height=this.image.height}},setSection:function Image_setSection(x,y,width,height){if(arguments.length===1){jayus.debug.match("Image.setSection()",x,"section",20);height=
x.height;width=x.width;y=x.y;x=x.x}else if(arguments.length===3){jayus.debug.matchArguments("Image.setSection()",arguments,"origin",13,"width",4,"height",4);height=width;width=y;y=x.y;x=x.x}else jayus.debug.matchArgumentsAs("Image.setSection()",arguments,4,"x","y","width","height");this.hasSection=true;if(this.width!==width||this.height!==height){this.sectionY=y;this.sectionX=x;this.width=width;this.height=height;this.dirty(4)}else if(this.sectionX!==x||this.sectionY!==y){this.sectionY=y;this.sectionX=
x;this.dirty(16)}return this},clearSection:function Image_clearSection(){if(this.hasSection){this.hasSection=false;this.setSize(this.image.width,this.image.height)}return this},toPattern:function Image_toPattern(repetition){if(!arguments.length)repetition="repeat";jayus.debug.match("Image.toPattern",repetition,"repetition",2);if(["repeat","repeat-x","repeat-y","no-repeat"].indexOf(repetition)===-1)throw new Error("Image.toPattern() - Invalid repetition"+jayus.debug.toString(repetition)+' sent, "repeat", "repeat-x", "repeat-y", or "no-repeat" required');
...
jayus.Sprite=jayus.Image.extend({animation:null,animator:null,spriteIndexX:null,spriteIndexY:null,toObject:function Sprite_toObject(){var object=jayus.Image.prototype.toObject.call(this);object.__type__="Sprite";if(this.spriteIndexX!==null||this.spriteIndexY!==null){object.spriteIndexX=this.spriteIndexX;object.spriteIndexY=this.spriteIndexY}return object},initFromObject:function Sprite_initFromObject(object){jayus.Image.prototype.initFromObject.call(this,object);if(typeof object.spriteIndexX==="number"&&
typeof object.spriteIndexY==="number")this.setSprite(object.spriteIndexX,object.spriteIndexY);else if(typeof object.spriteIndexX!==typeof object.spriteIndexY)console.warn("Sprite.initFromObject() - Properties spriteIndexX and spriteIndexY must both be present if one is");return this.dirty(jayus.DIRTY.ALL)},setSpriteSheet:function Sprite_setSpriteSheet(sheet){this.sheet=sheet;return this},setSprite:function Sprite_setSprite(x,y){jayus.debug.matchCoordinate("Sprite.setSprite",x,y);if(arguments.length===
1){y=x.y;x=x.x}if(this.sheet!==null||typeof this.image.sheet==="object"){if(this.spriteIndexX!==x||this.spriteIndexY!==y){var sheet=this.sheet||this.image.sheet;this.spriteIndexX=x;this.spriteIndexY=y;this.setSection(sheet.marginX+x*sheet.spriteWidth,sheet.marginY+y*sheet.spriteHeight,sheet.spriteWidth,sheet.spriteHeight);this.dirty(4)}}else console.warn("Sprite.setSprite() - Image does not contain attached spritesheet");return this},playAnimation:function Sprite_playAnimation(name,func){jayus.debug.match("Sprite.playAnimation",
name,"name",2);jayus.debug.matchOptional("Sprite.playAnimation",func,"func",3);this.setAnimation(name,false);if(arguments.length>1)this.animator.addHandler("finished",func);return this},loopAnimation:function Sprite_loopAnimation(name){jayus.debug.match("Sprite.loopAnimation",name,"name",2);return this.setAnimation(name,true)},stopAnimation:function Sprite_stopAnimation(){if(this.animator!==null)this.animator.stop()},setAnimation:function Sprite_setAnimation(name,looped){jayus.debug.matchArguments("Sprite.setAnimation",
arguments,"name",2,"looped",1);if(this.sheet!==null||typeof this.image.sheet==="object"){var sheet=this.sheet||this.image.sheet;if(sheet.hasAnimation(name)){if(this.animator!==null)this.animator.stop();this.animation=name;var data=sheet.animations[name],sequence=data.sprites,sprite=this;if(data.flipX||data.flipY){this.setAnchor(this.width/2,this.height/2);if(data.flipX)this.setScale(-1,this.yScale);else this.setScale(1,this.yScale);if(data.flipY)this.setScale(this.xScale,-1);else this.setScale(this.xScale,
1)}else this.setScale(1,1);this.animator=new jayus.Animator.Discrete(sequence.length,function(index){sprite.setSprite(sequence[index])});this.animator.setDuration(sheet.animations[name].duration).setLooped(looped).start()}else throw new Error("Sprite.setAnimation() - Spritesheet does not contain sprite/animation: "+name);}else throw new Error("Sprite.setAnimation() - Image does not contain attached spritesheet");return this},playSequence:function Sprite_playSequence(names){var i,anim,seq=new jayus.AnimatorSequence,
sheet=this.sheet||this.image.sheet;for(i=0;i<names.length;i++)seq.add((new jayus.Sprite.SpriteSequence(this,sheet.animations[names[i]].sprites)).setDuration(sheet.animations[names[i]].duration));this.animator.stop();this.animator=seq;anim=this.animation;this.animation=names;seq.addHandler("finished",function(e){this.setAnimation(anim)},{context:this});seq.start();return seq}});jayus.debug.className="SpriteSheet";
...
jayus.Surface=jayus.RectEntity.extend({width:300,height:150,canvas:null,context:null,negateBlur:false,init:function Surface_init(width,height){jayus.Entity.prototype.init.apply(this);if(arguments.length===2){jayus.debug.matchArgumentsAs("Surface.init",arguments,4,"width","height");this.width=width;this.height=height}this.canvas=document.createElement("canvas");this.canvas.width=this.width;this.canvas.height=this.height;this.context=this.canvas.getContext("2d")},formContents:function Display_formContents(width,height){this.canvas.width=
width;this.canvas.height=height},clear:function Surface_clear(){this.context.clearRect(0,0,this.width,this.height);return this},fill:overloadArgumentCount({1:function Surface_fill(color){return this.fill(color.r,color.g,color.b,color.a)},2:function Surface_fill(color,alpha){return this.fill(color.r,color.g,color.b,color.a*alpha)},3:function Surface_fill(r,g,b){return this.fill(r,g,b,1)},4:function Surface_fill(r,g,b,a){jayus.debug.matchArgumentsAs("Surface.fill",arguments,4,"r","g","b","a");var ctx=
this.context;ctx.save();ctx.fillStyle="rgba("+r+","+g+","+b+","+a+")";ctx.globalCompositeOperation="copy";ctx.fillRect(0,0,this.width,this.height);ctx.restore();return this}}),getBuffer:function Surface_getBuffer(x,y,width,height){if(arguments.length===0){height=this.height;width=this.width;y=0;x=0}else if(arguments.length===1){jayus.debug.match("Surface.getBuffer()",x,"section",20);height=x.height;width=x.width;y=x.y;x=x.x}else if(arguments.length===3){jayus.debug.matchArguments("Surface.getBuffer()",
...
jayus.Scene=jayus.RectEntity.extend({hasWorld:false,world:null,childrenAdded:null,componentDirtied:function Scene_componentDirtied(component,type){if(component.componentType==="ENTITY")if(type&jayus.DIRTY.SCOPE)component.scopeChanged=true;jayus.RectEntity.prototype.componentDirtied.call(this,component,type);this.dirty(16)},listItemAdded:function Scene_listItemAdded(list,item){if(this.childrenAdded===null)this.childrenAdded=[];this.childrenAdded.push(item);item.prevScope=item.getScope();item.setParent(this);
this.dirty(16)},listItemsAdded:function Scene_listItemsAdded(list,items){if(this.childrenAdded===null)this.childrenAdded=[];for(var item,i=0;i<items.length;i++){item=items[i];item.setParent(this);this.childrenAdded.push(item);item.prevScope=item.getScope()}this.dirty(16)},listItemRemoved:function Scene_listItemRemoved(list,item){item.removeParent();this.redrawAll=true;this.dirty(16)},listItemsRemoved:function Scene_listItemsRemoved(list,items){for(var i=0;i<items.length;i++)items[i].removeParent();
this.redrawAll=true;this.dirty(16)},formContents:function Scene_formContents(width,height){if(this.buffered){this.canvas.width=width;this.canvas.height=height}},init:function Scene_init(width,height){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);this.children.typeId=12;if(arguments.length===1)this.initFromObject(width);else if(arguments.length===2){jayus.debug.matchArgumentsAs("Scene.init",arguments,4,"width","height");this.width=width;this.height=height}},toObject:function Scene_toObject(){var object=
jayus.RectEntity.prototype.toObject.apply(this);jayus.groupToObject.call(this,object);object.__type__="Scene";if(this.optimizeBuffering!==false)object.optimizeBuffering=this.optimizeBuffering;if(this.groupDamagedRegions!==true)object.groupDamagedRegions=this.groupDamagedRegions;if(this.damagedRegionPadding!==2)object.damagedRegionPadding=this.damagedRegionPadding;return object},initFromObject:function Scene_initFromObject(object){jayus.debug.match("Scene.initFromObject",object,"object",5);this.frozen++;
jayus.RectEntity.prototype.initFromObject.call(this,object);jayus.groupInitFromObject.call(this,object);if(typeof object.optimizeBuffering==="boolean")this.optimizeBuffering=object.optimizeBuffering;if(typeof object.groupDamagedRegions==="boolean")this.groupDamagedRegions=object.groupDamagedRegions;if(typeof object.damagedRegionPadding==="number")this.damagedRegionPadding=object.damagedRegionPadding;if(typeof object.widthPolicy==="object")this.widthPolicy=new jayus.SizePolicy(object.widthPolicy);
if(typeof object.heightPolicy==="object")this.heightPolicy=new jayus.SizePolicy(object.heightPolicy);this.frozen--;this.dirty(jayus.DIRTY.ALL)},optimizeBuffering:false,groupDamagedRegions:true,damagedRegionPadding:2,setOptimizedBuffering:function Scene_setOptimizedBuffering(on){jayus.debug.match("Scene.setOptimizedBuffering",on,"on",1);if(this.optimizeBuffering!==on){this.optimizeBuffering=on;this.redrawAll=true;this.dirty(16)}return this},refreshBuffer:function Scene_refreshBuffer(){if(this.buffered){var width=
this.width,height=this.height,canvas=this.canvas,ctx=this.context;if(width!==canvas.width||height!==canvas.height){canvas.width=width;canvas.height=height}if(this.optimizeBuffering&&!this.redrawAll)this.optimizedRefreshBuffer();else{this.fullyRefreshBuffer();this.redrawAll=false}this.dirtied=false}},optimizedRefreshBuffer:function Scene_optimizedRefreshBuffer(){if(this.buffered){var ctx=this.context,i,item,region,damagedRegions=[];for(i=0;i<this.children.items.length;i++){item=this.children.items[i];
item.clean=true;if(item.scopeChanged){damagedRegions.push(item.getScope().includeRectangle(item.prevScope));item.prevScope=item.getScope();item.scopeChanged=false;item.clean=false}else if(item.dirtied){damagedRegions.push(item.getScope());item.dirtied=false;item.clean=false}}var done=false;if(this.groupDamagedRegions)while(!done){done=true;for(i=0;i<damagedRegions.length;i++){region=damagedRegions[i];for(i2=0;i2<damagedRegions.length;i2++){region2=damagedRegions[i2];if(region!==region2&&jayus.intersectTest(region,
region2)){damagedRegions[i].includeRectangle(region2);damagedRegions.splice(i2,1);done=false;if(i>=i2)i--;i2--}}}}var padding=this.damagedRegionPadding;for(i=0;i<damagedRegions.length;i++){region=damagedRegions[i];region.translate(-padding,-padding);region.setSize(region.width+padding*2,region.height+padding*2)}for(i=0;i<damagedRegions.length;i++){region=damagedRegions[i];for(i2=0;i2<this.children.items.length;i2++){item=this.children.items[i2];if(item.clean&&jayus.intersectTest(item,region))item.clean=
false}}if(this.showDamage)this.clearDamage();ctx.save();for(i=0;i<damagedRegions.length;i++){region=damagedRegions[i];ctx.clearRect(region.x,region.y,region.width,region.height)}ctx.beginPath();for(i=0;i<damagedRegions.length;i++){region=damagedRegions[i];ctx.rect(region.x,region.y,region.width,region.height)}ctx.clip();if(this.bufferBg&&this.hasBg)if(this.alignBg)this.bg.paintRect(ctx,0.5,0.5,Math.round(this.width),Math.round(this.height));else this.bg.paintRect(ctx,0,0,this.width,this.height);for(i=
0;i<this.children.items.length;i++){item=this.children.items[i];if(!item.clean&&item.visible){item.drawOntoContext(ctx);if(this.showDamage)this.paintRedraw(item)}}if(this.showDamage)for(i=0;i<damagedRegions.length;i++)this.paintDamage(damagedRegions[i]);ctx.restore()}},paintContents:function Scene_paintContents(ctx){var i,item;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(item.visible)item.drawOntoContext(ctx)}}});jayus.applyObject(jayus.Group,jayus.Scene.prototype);jayus.debug.className="Display";
...
jayus.Frame=jayus.RectEntity.extend(jayus.applyObject({brush:null,hasBrush:false,marginLeft:6,marginRight:6,marginTop:6,marginBottom:6,forming:false,updateCursorOnChildren:function Frame_updateCursorOnChildren(x,y){if(this.propagateCursor){var pos=this.parentToLocal(x,y);x=pos.x;y=pos.y;this.localX=x;this.localY=y;this.child.updateCursor(x-this.marginLeft,y-this.marginTop)}},init:function Frame_init(child){jayus.debug.match("Frame.init",child,"child",12);jayus.chart.tallyInit(12);jayus.Entity.prototype.init.apply(this);
this.setChild(child);this.formContents();this.addHandler("dirty",function(type){if(type&4||type&2)this.formContents()})},setChild:function Frame_setChild(child){jayus.Wrapper.setChild.call(this,child);this.changeSize(child.width+this.marginLeft+this.marginRight,child.height+this.marginTop+this.marginBottom);return this},componentDirtied:function Frame_componentDirtied(component,type){if(!this.forming)if(type&4)this.changeSize(this.child.width+this.marginLeft+this.marginRight,this.child.height+this.marginTop+
this.marginBottom);else this.dirty(jayus.DIRTY.ALL)},setBrush:function Frame_setBrush(brush){jayus.debug.match("Frame.setBrush",brush,"brush",5);if(this.hasBrush)this.brush.detach(this);if(!(brush instanceof jayus.Brush))brush=new jayus.Brush(brush);this.brush=brush;this.brush.attach(this);this.hasBrush=true;return this.dirty(16)},clearBrush:function Frame_clearBrush(){if(this.hasBrush){this.brush.detach(this);this.brush=null;this.hasBrush=false;this.dirty(16)}return this},formContents:function Frame_formContents(){this.forming=
true;this.child.setOrigin(this.marginLeft,this.marginTop);this.child.changeSize(this.width-this.marginLeft-this.marginRight,this.height-this.marginTop-this.marginBottom);this.forming=false},setMargin:function Frame_setMargin(left,right,top,bottom){if(arguments.length===1){jayus.debug.match("Frame.setMargin",left,"margin",4);bottom=top=right=left}else jayus.debug.matchArgumentsAs("Frame.setMargin",arguments,4,"left","right","top","bottom");this.marginLeft=left;this.marginRight=right;this.marginTop=
top;this.marginBottom=bottom;this.child.frozen++;if(this.child.hasFlexibleWidth)this.child.setWidth(this.width-this.marginLeft-this.marginRight);else this.changeSize(this.child.width+left+right,this.child.height);if(this.child.hasFlexibleHeight)this.child.setHeight(this.height-this.marginTop-this.marginBottom);else this.changeSize(this.child.width,this.child.height+top+bottom);this.child.frozen--;return this},setMarginLeft:function Frame_setMarginLeft(margin){jayus.debug.match("Frame.setMarginLeft",
margin,"margin",4);return this.setMargin(margin,this.marginRight,this.marginTop,this.marginBottom)},setMarginRight:function Frame_setMarginRight(margin){jayus.debug.match("Frame.setMarginRight",margin,"margin",4);return this.setMargin(this.marginLeft,margin,this.marginTop,this.marginBottom)},setMarginTop:function Frame_setMarginTop(margin){jayus.debug.match("Frame.setMarginTop",margin,"margin",4);return this.setMargin(this.marginLeft,this.marginRight,margin,this.marginBottom)},setMarginBottom:function Frame_setMarginBottom(margin){jayus.debug.match("Frame.setMarginBottom",
margin,"margin",4);return this.setMargin(this.marginLeft,this.marginRight,this.marginTop,margin)},paintContents:function Frame_paintContents(ctx){jayus.debug.matchContext("Frame.paintContents",ctx);if(this.hasBrush)if(this.brush.fill||this.brush.stroking){ctx.save();this.brush.applyTo(ctx);ctx.beginPath();ctx.rect(0,0,this.width,this.height);if(this.brush.stroking&&this.brush.strokeFirst)ctx.stroke();if(this.brush.filling)ctx.fill();if(this.brush.stroking&&!this.brush.strokeFirst)ctx.stroke();ctx.restore()}ctx.save();
this.child.drawOntoContext(ctx);this.child.dirtied=false;ctx.restore()}},jayus.copyObject(jayus.Wrapper)));jayus.debug.className="EditableFrame";
jayus.EditableFrame=jayus.Frame.extend({minW:20,minH:20,maxW:Infinity,maxH:Infinity,enabledHandles:{nw:1,n:1,ne:1,e:1,se:1,s:1,sw:1,w:1},cornerThreshold:6,edgeThreshold:5,state:"",display:null,init:function EditableFrame_init(child){jayus.debug.match("Frame.init",child,"child",12);jayus.Frame.prototype.init.call(this,child);this.addHandler("cursorMove",function(e){if(!this.dragging){this.state="";this.display=e.display;var cursor=e.display.cursor,varWidth=this.child.hasFlexibleWidth,varHeight=this.child.hasFlexibleHeight;
if(varWidth&&varHeight)if(this.enabledHandles.nw&&jayus.distance(this.getPosAt(0,0),cursor)<this.cornerThreshold)this.state="nw";else if(this.enabledHandles.ne&&jayus.distance(this.getPosAt(1,0),cursor)<this.cornerThreshold)this.state="ne";else if(this.enabledHandles.sw&&jayus.distance(this.getPosAt(0,1),cursor)<this.cornerThreshold)this.state="sw";else if(this.enabledHandles.se&&jayus.distance(this.getPosAt(1,1),cursor)<this.cornerThreshold)this.state="se";if(this.state===""&&varWidth)if(this.enabledHandles.w&&
Math.abs(this.x-e.x)<this.edgeThreshold)this.state="w";else if(this.enabledHandles.e&&Math.abs(this.x+this.width-e.x)<this.edgeThreshold)this.state="e";if(this.state===""&&varHeight)if(this.enabledHandles.n&&Math.abs(this.y-e.y)<this.edgeThreshold)this.state="n";else if(this.enabledHandles.s&&Math.abs(this.y+this.height-e.y)<this.edgeThreshold)this.state="s";if(this.state!=="")this.display.setCursor(this.state+"-resize");else this.display.resetCursor()}});this.addHandler("cursorOut",function(e){if(!this.dragging){this.display.resetCursor();
this.state=""}});this.addHandler("leftPress",function(e){this.dragging=true;if(this.state===""){this.display.setCursor("move");this.fire("dragStart")}else if(this.state==="nw")this.start=this.getPosAt(1,1);else if(this.state==="n"||this.state==="e"||this.state==="ne")this.start=this.getPosAt(0,1);else if(this.state==="sw"||this.state==="w")this.start=this.getPosAt(1,0);else if(this.state==="s"||this.state==="se")this.start=this.getPosAt(0,0);var that=this;jayus.interceptNextEvent("leftRelease",function(e){this.dragging=
false;if(this.state===""){this.fire("dragEnd");this.display.resetCursor()}},{context:this});var dragHandler=function(e){if(that.dragging){if(that.state==="")that.translate(e.deltaX,e.deltaY);else{var width=null,height=null,anchorX=0,anchorY=0;if(that.state==="nw"){anchorX=1;anchorY=1;width=that.start.x-e.x;height=that.start.y-e.y}else if(that.state==="n"){anchorY=1;height=that.start.y-e.y}else if(that.state==="ne"){anchorY=1;width=e.x-that.start.x;height=that.start.y-e.y}else if(that.state==="sw"){anchorX=
1;width=that.start.x-e.x;height=e.y-that.start.y}else if(that.state==="s")height=e.y-that.start.y;else if(that.state==="se"){width=e.x-that.start.x;height=e.y-that.start.y}else if(that.state==="e")width=e.x-that.start.x;else if(that.state==="w"){anchorX=1;width=that.start.x-e.x}var pos=that.getPosAt(anchorX,anchorY);if(width!==null)if(width<that.minW+that.marginLeft+that.marginRight)width=that.minW+that.marginLeft+that.marginRight;else if(width>that.maxW+that.marginLeft+that.marginRight)width=that.maxW+
that.marginLeft+that.marginRight;if(height!==null)if(height<that.minH+that.marginTop+that.marginBottom)height=that.minH+that.marginTop+that.marginBottom;else if(height>that.maxH+that.marginTop+that.marginBottom)height=that.maxH+that.marginTop+that.marginBottom;if(width===null)width=that.width;if(height===null)height=that.height;that.setSize(width,height);that.setPosAt(anchorX,anchorY,pos)}that.display.interceptNextEvent("cursorMove",dragHandler);return true}};e.display.interceptNextEvent("cursorMove",
dragHandler);return true})},setMinimumSize:function EditableFrame_setMinimumSize(width,height){jayus.debug.matchArgumentsAs("EditableFrame.setMinimumSize",arguments,4,"width","height");this.minW=width;this.minH=height;return this}});jayus.debug.className="FlowLayout";
jayus.FlowLayout=jayus.RectEntity.extend({alignment:0,width:100,height:100,forming:false,init:function FlowLayout_init(){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function(type){if(type&4)this.formContents()})},componentDirtied:function Box_componentDirtied(component,type){if(!this.forming)this.dirty(jayus.DIRTY.ALL)},listItemAdded:function FlowLayout_listItemAdded(list,item){item.setParent(this);this.dirty(jayus.DIRTY.ALL)},
listItemsAdded:function FlowLayout_listItemsAdded(list,items){for(var i=0;i<items.length;i++)items[i].setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemRemoved:function FlowLayout_listItemRemoved(list,item){item.removeParent();this.dirty(jayus.DIRTY.ALL)},listItemsRemoved:function FlowLayout_listItemsRemoved(list,items){for(var i=0;i<items.length;i++)items[i].removeParent();this.dirty(jayus.DIRTY.ALL)},setAlignment:function FlowLayout_setAlignment(alignment){jayus.debug.match("FlowLayout.setAlignment",
alignment,"alignment",4);if(this.alignment!==alignment){this.alignment=alignment;this.formContents()}return this},getAlignment:function FlowLayout_getAlignment(){return this.alignment},formContents:function FlowLayout_formContents(){this.forming=true;var i=0,j,y=0,currentLine,currentLineWidth,h,x,item,nextItem,nextItemWidth,lineHeights=[];while(i!==this.children.items.length){currentLine=[];currentLineWidth=0;h=0;x=0;nextItem=this.children.items[i];nextItemWidth=nextItem.width;do{nextItem.x=x;nextItem.y=
y;currentLine.push(nextItem);currentLineWidth+=nextItemWidth;x+=nextItemWidth;if(nextItem.height>h)h=nextItem.height;i++;if(i===this.children.items.length)break;nextItem=this.children.items[i];nextItemWidth=nextItem.width}while(currentLineWidth+nextItemWidth<this.width);if(this.alignment!==0)for(j=0;j<currentLine.length;j++){item=currentLine[j];item.x+=(this.width-currentLineWidth)*this.alignment}for(j=0;j<currentLine.length;j++){item=currentLine[j];if(typeof item.verticalAlign==="number")item.y=
item.y+item.verticalAlign*(h-item.height)}y+=h;lineHeights.push(h)}for(i=0;i<this.children.items.length;i++){item=this.children.items[i];item.dirty(2)}this.lineHeights=lineHeights;this.forming=false},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.FlowLayout.prototype);jayus.debug.className="Stack";
jayus.Stack=jayus.RectEntity.extend({spacing:8,reversed:false,isParent:true,hasFlexibleWidth:false,hasFlexibleHeight:false,init:function Stack_init(){jayus.RectEntity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function(type){if(type&4);})},toObject:function Stack_toObject(){var object=jayus.RectEntity.prototype.toObject.apply(this);jayus.groupToObject.call(this,object);object.__type__="Stack";if(this.spacing!==8)object.spacing=
this.spacing;if(this.reversed!==8)object.reversed=this.reversed;return object},initFromObject:function Stack_initFromObject(object){jayus.debug.match("Stack.initFromObject",object,"object",5);this.frozen++;jayus.RectEntity.prototype.initFromObject.call(this,object);jayus.groupFromObject.call(this,object);if(typeof object.spacing==="number")this.spacing=object.spacing;if(typeof object.reversed==="boolean")this.reversed=object.reversed;this.frozen--;return this.dirty(jayus.DIRTY.ALL)},componentDirtied:function Stack_componentDirtied(component,
type){this.dirty(jayus.DIRTY.ALL)},listItemAdded:function Stack_listItemAdded(list,item){this.formContents();item.setParent(this);this.dirty(16)},listItemRemoved:function Stack_listItemRemoved(list,item){this.formContents();item.removeParent();this.dirty(16)},listItemsAdded:function Stack_listItemsAdded(list,items){this.formContents();for(var i=0;i<items.length;i++){item=items[i];item.setParent(this)}this.dirty(16)},listItemsRemoved:function Stack_listItemsRemoved(list,items){this.formContents();
for(var i=0;i<items.length;i++){item=items[i];item.removeParent()}this.dirty(16)},listItemsMoved:function Stack_listItemsMoved(){this.formContents();this.dirty(16)},setReversed:function Stack_setReversed(on){jayus.debug.match("Stack.setReversed",on,"on",1);if(this.reversed!==on){this.reversed=on;this.formContents()}return this},setSpacing:function Stack_setSpacing(spacing){jayus.debug.match("Stack.setSpacing",spacing,"spacing",4);if(this.spacing!==spacing){this.spacing=spacing;this.formContents()}return this},
findGreatestMinimalChildWidth:function Stack_findGreatestMinimalChildWidth(){var i,width=0;for(i=0;i<this.children.items.length;i++)width=Math.max(width,this.children.items[i].minW);return width},findGreatestMinimalChildHeight:function Stack_findGreatestMinimalChildHeight(){var i,height=0;for(i=0;i<this.children.items.length;i++)height=Math.max(height,this.children.items[i].minH);return height},reform:function Stack_reform(){this.formContents(this.width,this.height)},paintContents:jayus.Scene.prototype.paintContents});
jayus.applyObject(jayus.Group,jayus.Stack.prototype);jayus.debug.className="hStack";
jayus.hStack=jayus.Stack.extend({automaticHeight:true,toObject:function hStack_toObject(){var object=jayus.Stack.prototype.toObject.apply(this);object.__type__="hStack";if(this.automaticHeight!==true)object.automaticHeight=this.automaticHeight.toObject();return object},formContents:function hStack_formContents(){var w=0,h=0,newH,i,item;if(this.reversed)for(i=this.children.items.length-1;i>=0;i--){item=this.children.items[i];item.setX(w);w+=item.width+this.spacing;if(this.automaticHeight){newH=item.height;
if(newH>h)h=newH}}else for(i=0;i<this.children.items.length;i++){item=this.children.items[i];item.setX(w);w+=item.width+this.spacing;if(this.automaticHeight){newH=item.height;if(newH>h)h=newH}}var width=w-this.spacing,heigth=this.height;if(this.automaticHeight)height=h;this.changeSize(width,height)}});jayus.debug.className="vStack";
jayus.vStack=jayus.Stack.extend({automaticWidth:true,getAutomaticWidth:function vStack_getAutomaticWidth(){return this.automaticWidth},setAutomaticWidth:function vStack_setAutomaticWidth(on){jayus.debug.match("vStack.setAutomaticWidth",on,"on",1);if(this.automaticWidth!==on){this.automaticWidth=on;this.reform()}return this},toObject:function vStack_toObject(){var object=jayus.Stack.prototype.toObject.apply(this);object.__type__="vStack";if(this.automaticWidth!==true)object.automaticWidth=this.automaticWidth.toObject();
return object},formContents:function vStack_formContents(){var h=0,w=0;for(var i=0;i<this.children.items.length;i++){var item=this.children.items[i];item.frozen++;item.setY(h);item.frozen--;h+=item.height+this.spacing;if(this.automaticWidth){var newW=item.width;if(newW>w)w=newW}}var height=h-this.spacing,width=this.width;if(this.automaticWidth)width=w;this.changeSize(width,height)}});jayus.debug.className="Box";
jayus.Box=jayus.RectEntity.extend({width:100,height:100,spacing:8,reversed:false,isParent:true,forming:false,init:function Box_init(object){jayus.Entity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.children.typeId=12;this.addHandler("dirty",function Box_dirtyHandler(type){if(type&4)this.formContents()});if(arguments.length)this.initFromObject(object)},toObject:function Box_toObject(){var object=jayus.RectEntity.prototype.toObject.call(this);jayus.groupToObject.call(this,
object);object.id="__Box__";if(this.spacing!==jayus.Box.prototype.spacing)object.spacing=this.spacing;if(this.reversed!==false)object.reversed=this.reversed;return object},initFromObject:function Box_initFromObject(object){jayus.RectEntity.prototype.initFromObject.call(this,object);jayus.groupInitFromObject.call(this,object);if(typeof object.spacing==="number")this.setSpacing(object.spacing);if(typeof object.reversed==="boolean")this.setReversed(object.reversed);return this.dirty(jayus.DIRTY.ALL)},
componentDirtied:function Box_componentDirtied(component,type){if(!this.forming)this.dirty(jayus.DIRTY.ALL)},listItemAdded:function Box_listItemAdded(list,item){item.setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemsAdded:function Box_listItemsAdded(list,items){for(var i=0;i<items.length;i++)items[i].setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemRemoved:function Box_listItemRemoved(list,item){item.removeParent();this.dirty(jayus.DIRTY.ALL)},listItemsRemoved:function Box_listItemsRemoved(list,
items){for(var i=0;i<items.length;i++)items[i].removeParent();this.dirty(jayus.DIRTY.ALL)},setReversed:function Box_setReversed(on){jayus.debug.match("Box.setReversed",on,"on",1);if(this.reversed!==on){this.reversed=on;this.formContents()}return this},setSpacing:function Box_setSpacing(spacing){jayus.debug.match("Box.setSpacing",spacing,"spacing",4);if(this.spacing!==spacing){this.spacing=spacing;this.formContents()}return this},findGreatestMinimalChildWidth:function Box_findGreatestMinimalChildWidth(){var width=
0;for(var i=0;i<this.children.items.length;i++)width=Math.max(width,this.children.items[i].minW);return width},findGreatestMinimalChildHeight:function Box_findGreatestMinimalChildHeight(){var height=0;for(var i=0;i<this.children.items.length;i++)height=Math.max(height,this.children.items[i].minH);return height},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.Box.prototype);jayus.debug.className="hBox";
jayus.hBox=jayus.Box.extend({toObject:function hBox_toObject(){var object=jayus.Box.prototype.toObject.call(this);object.id="__hBox__";return object},formContents:function hBox_formContents(){this.forming=true;var i,item,x=0,space=this.width-(this.children.items.length-1)*this.spacing,totalWeight=0,totalFixedSize=0,itemWidth,itemHeight;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(typeof item.policy!=="object")item.policy=new jayus.SizePolicy;if(item.hasFlexibleWidth){totalFixedSize+=
item.policy.size;if(item.policy.expand)totalWeight+=item.policy.weight}else totalFixedSize+=item.width}var extraSpace=space-totalFixedSize;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(item.hasFlexibleHeight)itemHeight=this.height;else itemHeight=item.height;if(item.hasFlexibleWidth)if(item.policy.expand)itemWidth=item.policy.size+extraSpace*(item.policy.weight/totalWeight);else itemWidth=item.policy.size;else itemWidth=item.width;item.x=x;x+=itemWidth+this.spacing;item.changeSize(itemWidth,
itemHeight)}this.forming=false}});jayus.debug.className="vBox";
jayus.vBox=jayus.Box.extend({toObject:function vBox_toObject(){var object=jayus.Box.prototype.toObject.call(this);object.id="__vBox__";return object},formContents:function vBox_formContents(){this.forming=true;var i,item,y=0,space=this.height-(this.children.items.length-1)*this.spacing,totalWeight=0,totalFixedSize=0,itemWidth,itemHeight;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(typeof item.heightPolicy!=="object")item.heightPolicy=new jayus.SizePolicy;if(item.hasFlexibleHeight){totalFixedSize+=
item.heightPolicy.size;if(item.heightPolicy.expand)totalWeight+=item.heightPolicy.weight}else totalFixedSize+=item.height}var extraSpace=space-totalFixedSize;for(i=0;i<this.children.items.length;i++){item=this.children.items[i];if(item.hasFlexibleWidth)itemWidth=this.width;else itemWidth=item.width;if(item.hasFlexibleHeight)if(item.heightPolicy.expand)itemHeight=item.heightPolicy.size+extraSpace*(item.heightPolicy.weight/totalWeight);else itemHeight=item.heightPolicy.size;else itemHeight=item.height;
item.y=y;y+=itemHeight+this.spacing;item.changeSize(itemWidth,itemHeight)}this.forming=false}});jayus.debug.className="TiledMap";
jayus.TiledMap=jayus.RectEntity.extend({loaded:false,filename:null,map:null,tileWidth:0,tileHeight:0,layerVisibility:null,init:function TiledMap_init(map){jayus.Entity.prototype.init.apply(this);if(arguments.length)this.setMap(map)},checkLoaded:function TiledMap_checkLoaded(){if(!this.loaded)throw new Error("TiledMap.checkLoaded() - Error, map not yet loaded");},setMap:overloadArgumentType({object:function TiledMap_setMap(map){this.map=map;this.loaded=true},string:function TiledMap_setMap(filepath){this.map=jayus.objects.get(filepath);
for(var i=0;i<this.map.tilesets.length;i++){var data=this.map.tilesets[i];jayus.images.load(data.image);var sheet=new jayus.SpriteSheet;sheet.setSpriteSize(data.tilewidth,data.tileheight);jayus.images.get(data.image).sheet=sheet;this.tileWidth=data.tilewidth;this.tileHeight=data.tileheight}this.layerVisibility=[];for(i=0;i<this.map.layers.length;i++)this.layerVisibility.push(this.map.layers[i].visible);this.loaded=true;this.setSize(this.map.width*this.map.tilewidth,this.map.height*this.map.tileheight)}}),
getTileAt:function TiledMap_getTileAt(x,y){jayus.debug.matchCoordinate("TiledMap.getTileAt",x,y);if(arguments.length===1){y=x.y;x=x.x}x-=this.x;y-=this.y;return new jayus.Point(Math.floor(x/this.map.tilewidth),Math.floor(y/this.map.tileheight))},getSlotFrame:function TiledMap_getSlotFrame(x,y){jayus.debug.matchCoordinate("TiledMap.getSlotFrame",x,y);this.checkLoaded();if(arguments.length===1){y=x.y;x=x.x}return new jayus.Rectangle(this.x+x*this.map.tilewidth,this.y+y*this.map.tileheight,this.map.tilewidth,
this.map.tileheight)},isLayerVisible:function TiledMap_isLayerVisible(index){jayus.debug.match("TiledMap.isLayerVisible",index,"index",4);this.checkLoaded();return this.layerVisibility[index]},setLayerVisibility:function TiledMap_setLayerVisibility(index,visible){jayus.debug.matchArguments("TiledMap.setLayerVisibility",arguments,"index",4,"visible",1);this.checkLoaded();if(this.layerVisibility[index]!==visible){this.layerVisibility[index]=visible;this.dirty()}return this},showLayer:function TiledMap_showLayer(index){jayus.debug.match("TiledMap.showLayer",
index,"index",4);this.checkLoaded();return this.setLayerVisibility(index,true)},hideLayer:function TiledMap_hideLayer(index){jayus.debug.match("TiledMap.hideLayer",index,"index",4);this.checkLoaded();return this.setLayerVisibility(index,false)},paintContents:function TiledMap_paintContents(ctx){jayus.debug.matchContext("TiledMap.paintContents",ctx);this.checkLoaded();var i,image,tileset=this.map.tilesets[0],tileSheet=jayus.images.get(tileset.image),sheetInfo=tileSheet.sheet,tileWidth=sheetInfo.spriteWidth,
tileHeight=sheetInfo.spriteHeight,layer,tileY,tileX,index,sourceTileX,sourceTileY,x,y;for(i=0;i<this.map.tilesets.length;i++){tileset=this.map.tilesets[i];if(!jayus.images.isLoaded(tileset.image))console.warn('TiledMap.paintContents() - Tileset "'+tileset.image+'" not yet loaded')}for(i=0;i<this.map.layers.length;i++){layer=this.map.layers[i];if(layer.type==="tilelayer"&&this.layerVisibility[i])for(tileY=0;tileY<layer.height;tileY++)for(tileX=0;tileX<layer.width;tileX++){index=layer.data[tileY*layer.width+
tileX]-1;if(index!==-1){sourceTileX=index%(tileset.imagewidth/tileset.tilewidth);sourceTileY=(index-sourceTileX)/(tileset.imagewidth/tileset.tilewidth);x=sheetInfo.marginX+sourceTileX*tileWidth;y=sheetInfo.marginY+sourceTileY*tileHeight;ctx.drawImage(tileSheet,x,y,tileWidth,tileHeight,tileX*tileWidth,tileY*tileHeight,tileWidth,tileHeight)}}}}});
