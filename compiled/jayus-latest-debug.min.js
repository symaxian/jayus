...
return}if(!this.running){if(this.isHandler===null)this.isHandler={frame:false,step:false};this.running=true;if(!this.useReqAnimFrame||!this.ua.requestAnimationFrame)this.frameInterval=setInterval(jayus.frame,1E3/this.framerate);else if(!this.frameIntervalRunning){this.lastFrameTime=Date.now();this.lastStepTime=Date.now();this.frameIntervalRunning=true;this.frame()}this.stepInterval=setInterval(jayus.step,1E3/this.steprate)}else console.warn("jayus.start() - Jayus is already running")},stop:function jayus_stop(){if(this.running){this.running=
false;clearInterval(this.frameInterval);clearInterval(this.stepInterval);this.frameInterval=null;this.stepInterval=null}else console.warn("jayus.stop() - Called when not running")},step:function jayus_step(){if(jayus.running){jayus.chart.begin("Frame");var i,item,now=Date.now(),elapsedMilliSecs=now-jayus.lastStepTime,elapsedSecs=elapsedMilliSecs/1E3;jayus.lastStepTime=now;if(!jayus.isHandler.step||!jayus.fire("step",elapsedSecs)){jayus.chart.begin("Cursor");for(i=0;i<jayus.displays.length;i++){item=
jayus.displays[i];if(item.hasPendingMousemoveEvent){item.processPendingMousemove();item.hasPendingMousemoveEvent=false}}jayus.chart.end();jayus.chart.begin("Animation");for(i=0;i<jayus.animators.length;i++)jayus.animators[i].tick(now,elapsedSecs);jayus.chart.end();if(jayus.box2d.enabled)jayus.box2d.step(elapsedSecs);jayus.chart.update()}jayus.chart.end()}},frame:function jayus_frame(){if(jayus.running){jayus.chart.begin("Frame");if(jayus.useReqAnimFrame&&jayus.ua.requestAnimationFrame)window.requestAnimationFrame(jayus.frame);
var i,item,now=Date.now(),elapsedMilliSecs=now-jayus.lastFrameTime;jayus.fps=1E3/elapsedMilliSecs;jayus.lastFrameTime=now;if(!jayus.isHandler.frame||!jayus.fire("frame",elapsedMilliSecs/1E3)){jayus.chart.begin("Render");for(i=0;i<jayus.displays.length;i++){jayus.chart.begin("Display: "+i);item=jayus.displays[i];if(item.dirtied)item.refreshBuffer();jayus.chart.end()}if(jayus.box2d.enabled){jayus.chart.begin("Box2D Debug");jayus.box2d.drawDebug();jayus.chart.end()}jayus.chart.end();jayus.chart.update()}jayus.chart.end()}else jayus.frameIntervalRunning=
false},addDefaultHandlers:function jayus_addDefaultHandlers(){jayus.addHandler("keyPress",function(e){if(e.key==="grave"&&e.event.ctrlKey){if(jayus.chart.visible)jayus.chart.hide();else jayus.chart.show();return true}})},manuallyDrawImage:function jayus_manuallyDrawImage(source,destination,width,height){if(typeof destination.imageSmoothingEnabled==="boolean"){var prev=destination.imageSmoothingEnabled;destination.imageSmoothingEnabled=false;destination.drawImage(source.canvas,0,0);destination.imageSmoothingEnabled=
prev}else{var data=source.getImageData(0,0,width,height).data,x,y,pos;for(y=height-1;y>=0;y--)for(x=width-1;x>=0;x--){pos=4*(x+width*y);destination.fillStyle="rgba("+data[pos]+","+data[pos+1]+","+data[pos+2]+","+data[pos+3]/255+")";destination.fillRect(x,y,1,1)}}},chart:{markColor:"grey",markFont:"10px sans-serif",display:null,graphSurface:null,topMS:40,index:0,width:400,height:380,visible:false,initialized:false,windowSize:120,windowIndex:0,rootRoutine:null,currentRoutine:null,timeLabelColumnWidth:30,
TIME_LABEL_COLUMN_WIDTH_PADDING:8,TOP_STACK_HEIGHT:20,CHILD_ROW_SPACING:8,LATENCY_DISPLAY_THRESHOLD:0.5,MAX_ROUTINE_DEPTH:8,defaultRoutineColors:{"Total Latency":"grey","Render":"saddlebrown","Box2d":"blue","Cursor":"Chartreuse","Animation":"orange","DebugPanel":"yellow"},init:function jayus_chart_init(){this.display=jayus.parse({type:"Display",id:"DebugPanelDisplay",width:this.width,height:this.height,children:[{id:"vBox",type:"vBox",constraints:{width:"parent.width",height:"parent.height"},children:[{id:"TopRow",
type:"Scene",constraints:{width:"parent.width"},height:30,heightPolicy:{type:"SizePolicy",size:30,expand:false,flexible:false},bg:{fill:{type:"LinearGradient",start:{x:0,y:0},end:{x:0,y:30},stopPositions:[0,1],stopColors:["#DDD","#CCC"]}},children:[{type:"Text",id:"fpsLabel",font:"12px sans-serif",x:0,y:4,brush:{fill:"#111"}}]},{id:"Graph",type:"Scene",children:[{id:"graphSurface",type:"Surface",constraints:{width:"parent.width",height:"parent.height"}},{id:"line16ms",type:"PaintedShape",brush:{stroke:"dimgrey"},
shape:new jayus.Polygon.Line(0,0,0,0)},{id:"line33ms",type:"PaintedShape",brush:{stroke:"dimgrey"},shape:new jayus.Polygon.Line(0,0,0,0)},{id:"text16ms",type:"Text",font:"10px sans-serif",brush:{fill:"dimgrey"}},{id:"text33ms",type:"Text",font:"10px sans-serif",brush:{fill:"dimgrey"}}]},{type:"FlexibleFrame",id:"RoutineBoxFrame",marginLeft:2,marginRight:2,marginTop:8,marginBottom:8,constraints:{width:"parent.width"},bg:{fill:"white"},child:{id:"RoutineBox",type:"vStack",constraints:{width:"parent.width"},
spacing:8}}]}]});jayus.displays.splice(jayus.displays.indexOf(this.display),1);this.vBox=this.display.children.find("vBox");this.topRow=this.display.children.find("TopRow");this.graph=this.display.children.find("Graph");this.routineBox=this.display.children.find("RoutineBox");this.graphSurface=this.display.children.find("graphSurface");this.fpsLabel=this.topRow.children.find("fpsLabel");this.fpsLabel.ignoreDirty++;setInterval(function(){var label=jayus.chart.fpsLabel;var fps=Math.round(jayus.fps*
10)/10+"";if(fps.length===2)fps+=".0";label.setText("Framerate: "+fps);label.setX(label.parent.width-4-label.width)},100);var line16ms=this.display.children.find("line16ms"),line33ms=this.display.children.find("line33ms"),text16ms=this.display.children.find("text16ms"),text33ms=this.display.children.find("text33ms"),updateLabel=function GraphLabelUpdater(ms,line,text){var y=jayus.chart.graph.height-ms/jayus.chart.topMS*jayus.chart.graph.height;line.shape.setPoint(0,0,Math.floor(y)+0.5);line.shape.setPoint(1,
jayus.chart.graph.width,Math.floor(y)+0.5);text.setText(ms+" ms");text.setOrigin(4,y-12)};line16ms.ignoreDirty++;line33ms.ignoreDirty++;text16ms.ignoreDirty++;text33ms.ignoreDirty++;this.graph.addHandler("dirty",function(dirty){if(dirty&4){updateLabel(16,line16ms,text16ms);updateLabel(33,line33ms,text33ms)}if(jayus.chart.rootRoutine!==null)jayus.chart.rootRoutine.setFrameCount(jayus.chart.graphSurface.width)});this.graph.dirty(4);var div=document.createElement("div");var canvas=this.display.canvas;
canvas.style.border="#aaa 1px solid";canvas.style.backgroundColor="black";div.appendChild(canvas);$(div).dialog({title:"Jayus Debug Panel",open:function(){$(this).data("width.dialog",$(canvas).width()+60)},resize:function(){jayus.chart.display.setWidth($(jayus.chart.dialogContainer).width()-4);jayus.chart.display.setHeight($(jayus.chart.dialogContainer).height()-30);return false},close:function(){jayus.chart.visible=false}});jayus.chart.dialog=div.parentNode;jayus.chart.dialogContainer=jayus.chart.dialog.childNodes[1];
this.initialized=true},getRoutineColor:function jayus_chart_getRoutineColor(name){if(typeof this.defaultRoutineColors[name]==="string")return this.defaultRoutineColors[name];return jayus.colors.cssNames[jayus.math.randomBetween(0,jayus.colors.count-1)]},begin:function jayus_chart_begin(name){if(this.initialized){var data;if(this.rootRoutine===null){this.rootRoutine=this.createRoutineDataObject("Total Latency",null);this.currentRoutine=this.rootRoutine;this.routineBox.children.add(this.rootRoutine.entity)}if(name===
"Frame")data=this.rootRoutine;else{data=this.currentRoutine.children[name];if(data===undefined){data=this.createRoutineDataObject(name);this.currentRoutine.childrenStack.children.add(data.entity);this.currentRoutine.children[name]=data}this.currentRoutine=data}data.startTime=Date.now()}},end:function jayus_chart_end(){var time,data;if(this.initialized){data=this.currentRoutine;time=Date.now()-data.startTime;data.time+=time;data.latencies[this.index]=time;if(data!==this.rootRoutine)this.currentRoutine=
data.parent}},createRoutineDataObject:function jayus_chart_createRoutineDataObject(name,parent){var data={name:name,latencies:[],inits:[],averageLatency:0,initTotal:0,color:this.getRoutineColor(name),parent:this.currentRoutine,children:{},sortedRoutineNames:[],depth:0};if(this.currentRoutine!==null)data.depth=this.currentRoutine.depth+1;if(data.parent!==null)data.parent.sortedRoutineNames.push(data.name);for(var i=0;i<this.graphSurface.width;i++)data.latencies.push(0);var x;if(data.parent===null)x=
0;else x=20;data.entity=jayus.parse({type:"Scene",x:x,parent:parent,constraints:{width:"parent.width - x - 2"},heightPolicy:{type:"SizePolicy",expand:true},bg:{fill:"#BBB",stroke:"black",lineWidth:2},children:[{type:"Scene",x:1,y:1,height:20,constraints:{width:"parent.width"},bg:{},children:[{id:"label",type:"Text",text:name,font:"12px sans-serif",brush:{fill:"black"},constraints:{y:"parent.height/2 - height/2"}},{id:"bar",type:"Scene",bg:{fill:data.color},width:10,constraints:{height:"parent.height - 2"}},
{id:"timeLabel",type:"Text",text:"0 ms",font:"12px sans-serif",brush:{fill:"black"},constraints:{y:"parent.height/2 - height/2"}}]},{id:"childrenStack",type:"vStack",visible:true,x:0,y:20+8,height:300,spacing:8,constraints:{width:"parent.width"}}]});data.label=data.entity.find("label");data.bar=data.entity.find("bar");data.timeLabel=data.entity.find("timeLabel");data.childrenStack=data.entity.find("childrenStack");data.label.ignoreDirty++;data.bar.ignoreDirty++;data.timeLabel.ignoreDirty++;data.bar.addHandler("leftClick",
function(){data.color=jayus.chart.getRoutineColor(data.name);this.bg.setFill(data.color)});data.showChildren=true;data.label.data=data;data.label.handle({cursorOver:function(){this.setBg({fill:"white"})},cursorOut:function(){this.clearBg()},leftClick:function(){this.data.showChildren=!this.data.showChildren}});data.setFrameCount=function(count){for(var i=count-1;i>=0;i--)this.latencies[i]=-1;for(var routine in this.children)this.children[routine].setFrameCount(count)};data.update=function(){if(this.depth>
jayus.chart.MAX_ROUTINE_DEPTH)return;var i,routine,childData,width,val,total,count,labelColumnWidth=0,childrenVisible=false;total=0;count=0;for(i=0;i<this.latencies.length-1;i++){val=this.latencies[i];if(val!==-1){total+=val;count++}}this.averageLatency=total/count;for(routine in this.children){childData=this.children[routine];width=childData.label.width;if(width>labelColumnWidth)labelColumnWidth=width}for(routine in this.children){childData=this.children[routine];childData.labelColumnWidth=labelColumnWidth;
childData.update();width=childData.timeLabel.width;if(width+8>jayus.chart.timeLabelColumnWidth)jayus.chart.timeLabelColumnWidth=width+8;if(childData.entity.visible)childrenVisible=true}this.entity.averageLatency=this.averageLatency;this.entity.setVisible(this.averageLatency>=0.5);this.entity.setIncluded(this.averageLatency>=0.5);this.childrenVisible=childrenVisible&&this.showChildren;if(this.entity.visible){this.timeLabel.setText(Math.round(this.averageLatency*10)/10+" ms");this.entity.children.at(0).setHeight(20);
this.entity.children.at(1).setVisible(this.childrenVisible);this.entity.children.at(1).setIncluded(this.childrenVisible);if(this.childrenVisible){this.entity.setHeight(this.entity.children.at(0).height+8+this.entity.children.at(1).height);var children=this.children;this.sortedRoutineNames.sort(function(a,b){return children[a].averageLatency<children[b].averageLatency});this.childrenStack.children.sort(function(a,b){return a.averageLatency<b.averageLatency})}else this.entity.setHeight(this.entity.children.at(0).height);
if(this===jayus.chart.rootRoutine)labelColumnWidth=this.label.width;else labelColumnWidth=this.labelColumnWidth;labelColumnWidth+=10;this.label.setX(labelColumnWidth-this.label.width-4);this.bar.setX(labelColumnWidth);width=this.entity.parent.width-labelColumnWidth-jayus.chart.timeLabelColumnWidth-4;if(this!==jayus.chart.rootRoutine)width*=this.averageLatency/this.parent.averageLatency;if(!isNaN(width)&&isFinite(width))this.bar.setWidth(width);this.timeLabel.setX(this.entity.width-this.timeLabel.width-
4)}};data.drawBars=function(ctx,y){var totalHeight=Math.floor(this.time/jayus.chart.topMS*jayus.chart.graph.height),childrenHeight=0,selfHeight,temp;if(this.childrenVisible){var routineNames=this.sortedRoutineNames;for(i=0;i<routineNames.length;i++){data=this.children[routineNames[i]];if(data.entity.visible){temp=data.drawBars(ctx,y);y-=temp;childrenHeight+=temp}}}selfHeight=totalHeight-childrenHeight;ctx.fillStyle=this.color;ctx.fillRect(jayus.chart.index,y-selfHeight,1,selfHeight-0.5);this.time=
0;return totalHeight};return data},tallyInit:function jayus_chart_tallyInit(type){},show:function jayus_chart_show(){if(!this.initialized)this.init();this.visible=true;$(jayus.chart.dialog).show();jayus.fire("debugPanelShown")},hide:function jayus_chart_hide(){if(this.initialized&&this.visible){$(jayus.chart.dialog).hide();this.graphSurface.clear();this.index=0;this.visible=false}jayus.fire("debugPanelHidden")},mark:function jayus_chart_mark(text){if(this.visible){var ctx=this.graphSurface.context,
metrics=jayus.getTextMetrics(text,this.markFont);ctx.fillStyle=this.markColor;ctx.fillRect(this.index,0,1,this.graph.height-0.5);ctx.fillText(text,this.index-metrics.width-2,metrics.ascent)}},update:function jayus_chart_update(){if(this.visible){this.begin("DebugPanel");var ctx=this.graphSurface.context,y=this.graphSurface.height,i,data,routineNames,height;if(this.display.hasPendingMousemoveEvent){this.begin("MouseMoveEvent");this.display.processPendingMousemove();this.end()}this.begin("UpdateRoutineTable");
this.rootRoutine.update();this.end();this.begin("Rendering");this.rootRoutine.drawBars(ctx,this.graphSurface.height);ctx.clearRect(this.index+1,0,2,this.graph.height);this.index++;if(this.index>=this.graph.width)this.index=-1;this.display.refreshBuffer();this.end();this.end()}}},cacheFont:function jayus_cacheFont(font){this.debug.match("jayus.cacheFont",font,"font",2);var i,ctx,charWidths,descriptor;if(!this.isFontCached(font)){charWidths=[];ctx=this.getContext();ctx.save();ctx.font=font;for(i=0;i<
this.fontCacheMaxChar+1;i++)charWidths[i]=ctx.measureText(String.fromCharCode(i)).width;ctx.restore();descriptor=this.getVerticalFontMetrics(font);descriptor.charWidths=charWidths;this.fontCache[font]=descriptor}},cacheFonts:function jayus_cacheFonts(fonts){this.debug.matchArray("jayus.cacheFonts",fonts,"fonts",2);for(var i=0;i<fonts.length;i++)this.cacheFont(fonts[i])},isFontCached:function jayus_isFontCached(font){this.debug.match("jayus.isFontCached",font,"font",2);return typeof this.fontCache[font]===
"object"},getFontDescriptor:function jayus_getFontDescriptor(font){this.debug.match("jayus.getFontDescriptor",font,"font",2);if(!this.isFontCached(font))this.cacheFont(font);return this.fontCache[font]},getTextMetrics:function jayus_getTextMetrics(text,font){this.debug.matchArguments("jayus.getTextMetrics",arguments,"text",2,"font",2);if(typeof this.fontCache[font]!=="object")this.cacheFont(font);var descriptor=jayus.fontCache[font],charWidths=descriptor.charWidths,i,width=0;for(i=text.length-1;i>=
0;i--)width+=charWidths[text.charCodeAt(i)];return{width:width,ascent:descriptor.ascent,descent:descriptor.descent,height:descriptor.height}},getTextWidth:function jayus_getTextWidth(text,font){this.debug.matchArguments("jayus.getTextWidth",arguments,"text",2,"font",2);if(typeof this.fontCache[font]!=="object")this.cacheFont(font);var charWidths=jayus.fontCache[font].charWidths,i,width=0;for(i=text.length-1;i>=0;i--)width+=charWidths[text.charCodeAt(i)];return width},getVerticalFontMetrics:function jayus_getVerticalFontMetrics(font){var result=
...
this.toString(object)+" does not have method "+method);}},toString:function jayus_debug_toString(v){switch(typeof v){case "undefined":return"(Undefined)";case "boolean":return"(Boolean:"+v+")";case "number":return"(Number:"+v+")";case "string":return'(String:"'+v+'")';case "object":if(v===null)return"(null)";if(v instanceof Array)return"(Array:["+v+"])";if(typeof v.toString==="function")return v.toString();if(typeof v.toObject==="function")return JSON.stringify(v.toObject());return"(Object:"+v+")";
case "function":return"(Function:"+v+")"}return"(Unknown)"},exposedOriginColor:"blue",exposedBoundsColor:"red",exposedScopeColor:"green",exposedFrameColor:"orange",exposedTransformedDashing:[5,5],defaultDebugRenderer:function jayus_debug_defaultDebugRenderer(ctx){var item;ctx.save();ctx.lineWidth=1;if(this.shapeType===0){item=this.getBounds().alignToCanvas();item.etchOntoContext(ctx);ctx.strokeStyle=jayus.debug.exposedBoundsColor;ctx.stroke()}ctx.strokeStyle=jayus.debug.exposedScopeColor;item=this.getScope().alignToCanvas();
ctx.strokeRect(item.x,item.y,item.width,item.height);if(typeof this.getFrame==="function"){item=this.getFrame().alignToCanvas();item.etchOntoContext(ctx);ctx.strokeStyle=jayus.debug.exposedFrameColor;ctx.stroke();if(this.isTransformed){ctx.setLineDash(jayus.debug.exposedTransformedDashing);item=this.getUnFrame().alignToCanvas();ctx.strokeRect(item.x,item.y,item.width,item.height)}}ctx.restore()},showPoint:function jayus_debug_showPoint(display,x,y,duration){if(arguments.length===3)duration=1E3;var spec=
new jayus.Scene(3,3);spec.setBg({fill:"red"});spec.setOrigin(x,y);spec.animate().setAlpha(0).setDuration(duration).addHandler("finished",function(){display.children.remove(spec)}).start();display.children.add(spec)},pause:function jayus_debug_pause(){var i,display,ctx;if(jayus.running){jayus.stop();for(i=0;i<jayus.displays.length;i++){display=jayus.displays[i];ctx=display.context;ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle="rgba(0, 0, 0, 0.5)";ctx.fillRect(0,0,display.width,display.height);
ctx.fillStyle="white";ctx.font="24px sans-serif";ctx.textAlign="center";ctx.fillText("Paused",display.width/2,display.height/2);ctx.restore()}}else{for(i=0;i<jayus.displays.length;i++)jayus.displays[i].fullyRefreshBuffer();jayus.start()}}},intersectTest:function jayus_intersectTest(a,b){var i,temp,ctx,ret,at=a.shapeType,bt=b.shapeType;while(!at){if(a.getBounds()===a)throw new Error("jayus.intersectTest() - a.getBounds() === a, indefinite loop would result");a=a.getBounds();at=a.shapeType}while(!bt){if(b.getBounds()===
b)throw new Error("jayus.intersectTest() - b.getBounds() === b, indefinite loop would result");b=b.getBounds();bt=b.shapeType}if(at===-1){for(i=0;i<a.items.length;i++)if(this.intersectTest(a[i],b))return true;return false}if(bt===-1){for(i=0;i<b.items.length;i++)if(this.intersectTest(a,b[i]))return true;return false}if(at>bt){temp=a;a=b;b=temp}switch(a.shapeType+b.shapeType){case 1+1:return a.x===b.x&&a.y===b.y;case 1+2:return b.intersectsAt(a.x,a.y);case 2+2:return(a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-
b.y)<=(a.radius+b.radius)*(a.radius+b.radius);case 1+4:return b.intersectsAt(a.x,a.y);case 2+4:var x=2*Math.abs(a.x-(b.x+b.width/2)),y=2*Math.abs(a.y-(b.y+b.height/2));if(x>b.width+2*a.radius||y>b.height+2*a.radius)return false;if(x<=b.width||y<=b.height)return true;return(x-b.width)*(x-b.width)+(y-b.height)*(y-b.height)<=4*a.radius*a.radius;case 4+4:return!(a.x+a.width<b.x||(a.y+a.height<b.y||(a.x>b.x+b.width||a.y>b.y+b.height)));case 1+8:return b.intersectsAt(a.x,a.y);case 2+8:return this.intersectsPolygonPolygon(a.toPolygon(),
b);case 4+8:return this.intersectPolygonPolygon(a.toPolygon(),b);case 8+8:return this.intersectsPolygonPolygon(a,b);case 1+16:return b.intersectsAt(a.x,a.y);case 2+16:return this.intersectsPolygonPolygon(a.toPolygon(),b.toPolygon());case 4+16:return this.intersectsPolygonPolygon(a.toPolygon(),b.toPolygon());case 8+16:return this.intersectsPolygonPolygon(a,b.toPolygon());case 16+16:return this.intersectsPolygonPolygon(a.toPolygon(),b.toPolygon())}},intersectLineLine:function jayus_intersectLineLine(a1,
a2,b1,b2){jayus.debug.matchArgumentsAs("jayus.intersectLineLine",arguments,13,"a1","a2","b1","b2");var ua_t=(b2.x-b1.x)*(a1.y-b1.y)-(b2.y-b1.y)*(a1.x-b1.x),ub_t=(a2.x-a1.x)*(a1.y-b1.y)-(a2.y-a1.y)*(a1.x-b1.x),u_b=(b2.y-b1.y)*(a2.x-a1.x)-(b2.x-b1.x)*(a2.y-a1.y);if(u_b!==0){var ua=ua_t/u_b,ub=ub_t/u_b;return 0<=ua&&(ua<=1&&(0<=ub&&ub<=1))}return ua_t===0||ub_t===0},intersectLinePolygon:function jayus_intersectLinePolygon(a1,a2,poly){jayus.debug.matchArguments("jayus.intersectLinePolygon",arguments,
"a1",13,"a2",13,"poly",15);for(var i=poly.xPoints.length-1;i>=0;i--)if(this.intersectLineLine(a1,a2,poly.xPoints[i],poly.yPoints[i]))return true;return false},intersectPolygonPolygon:function jayus_intersectPolygonPolygon(poly1,poly2){jayus.debug.matchArgumentsAs("jayus.intersectPolygonPolygon",arguments,15,"poly1","poly2");for(var i=poly1.xPoints.length-1;i>=0;i--)if(this.intersectLinePolygon(poly1.xPoints[i],poly1.yPoints[i],poly2))return true;return false},distance:function jayus_distance(a,b){this.debug.matchArgumentsAs("jayus.distance",
arguments,13,"a","b");var x=a.x-b.x,y=a.y-b.y;return Math.sqrt(x*x+y*y)},math:{randomFrom:function jayus_math_randomFrom(array){jayus.debug.match("jayus.math.randomFrom",array,"array",6);if(!array.length)throw new RangeError("jayus.math.randomFrom() - Invalid array"+jayus.debug.toString(array)+" sent, array or at least length 1 required");return array[this.randomBetween(0,array.length-1)]},randomBetween:function jayus_math_randomBetween(min,max){jayus.debug.matchArgumentsAs("jayus.math.randomBetween",
...
angle2){jayus.debug.matchArgumentsAs("jayus.math.getCcwAngleBetween",arguments,4,"angle1","angle2");var distance=angle2-angle1;if(distance<0)distance+=2*Math.PI;return distance}},colors:{count:140,displayNames:["Alice Blue","Antique White","Aqua","Aquamarine","Azure","Beige","Bisque","Black","Blanched Almond","Blue","Blue Violet","Brown","Burly Wood","Cadet Blue","Chartreuse","Chocolate","Coral","Cornflower Blue","Cornsilk","Crimson","Cyan","Dark Blue","Dark Cyan","Dark Goldenrod","Dark Grey","Dark Green",
...
handlers[i];if(options.id===handler)handlers.splice(i,1)}if(!handlers.length){delete this.handlers[event];this.isHandler[event]=false}}return this},removeHandlers:function Responder_removeHandlers(event){jayus.debug.match("Responder.removeHandlers",event,"event",2);if(this.isHandler[event]){delete this.handlers[event];this.isHandler[event]=false}return this},removeAllHandlers:function Responder_removeAllHandlers(){this.hasHandlers=false;this.isHandler=null;this.handlers=null;if(typeof this.events===
"object"&&this.events!==null)this.handle(this.events);return this},fire:function Responder_fire(event,data){jayus.debug.match("Responder.fire",event,"event",2);jayus.debug.matchOptional("Responder.fire",data,"data",0);var i,opts,result,handlers;if(this.hasHandlers&&this.isHandler[event]){handlers=this.handlers[event].slice(0);for(i=0;i<handlers.length;i++){opts=handlers[i];if(opts.remove)this.removeHandler(event,opts.handler);else if(opts.enabled)if(opts.handler.call(opts.hasContext?opts.context:
this,data,opts))return true}}return false}});jayus.Animator=jayus.Responder.extend({isAnimator:true,running:false,attachToJayus:true,startTime:null,duration:1E3,easing:jayus.easing.linear,looped:false,oscillate:false,init:function Animator(updater){jayus.debug.match("Animator",updater,"updater",3);this.update=updater},initFromObject:function Animator_initFromObject(object){jayus.debug.match("Animator.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);if(typeof object.duration==="number")this.duration=
...
index,"index",4);if(0<=index&&index<this.items.length)return this.items[index];return null},get:function List_get(id){jayus.debug.match("List.get",id,"id",0);var i,item;for(i=this.items.length-1;i>=0;i--){item=this.items[i];if(item.id===id)return item}return null},find:function List_find(id){jayus.debug.match("List.get",id,"id",0);var i,item;for(i=this.items.length-1;i>=0;i--){item=this.items[i];if(item.id===id)return item;if(item.isParent){item=item.find(id);if(item!==null)return item}}},add:function List_add(item){var tempItem;
if(arguments.length===1){tempItem=item;if(typeof tempItem.frozen!=="number")tempItem=jayus.parse(tempItem);jayus.debug.match("List.add",item,"item",this.typeId);this.items.push(item);this.added(item)}else{var items=Array.prototype.slice.call(arguments);for(var i=0;i<items.length;i++){tempItem=items[i];if(typeof tempItem.frozen!=="number"){tempItem=jayus.parse(tempItem);items[i]=tempItem}this.items.push(tempItem)}this.addedMany(items)}return this},append:function List_append(items){jayus.debug.matchArray("List.append",
items,"items",this.typeId);for(var i=0;i<items.length;i++)this.items.push(items[i]);this.addedMany(items);return this},insert:function List_insert(item,index){jayus.debug.matchArguments("List.insert",arguments,"item",this.typeId,"index",4);this.items.splice(index,0,item);this.added(item);return this},insertBefore:function List_insertBefore(item,pivot){jayus.debug.matchArguments("List.insertBefore",arguments,"item",this.typeId,"pivot",this.typeId);var index=this.items.indexOf(pivot);if(index>0){this.items.splice(index-
1,0,item);this.added(item)}return this},insertAfter:function List_insertAfter(item,pivot){jayus.debug.matchArguments("List.insertAfter",arguments,"item",this.typeId,"pivot",this.typeId);var index=this.items.indexOf(pivot);if(index!==-1){this.items.splice(index,0,item);this.added(item)}return this},remove:function List_remove(item){jayus.debug.match("List.remove",item,"item",this.typeId);var i,index;if(arguments.length===1){index=this.items.indexOf(item);if(index>=0){this.items.splice(index,1);this.removed(item)}}else{for(i=
0;i<arguments.length;i++){index=this.items.indexOf(arguments[i]);if(index>=0)this.items.splice(index,1)}this.removedMany(arguments)}return this},removeAt:function List_removeAt(index){jayus.debug.match("List.removeAt",index,"index",4);if(0<=index&&index<this.items.length){var item=this.items[index];this.items.splice(index,1);this.removed(item)}return this},replace:function List_replace(oldItem,newItem){if(typeof oldItem==="string")jayus.debug.matchArguments("List.replace",arguments,"oldId",2,"newItem",
this.typeId);else jayus.debug.matchArguments("List.replace",arguments,"oldItem",this.typeId,"newItem",this.typeId);return this.replaceAt(this.indexOf(oldItem),newItem)},update:function List_update(oldItem,newItem){if(typeof oldItem==="string")jayus.debug.matchArguments("List.update",arguments,"oldId",2,"newItem",this.typeId);else jayus.debug.matchArguments("List.update",arguments,"oldItem",this.typeId,"newItem",this.typeId);var index=this.indexOf(oldItem);if(index===-1)this.add(newItem);else this.replaceAt(index,
newItem);return this},replaceAt:function List_replaceAt(index,newItem){jayus.debug.matchArguments("List.replaceAt",arguments,"index",4,"newItem",this.typeId);if(index>=0){this.removed(this.items[index]);this.items[index]=newItem;this.added(newItem)}return this},purge:function List_purge(){var items=this.items;this.removedMany(items);this.items=[];if(this.parent.items===items)this.parent.items=this.items;return this},onEach:function List_onEach(method,args){jayus.debug.match("List.onEach",method,"method",
2);jayus.debug.matchOptional("List.onEach",args,"args",6);for(var i=0;i<this.items.length;i++)this.items[i][method].apply(this.items[i],args);return this},forEach:function List_forEach(func,args){jayus.debug.match("List.forEach",func,"func",3);jayus.debug.matchOptional("List.forEach",args,"args",6);var i,ret;for(i=0;i<this.items.length;i++){ret=func.apply(this.items[i],args);if(ret!==undefined)return ret}return null},forEvery:function List_forEvery(func,args){jayus.debug.match("List.forEvery",func,
"func",3);jayus.debug.matchOptional("List.forEvery",args,"args",6);var i,ret;for(i=0;i<this.items.length;i++){ret=func.apply(this.items[i],args);if(ret!==undefined)return ret}return null},sort:function List_sort(comparator){jayus.debug.match("List.sort",comparator,"comparator",3);this.items.sort(comparator);this.parent.listItemsMoved();return this}});jayus.Color=jayus.Dependency.extend({r:0,g:0,b:0,a:1,init:function Color(){if(arguments.length)this.set.apply(this,arguments)},toObject:function Color_toObject(){var object={type:"Color",r:this.r,g:this.g,b:this.b,a:this.a};if(this.id!=="")object.id=this.id;return object},initFromObject:function Color_initFromObject(object){jayus.debug.match("Color.initFromObject",object,"object",5);this.r=object.r;this.g=object.g;this.b=object.b;this.a=object.a;return this.dirty(127)},set:function Color_set(r,g,
...
false;if(this.isParent)this.forEachChild(function(){this.unexposeAll()});this.dirty(127)},animate:function Animatable_animate(){this.actionsToAnimate++;return this},init:function Entity(){if(typeof this.events==="object"&&this.events!==null)this.handle(this.events);if(this.enableDragEvents)this.handle({leftPress:function Entity_startLeftDrag(e){this.leftDragging=true;return this.fire("leftDragStart",e)},leftRelease:function Entity_endLeftDrag(e){if(this.leftDragging){this.leftDragging=false;return this.fire("leftDragEnd",
e)}},middlePress:function Entity_startMiddleDrag(e){this.middleDragging=true;return this.fire("middleDragStart",e)},middleRelease:function Entity_endMiddleDrag(e){if(this.middleDragging){this.middleDragging=false;return this.fire("middleDragEnd",e)}},rightPress:function Entity_startRightDrag(e){this.rightDragging=true;return this.fire("rightDragStart",e)},rightRelease:function Entity_endRightDrag(e){if(this.rightDragging){this.rightDragging=false;return this.fire("rightDragEnd",e)}},cursorMove:function Entity_handleDragging(e){var ret=
false;if(this.leftDragging)ret=this.fire("leftDrag",e)||ret;if(this.middleDragging)ret=this.fire("middleDrag",e)||ret;if(this.rightDragging)ret=this.fire("rightDrag",e)||ret;return ret},cursorOut:function Entity_endDragging(){if(this.leftDragging)this.fire("leftDragEnd");if(this.middleDragging)this.fire("middleDragEnd");if(this.rightDragging)this.fire("rightDragEnd");this.leftDragging=this.middleDragging=this.rightDragging=false}})},toObject:function Entity_toObject(){var object={type:"Entity"};if(this.id!==
"")object.id=this.id;var i,key,val,valType;for(i=0;i<jayus.Entity.prototype.properties.length;i++){key=jayus.Entity.prototype.properties[i];val=this[key];valType=typeof val;if(val!==jayus.Entity.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}return object},initFromObject:function Entity_initFromObject(object){jayus.debug.match("Entity.initFromObject",object,"object",5);jayus.Dependency.prototype.initFromObject.call(this,object);this.frozen++;var i,key,val,valType;for(i=
0;i<jayus.Entity.prototype.properties.length;i++){key=jayus.Entity.prototype.properties[i];val=object[key];valType=typeof val;if(val!==undefined){if(valType==="object")val=jayus.parse(val);this[key]=val}}if(typeof object.tooltip==="string")this.setTooltip(object.tooltip);if(typeof object.xScale==="number")this.setXScale(object.xScale);if(typeof object.yScale==="number")this.setYScale(object.yScale);if(typeof object.scale==="number")this.setScale(object.scale);if(typeof object.xVelocity==="number")this.setXVelocity(object.xVelocity);
if(typeof object.yVelocity==="number")this.setYVelocity(object.yVelocity);if(typeof object.dragButton==="string")this.setDragButton(object.dragButton);if(typeof object.parent==="object")this.setParent(object.parent);if(typeof object.constraints==="object")this.setConstraints(object.constraints);if(typeof object.events==="object"&&object.events!==null)this.handle(object.events);this.frozen--;return this.dirty(127)},setId:function Entity_setId(id){jayus.Dependency.prototype.setId.call(this,id);if(jayus.declareChildrenAsProperties&&
(this.parent!==null&&id)){if(typeof this.parent[id]!=="undefined"){console.error('Entity.setParent() - Collision between child id("'+id+'") and parent property('+this.parent[id]+")");return}this.parent[id]=this}},addHandler:function Entity_addHandler(event){jayus.Responder.prototype.addHandler.apply(this,arguments);if(event==="leftClick")this.addHandler("leftRelease",function(e){if(jayus.entityPressedOn===this)this.fire("leftClick",e)});else if(event==="middleClick")this.addHandler("middleRelease",
function(e){if(jayus.entityPressedOn===this)this.fire("middleClick",e)});else if(event==="rightClick")this.addHandler("rightRelease",function(e){if(jayus.entityPressedOn===this)this.fire("rightClick",e)});else if(!this.trackCursor)if(event.indexOf("cursor")===0||(event.indexOf("Press")>=0||event.indexOf("Release")>=0)){this.trackCursor=true;if(this.parent!==null)this.parent.childCursorTrackingChanged(this,true)}},dirtied:true,frozen:0,pendingDirty:0,ignoreDirty:0,freeze:function Entity_freeze(){this.frozen++;
return this},unfreeze:function Entity_unfreeze(){this.frozen--;if(!this.frozen)this.thaw();return this},thaw:function Entity_thaw(){jayus.dirtyCount++;this.fire("dirty",this.pendingDirty);this.dirtied=true;this.informDependents(this.pendingDirty);if(this.constraints!==null)this.constrain();this.pendingDirty=0;return this},dirty:function Entity_dirty(type){jayus.debug.match("Entity.dirty",type,"type",4);if(!this.ignoreDirty)if(this.frozen)this.pendingDirty|=type;else{jayus.dirtyCount++;if(this.constraints!==
null)this.constrain();if(this.isTransformed&&type&8+2)this.matrixDirty=true;this.fire("dirty",type);this.dirtied=true;this.informDependents(type);if(this.parentDisplay!==null&&type&6+8)this.parentDisplay.startCursorUpdate()}return this},parent:null,parentDisplay:null,getAbsoluteParent:function Entity_getAbsoluteParent(){return this.parent!==null?this.parent.getAbsoluteParent():this},setParent:function Entity_setParent(parent){if(this.parent!==parent){if(this.parent!==null)throw new Error("Entity.setParent() - Entity already has a parent");
if(parent.exposingAll)this.expose();this.parent=parent;this.parentDisplay=parent.parentDisplay;this.attach(parent);if(jayus.declareChildrenAsProperties&&this.id)if(typeof parent[this.id]!=="undefined")console.error('Entity.setParent() - Collision between child id("'+this.id+'") and parent property('+parent[this.id]+")");else parent[this.id]=this;if(this.trackCursor)parent.childCursorTrackingChanged(this,true);if(this.constraintsNeedParent){this.parentConstrainerOptions={};var that=this;this.parent.addHandler("dirty",
function(type){that.constrain()},this.parentConstrainerOptions)}this.fire("added")}},removeParent:function Entity_removeParent(){if(this.constraints!==null&&this.constraints.needsParent){this.parentConstrainerOptions.remove=true;this.parentConstrainerOptions=null}if(jayus.declareChildrenAsProperties&&this.id)delete this.parent[this.id];this.detach(this.parent);this.parent=null;this.underCursor=false;this.fire("removed")},setIncluded:function Entity_setIncluded(on){jayus.debug.match("Entity.setIncluded",
on,"on",1);if(this.included!==on){this.included=on;this.dirty(127)}return this},include:function Entity_include(){return this.setIncluded(true)},exclude:function Entity_exclude(){return this.setIncluded(false)},setVisible:function Entity_setVisible(on){jayus.debug.match("Entity.setVisible",on,"on",1);if(this.visible!==on){this.visible=on;this.dirty(1)}return this},show:function Entity_show(){return this.setVisible(true)},hide:function Entity_hide(){return this.setVisible(false)},constraints:null,
hasConstraint:null,constraintsNeedParent:false,parentConstrainerOptions:null,setConstraint:function Entity_setConstraint(key,formula){if(typeof this.constraints!=="object"){var data={};data[key]=formula;this.setConstraints(data)}else{this.constraints[key]=formula;this.hasConstraint[key]=true;this.constrain()}return this},clearConstraint:function Entity_clearConstraint(key){if(typeof this.constraints==="object"){delete this.constraints[key];this.hasConstraint[key]=false}return this},setConstraints:function Entity_setConstraints(data){var key,
formula,fromParent=false;this.hasConstraint={x:false,y:false,width:false,height:false};for(key in data){formula=data[key];this.hasConstraint[key]=true;if(formula.indexOf("parent")!==-1)fromParent=true}if(fromParent&&this.parent!==null){var that=this;this.parent.addHandler("dirty",function(){that.constrain()})}this.constraints=data;this.constraintsNeedParent=fromParent;this.constrain();return this},constrain:function Entity_constrain(){if(this.constraints!==null&&(!this.constraintsNeedParent||this.parent!==
null)){var hasData=this.hasConstraint,data=this.constraints;if(hasData.x)this.setX(jayus.executeFormula(this,data.x));if(hasData.y)this.setY(jayus.executeFormula(this,data.y));if(hasData.width)this.setWidth(jayus.executeFormula(this,data.width));if(hasData.height)this.setHeight(jayus.executeFormula(this,data.height))}return this},setAlpha:function Entity_setAlpha(alpha){jayus.debug.match("Entity.setAlpha",alpha,"alpha",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,
this.setAlpha,this.alpha,alpha)}if(this.alpha!==alpha){this.alpha=alpha;this.dirty(16)}return this},trackCursor:false,canAcceptCursor:true,canReleaseCursor:true,underCursor:false,hasCursor:false,cursorX:0,cursorY:0,tooltip:"",setTooltip:function Entity_setTooltip(text){this.trackCursor=true;if(this.parent!==null)this.parent.childCursorTrackingChanged(this,true);this.canAcceptCursor=true;this.tooltip=text},updateCursor:function Entity_updateCursor(x,y){var pos=new jayus.Point(x,y);if(this.isTransformed)this.getMatrix().inverseTransformPointOnto(x,
y,pos);this.cursorX=pos.x;this.cursorY=pos.y;var intersects=this.cursorHitTest(pos.x,pos.y);if(this.underCursor)if(!intersects){this.underCursor=false;this.fire("cursorOut",pos);if(this.isParent)this.removeCursorFromChildren()}else{if(this.isParent&&this.propagateCursor)this.updateCursorOnChildren(pos.x,pos.y)}else if(intersects){this.underCursor=true;this.fire("cursorOver",pos);if(this.isParent&&this.propagateCursor)this.updateCursorOnChildren(pos.x,pos.y)}},cursorHitTest:function Entity_cursorHitTest(x,
y){return true},removeCursor:function Entity_removeCursor(){this.underCursor=false;this.fire("cursorOut",null);if(this.isParent)this.removeCursorFromChildren()},dragButton:null,dragging:false,dragStarterOptions:null,dragEnderOptions:null,dragHandlerOptions:null,setDragButton:function Entity_setDragButton(button){if(button!==null&&(button!=="left"&&(button!=="middle"&&button!=="right")))throw"Entity.setDragButton() - Error: button("+button+') must be null, "left", "middle", or "right"';if(this.dragButton===
null&&typeof button==="string"){this.dragStarterOptions={};this.dragHandlerOptions={};this.dragEnderOptions={};this.addHandler(button+"Press",function(e){this.dragging=false;this.dragHandlerOptions.remove=false;this.dragEnderOptions.remove=false;var that=this;var dragHandler=function(e){that.canReleaseCursor=false;if(!that.dragging){that.parentDisplay.setCursor("move");that.fire("dragStart");that.dragging=true}if(!that.fire("dragged",e))that.translate(e.deltaX,e.deltaY);return true};this.parentDisplay.addHandler("cursorMove",
dragHandler,this.dragHandlerOptions);jayus.addHandler(button+"Release",function(e,options){if(that.dragging){that.canReleaseCursor=true;that.parentDisplay.updateCursor(that.parentDisplay.cursor.x,that.parentDisplay.cursor.y);that.fire("dragEnd");that.parentDisplay.resetCursor()}that.dragHandlerOptions.remove=true;options.remove=true;if(that.dragging){that.dragging=false;return true}},this.dragEnderOptions)},this.dragStarterOptions)}else if(typeof this.dragButton==="string"&&button===null){this.dragStarterOptions.remove=
true;this.dragStarterOptions=null;this.dragHandlerOptions.remove=true;this.dragHandlerOptions=null;this.dragEnderOptions.remove=true;this.dragEnderOptions=null}else if(typeof this.dragButton==="string"&&(typeof button==="string"&&this.dragButton!==button)){this.setDragButton(null);this.setDragButton(button)}this.dragButton=button;return this},matrix:null,matrixDirty:true,getMatrix:function Entity_getMatrix(){if(this.matrixDirty){if(this.matrix===null)this.matrix=new jayus.Matrix;else this.matrix.identity();
this.applyTransforms(this.matrix);this.matrixDirty=false}return this.matrix},applyTransforms:function Entity_applyTransforms(ctx){if(this.isTransformed){var xScale=this.xScale,yScale=this.yScale;if(this.flipX)xScale*=-1;if(this.flipY)yScale*=-1;if(this.xAnchor||this.yAnchor)ctx.translate(this.xAnchor,this.yAnchor);ctx.scale(xScale,yScale);ctx.rotate(this.angle);if(this.xAnchor||this.yAnchor)ctx.translate(-this.xAnchor,-this.yAnchor)}return this},angle:0,setAngle:function Entity_setAngle(angle){jayus.debug.match("Entity.setAngle",
angle,"angle",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setAngle,this.angle,angle)}if(this.angle!==angle){this.angle=angle;this.isTransformed=true;this.dirty(8)}return this},rotate:function Entity_rotate(angle){jayus.debug.match("Entity.rotate",angle,"angle",4);return this.setAngle(this.angle+angle)},xScale:1,yScale:1,setScale:function Entity_setScale(x,y){if(arguments.length===1)y=x;if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,
this.setScale,[this.xScale,this.yScale],[x,y])}if(this.xScale!==x||this.yScale!==y){this.xScale=x;this.yScale=y;this.isTransformed=true;this.dirty(8)}return this},setScaleAround:function Entity_setScaleAround(x,y,xAnchor,yAnchor){if(arguments.length===3){jayus.debug.matchArguments("Entity.setScaleAround",arguments,"x",4,"y",4,"xAnchor",4);return this.setScaleAround(x,x,y,xAnchor)}else jayus.debug.matchArgumentsAs("Entity.scale",arguments,4,"x","y","xAnchor","yAnchor");var pos=this.localToParent(xAnchor,
yAnchor);this.setScale(x,y);var pos2=this.localToParent(xAnchor,yAnchor);this.translate(pos.x-pos2.x,pos.y-pos2.y);return this.dirty(127)},scale:function Entity_scale(x,y){if(arguments.length===1){jayus.debug.match("Entity.scale",x,"scale",4);y=x}else jayus.debug.matchArgumentsAs("Entity.scale",arguments,4,"x","y");return this.setScale(this.xScale*x,this.yScale*y)},setXScale:function Entity_setXScale(scale){jayus.debug.match("Entity.scale",scale,"scale",4);if(this.actionsToAnimate){this.actionsToAnimate--;
return new jayus.MethodAnimator(this,this.setXScale,this.xScale,scale)}if(this.xScale!==scale){this.xScale=scale;this.isTransformed=true;this.dirty(8)}return this},setYScale:function Entity_setYScale(scale){jayus.debug.match("Entity.scale",scale,"scale",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setYScale,this.yScale,scale)}if(this.yScale!==scale){this.yScale=scale;this.isTransformed=true;this.dirty(8)}return this},flipX:false,flipY:false,setFlipX:function Entity_setFlipX(on){jayus.debug.match("Entity.setFlipX",
...
yAnchor:0,setAnchor:function Entity_setAnchor(x,y){jayus.debug.matchCoordinate("Entity.setAnchor",x,y);if(arguments.length===1)return this.setAnchor(x.x,x.y);if(this.xAnchor!==x||this.yAnchor!==y){var pos=this.getPosAt(this.xAnchor/this.width,this.yAnchor/this.height);this.xAnchor=x;this.yAnchor=y;var pos2=this.getPosAt(this.xAnchor/this.width,this.yAnchor/this.height);console.log(pos2.x-pos.x,pos2.y-pos.y);this.dirty(8)}return this},screenToLocal:function Entity_screenToLocal(x,y){jayus.debug.matchCoordinate("Entity.screenToLocal",
x,y);if(arguments.length===1)return this.screenToLocal(x.x,x.y);if(this.parent!==null){var pos=this.parent.screenToLocal(x,y);return this.parentToLocal(pos)}return new jayus.Point(x,y)},parentToLocal:function Entity_parentToLocal(x,y){jayus.debug.matchCoordinate("Entity.parentToLocal",x,y);if(arguments.length===1)return this.parentToLocal(x.x,x.y);var pos=new jayus.Point(x,y);if(this.isTransformed)this.getMatrix().inverseTransformPointOnto(x,y,pos);else{pos.x-=this.x;pos.y-=this.y}return pos},localToScreen:function Entity_localToScreen(x,
y){jayus.debug.matchCoordinate("Entity.localToScreen",x,y);if(arguments.length===1)return this.localToScreen(x.x,x.y);if(this.parent!==null)return this.parent.localToScreen(this.localToParent(x,y));return new jayus.Point(x,y)},localToScreenOnto:function Entity_localToScreenOnto(x,y,ret){jayus.debug.matchArguments("Entity.localToScreenOnto",arguments,"x",4,"y",4,"ret",13);if(this.parent!==null){this.localToParentOnto(x,y,ret);this.parent.localToScreenOnto(ret.x,ret.y,ret)}else{ret.x=x;ret.y=y}return ret},
localToParent:function Entity_localToParent(x,y){jayus.debug.matchCoordinate("Entity.localToParent",x,y);if(arguments.length===1)return this.localToParent(x.x,x.y);return this.localToParentOnto(x,y,new jayus.Point)},localToParentOnto:function Entity_localToParentOnto(x,y,ret){jayus.debug.matchArguments("Entity.localToParentOnto",arguments,"x",4,"y",4,"ret",13);if(this.isTransformed)return this.getMatrix().transformPointOnto(x,y,ret);ret.x=x+this.x;ret.y=y+this.y;return ret},running:false,xVelocity:0,
yVelocity:0,setXVelocity:function Entity_setXVelocity(vel){jayus.debug.match("Entity.setXVelocity",vel,"vel",4);this.xVelocity=vel;if(!this.running){jayus.animators.push(this);this.running=true}return this},setYVelocity:function Entity_setYVelocity(vel){jayus.debug.match("Entity.setYVelocity",vel,"vel",4);this.yVelocity=vel;if(!this.running){jayus.animators.push(this);this.running=true}return this},setVelocity:function Entity_setVelocity(x,y){jayus.debug.matchCoordinate("Entity.setVelocity",x,y);
if(arguments.length===1)return this.setVelocity(x.x,x.y);this.xVelocity=x;this.yVelocity=y;if(!this.running){jayus.animators.push(this);this.running=true}return this},clearVelocity:function Entity_clearVelocity(){if(this.running){this.xVelocity=0;this.yVelocity=0;jayus.animators.splice(jayus.animators.indexOf(this),1);this.running=false}return this},tick:function Entity_tick(now,elapsed){this.translate(this.xVelocity*elapsed,this.yVelocity*elapsed)},intersects:function Entity_intersects(object){return jayus.intersectTest(this,
object)},intersectCount:function Entity_intersectCount(entities){jayus.debug.matchArray("Entity.intersectCount",entities,"entities",12);if(!entities.length)throw new RangeError("Entity.intersectCount() - Invalid entities"+jayus.debug.toString(entities)+" sent, length of at least 1 required");var i,count=0;for(i=entities.length-1;i>=0;i--)if(jayus.intersectTest(this,entities[i]))count++;return count},intersectsAny:function Entity_intersectsAny(entities){if(entities instanceof jayus.List)entities=entities.items;
jayus.debug.matchArray("Entity.intersectsAny",entities,"entities",12);for(var i=0;i<entities.length;i++)if(jayus.intersectTest(this,entities[i]))return true;return false},intersectsAll:function Entity_intersectsAll(entities){if(entities instanceof jayus.List)entities=entities.items;jayus.debug.matchArray("Entity.intersectsAll",entities,"entities",12);if(!entities.length)throw new RangeError("Entity.intersectsAll() - Invalid entities"+jayus.debug.toString(entities)+" sent, length of at least 1 required");
for(var i=0;i<entities.length;i++)if(!jayus.intersectTest(this,entities[i]))return false;return true},intersectsWhich:function Entity_intersectsWhich(entities){if(entities instanceof jayus.List)entities=entities.items;jayus.debug.matchArray("Entity.intersectsWhich",entities,"entities",12);var i,ret=[];for(i=0;i<entities.length;i++)if(jayus.intersectTest(this,entities[i]))ret.push(entities[i]);return ret},rasterize:function Entity_rasterize(){var scope=this.getScope(),surface=new jayus.Surface(scope.width,
scope.height),x=this.x,y=this.y;this.x=0;this.y=0;this.drawOnto(surface);this.x=x;this.y=y;return surface},drawOnto:function Entity_drawOnto(surface){jayus.debug.match("Entity.drawOnto",surface,"surface",17);var ctx=surface.context;ctx.save();this.drawOntoContext(ctx,0,0);ctx.restore();surface.dirty(16);return this}})})();jayus.RectEntity=jayus.Entity.extend({shapeType:4,hasFlexibleWidth:true,hasFlexibleHeight:true,properties:["x","y","roundX","roundY","alignBg"],updateCursor:function RectEntity_updateCursor(x,y){var pos=new jayus.Point(x,y);if(this.isTransformed)this.getMatrix().inverseTransformPointOnto(x,y,pos);else{pos.x-=this.x;pos.y-=this.y}this.cursorX=pos.x;this.cursorY=pos.y;var intersects=this.cursorHitTest(pos.x,pos.y);if(this.underCursor)if(!intersects){this.underCursor=false;this.fire("cursorOut",pos);
if(this.isParent)this.removeCursorFromChildren()}else{if(this.isParent&&this.propagateCursor)this.updateCursorOnChildren(pos.x,pos.y)}else if(intersects){this.underCursor=true;this.fire("cursorOver",pos);if(this.isParent&&this.propagateCursor)this.updateCursorOnChildren(pos.x,pos.y)}},cursorHitTest:function RectEntity_cursorHitTest(x,y){return 0<=x&&(0<=y&&(x<=this.width&&y<=this.height))},toObject:function RectEntity_toObject(){var object=jayus.Entity.prototype.toObject.apply(this);object.type="RectEntity";
var i,key,val,valType;for(i=0;i<jayus.RectEntity.prototype.properties.length;i++){key=jayus.RectEntity.prototype.properties[i];val=this[key];valType=typeof val;if(val!==jayus.RectEntity.prototype[key]){if(valType==="object")val=val.toObject();object[key]=val}}if(this.bg!==null)object.bg=this.bg.toObject();if(this.bounds!==null)object.bounds=this.bounds.toObject();return object},initFromObject:function RectEntity_initFromObject(object){jayus.debug.match("RectEntity.initFromObject",object,"object",
5);jayus.Entity.prototype.initFromObject.call(this,object);this.ignoreDirty++;var i,key,val,valType;for(i=0;i<jayus.RectEntity.prototype.properties.length;i++){key=jayus.RectEntity.prototype.properties[i];val=object[key];if(typeof val!=="undefined")this[key]=val}if(typeof object.width==="number")this.setWidth(object.width);if(typeof object.height==="number")this.setHeight(object.height);if(typeof object.bounds==="object")this.bounds=jayus.parse(object.bounds);if(typeof object.negateBlur==="boolean")this.negateBlur=
object.negateBlur;if(typeof object.buffering==="boolean")this.setBuffering(object.buffering);if(typeof object.bg==="object")this.setBg(object.bg);if(typeof object.widthPolicy==="object")this.widthPolicy=jayus.parse(object.widthPolicy);if(typeof object.heightPolicy==="object")this.heightPolicy=jayus.parse(object.heightPolicy);this.ignoreDirty--;return this.dirty(127)},x:0,y:0,setX:function Entity_setX(x){jayus.debug.match("Entity.setX",x,"x",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,
this.setX,this.x,x)}if(this.x!==x){this.x=x;this.dirty(2)}return this},setY:function Entity_setY(y){jayus.debug.match("Entity.setY",y,"y",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setY,this.y,y)}if(this.y!==y){this.y=y;this.dirty(2)}return this},getOrigin:jayus.Rectangle.prototype.getOrigin,setOrigin:jayus.Rectangle.prototype.setOrigin,translate:jayus.Rectangle.prototype.translate,getLeft:jayus.Rectangle.prototype.getLeft,getRight:jayus.Rectangle.prototype.getRight,
getTop:jayus.Rectangle.prototype.getTop,getBottom:jayus.Rectangle.prototype.getBottom,setLeft:jayus.Rectangle.prototype.setLeft,setRight:jayus.Rectangle.prototype.setRight,setTop:jayus.Rectangle.prototype.setTop,setBottom:jayus.Rectangle.prototype.setBottom,getPosAt:function RectEntity_getPosAt(anchorX,anchorY){jayus.debug.matchCoordinate("RectEntity.getPosAt",anchorX,anchorY);if(arguments.length===1)return this.getPosAt(anchorX.x,anchorX.y);return this.localToParent(anchorX*this.width,anchorY*this.height)},
setPosAt:function RectEntity_setPosAt(x,y,anchorX,anchorY){if(arguments.length===3)return this.setPosAt(x.x,x.y,y,anchorX);jayus.debug.matchArgumentsAs("RectEntity.setPosAt",arguments,4,"x","y","anchorX","anchorY");var pos=this.getPosAt(anchorX,anchorY);return this.translate(x-pos.x,y-pos.y)},setRelativeAnchor:function Entity_setRelativeAnchor(x,y){jayus.debug.matchCoordinate("RectEntity.setRelativeAnchor",x,y);if(arguments.length===1)return this.setAnchor(this.width*x.x,this.height*x.y);return this.setAnchor(this.width*
x,this.height*y)},roundX:false,roundY:false,setRounding:function RectEntity_setRounding(x,y){if(arguments.length===1){jayus.debug.match("RectEntity.setRounding",x,"x",1);this.roundX=x;this.roundY=y}else{jayus.debug.matchArgumentsAs("RectEntity.setRounding",arguments,1,"x","y");if(this.roundX!==x||this.roundY!==y){this.roundX=x;this.roundY=y;this.dirty(127)}}return this},body:null,setBody:function RectEntity_setBody(body){jayus.debug.match("RectEntity.setBody",body,"body",5);if(this.body!==null)delete this.body.entity;
else{jayus.box2d.physicalEntities.push(this);this.setAnchor(this.width/2,this.height/2)}this.body=body;this.body.entity=this;return this},removeBody:function RectEntity_removeBody(){if(this.body!==null){delete this.body.entity;this.body=null;jayus.box2d.physicalEntities.splice(jayus.box2d.physicalEntities.indexOf(this),1)}return this},updateFromBody:function RectEntity_updateFromBody(){if(this.body!==null&&this.body.IsAwake()){var pos=this.body.GetPosition();this.setPosAt(pos.x,pos.y,0.5,0.5);this.angle=
this.body.GetAngle();this.isTransformed=true;this.matrixDirty=true}},width:0,height:0,setWidth:function RectEntity_setWidth(width){jayus.debug.match("RectEntity.setWidth",width,"width",4);if(!this.hasFlexibleWidth)throw new Error("RectEntity.setWidth() - Entity width is not flexible");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setWidth,this.width,width)}if(width!==this.width)this.changeSize(width,this.height);return this},setHeight:function RectEntity_setHeight(height){jayus.debug.match("RectEntity.setHeight",
height,"height",4);if(!this.hasFlexibleHeight)throw new Error("RectEntity.setHeight() - Entity height is not flexible");if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setHeight,this.height,height)}if(height!==this.height)this.changeSize(this.width,height);return this},setSize:function RectEntity_setSize(width,height){jayus.debug.matchSize("RectEntity.setSize",width,height);if(!this.hasFlexibleWidth||!this.hasFlexibleHeight)throw new Error("RectEntity.setSize() - Entity size is not flexible");
if(arguments.length===1)return this.setSize(width.width,width.height);if(this.width!==width||this.height!==height)this.changeSize(width,height);return this},changeSize:function RectEntity_changeSize(width,height){this.width=width;this.height=height;this.dirty(4)},intersectsAt:function RectEntity_intersectsAt(x,y){return this.getBounds().intersectsAt(x,y)},getUnFrame:function RectEntity_getUnFrame(){return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getFrame:function RectEntity_getFrame(){if(this.isTransformed)return(new jayus.Polygon.Rectangle(0,
0,this.width,this.height)).transform(this.getMatrix());return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getScope:function RectEntity_getScope(){return this.getFrame().getScope()},bounds:null,boundsClone:null,getBounds:function RectEntity_getBounds(){if(this.bounds!==null){this.bounds.cloneOnto(this.boundsClone);return this.boundsClone.translate(this.x,this.y)}return this.getFrame().getScope()},setBounds:function RectEntity_setBounds(shape){jayus.debug.match("RectEntity.setBounds",
shape,"shape",0);if(shape.shapeType!=="number")shape=jayus.parse(shape);if(this.bounds!==null)this.bounds.detach(this);this.bounds=shape;this.bounds.attach(this);this.boundsClone=this.bounds.clone();this.shapeType=0;return this},clearBounds:function RectEntity_clearBounds(){if(this.bounds!==null){this.bounds.detach(this);this.bounds=null;this.boundsClone=null;this.shapeType=4}return this},componentDirtied:function RectEntity_componentDirtied(component,type){if(component===this.bounds)this.bounds.cloneOnto(this.boundsClone)},
bg:null,alignBg:true,setBg:function RectEntity_setBg(brush){jayus.debug.match("RectEntity.setBg",brush,"brush",0);if(!(brush instanceof jayus.Brush))brush=new jayus.Brush(brush);if(this.bg!==null)this.bg.detach(this);this.bg=brush;this.bg.attach(this);return this.dirty(16)},clearBg:function RectEntity_clearBg(){if(this.bg!==null){this.bg.detach(this);this.bg=null;this.dirty(16)}return this},buffered:false,bufferBg:true,canvas:null,context:null,negateBlur:false,setBuffering:function RectEntity_setBuffering(on){jayus.debug.match("RectEntity.setBuffering",
on,"on",1);if(!this.buffered&&on){this.canvas=document.createElement("canvas");this.context=this.canvas.getContext("2d");this.refreshBuffer()}else if(this.buffered&&!on){this.canvas=null;this.context=null}this.buffered=on;return this},refreshBuffer:function RectEntity_refreshBuffer(){if(this.buffered){var canvas=this.canvas;if(this.width!==canvas.width||this.height!==canvas.height){canvas.width=this.width;canvas.height=this.height}this.fullyRefreshBuffer()}},fullyRefreshBuffer:function RectEntity_fullyRefreshBuffer(){if(this.buffered){var ctx=
...
y){if(this.isTransformed){var pos=this.getMatrix().inverseTransformPoint(x,y);x=pos.x;y=pos.y}return this.shape.intersectsAt(x,y)},cursorHitTest:function Shape_cursorHitTest(x,y){return this.shape.intersectsAt(x,y)},setShape:function PaintedShape_setShape(shape){jayus.debug.match("PaintedShape.setShape",shape,"shape",5);if(this.shape!==shape){if(typeof shape.frozen!=="number")shape=jayus.parse(shape);if(this.shape!==null)this.shape.detach(this);this.shape=shape;shape.attach(this);this.dirty(127)}return this},
setBrush:function PaintedShape_setBrush(brush){jayus.debug.match("PaintedShape.setBrush",brush,"brush",5);if(this.brush!==null)this.brush.detach(this);if(!(brush instanceof jayus.Brush))brush=new jayus.Brush(brush);this.brush=brush;brush.attach(this);return this.dirty(16)},clearBrush:function PaintedShape_clearBrush(){if(this.brush!==null){this.brush.detach(this);this.brush=null;this.dirty(16)}return this},getScope:function PaintedShape_getScope(){if(!this.isTransformed)return this.shape.getScope();
var scope=this.shape.getScope().getTransformed(this.getMatrix()).getScope();if(this.brush.stroking||this.brush.shadowing){var scale=Math.max(this.xScale,this.yScale),left=0,right=0,top=0,bottom=0;if(this.brush.stroking){var miterLimit=10;if(typeof this.brush.miterLimit==="number")miterLimit=this.brush.miterLimit;left=right=top=bottom=miterLimit*scale/1.5}if(this.brush.shadowing){left=Math.max(left,left+this.brush.shadowBlur-this.brush.shadowOffsetX);right=Math.max(right,right+this.brush.shadowBlur+
this.brush.shadowOffsetX);top=Math.max(top,top+this.brush.shadowBlur-this.brush.shadowOffsetY);bottom=Math.max(bottom,bottom+this.brush.shadowBlur+this.brush.shadowOffsetY)}scope.setSize(scope.width+left+right,scope.height+top+bottom);scope.translate(-left,-top)}return scope},getBounds:function PaintedShape_getBounds(){return this.shape},drawOntoContext:function PaintedShape_drawOntoContext(ctx){jayus.debug.matchContext("PaintedShape.drawOntoContext",ctx);if(this.brush!==null){ctx.save();if(this.alpha!==
1)ctx.globalAlpha*=this.alpha;this.applyTransforms(ctx);this.shape.etchOntoContext(ctx);this.brush.paintShape(ctx);ctx.restore();if(this.debugRenderer!==null)this.debugRenderer(ctx)}}});jayus.Text=jayus.RectEntity.extend({text:"",font:"12pt sans-serif",brush:null,alignment:0,fontDesc:null,lines:null,lineWidths:null,init:function Text(text,font,brush){jayus.Entity.prototype.init.apply(this);this.fontDesc=jayus.getFontDescriptor(this.font);this.lines=[];this.lineWidths=[];if(arguments.length===1)jayus.debug.match("Text",text,"text",2);else if(arguments.length===2)jayus.debug.matchArguments("Text",arguments,"text",2,"font",2);else if(arguments.length===3)jayus.debug.matchArguments("Text",
...
object,"object",5);this.ignoreDirty++;jayus.Scene.prototype.initFromObject.call(this,object);if(typeof object.negateBlur==="boolean")this.negateBlur=object.negateBlur;this.ignoreDirty--;this.dirty(127)},changeSize:function Surface_changeSize(width,height){this.width=width;this.height=height;this.canvas.width=width;this.canvas.height=height;this.dirty(4)},clear:function Surface_clear(){this.context.clearRect(0,0,this.width,this.height);return this.dirty(16)},fill:function Surface_fill(r,g,b,a){switch(arguments.length){case 1:return this.fill(r.r,
r.g,r.b,r.a);case 2:return this.fill(r.r,r.g,r.b,r.a*g);case 3:return this.fill(r,g,b,1);case 4:jayus.debug.matchArgumentsAs("Surface.fill",arguments,4,"r","g","b","a");var ctx=this.context;ctx.save();ctx.fillStyle="rgba("+r+","+g+","+b+","+a+")";ctx.globalCompositeOperation="copy";ctx.fillRect(0,0,this.width,this.height);ctx.restore();return this}},getBuffer:function Surface_getBuffer(x,y,width,height){switch(arguments.length){case 0:return this.getBuffer(0,0,this.width,this.height);case 1:jayus.debug.match("Surface.getBuffer()",
x,"section",20);return this.getBuffer(x.x,x.y,x.width,x.height);case 3:jayus.debug.matchArguments("Surface.getBuffer()",arguments,"origin",13,"width",4,"height",4);return this.getBuffer(x.x,x.y,width,height)}jayus.debug.matchArgumentsAs("Surface.getBuffer()",arguments,4,"x","y","width","height");return this.context.getImageData(x,y,width,height)},putBuffer:function Surface_putBuffer(x,y,buffer){switch(arguments.length){case 1:jayus.debug.match("Surface.putBuffer",x,"buffer",21);return this.putBuffer(0,
0,x);case 2:jayus.debug.matchArguments("Surface.putBuffer",arguments,"position",13,"buffer",21);return this.putBuffer(x.x,x.y,y)}jayus.debug.matchArguments("Surface.putBuffer",arguments,"x",4,"y",4,"buffer",21);this.context.putImageData(buffer,x,y);return this.dirty(16)},getPixel:function Surface_getPixel(x,y){jayus.debug.matchCoordinate("Surface.getPixel",x,y);if(arguments.length===1)return this.getPixel(x.x,x.y);var data=this.getBuffer(x,y,1,1).data;return new jayus.Color(data[0],data[1],data[2],
data[3]/255)},setPixel:function Surface_setPixel(x,y,r,g,b,a){switch(arguments.length){case 2:jayus.debug.matchArguments("Surface.setPixel",arguments,"pixel",13,"color",8);return this.setPixel(x.x,x.y,y.r,y.g,y.b,y.a);case 3:jayus.debug.matchArguments("Surface.setPixel",arguments,"x",4,"y",4,"color",8);return this.setPixel(x,y,r.r,r.g,r.b,r.a);case 4:jayus.debug.matchArguments("Surface.setPixel",arguments,"pixel",13,"r",4,"g",4,"b",4);return this.setPixel(x.x,x.y,y,r,g,1);case 5:jayus.debug.matchArgumentsAs("Surface.setPixel",
arguments,4,"x","y","r","g","b");return this.setPixel(x,y,r,g,b,1);case 6:jayus.debug.matchArgumentsAs("Surface.setPixel",arguments,4,"x","y","r","g","b","a");var buffer=this.context.createImageData(1,1),data=buffer.data;data[0]=r;data[1]=g;data[2]=b;data[3]=a*255;return this.putBuffer(x,y,buffer)}},getPixelFrame:function Surface_getPixelFrame(x,y){jayus.debug.matchCoordinate("Surface.getPixelFrame",x,y);if(arguments.length===1)return this.getPixelFrame(x.x,x.y);return new jayus.Rectangle(this.x+
x*this.xScale,this.y+y*this.yScale,this.xScale,this.yScale)},getPixelAt:function Surface_getPixelAt(x,y){jayus.debug.matchCoordinate("Surface.getPixelAt",x,y);if(arguments.length===1)return this.getPixelAt(x.x,x.y);if(this.intersectsAt(x,y)){if(x===this.x+this.width)x--;if(y===this.y+this.height)y--;return new jayus.Point(Math.floor((x-this.x)/this.xScale),Math.floor((y-this.y)/this.yScale))}return null},blurImage:function Surface_blurImage(intensity){jayus.debug.match("Surface.blurImage",intensity,
"intensity",4);var data=this.getBuffer().data,w=this.width,h=this.height,newBuffer=this.context.createImageData(w,h),newData=newBuffer.data,x,y,sx,sy,r,b,g,a,c,z,i=0;for(y=h-1;y>=0;y--)for(x=w-1;x>=0;x--){r=g=b=a=c=0;for(sy=y-intensity;sy<y+1+intensity;sy++)if(sy>=0&&sy<h)for(sx=x-intensity;sx<x+1+intensity;sx++)if(sx>=0&&sx<w){z=(sy*w+sx)*4;r+=data[z];g+=data[z+1];b+=data[z+2];a+=data[z+3];c++}newData[i]=r/c;newData[i+1]=g/c;newData[i+2]=b/c;newData[i+3]=a/c;i+=4}return this.putBuffer(newBuffer)},
exportPNG:function Surface_exportPNG(){window.open(this.canvas.toDataURL("image/png"),"_blank");return this},toPattern:function Surface_toPattern(repetition){if(!arguments.length)repetition="repeat";else{jayus.debug.match("Surface.toPattern",repetition,"repetition",2);if(["repeat","repeat-x","repeat-y","no-repeat"].indexOf(repetition)===-1)throw new Error("Surface.toPattern() - Invalid repetition"+jayus.debug.toString(repetition)+' sent, "repeat", "repeat-x", "repeat-y", or "no-repeat" required');
}return this.context.createPattern(this.canvas,repetition)},paintContents:function Surface_paintContents(ctx){if(this.negateBlur)jayus.manuallyDrawImage(this.context,ctx,this.width,this.height);else ctx.drawImage(this.canvas,0,0)}});jayus.Group={children:null,isParent:true,find:function Group_find(id){jayus.debug.match("Entity.setId",id,"id",0);var i,item;for(i=this.items.length-1;i>=0;i--){item=this.items[i];if(item.id===id)return item;if(item.isParent){item=item.find(id);if(item!==null)return item}}return null},getAllChildren:function Group_getAllChildren(parentsAfter){var i,item,children=[];for(i=0;i<this.items.length;i++){item=this.items[i];if(item.isParent)if(parentsAfter)children=children.concat(item,item.getAllChildren(parentsAfter));
...
items[i];if(item.isParent&&item.fireOnChildren(event,data))return true;if(item.fire(event,data))return true}return false},fireOnCursor:function Group_fireOnCursor(event,data){jayus.debug.match("Group.fireOnCursor",event,"event",2);var i,item,items=this.items.slice(0);for(i=items.length-1;i>=0;i--){item=items[i];if(item.underCursor){if(item.isParent&&item.fireOnCursor(event,data))return true;if(item.fire(event,data))return true}}return false},propagateCursor:false,updateCursorOnChildren:function Group_updateCursorOnChildren(x,
...
arguments,"width",4,"height",4);else throw new TypeError("Display() - Invalid number of arguments sent, 0, 1, or 2 required");var canvas;if(arguments.length===1){canvas=width;canvas.parentElement.style.position="relative"}else{canvas=document.createElement("canvas");if(arguments.length){canvas.width=width;canvas.height=height}}this.canvas=canvas;this.width=canvas.width;this.height=canvas.height;this.context=canvas.getContext("2d");jayus.displays.push(this);this.hookCursorListeners(this.canvas);var display=
this;jayus.addHandler("keyPress",function(e){if(display.underCursor&&e.key==="grave"){var i,target,targets=display.getChildrenUnderCursor();if(targets.length===0)targets.push(display);for(i=0;i<targets.length;i++){target=targets[i];if(e.event.shiftKey)if(target.exposingAll)target.exposeAll();else target.unexposeAll();else if(target.debugRenderer===null)target.expose();else target.unexpose()}}});this.addHandler("dirty",function Display_dirty_updateCursor(e){if(e&2+4+8)this.startCursorUpdate()})},listItemAdded:function Display_listItemAdded(list,
item){jayus.Scene.prototype.listItemAdded.call(this,list,item);item.parentDisplay=this;if(item.isParent){var display=this;item.forEveryChild(function(){this.parentDisplay=display})}},listItemsAdded:function Display_listItemsAdded(list,items){jayus.Scene.prototype.listItemsAdded.call(this,list,items);var i,item,displaySetter=function(){this.parentDisplay=display};for(i=0;i<items.length;i++){item=items[i];item.parentDisplay=this;if(item.isParent){var display=this;item.forEveryChild(displaySetter)}}},
setBuffering:function Display_setBuffering(){},processPendingMousemove:function Display_processPendingMousemove(e){this.hasPendingMousemoveEvent=false;return this.processMousemove(this.pendingMousemoveEvent)},processMousemove:function Display_processMousemove(e){if(!this.underCursor){this.underCursor=true;this.fire("cursorOver")}var x=e.offsetX===undefined?e.layerX:e.offsetX,y=e.offsetY===undefined?e.layerY:e.offsetY,data={display:this,event:e,x:x,y:y,deltaX:x-this.cursor.x,deltaY:y-this.cursor.y};
this.cursor.x=x;this.cursor.y=y;if(this.cursor.dependentCount)this.cursor.dirty(2);if(!this.fireOnCursor("cursorMove",data))this.fire("cursorMove",data);this.startCursorUpdate()},hookCursorListeners:function Display_hookCursorListeners(canvas){var display=this,cursorEventObject={event:null,display:display,x:null,y:null},scrollEventObject={event:null,display:display,x:null,y:null,scroll:null,xScroll:null,yScroll:null},updateCursorEventObject=function(e){cursorEventObject.event=e;cursorEventObject.x=
...
if(jayus.fire("scroll",scrollEventObject)||(display.fireOnCursor("scroll",scrollEventObject)||display.fire("scroll",scrollEventObject)))e.preventDefault()})},startCursorUpdate:function Display_startCursorUpdate(){var acceptor,tooltip;if(this.underCursor&&this.propagateCursor){this.updateCursorOnChildren(this.cursor.x,this.cursor.y);acceptor=this.findCursorAcceptor();this.cursorFocus=acceptor;if(jayus.cursorFocus!==acceptor)if(jayus.cursorFocus===null||jayus.cursorFocus.canReleaseCursor){if(jayus.cursorFocus!==
null)jayus.cursorFocus.fire("cursorLeave");jayus.cursorFocus=acceptor;if(acceptor!==null){acceptor.fire("cursorEnter");tooltip=acceptor.tooltip;if(this.canvas.title!==tooltip)this.canvas.setAttribute("title",tooltip)}else if(this.canvas.title)this.canvas.removeAttribute("title")}}},getTopCanvas:function Display_getTopCanvas(){return this.overlayCanvas!==null?this.overlayCanvas:this.canvas},showDamage:false,overlayCanvas:null,overlayContext:null,overlayFadeAnimator:null,initOverlay:function Display_initOverlay(){var canvas=
document.createElement("canvas");canvas.width=this.width;canvas.height=this.height;canvas.style.position="absolute";canvas.style.left=this.canvas.offsetLeft;canvas.style.top=this.canvas.offsetTop;canvas.style.opacity="0.5";this.overlayCanvas=canvas;this.overlayContext=canvas.getContext("2d");this.overlayContext.lineWidth=4;this.canvas.parentElement.appendChild(this.overlayCanvas);this.hookCursorListeners(canvas);this.overlayFadeAnimator=(new jayus.Animator(function update(pos){canvas.style.opacity=
0.5-0.5*pos})).setDuration(500)},clearDamage:function Display_clearDamage(){if(this.showDamage){this.initOverlay();var ctx=this.overlayContext;ctx.clearRect(0,0,this.width,this.height)}},paintDamage:function Display_paintDamage(rect){this.paintOverlayRect(rect,"red")},paintRedraw:function Display_paintRedraw(rect){this.paintOverlayRect(rect,"yellow")},paintOverlayRect:function Display_paintOverlayRect(rect,color){if(this.showDamage){this.initOverlay();var ctx=this.overlayContext;ctx.fillStyle=color;
ctx.fillRect(rect.x,rect.y,rect.width,rect.height);this.overlayCanvas.style.opacity="0.5";this.overlayFadeAnimator.restart()}},setVisible:function Display_setVisible(on){jayus.debug.match("Display.setVisible",on,"on",1);this.visible=on;if(on)this.canvas.style.visibility="";else this.canvas.style.visibility="hidden";return this},setAlpha:function Display_setAlpha(alpha){jayus.debug.match("Display.setAlpha",alpha,"alpha",4);if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,
this.setAlpha,this.alpha,alpha)}if(this.alpha!==alpha){this.alpha=alpha;this.canvas.style.opacity=alpha}return this},setInto:function Display_setInto(element){element.style.position="relative";if(this.visible)element.appendChild(this.canvas);return this},focus:function Display_focus(){this.canvas.focus();return this},blur:function Display_blur(){this.canvas.blur();return this},isFullScreen:function Display_isFullScreen(){return(document.fullscreenEnabled||(document.mozFullScreenEnabled||document.webkitFullScreenEnabled))&&
...
else if(document.mozCancelFullScreen)document.mozCancelFullScreen();else if(document.webkitCancelFullScreen)document.webkitCancelFullScreen();return this},getCursor:function Display_getCursor(){return this.customIcon?this.customCursor:this.getTopCanvas().style.cursor},setCursor:function Display_setCursor(cursor){if(typeof cursor==="string"){jayus.debug.match("Display.setCursor",cursor,"cursor",2);this.getTopCanvas().style.cursor=cursor;this.hasCustomCursor=false}else{jayus.debug.match("Display.setCursor",
cursor,"cursor",12);this.getTopCanvas().style.cursor="none";this.hasCustomCursor=true;this.customCursor=cursor}return this},resetCursor:function Display_resetCursor(){this.getTopCanvas().style.cursor="";this.hasCustomCursor=false;this.customCursor=null}});jayus.RESIZE_SELF=0;jayus.RESIZE_CHILD=1;jayus.RESIZE_MARGINS=2;
