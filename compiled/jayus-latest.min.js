...
this.getPoint(b);break;case "arc":a.arc(c[1],c[2],c[3],c[4],c[5],c[6]);this.getPoint(b);break;case "rect":d=c[1];e=c[2];a.rect(d,e,c[3],c[4]);break;case "ellipse":throw Error("Path.etchOntoContext() - Unimplemented for command ellipse");}}return this}});jayus.Path.Circle=function(a,b,c){if(arguments.length===2){c=b;b=a.y;a=a.x}return(new jayus.Path).arc(a,b,c,0,Math.PI*2,false)};jayus.Path.Rectangle=function(a,b,c,d){if(arguments.length===3){d=c;c=b;b=a.y;a=a.x}return(new jayus.Path).rect(a,b,c,d)};
...
this.angle=this.body.GetAngle();this.matrixDirty=true}},setWidth:function(a){if(a!==this.width){this.width=a;this.dirty(4)}return this},setHeight:function(a){if(a!==this.height){this.height=a;this.dirty(4)}return this},setSize:function(a,b){if(arguments.length===1){b=a.height;a=a.width}this.width=a;this.height=b;this.dirty(4);return this},intersectsAt:function(a,b){return this.getBounds().intersectsAt(a,b)},getUnFrame:function(){return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getFrame:function(){if(this.isTransformed)return(new jayus.Polygon.Rectangle(0,
0,this.width,this.height)).transform(this.getMatrix());return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getScope:function(){return this.getFrame().getScope()},getBounds:function(){if(this.bounds!==null){this.bounds.cloneOnto(this.boundsClone);return this.boundsClone.translate(this.x,this.y)}return this.getFrame().getScope()},setBounds:function(a){this.bounds!==null&&this.bounds.detach(this);this.bounds=a;this.bounds.attach(this);this.boundsClone=this.bounds.clone();this.shapeType=
0;return this},clearBounds:function(){if(this.bounds!==null){this.bounds.detach(this);this.boundsClone=this.bounds=null;this.shapeType=4}return this},componentDirtied:function(a){a===this.bounds&&this.bounds.cloneOnto(this.boundsClone)},setBg:function(a){this.hasBg&&this.bg.detach(this);a instanceof jayus.Brush||(a=new jayus.Brush(a));this.bg=a;this.bg.attach(this);this.hasBg=true;return this.dirty(jayus.DIRTY.BACKGROUND)},clearBg:function(){if(this.hasBg){this.bg.detach(this);this.bg=null;this.hasBg=
false;this.dirty(jayus.DIRTY.BACKGROUND)}return this},buffered:false,bufferBg:true,canvas:null,context:null,setBuffering:function(a){if(!this.buffered&&a){this.canvas=document.createElement("canvas");this.context=this.canvas.getContext("2d");this.refreshBuffer()}else if(this.buffered&&!a)this.context=this.canvas=null;this.buffered=a;return this},refreshBuffer:function(){if(this.buffered){var a=this.canvas;if(this.width!==a.width||this.height!==a.height){a.width=this.width;a.height=this.height}this.fullyRefreshBuffer()}},
fullyRefreshBuffer:function(){if(this.buffered){var a=this.context;a.clearRect(0,0,this.width,this.height);a.save();this.bufferBg&&this.hasBg&&this.bg.paintRect(a,0,0,this.width,this.height);this.paintContents(a);a.restore()}},keepAligned:false,applyTransforms:function(a){if(this.xScale!==1||this.yScale!==1||this.angle)if(this.xAnchor||this.yAnchor){this.keepAligned?a.translate(Math.round(this.x)+this.xAnchor,Math.round(this.y)+this.yAnchor):a.translate(this.x+this.xAnchor,this.y+this.yAnchor);if(this.xScale!==
1||this.yScale!==1)a.scale(this.xScale,this.yScale);this.angle!==0&&a.rotate(this.angle);a.translate(-this.xAnchor,-this.yAnchor)}else{this.keepAligned?a.translate(Math.round(this.x),Math.round(this.y)):a.translate(this.x,this.y);if(this.xScale!==1||this.yScale!==1)a.scale(this.xScale,this.yScale);this.angle!==0&&a.rotate(this.angle)}else if(this.x||this.y)this.keepAligned?a.translate(Math.round(this.x),Math.round(this.y)):a.translate(this.x,this.y);return this},drawOntoContext:function(a){a.save();
if(this.alpha!==1)a.globalAlpha*=this.alpha;this.applyTransforms(a);if(this.buffered){this.dirtied&&this.refreshBuffer();if(!this.bufferBg&&this.hasBg)this.alignBg?this.bg.paintRect(a,0.5,0.5,Math.round(this.width),Math.round(this.height)):this.bg.paintRect(a,0,0,this.width,this.height);a.drawImage(this.canvas,0,0)}else{if(this.hasBg)this.alignBg?this.bg.paintRect(a,0.5,0.5,Math.round(this.width),Math.round(this.height)):this.bg.paintRect(a,0,0,this.width,this.height);this.paintContents(a)}this.dirtied=
...
[];for(var c,d=0;d<b.length;d++){c=b[d];c.setParent(this);this.childrenAdded.push(c);c.prevScope=c.getScope()}this.dirty(16)},listItemRemoved:function(a,b){b.removeParent();this.redrawAll=true;this.dirty(16)},listItemsRemoved:function(a,b){for(var c=0;c<b.length;c++)b[c].removeParent();this.redrawAll=true;this.dirty(16)},hasFlexibleWidth:function(){return true},hasFlexibleHeight:function(){return true},formContents:function(a,b){if(this.buffered){this.canvas.width=a;this.canvas.height=b}},init:function(a,
b){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);if(arguments.length){this.width=a;this.height=b}},optimizeBuffering:false,groupDamagedRegions:true,damagedRegionPadding:2,setOptimizedBuffering:function(a){if(this.optimizeBuffering!==a){this.optimizeBuffering=a;this.redrawAll=true;this.dirty(16)}return this},refreshBuffer:function(){if(this.buffered){var a=this.width,b=this.height,c=this.canvas;if(a!==c.width||b!==c.height){c.width=a;c.height=b}if(this.optimizeBuffering&&
!this.redrawAll)this.optimizedRefreshBuffer();else{this.fullyRefreshBuffer();this.redrawAll=false}this.dirtied=false}},optimizedRefreshBuffer:function(){if(this.buffered){var a=this.context,b,c,d,e=[];for(b=0;b<this.children.items.length;b++){c=this.children.items[b];c.clean=true;if(c.scopeChanged){e.push(c.getScope().includeRectangle(c.prevScope));c.prevScope=c.getScope();c.scopeChanged=false;c.clean=false}else if(c.dirtied){e.push(c.getScope());c.dirtied=false;c.clean=false}}c=false;if(this.groupDamagedRegions)for(;!c;){c=
true;for(b=0;b<e.length;b++){d=e[b];for(i2=0;i2<e.length;i2++){region2=e[i2];if(d!==region2&&jayus.intersectTest(d,region2)){e[b].includeRectangle(region2);e.splice(i2,1);c=false;b>=i2&&b--;i2--}}}}c=this.damagedRegionPadding;for(b=0;b<e.length;b++){d=e[b];d.translate(-c,-c);d.setSize(d.width+c*2,d.height+c*2)}for(b=0;b<e.length;b++){d=e[b];for(i2=0;i2<this.children.items.length;i2++){c=this.children.items[i2];if(c.clean&&jayus.intersectTest(c,d))c.clean=false}}a.save();for(b=0;b<e.length;b++){d=
e[b];a.clearRect(d.x,d.y,d.width,d.height)}a.beginPath();for(b=0;b<e.length;b++){d=e[b];a.rect(d.x,d.y,d.width,d.height)}a.clip();if(this.bufferBg&&this.hasBg)this.alignBg?this.bg.paintRect(a,0.5,0.5,Math.round(this.width),Math.round(this.height)):this.bg.paintRect(a,0,0,this.width,this.height);for(b=0;b<this.children.items.length;b++){c=this.children.items[b];!c.clean&&c.visible&&c.drawOntoContext(a)}a.restore()}},paintContents:function(a){var b,c;for(b=0;b<this.children.items.length;b++){c=this.children.items[b];
c.visible&&c.drawOntoContext(a)}}});jayus.applyObject(jayus.Group,jayus.Scene.prototype);jayus.Display=jayus.Scene.extend({buffered:true,cursor:new jayus.Point,suppressContextMenu:true,suppressSelection:true,pendMousemoveEvents:true,hasPendingMousemoveEvent:false,pendingMousemoveEvent:null,init:function(a,b){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);if(arguments.length===1){var c=a;c.parentElement.style.position="relative"}else{c=document.createElement("canvas");if(arguments.length){c.width=a;c.height=b}}this.canvas=c;this.width=c.width;this.height=c.height;
...
return this},getCursor:function(){return this.customIcon?this.customCursor:this.getTopCanvas().style.cursor},setCursor:function(a){if(typeof a==="string"){this.getTopCanvas().style.cursor=a;this.hasCustomCursor=false}else{this.getTopCanvas().style.cursor="none";this.hasCustomCursor=true;this.customCursor=a}return this},resetCursor:function(){this.getTopCanvas().style.cursor="";this.hasCustomCursor=false;this.customCursor=null}});jayus.Frame=jayus.RectEntity.extend(jayus.applyObject({brush:null,hasBrush:false,marginLeft:6,marginRight:6,marginTop:6,marginBottom:6,forming:false,updateCursorOnChildren:function(a,b){if(this.propagateCursor){var c=this.parentToLocal(a,b);a=c.x;b=c.y;this.localX=a;this.localY=b;this.child.updateCursor(a-this.marginLeft,b-this.marginTop)}},init:function(a){jayus.Entity.prototype.init.apply(this);this.setChild(a);this.addHandler("dirty",function(b){if(b&4||b&2)this.formContents()})},setChild:function(a){jayus.Wrapper.setChild.call(this,
a);this.setSize(a.width+this.marginLeft+this.marginRight,a.height+this.marginTop+this.marginBottom);return this},componentDirtied:function(a,b){this.forming||(b&4?this.setSize(this.child.width+this.marginLeft+this.marginRight,this.child.height+this.marginTop+this.marginBottom):this.dirty(jayus.DIRTY.ALL))},setBrush:function(a){this.hasBrush&&this.brush.detach(this);a instanceof jayus.Brush||(a=new jayus.Brush(a));this.brush=a;this.brush.attach(this);this.hasBrush=true;return this.dirty(16)},clearBrush:function(){if(this.hasBrush){this.brush.detach(this);
this.brush=null;this.hasBrush=false;this.dirty(16)}return this},hasFlexibleWidth:function(){return this.child.hasFlexibleWidth()},hasFlexibleHeight:function(){return this.child.hasFlexibleHeight()},formContents:function(){this.forming=true;this.child.setOrigin(this.marginLeft,this.marginTop);this.child.setSize(this.width-this.marginLeft-this.marginRight,this.height-this.marginTop-this.marginBottom);this.forming=false},setMargin:function(a,b,c,d){if(arguments.length===1)d=c=b=a;this.marginLeft=a;this.marginRight=
b;this.marginTop=c;this.marginBottom=d;this.child.frozen++;this.child.hasFlexibleWidth()?this.child.setWidth(this.width-this.marginLeft-this.marginRight):this.setSize(this.child.width+a+b,this.child.height);this.child.hasFlexibleHeight()?this.child.setHeight(this.height-this.marginTop-this.marginBottom):this.setSize(this.child.width,this.child.height+c+d);this.child.frozen--;return this},setMarginLeft:function(a){return this.setMargin(a,this.marginRight,this.marginTop,this.marginBottom)},setMarginRight:function(a){return this.setMargin(this.marginLeft,
...
b.display.interceptNextEvent("cursorMove",d);return true})},setMinimumSize:function(a,b){this.minW=a;this.minH=b;return this}});jayus.FlowLayout=jayus.RectEntity.extend({alignment:0,width:100,height:100,forming:false,init:function(){jayus.Entity.prototype.init.apply(this);this.children=new jayus.List(this);this.addHandler("dirty",function(a){a&4&&this.formContents()})},hasFlexibleWidth:function(){return true},hasFlexibleHeight:function(){return false},componentDirtied:function(){this.forming||this.dirty(jayus.DIRTY.ALL)},listItemAdded:function(a,b){b.setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemsAdded:function(a,b){for(var c=
0;c<b.length;c++)b[c].setParent(this);this.dirty(jayus.DIRTY.ALL)},listItemRemoved:function(a,b){b.removeParent();this.dirty(jayus.DIRTY.ALL)},listItemsRemoved:function(a,b){for(var c=0;c<b.length;c++)b[c].removeParent();this.dirty(jayus.DIRTY.ALL)},setAlignment:function(a){if(this.alignment!==a){this.alignment=a;this.formContents()}return this},getAlignment:function(){return this.alignment},formContents:function(){this.forming=true;for(var a=0,b,c=0,d,e,f,h,g,i=[];a!==this.children.items.length;){d=
[];b=f=e=0;h=this.children.items[a];g=h.width;do{h.x=b;h.y=c;d.push(h);e+=g;b+=g;if(h.height>f)f=h.height;a++;if(a===this.children.items.length)break;h=this.children.items[a];g=h.width}while(e+g<this.width);if(this.alignment!==0)for(b=0;b<d.length;b++){h=d[b];h.x+=(this.width-e)*this.alignment}for(b=0;b<d.length;b++){h=d[b];if(typeof h.verticalAlign==="number")h.y+=h.verticalAlign*(f-h.height)}c+=f;i.push(f)}for(a=0;a<this.children.items.length;a++){h=this.children.items[a];h.dirty(2)}this.lineHeights=
i;this.forming=false},paintContents:jayus.Scene.prototype.paintContents});jayus.applyObject(jayus.Group,jayus.FlowLayout.prototype);jayus.Stack=jayus.RectEntity.extend({spacing:8,reversed:false,isParent:true,init:function(){jayus.RectEntity.prototype.init.apply(this,arguments);this.children=new jayus.List(this);this.addHandler("dirty",function(){})},listItemAdded:function(){this.formContents()},listItemRemoved:function(){this.formContents()},listItemsAdded:function(){this.formContents()},listItemsRemoved:function(){this.formContents()},hasFlexibleWidth:function(){return false},hasFlexibleHeight:function(){return false},setReversed:function(a){if(this.reversed!==
a){this.reversed=a;this.formContents()}return this},setSpacing:function(a){if(this.spacing!==a){this.spacing=a;this.formContents()}return this},findGreatestMinimalChildWidth:function(){var a,b=0;for(a=0;a<this.children.items.length;a++)b=Math.max(b,this.children.items[a].minW);return b},findGreatestMinimalChildHeight:function(){var a,b=0;for(a=0;a<this.children.items.length;a++)b=Math.max(b,this.children.items[a].minH);return b},reform:function(){this.formContents(this.width,this.height)},paintContents:jayus.Scene.prototype.paintContents});
