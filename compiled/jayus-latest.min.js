...
this.applyObject(this.Responder.prototype,this);this.addDefaultHandlers();this.ua=this.detectBrowser();this.ua.lineDash=typeof this.getContext().setLineDash==="function";window.requestAnimationFrame=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame;this.ua.requestAnimationFrame=typeof window.requestAnimationFrame==="function";this.keyboard.init();this.colors.cssNames=[];for(a=0;a<this.colors.displayNames.length;a++)this.colors.cssNames.push(this.colors.displayNames[a].replace(" ",
"").replace(" ","").toLowerCase());this.initialized=true},detectBrowser:function(){var a=[{src:navigator.platform,str:"Win",id:"Windows"},{src:navigator.platform,str:"Mac",id:"Mac"},{src:navigator.userAgent,str:"iPhone",id:"iPhone/iPod"},{src:navigator.platform,str:"Linux",id:"Linux"}],b,c=function(e){for(var f=0;f<e.length;f++){var h=e[f].src,g=e[f].prop;b=e[f].ver||e[f].id;if(h){if(h.indexOf(e[f].str)!==-1)return e[f].id}else if(g)return e[f].id}},d=function(e){var f=e.indexOf(b);if(f!==-1)return parseFloat(e.substring(f+
...
jayus.displays[a];if(b.hasPendingMousemoveEvent){b.processPendingMousemove();b.hasPendingMousemoveEvent=false}}for(a=0;a<jayus.animators.length;a++)jayus.animators[a].tick(c,d);jayus.box2d.enabled&&jayus.box2d.step(d);for(a=0;a<jayus.displays.length;a++){b=jayus.displays[a];b.dirtied&&b.refreshBuffer()}}}else jayus.frameIntervalRunning=false},parse:function(a){var b=a.type;if(typeof jayus[b]==="function"){for(var c in a){var d=a[c];if(d instanceof Array)for(var e=0;e<d.length;e++){if(typeof d[e]===
"object"&&typeof d[e].type==="string")d[e]=this.parse(d[e])}else if(typeof d==="object"&&typeof d.type==="string")a[c]=this.parse(d)}b=new jayus[b];b.apply(a);return b}},addDefaultHandlers:function(){jayus.addHandler("keyPress",function(a){if(a.key==="grave"&&a.event.ctrlKey){jayus.chart.visible?jayus.chart.hide():jayus.chart.show();return true}})},chart:{routines:{render:{description:"Rendering",time:0,color:"saddlebrown"},box2d:{description:"Box2D updating",time:0,color:"blue"},animation:{description:"Animation",
time:0,color:"orange"},cursor:{description:"Cursor events",time:0,color:"Chartreuse"}},routineNameStack:[],routineTimeStack:[],routineInitStack:[],markColor:"grey",markFont:"10px sans-serif",display:null,graphSurface:null,topMS:40,index:0,width:500,height:380,visible:false,initialized:false,windowSize:120,windowIndex:0,barHeight:20,barSpacing:10,routineData:{},getRoutineColor:function(a){switch(a){case "render":return"saddlebrown";case "box2d":return"blue";case "cursor":return"Chartreuse";case "animation":return"orange"}return"white"},
begin:function(a){this.routineNameStack.push(a);this.routineTimeStack.push(Date.now());this.routineInitStack.push(0)},end:function(){var a=Date.now()-this.routineTimeStack.pop(),b=this.routineNameStack.pop(),c=this.routineInitStack.pop(),d,e;if(typeof this.routines[b]!=="object")this.routines[b]={description:"",time:0,color:"red"};if(this.initialized){d=this.routineData[b];if(d===undefined){d={visible:true,latencies:[],averageLatency:0,inits:[],initTotal:0,color:this.getRoutineColor(b),widget:null};
for(e=0;e<this.windowSize;e++){d.latencies.push(0);d.inits.push(0)}this.routineData[b]=d;d.row=document.createElement("tr");e=document.createElement("td");e.style.color="#CCC";e.style.fontSize="11pt";e.innerText=b;d.latencyLabel=document.createElement("td");d.latencyLabel.style.color="#BBB";d.latencyLabel.style.fontSize="10pt";d.latencyLabel.style.textAlign="right";d.latencyLabel.innerText="0 ms";d.progressBar=document.createElement("div");d.progressBar.style.margin="4px";$(d.progressBar).progressbar({value:0});
d.progressBarValue=d.progressBar.childNodes[0];$(d.progressBarValue).css("background",d.color);d.row.appendChild(e);d.row.appendChild(d.latencyLabel);d.row.appendChild(d.progressBar);jayus.chart.latencyTable.appendChild(d.row)}d.time+=a;d.latencyTotal-=d.latencies[this.windowIndex];d.latencies[this.windowIndex]=a;d.inits+=c}this.routines[b].time+=a},tallyInit:function(){this.routineInitStack[this.routineInitStack.length-1]++},show:function(){if(!this.initialized){this.vBox=new jayus.vBox;this.vBox.setSize(this.width,
this.height);this.topRow=new jayus.Scene(this.width,30);var a=new jayus.LinearGradient(0,0,0,30);a.addColorStop(0,"#DDD");a.addColorStop(1,"#CCC");this.topRow.setBg({fill:a});this.topRow.heightPolicy=new jayus.SizePolicy;this.topRow.heightPolicy.size=30;this.topRow.heightPolicy.expand=false;this.fpsLabel=jayus.parse({type:"Text",setText:"",setFont:"12px sans-serif",setBrush:{fill:"#111"},setOrigin:[0,4]});this.topRow.children.add(this.fpsLabel);setInterval(function(){var c=jayus.chart.fpsLabel,d=
Math.round(jayus.fps*10)/10+"";if(d.length===2)d+=".0";c.setText("Framerate: "+d);c.setX(c.parent.width-4-c.width)},100);this.graph=new jayus.Scene;this.graph.setOptimizedBuffering(false);this.vBox.children.add(this.topRow,this.graph);this.graphSurface=new jayus.Surface(this.graph.width,this.graph.height);this.graph.children.add(this.graphSurface);this.graph.addHandler("dirty",function(c){if(c&4){c=function(d){var e=jayus.chart.graph.height-d/jayus.chart.topMS*jayus.chart.graph.height,f=new jayus.PaintedShape(new jayus.Polygon.Line(0,
Math.floor(e)+0.5,jayus.chart.graph.width,Math.floor(e)+0.5),{stroke:"dimgrey"});e=(new jayus.Text(d+" ms","10px sans-serif",{fill:"dimgrey"})).setOrigin(4,e-12);jayus.chart.graph.children.update(d+"line",f.setId(d+"line")).update(d+"text",e.setId(d+"text"))};c(16);c(33);jayus.chart.graphSurface.setHeight(this.height)}});this.graph.fire("resized");this.display=new jayus.Display(this.width,this.height);this.display.children.add(this.vBox);this.display.addHandler("dirty",function(c){c&4&&jayus.chart.vBox.setWidth(this.width)});
a=document.createElement("div");var b=this.display.canvas;b.style.border="#aaa 1px solid";b.style.backgroundColor="black";a.appendChild(b);table=document.createElement("table");table.style.width="100%";a.appendChild(table);$(table).append('<colgroup><col span="1" style="width: 15%;"><col span="1" style="width: 15%;"><col span="1" style="width: 70%;"></colgroup>');jayus.chart.latencyTable=table;$(a).dialog({title:"Jayus Debug Panel",open:function(){$(this).data("width.dialog",$(b).width()+60)},resize:function(){jayus.chart.display.setWidth($(jayus.chart.dialogContainer).width()-
4);return false},close:function(){jayus.chart.visible=false}});jayus.chart.dialog=a.parentNode;jayus.chart.dialogContainer=jayus.chart.dialog.childNodes[1];this.display.setSize($(jayus.chart.dialogContainer).width()-4,$(jayus.chart.dialogContainer).height()-4);this.initialized=true}this.visible=true;$(jayus.chart.dialog).show();jayus.fire("debugPanelShown")},hide:function(){if(this.initialized&&this.visible){$(jayus.chart.dialog).hide();this.graphSurface.clear();this.index=0;this.visible=false}jayus.fire("debugPanelHidden")},
mark:function(a){if(this.visible){var b=this.graphSurface.context,c=jayus.measureText(a,this.markFont);b.fillStyle=this.markColor;b.fillRect(this.index,0,1,this.graph.height-0.5);b.fillText(a,this.index-c.width-2,c.ascent)}},order:["render","box2d","animation","cursor"],update:function(){if(this.visible){var a=this.graphSurface.context,b=this.graphSurface.height,c,d,e,f=0,h=[];for(c in this.routineData){d=this.routineData[c];h.push(c);d.averageLatency=jayus.math.average(d.latencies);f+=d.averageLatency}for(c=
0;c<h.length;c++){d=this.routineData[h[c]];e=Math.floor(d.time/this.topMS*this.graph.height);a.fillStyle=d.color;a.fillRect(this.index,b-e,1,e-0.5);b-=e;d.time=0;d.inits=0}a.clearRect(this.index+1,0,2,this.graph.height);this.index++;if(this.index>=this.graph.width)this.index=-1;for(c=0;c<h.length;c++){d=this.routineData[h[c]];$(d.progressBar).progressbar({value:100*d.averageLatency/f});a=Math.round(10*d.averageLatency)/10+"";if(a.length===1&&a!=="0")a+=".0";$(d.latencyLabel).text(a+" ms")}d=$(jayus.chart.latencyTable);
f=d.children();f.sort(function(g,i){if(g.nodeName==="TR"&&i.nodeName==="TR"){var k=parseFloat(g.childNodes[2].attributes["aria-valuenow"].value),l=parseFloat(i.childNodes[2].attributes["aria-valuenow"].value);return k<l?1:0}});d.append(f);this.windowIndex++;if(this.windowIndex===this.windowSize)this.windowIndex=0;this.graphSurface.dirty(jayus.DIRTY.ALL);this.display.refreshBuffer()}}},getFontDescriptor:function(a){this.isFontCached(a)||this.cacheFont(a);return this.fontCache[a]},cacheFont:function(a){var b,
c,d;if(!this.isFontCached(a)){d=[];c=this.getContext();c.save();c.font=a;for(b=0;b<this.fontCacheMaxChar+1;b++)d[b]=c.measureText(String.fromCharCode(b)).width;c.restore();b=this.getVerticalFontMetrics(a);b.charWidths=d;this.fontCache[a]=b}},cacheFonts:function(a){for(var b=0;b<a.length;b++)this.cacheFont(a[b])},isFontCached:function(a){return typeof this.fontCache[a]==="object"},measureText:function(a,b){return this.measureTextOnto(a,b,{})},measureTextOnto:function(a,b,c){this.cacheFont(b);b=jayus.fontCache[b];
var d=b.charWidths,e,f=0;for(e=0;e<a.length;e++)f+=d[a.charCodeAt(e)];c.width=f;c.ascent=b.ascent;c.descent=b.descent;c.height=b.height;return c},getVerticalFontMetrics:function(a){var b={},c=document.createElement("span"),d=document.createElement("div"),e=document.createElement("div");c.style.font=a;c.innerText="Hg";d.style.display="inline-block";d.style.width="1px";d.style.height="0";e.appendChild(c);e.appendChild(d);document.body.appendChild(e);try{d.style.verticalAlign="baseline";b.ascent=d.offsetTop-
c.offsetTop;d.style.verticalAlign="bottom";b.height=d.offsetTop-c.offsetTop;b.descent=b.height-b.ascent}finally{document.body.removeChild(e)}return b},box2d:{enabled:true,scale:0.5,worlds:[],velocityIterations:10,positionIterations:10,physicalEntities:[],addWorld:function(a){this.worlds.push(a)},removeWorld:function(){this.worlds.splice(this.worlds.indexOf(this.world),1)},step:function(a){var b,c;for(b=0;b<this.worlds.length;b++){c=this.worlds[b];c.Step(a,this.velocityIterations,this.positionIterations);
c.ClearForces()}for(b=0;b<this.physicalEntities.length;b++)this.physicalEntities[b].updateFromBody()},drawDebug:function(){for(var a=0;a<this.worlds.length;a++)this.worlds[a].DrawDebugData()}},getContext:function(){if(this.displays.length)return this.displays[0].context;return document.createElement("canvas").getContext("2d")},math:{randomFrom:function(a){return a[this.randomBetween(0,a.length-1)]},randomBetween:function(a,b){return a+Math.floor((b-a+1)*Math.random())},clamp:function(a,b,c){if(b<
a)return a;if(b>c)return c;return b},min:function(a){var b,c,d=a[0];for(b=1;b<a.length;b++){c=a[b];if(c<d)d=c}return d},max:function(a){var b,c,d=a[0];for(b=1;b<a.length;b++){c=a[b];if(c>d)d=c}return d},average:function(a){var b,c=0;for(b=0;b<a.length;b++)c+=a[b];return c/a.length},toDegrees:function(a){return a*(180/Math.PI)},toRadians:function(a){return a*(Math.PI/180)},vFlipAngle:function(a){return 2*Math.PI-a},hFlipAngle:function(a){a=Math.PI-a;if(a<0)a+=2*Math.PI;return a},getCwAngleBetween:function(a,
b){var c=a-b;if(c<0)c+=2*Math.PI;return c},getCcwAngleBetween:function(a,b){var c=b-a;if(c<0)c+=2*Math.PI;return c}},colors:{displayNames:["Alice Blue","Antique White","Aqua","Aquamarine","Azure","Beige","Bisque","Black","Blanched Almond","Blue","Blue Violet","Brown","Burly Wood","Cadet Blue","Chartreuse","Chocolate","Coral","Cornflower Blue","Cornsilk","Crimson","Cyan","Dark Blue","Dark Cyan","Dark Goldenrod","Dark Grey","Dark Green","Dark Khaki","Dark Magenta","Dark Olive Green","Dark Orange","Dark Orchid",
...
this.initialValue+a*(this.finalValue-this.initialValue))}});jayus.Animator.Discrete=jayus.Animator.extend({count:NaN,updater:null,init:function(a,b){this.count=a;this.updater=b},update:function(a){a=Math.floor(a*this.count);a===this.count&&a--;this.updater(a)}});jayus.Animator.Sequence=jayus.Animator.extend({animators:null,init:function(){this.animators=new jayus.List},update:function(a){var b=Math.floor(a*counts);b===counts&&b--;a=a*counts-b;this.animators.items[b].update(a)}});jayus.Dependency=jayus.Animatable.extend({dependents:null,dependentCount:0,frozen:0,attach:function(a){if(this.dependentCount===1)this.dependents=[this.dependents,a];else if(this.dependentCount)this.dependents.push(a);else this.dependents=a;this.dependentCount++},detach:function(a){if(this.dependentCount){if(this.dependentCount===1)this.dependents=null;else if(this.dependenCount===2)this.depdendents=this.dependents[1];else this.dependents.splice(this.dependents.indexOf(a),1);this.dependentCount--}},
dirty:function(a){this.frozen||this.informDependents(a)},informDependents:function(a){if(this.dependentCount===1)this.dependents.componentDirtied(this,a);else for(var b=0;b<this.dependentCount;b++)this.dependents[b].componentDirtied(this,a)},forEachDependent:function(a,b){if(this.dependentCount===1)a.apply(this.dependents,b);else for(var c=0;c<this.dependentCount;c++)a.apply(this.dependents[c],b)}});jayus.List=jayus.createClass({shapeType:-1,parent:null,items:null,init:function(a){if(arguments.length===1)this.parent=a;this.items=[]},added:function(a){this.parent.listItemAdded(this,a)},addedMany:function(a){this.parent.listItemsAdded(this,a)},removed:function(a){this.parent.listItemRemoved(this,a)},removedMany:function(a){this.parent.listItemsRemoved(this,a)},indexOf:function(a){var b;if(typeof a==="string"){for(b=0;b<this.items.length;b++)if(this.items[b].id===a)return b;return-1}return this.items.indexOf(a)},
...
jayus.Entity=jayus.Responder.extend({componentType:"ENTITY",shapeType:0,isEntity:true,isRect:false,isParent:false,id:"",visible:true,alpha:1,dirtied:true,frozen:0,hasParent:false,parent:null,angle:0,xScale:1,yScale:1,xAnchor:0,yAnchor:0,trackCursor:true,canAcceptCursor:false,canReleaseCursor:true,underCursor:false,hasCursor:false,draggable:false,dragButton:"left",dragging:false,enableDragEvents:true,leftDragging:false,middleDragging:false,rightDragging:false,apply:function(a){jayus.debug.match("Entity.apply",
a,"type",5);for(var b in a)if(a.hasOwnProperty(b))if(typeof this[b]==="function")a[b]instanceof Array?this[b].apply(this,a[b]):this[b](a[b]);else this[b]=a[b];return this},isTransformed:false,transforms:null,actionsToAnimate:0,animate:function(){this.actionsToAnimate++;return this},init:function(){this.addHandler("dirty",function(a){if(a&2)this.matrixDirty=true});this.enableDragEvents&&this.handle({leftPress:function(a){this.leftDragging=true;return this.fire("leftDragStart",a)},leftRelease:function(a){if(this.leftDragging){this.leftDragging=
false;return this.fire("leftDragEnd",a)}},middlePress:function(a){this.middleDragging=true;return this.fire("middleDragStart",a)},middleRelease:function(a){if(this.middleDragging){this.middleDragging=false;return this.fire("middleDragEnd",a)}},rightPress:function(a){this.rightDragging=true;return this.fire("rightDragStart",a)},rightRelease:function(a){if(this.rightDragging){this.rightDragging=false;return this.fire("rightDragEnd",a)}},cursorMove:function(a){var b=false;if(this.leftDragging)b=this.fire("leftDrag",
a)||b;if(this.middleDragging)b=this.fire("middleDrag",a)||b;if(this.rightDragging)b=this.fire("rightDrag",a)||b;return b},cursorOut:function(){this.leftDragging&&this.fire("leftDragEnd");this.middleDragging&&this.fire("middleDragEnd");this.rightDragging&&this.fire("rightDragEnd");this.leftDragging=this.middleDragging=this.rightDragging=false}})},setId:function(a){this.id=a;return this},dirty:function(a){if(!this.frozen){this.fire("dirty",a);this.dirtied=true;this.informDependents(a)}return this},
getAbsoluteParent:function(){return this.hasParent?this.parent.getAbsoluteParent():this},setParent:function(a){this.hasParent=true;this.parent=a;this.attach(a);this.fire("added")},removeParent:function(){this.hasParent=false;this.parent=null;this.underCursor=false;this.detach(parent);this.fire("removed")},setVisible:function(a){if(this.visible!==a){this.visible=a;this.dirty(1)}return this},show:function(){return this.setVisible(true)},hide:function(){return this.setVisible(false)},setAlpha:function(a){if(this.actionsToAnimate){this.actionsToAnimate--;
return new jayus.MethodAnimator(this,this.setAlpha,this.alpha,a)}if(this.alpha!==a){this.alpha=a;this.dirty(16)}return this},updateCursor:function(a,b){if(this.underCursor)if(this.intersectsAt(a,b))this.isParent&&this.updateCursorOnChildren(a,b);else{this.underCursor=false;this.fire("cursorOut");this.isParent&&this.removeCursorFromChildren()}else if(this.intersectsAt(a,b)){this.underCursor=true;this.fire("cursorOver");this.isParent&&this.updateCursorOnChildren(a,b)}},removeCursor:function(){if(this.underCursor){this.underCursor=
false;this.fire("cursorOut");this.isParent&&this.removeCursorFromChildren()}},setDragButton:function(a){if(this.dragButton!==a){this.dragButton=a;if(this.draggable){this.setDraggable(false);this.setDraggable(true)}}return this},setDraggable:function(a){if(!this.draggable&&a){this.addHandler(this.dragButton+"Press",function(b){this.dragging=true;this.canReleaseCursor=false;this.fire("dragStart");jayus.interceptNextEvent(this.dragButton+"Release",function(){this.dragging=false;this.canReleaseCursor=
true;this.display.updateCursor(this.display.cursor.x,this.display.cursor.y)},{context:this});this.display=b.display;var c=this;b.display.addHandler("cursorMove",function(d){c.dragging?c.translate(d.deltaX,d.deltaY):c.display.removeHandler("cursorMove","dragHandler");return true},{id:"dragHandler"})});this.draggable=true}return this},matrix:null,matrixDirty:true,getMatrix:function(){if(this.matrixDirty){if(this.matrix===null)this.matrix=new jayus.Matrix;else this.matrix.identity();this.applyTransforms(this.matrix);
this.matrixDirty=false}return this.matrix},applyTransforms:function(a){if(this.isTransformed)if(this.xAnchor||this.yAnchor){a.translate(this.xAnchor,this.yAnchor);if(this.xScale!==1||this.yScale!==1)a.scale(this.xScale,this.yScale);this.angle!==0&&a.rotate(this.angle);a.translate(-this.xAnchor,-this.yAnchor)}else{if(this.xScale!==1||this.yScale!==1)a.scale(this.xScale,this.yScale);this.angle!==0&&a.rotate(this.angle)}return this},setAngle:function(a){if(this.actionsToAnimate){this.actionsToAnimate--;
return jayus.MethodAnimator(this,this.setAngle,this.angle,a)}if(this.angle!==a){this.angle=a;this.matrixDirty=this.isTransformed=true;this.dirty(8)}return this},rotate:function(a){return this.setAngle(this.angle+a)},setScale:function(a,b){if(arguments.length===1)b=a;if(this.actionsToAnimate){this.actionsToAnimate--;return new jayus.MethodAnimator(this,this.setScale,[this.xScale,this.yScale],[a,b])}if(this.xScale!==a||this.yScale!==b){this.xScale=a;this.yScale=b;this.matrixDirty=this.isTransformed=
true;this.dirty(8)}return this},scale:function(a,b){if(arguments.length===1)b=a;return this.setScale(this.xScale*a,this.yScale*b)},setAnchor:function(a,b){if(arguments.length===1){b=a.y;a=a.x}if(this.xAnchor!==a||this.yAnchor!==b){this.xAnchor=a;this.yAnchor=b;this.matrixDirty=true;this.dirty(8)}return this},screenToLocal:function(a,b){if(arguments.length===1){b=a.y;a=a.x}if(this.hasParent)return this.parentToLocal(this.parent.screenToLocal(a,b));return new jayus.Point(a,b)},parentToLocal:function(a,
b){if(arguments.length===1){b=a.y;a=a.x}if(this.hasParent){a-=this.x;b-=this.y;a/=this.xScale;b/=this.yScale;if(this.angle!==0){var c=a,d=b,e=Math.cos(this.angle),f=Math.sin(this.angle);a=e*c-f*d;b=e*d+f*c}}return new jayus.Point(a,b)},localToScreen:function(a,b){if(arguments.length===1){b=a.y;a=a.x}if(this.hasParent)return this.parent.localToScreen(this.localToParent(a,b));return new jayus.Point(a,b)},localToScreenOnto:function(a,b,c){if(this.hasParent){this.localToParentOnto(a,b,c);this.parent.localToScreenOnto(c.x,
c.y,c)}else{c.x=a;c.y=b}return c},localToParent:function(a,b){if(arguments.length===1){b=a.y;a=a.x}var c=new jayus.Point;this.localToParentOnto(a,b,c);return c},localToParentOnto:function(a,b,c){if(this.angle!==0){var d=a;b=b;var e=Math.cos(this.angle),f=Math.sin(this.angle);a=e*d-f*b;b=e*b+f*d}a*=this.xScale;b*=this.yScale;a+=this.x;b+=this.y;c.x=a;c.y=b;return c},localToParentOntoORIGINAL:function(a,b,c){if(this.hasParent){if(this.angle!==0){var d=a;b=b;var e=Math.cos(this.angle),f=Math.sin(this.angle);
a=e*d-f*b;b=e*b+f*d}a*=this.xScale;b*=this.yScale;a+=this.x;b+=this.y}c.x=a;c.y=b;return c},running:false,xVelocity:0,yVelocity:0,setVelocity:function(a,b){if(arguments.length===1){b=a.y;a=a.x}this.xVelocity=a;this.yVelocity=b;if(!this.running){jayus.animators.push(this);this.running=true}},clearVelocity:function(){this.velocity=null;jayus.animators.splice(jayus.animators.indexOf(this),1);this.running=false},tick:function(a,b){this.translate(this.xVelocity*b,this.yVelocity*b)},intersects:function(a){return jayus.intersectTest(this,
a)},intersectCount:function(a){var b,c=0;for(b=0;b<a.length;b++)jayus.intersectTest(this,a[b])&&c++;return c},intersectsAny:function(a){if(a instanceof jayus.List)a=a.items;for(var b=0;b<a.length;b++)if(jayus.intersectTest(this,a[b]))return true;return false},intersectsAll:function(a){if(a instanceof jayus.List)a=a.items;for(var b=0;b<a.length;b++)if(!jayus.intersectTest(this,a[b]))return false;return true},intersectsWhich:function(a){if(a instanceof jayus.List)a=a.items;var b,c=[];for(b=0;b<a.length;b++)jayus.intersectTest(this,
a[b])&&c.push(a[b]);return c},setStyle:function(a){this.hasStyle&&this.style.detach(this);a instanceof jayus.Brush||(a=new jayus.Brush(a));this.style=a;this.style.attach(this);this.hasStyle=true;return this.dirty(jayus.DIRTY.STYLE)},clearBg:function(){if(this.hasStyle){this.style.detach(this);this.style=null;this.hasStyle=false;this.dirty(jayus.DIRTY.STYLE)}return this},rasterize:function(){var a=this.getScope();a=new jayus.Surface(a.width,a.height);var b=this.x,c=this.y;this.y=this.x=0;this.drawOnto(a);
...
this.angle=this.body.GetAngle();this.matrixDirty=true}},setWidth:function(a){a!==this.width&&this.changeSize(a,this.height);return this},widthDesc:null,setRemoteWidth:function(a,b){this.widthDesc={source:a,key:b};var c=this;a.addHandler("dirty",function(){var d=this[b];d!==c.width&&c.changeSize(d,c.height)});return this},setHeight:function(a){a!==this.height&&this.changeSize(this.width,a);return this},setSize:function(a,b){if(arguments.length===1){b=a.height;a=a.width}if(this.width!==a||this.height!==
b)this.changeSize(a,b);return this},changeSize:function(a,b){this.width=a;this.height=b;this.dirty(4)},intersectsAt:function(a,b){return this.getBounds().intersectsAt(a,b)},getUnFrame:function(){return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getFrame:function(){if(this.isTransformed)return(new jayus.Polygon.Rectangle(0,0,this.width,this.height)).transform(this.getMatrix());return new jayus.Rectangle(this.x,this.y,this.width,this.height)},getScope:function(){return this.getFrame().getScope()},
getBounds:function(){if(this.bounds!==null){this.bounds.cloneOnto(this.boundsClone);return this.boundsClone.translate(this.x,this.y)}return this.getFrame().getScope()},setBounds:function(a){this.bounds!==null&&this.bounds.detach(this);this.bounds=a;this.bounds.attach(this);this.boundsClone=this.bounds.clone();this.shapeType=0;return this},clearBounds:function(){if(this.bounds!==null){this.bounds.detach(this);this.boundsClone=this.bounds=null;this.shapeType=4}return this},componentDirtied:function(a){a===
this.bounds&&this.bounds.cloneOnto(this.boundsClone)},setBg:function(a){this.hasBg&&this.bg.detach(this);a instanceof jayus.Brush||(a=new jayus.Brush(a));this.bg=a;this.bg.attach(this);this.hasBg=true;return this.dirty(jayus.DIRTY.BACKGROUND)},clearBg:function(){if(this.hasBg){this.bg.detach(this);this.bg=null;this.hasBg=false;this.dirty(jayus.DIRTY.BACKGROUND)}return this},buffered:false,bufferBg:true,canvas:null,context:null,setBuffering:function(a){if(!this.buffered&&a){this.canvas=document.createElement("canvas");
this.context=this.canvas.getContext("2d");this.refreshBuffer()}else if(this.buffered&&!a)this.context=this.canvas=null;this.buffered=a;return this},refreshBuffer:function(){if(this.buffered){var a=this.canvas;if(this.width!==a.width||this.height!==a.height){a.width=this.width;a.height=this.height}this.fullyRefreshBuffer()}},fullyRefreshBuffer:function(){if(this.buffered){var a=this.context;a.clearRect(0,0,this.width,this.height);a.save();this.bufferBg&&this.hasBg&&this.bg.paintRect(a,0,0,this.width,
this.height);this.paintContents(a);a.restore()}},keepAligned:false,applyTransforms:function(a){if(this.xScale!==1||this.yScale!==1||this.angle)if(this.xAnchor||this.yAnchor){this.keepAligned?a.translate(Math.round(this.x)+this.xAnchor,Math.round(this.y)+this.yAnchor):a.translate(this.x+this.xAnchor,this.y+this.yAnchor);if(this.xScale!==1||this.yScale!==1)a.scale(this.xScale,this.yScale);this.angle!==0&&a.rotate(this.angle);a.translate(-this.xAnchor,-this.yAnchor)}else{this.keepAligned?a.translate(Math.round(this.x),
Math.round(this.y)):a.translate(this.x,this.y);if(this.xScale!==1||this.yScale!==1)a.scale(this.xScale,this.yScale);this.angle!==0&&a.rotate(this.angle)}else if(this.x||this.y)this.keepAligned?a.translate(Math.round(this.x),Math.round(this.y)):a.translate(this.x,this.y);return this},drawOntoContext:function(a){a.save();this.hasStyle&&this.style.applyTo(a);if(this.alpha!==1)a.globalAlpha*=this.alpha;this.applyTransforms(a);if(this.buffered){this.dirtied&&this.refreshBuffer();if(!this.bufferBg&&this.hasBg)this.alignBg?
this.bg.paintRect(a,0.5,0.5,Math.round(this.width),Math.round(this.height)):this.bg.paintRect(a,0,0,this.width,this.height);a.drawImage(this.canvas,0,0)}else{if(this.hasBg)this.alignBg?this.bg.paintRect(a,0.5,0.5,Math.round(this.width),Math.round(this.height)):this.bg.paintRect(a,0,0,this.width,this.height);this.paintContents(a)}this.dirtied=false;a.restore();return this}});jayus.PaintedShape=jayus.Entity.extend({shape:null,brush:null,hasBrush:false,init:function(a,b){jayus.Entity.prototype.init.apply(this);this.shape=a;a.attach(this);b!==undefined&&this.setBrush(b)},componentDirtied:function(){this.dirty(16)},translate:function(a,b){this.shape.translate.call(this.shape,a,b);this.dirty(2)},intersectsAt:function(a,b){if(this.isTransformed){var c=this.getMatrix().inverseTransformPoint(a,b);a=c.x;b=c.y}return this.shape.intersectsAt(a,b)},setShape:function(a){if(this.shape!==
...
if(arguments.length){this.width=a;this.height=b}},optimizeBuffering:false,groupDamagedRegions:true,damagedRegionPadding:2,setOptimizedBuffering:function(a){if(this.optimizeBuffering!==a){this.optimizeBuffering=a;this.redrawAll=true;this.dirty(16)}return this},refreshBuffer:function(){if(this.buffered){var a=this.width,b=this.height,c=this.canvas;if(a!==c.width||b!==c.height){c.width=a;c.height=b}if(this.optimizeBuffering&&!this.redrawAll)this.optimizedRefreshBuffer();else{this.fullyRefreshBuffer();
this.redrawAll=false}this.dirtied=false}},optimizedRefreshBuffer:function(){if(this.buffered){var a=this.context,b,c,d,e=[];for(b=0;b<this.children.items.length;b++){c=this.children.items[b];c.clean=true;if(c.scopeChanged){e.push(c.getScope().includeRectangle(c.prevScope));c.prevScope=c.getScope();c.scopeChanged=false;c.clean=false}else if(c.dirtied){e.push(c.getScope());c.dirtied=false;c.clean=false}}c=false;if(this.groupDamagedRegions)for(;!c;){c=true;for(b=0;b<e.length;b++){d=e[b];for(i2=0;i2<
e.length;i2++){region2=e[i2];if(d!==region2&&jayus.intersectTest(d,region2)){e[b].includeRectangle(region2);e.splice(i2,1);c=false;b>=i2&&b--;i2--}}}}c=this.damagedRegionPadding;for(b=0;b<e.length;b++){d=e[b];d.translate(-c,-c);d.setSize(d.width+c*2,d.height+c*2)}for(b=0;b<e.length;b++){d=e[b];for(i2=0;i2<this.children.items.length;i2++){c=this.children.items[i2];if(c.clean&&jayus.intersectTest(c,d))c.clean=false}}a.save();for(b=0;b<e.length;b++){d=e[b];a.clearRect(d.x,d.y,d.width,d.height)}a.beginPath();
for(b=0;b<e.length;b++){d=e[b];a.rect(d.x,d.y,d.width,d.height)}a.clip();if(this.bufferBg&&this.hasBg)this.alignBg?this.bg.paintRect(a,0.5,0.5,Math.round(this.width),Math.round(this.height)):this.bg.paintRect(a,0,0,this.width,this.height);for(b=0;b<this.children.items.length;b++){c=this.children.items[b];!c.clean&&c.visible&&c.drawOntoContext(a)}a.restore()}},paintContents:function(a){var b,c;for(b=0;b<this.children.items.length;b++){c=this.children.items[b];c.visible&&c.drawOntoContext(a)}}});
